...
>>>
"""
__all__ = ['Decimal', 'Context', 'DecimalTuple', 'DefaultContext',
    'BasicContext', 'ExtendedContext', 'DecimalException', 'Clamped',
    'InvalidOperation', 'DivisionByZero', 'Inexact', 'Rounded', 'Subnormal',
    'Overflow', 'Underflow', 'FloatOperation', 'DivisionImpossible',
    'InvalidContext', 'ConversionSyntax', 'DivisionUndefined', 'ROUND_DOWN',
    'ROUND_HALF_UP', 'ROUND_HALF_EVEN', 'ROUND_CEILING', 'ROUND_FLOOR',
    'ROUND_UP', 'ROUND_HALF_DOWN', 'ROUND_05UP', 'setcontext', 'getcontext',
    'localcontext', 'MAX_PREC', 'MAX_EMAX', 'MIN_EMIN', 'MIN_ETINY',
    'HAVE_THREADS']
__xname__ = __name__
__name__ = 'decimal'
__version__ = '1.70'
    ...
    def handle(self, context, sign, *args):
        ...
        if sign == 0:
            if context.rounding == ROUND_CEILING:
                return _SignedInfinity[sign]
            return _dec_from_triple(sign, '9' * context.prec, context.Emax -
                context.prec + 1)
        if sign == 1:
...
_condition_map = {ConversionSyntax: InvalidOperation, DivisionImpossible:
    ...
    InvalidOperation}
_rounding_modes = (ROUND_DOWN, ROUND_HALF_UP, ROUND_HALF_EVEN,
    ROUND_CEILING, ROUND_FLOOR, ROUND_UP, ROUND_HALF_DOWN, ROUND_05UP)
try:
    import threading
except ImportError:
...
class Decimal(object):
    ...
    def _divide(self, other, context):
        ...
        if not self or other._isinfinity() or expdiff <= -2:
            return _dec_from_triple(sign, '0', 0), self._rescale(ideal_exp,
                context.rounding)
        if expdiff <= context.prec:
            ...
            if q < 10 ** context.prec:
                return _dec_from_triple(sign, str(q), 0), _dec_from_triple(self
                    ._sign, str(r), ideal_exp)
    ...
    def __divmod__(self, other, context=None):
        ...
        if self._isinfinity():
            if other._isinfinity():
                ...
                return _SignedInfinity[sign], context._raise_error(
    ...
    def __floordiv__(self, other, context=None):
        ...
        if not other:
            if self:
                return context._raise_error(DivisionByZero, 'x // 0', self.
                    _sign ^ other._sign)
            else:
                return context._raise_error(DivisionUndefined, '0 // 0')
    ...
    def same_quantum(self, other, context=None):
        ...
        other = _convert_other(other, raiseit=True)
        if self._is_special or other._is_special:
            return self.is_nan() and other.is_nan() or self.is_infinite(
    ...
    def _rescale(self, exp, rounding):
        ...
        if self._exp >= exp:
            return _dec_from_triple(self._sign, self._int + '0' * (self.
                _exp - exp), exp)
    ...
    def copy_sign(self, other, context=None):
        ...
        other = _convert_other(other, raiseit=True)
        return _dec_from_triple(other._sign, self._int, self._exp, self.
            _is_special)
    ...
    def rotate(self, other, context=None):
        ...
        return _dec_from_triple(self._sign, rotated.lstrip('0') or '0',
            self._exp)
    ...
    def shift(self, other, context=None):
        ...
        return _dec_from_triple(self._sign, shifted.lstrip('0') or '0',
            self._exp)
...
class Context(object):
    ...
    def __init__(self, prec=None, rounding=None, Emin=None, Emax=None,
        capitals=None, clamp=None, flags=None, traps=None, _ignored_flags=None
        ):
        try:
            dc = DefaultContext
        except NameError:
    ...
    def _set_integer_check(self, name, value, vmin, vmax):
        ...
        if vmin == '-inf':
            if value > vmax:
                raise ValueError('%s must be in [%s, %d]. got: %s' % (name,
                    vmin, vmax, value))
        elif vmax == 'inf':
            if value < vmin:
                raise ValueError('%s must be in [%d, %s]. got: %s' % (name,
                    vmin, vmax, value))
        elif value < vmin or value > vmax:
            raise ValueError('%s must be in [%d, %d]. got %s' % (name, vmin,
                vmax, value))
    ...
    def __reduce__(self):
        ...
        return self.__class__, (self.prec, self.rounding, self.Emin, self.
            Emax, self.capitals, self.clamp, flags, traps)
    ...
    def _shallow_copy(self):
        ...
        nc = Context(self.prec, self.rounding, self.Emin, self.Emax, self.
            capitals, self.clamp, self.flags, self.traps, self._ignored_flags)
        return nc

    def copy(self):
        """Returns a deep copy from self."""
        nc = Context(self.prec, self.rounding, self.Emin, self.Emax, self.
            capitals, self.clamp, self.flags.copy(), self.traps.copy(),
            self._ignored_flags)
        return nc
    __copy__ = copy
...
def _ilog(x, M, L=8):
    ...
    while R <= L and abs(y) << L - R >= M or R > L and abs(y) >> R - L >= M:
        y = _div_nearest(M * y << 1, M + _sqrt_nearest(M * (M +
...
def _convert_for_comparison(self, other, equality_op=False):
    ...
    if isinstance(other, _numbers.Rational):
        if not self._is_special:
            self = _dec_from_triple(self._sign, str(int(self._int) * other.
                denominator), self._exp)
...
DefaultContext = Context(prec=28, rounding=ROUND_HALF_EVEN, traps=[
    DivisionByZero, Overflow, InvalidOperation], flags=[], Emax=999999,
