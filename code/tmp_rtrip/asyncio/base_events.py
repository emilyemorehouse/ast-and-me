...
def _ensure_resolved(address, *, family=0, type=socket.SOCK_STREAM, proto=0,
    ...
    if info is not None:
        ...
        return loop.getaddrinfo(host, port, family=family, type=type, proto
            =proto, flags=flags)
...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def __init__(self):
        ...
        self.set_debug(not sys.flags.ignore_environment and bool(os.environ
            .get('PYTHONASYNCIODEBUG')))
    ...
    def __repr__(self):
        return '<%s running=%s closed=%s debug=%s>' % (self.__class__.
            __name__, self.is_running(), self.is_closed(), self.get_debug())
    ...
    def _make_socket_transport(self, sock, protocol, waiter=None, *, extra=
        None, server=None):
        """Create socket transport."""
        raise NotImplementedError

    def _make_ssl_transport(self, rawsock, protocol, sslcontext, waiter=
        None, *, server_side=False, server_hostname=None, extra=None,
        server=None):
        """Create SSL transport."""
        raise NotImplementedError

    def _make_datagram_transport(self, sock, protocol, address=None, waiter
        =None, extra=None):
    ...
    def _make_write_pipe_transport(self, pipe, protocol, waiter=None, extra
        =None):
    ...
    def _asyncgen_firstiter_hook(self, agen):
        if self._asyncgens_shutdown_called:
            warnings.warn(
                ...
                .format(agen), ResourceWarning, source=self)
    ...
    def shutdown_asyncgens(self):
        ...
        shutdown_coro = tasks.gather(*[ag.aclose() for ag in closing_agens],
            return_exceptions=True, loop=self)
        ...
        for result, agen in zip(results, closing_agens):
            if isinstance(result, Exception):
                self.call_exception_handler({'message':
                    ...
                    .format(agen), 'exception': result, 'asyncgen': agen})
    ...
    if compat.PY34:
        ...
        def __del__(self):
            if not self.is_closed():
                warnings.warn('unclosed event loop %r' % self,
    ...
    def _check_callback(self, callback, method):
        if coroutines.iscoroutine(callback) or coroutines.iscoroutinefunction(
            callback):
            raise TypeError('coroutines cannot be used with {}()'.format(
                method))
        if not callable(callback):
            raise TypeError('a callable object was expected by {}(), got {!r}'
                .format(method, callback))
    ...
    def _getaddrinfo_debug(self, host, port, family, type, proto, flags):
        ...
        dt = self.time() - t0
        msg = 'Getting address info %s took %.3f ms: %r' % (msg, dt * 
            1000.0, addrinfo)
    ...
    def getaddrinfo(self, host, port, *, family=0, type=0, proto=0, flags=0):
        if self._debug:
            return self.run_in_executor(None, self._getaddrinfo_debug, host,
                port, family, type, proto, flags)
        else:
            return self.run_in_executor(None, socket.getaddrinfo, host,
                port, family, type, proto, flags)
    ...
    def create_connection(self, protocol_factory, host=None, port=None, *,
        ...
        if host is not None or port is not None:
            ...
            f1 = _ensure_resolved((host, port), family=family, type=socket.
                SOCK_STREAM, proto=proto, flags=flags, loop=self)
            fs = [f1]
            if local_addr is not None:
                f2 = _ensure_resolved(local_addr, family=family, type=
                    socket.SOCK_STREAM, proto=proto, flags=flags, loop=self)
                fs.append(f2)
            else:
                f2 = None
            ...
            if not _is_stream_socket(sock):
                raise ValueError('A Stream Socket was expected, got {!r}'.
                    format(sock))
        transport, protocol = yield from self._create_connection_transport(sock
            , protocol_factory, ssl, server_hostname)
        if self._debug:
            sock = transport.get_extra_info('socket')
            logger.debug('%r connected to %s:%r: (%r, %r)', sock, host,
    ...
    def _create_connection_transport(self, sock, protocol_factory, ssl,
        ...
        if ssl:
            ...
            transport = self._make_ssl_transport(sock, protocol, sslcontext,
                waiter, server_side=server_side, server_hostname=
                server_hostname)
        else:
    ...
    def create_datagram_endpoint(self, protocol_factory, local_addr=None,
        ...
        if sock is not None:
            if not _is_dgram_socket(sock):
                raise ValueError('A UDP Socket was expected, got {!r}'.
                    format(sock))
            if (local_addr or remote_addr or family or proto or flags or
                reuse_address or reuse_port or allow_broadcast):
                opts = dict(local_addr=local_addr, remote_addr=remote_addr,
                    family=family, proto=proto, flags=flags, reuse_address=
                    reuse_address, reuse_port=reuse_port, allow_broadcast=
                    allow_broadcast)
                problems = ', '.join('{}={}'.format(k, v) for k, v in opts.
                    items() if v)
                raise ValueError(
            ...
            if not (local_addr or remote_addr):
                ...
                for idx, addr in ((0, local_addr), (1, remote_addr)):
                    if addr is not None:
                        assert isinstance(addr, tuple) and len(addr
                            ) == 2, '2-tuple is expected'
                        infos = yield from _ensure_resolved(addr, family=
                            family, type=socket.SOCK_DGRAM, proto=proto,
                ...
                addr_pairs_info = [(key, addr_pair) for key, addr_pair in
                    addr_infos.items() if not (local_addr and addr_pair[0] is
                    None or remote_addr and addr_pair[1] is None)]
                if not addr_pairs_info:
                    raise ValueError('can not get address information')
            exceptions = []
            if reuse_address is None:
                reuse_address = os.name == 'posix' and sys.platform != 'cygwin'
            for (family, proto), (local_address, remote_address
                ) in addr_pairs_info:
                sock = None
                r_addr = None
                try:
                    sock = socket.socket(family=family, type=socket.
                        SOCK_DGRAM, proto=proto)
                    if reuse_address:
    ...
    def _create_server_getaddrinfo(self, host, port, family, flags):
        infos = yield from _ensure_resolved((host, port), family=family,
            type=socket.SOCK_STREAM, flags=flags, loop=self)
        if not infos:
    ...
    def create_server(self, protocol_factory, host=None, port=None, *,
        ...
        if host is not None or port is not None:
            ...
            if host == '':
                hosts = [None]
            elif isinstance(host, str) or not isinstance(host, collections.
                Iterable):
                hosts = [host]
            else:
                hosts = host
            fs = [self._create_server_getaddrinfo(host, port, family=family,
                flags=flags) for host in hosts]
            ...
            try:
                for res in infos:
                    ...
                    if af == AF_INET6 and hasattr(socket, 'IPPROTO_IPV6'):
                        sock.setsockopt(socket.IPPROTO_IPV6, socket.
                            IPV6_V6ONLY, True)
                    try:
                        sock.bind(sa)
    ...
    @coroutine
    def subprocess_shell(self, protocol_factory, cmd, *, stdin=subprocess.
        ...
        transport = yield from self._make_subprocess_transport(protocol,
            cmd, True, stdin, stdout, stderr, bufsize, **kwargs)
        if self._debug:
            logger.info('%s: %r', debug_log, transport)
        return transport, protocol

    @coroutine
    def subprocess_exec(self, protocol_factory, program, *args, stdin=
        ...
        if self._debug:
            ...
            self._log_subprocess(debug_log, stdin, stdout, stderr)
        transport = yield from self._make_subprocess_transport(protocol,
            popen_args, False, stdin, stdout, stderr, bufsize, **kwargs)
        if self._debug:
            logger.info('%s: %r', debug_log, transport)
        return transport, protocol
    ...
    def set_exception_handler(self, handler):
        ...
        if handler is not None and not callable(handler):
            raise TypeError('A callable object or None is expected, got {!r}'
                .format(handler))
    ...
    def default_exception_handler(self, context):
        ...
        if exception is not None:
            ...
            exc_info = False
        if ('source_traceback' not in context and self._current_handle is not
            None and self._current_handle._source_traceback):
    ...
    def call_exception_handler(self, context):
        ...
        if self._exception_handler is None:
            ...
            try:
                ...
                try:
                    self.default_exception_handler({'message':
                        'Unhandled error in exception handler', 'exception':
                        exc, 'context': context})
                except Exception:
                    logger.error(
    ...
    def _run_once(self):
        ...
        if (sched_count > _MIN_SCHEDULED_TIMER_HANDLES and self.
            _timer_cancelled_count / sched_count >
        ...
        if self._debug and timeout != 0:
            ...
            if timeout is None:
                logger.log(level, 'poll took %.3f ms: %s events', dt * 
                    1000.0, nevent)
            elif nevent:
                logger.log(level, 'poll %.3f ms took %.3f ms: %s events', 
                    timeout * 1000.0, dt * 1000.0, nevent)
            elif dt >= 1.0:
                logger.log(level, 'poll %.3f ms took %.3f ms: timeout', 
                    timeout * 1000.0, dt * 1000.0)
        else:
            event_list = self._selector.select(timeout)
