    ...
    def close(self):
        ...
        for proto in self._pipes.values():
            if proto is None:
                continue
            proto.pipe.close()
        if (self._proc is not None and self._returncode is None and self.
            _proc.poll() is None):
            if self._loop.get_debug():
    ...
    def _connect_pipes(self, waiter):
        try:
            ...
            if proc.stdin is not None:
                _, pipe = yield from loop.connect_write_pipe(lambda :
                    WriteSubprocessPipeProto(self, 0), proc.stdin)
                self._pipes[0] = pipe
            if proc.stdout is not None:
                _, pipe = yield from loop.connect_read_pipe(lambda :
                    ReadSubprocessPipeProto(self, 1), proc.stdout)
                self._pipes[1] = pipe
            if proc.stderr is not None:
                _, pipe = yield from loop.connect_read_pipe(lambda :
                    ReadSubprocessPipeProto(self, 2), proc.stderr)
                self._pipes[2] = pipe
...
class WriteSubprocessPipeProto(protocols.BaseProtocol):
    ...
    def __repr__(self):
        return '<%s fd=%s pipe=%r>' % (self.__class__.__name__, self.fd,
            self.pipe)
