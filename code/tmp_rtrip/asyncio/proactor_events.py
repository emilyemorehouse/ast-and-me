...
class _ProactorReadPipeTransport(_ProactorBasePipeTransport, transports.
    ReadTransport):
...
class _ProactorBaseWritePipeTransport(_ProactorBasePipeTransport,
    ...
    def write(self, data):
        if not isinstance(data, (bytes, bytearray, memoryview)):
            msg = "data argument must be a bytes-like object, not '%s'" % type(
                data).__name__
            raise TypeError(msg)
        if self._eof_written:
...
class _ProactorDuplexPipeTransport(_ProactorReadPipeTransport,
    _ProactorBaseWritePipeTransport, transports.Transport):
...
class BaseProactorEventLoop(base_events.BaseEventLoop):
    ...
    def _make_write_pipe_transport(self, sock, protocol, waiter=None, extra
        =None):
            ...
            try:
                if f is not None:
                    ...
                    if sslcontext is not None:
                        self._make_ssl_transport(conn, protocol, sslcontext,
                            server_side=True, extra={'peername': addr},
                            server=server)
                    else:
                        self._make_socket_transport(conn, protocol, extra={
                            'peername': addr}, server=server)
                if self.is_closed():
                    return
                ...
                if sock.fileno() != -1:
                    self.call_exception_handler({'message':
                        'Accept failed on a socket', 'exception': exc,
                        'socket': sock})
                    sock.close()
                elif self._debug:
