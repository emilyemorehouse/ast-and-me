...
__all__ = ['Task', 'FIRST_COMPLETED', 'FIRST_EXCEPTION', 'ALL_COMPLETED',
    'wait', 'wait_for', 'as_completed', 'sleep', 'async', 'gather',
    'shield', 'ensure_future', 'run_coroutine_threadsafe']
...
class Task(futures.Future):
    ...
    def _step(self, exc=None):
        assert not self.done(), '_step(): already done: {!r}, {!r}'.format(self
            , exc)
        ...
        try:
            ...
            if blocking is not None:
                if result._loop is not self._loop:
                    self._loop.call_soon(self._step, RuntimeError(
...
def _wait(fs, timeout, return_when, loop):
    ...
    def _on_completion(f):
        nonlocal counter
        counter -= 1
        if (counter <= 0 or return_when == FIRST_COMPLETED or return_when ==
            FIRST_EXCEPTION and (not f.cancelled() and f.exception() is not
            None)):
