...
__all__ = ['SelectorEventLoop', 'AbstractChildWatcher', 'SafeChildWatcher',
...
class _UnixSelectorEventLoop(selector_events.BaseSelectorEventLoop):
    ...
    def _make_subprocess_transport(self, protocol, args, shell, stdin,
        ...
        with events.get_child_watcher() as watcher:
            ...
            transp = _UnixSubprocessTransport(self, protocol, args, shell,
                ...
                **kwargs)
            watcher.add_child_handler(transp.get_pid(), self.
    ...
    @coroutine
    def create_unix_connection(self, protocol_factory, path, *, ssl=None,
        ...
        if path is not None:
            ...
            if (sock.family != socket.AF_UNIX or not base_events.
                _is_stream_socket(sock)):
                raise ValueError(
    ...
    @coroutine
    def create_unix_server(self, protocol_factory, path=None, *, sock=None,
...
class _UnixReadPipeTransport(transports.ReadTransport):
    ...
    def __init__(self, loop, pipe, protocol, waiter=None, extra=None):
        ...
        if not (stat.S_ISFIFO(mode) or stat.S_ISSOCK(mode) or stat.S_ISCHR(
            mode)):
            self._pipe = None
            self._fileno = None
            self._protocol = None
        ...
        self._loop.call_soon(self._loop._add_reader, self._fileno, self.
            _read_ready)
    ...
    def __repr__(self):
        ...
        if self._pipe is not None and selector is not None:
            polling = selector_events._test_selector_event(selector, self.
                _fileno, selectors.EVENT_READ)
            if polling:
                info.append('polling')
            else:
                info.append('idle')
...
class _UnixWritePipeTransport(transports._FlowControlMixin, transports.
    WriteTransport):
...
class _UnixSubprocessTransport(base_subprocess.BaseSubprocessTransport):
    ...
    def _start(self, args, shell, stdin, stdout, stderr, bufsize, **kwargs):
        ...
        self._proc = subprocess.Popen(args, shell=shell, stdin=stdin,
            stdout=stdout, stderr=stderr, universal_newlines=False, bufsize
            =bufsize, **kwargs)
    ...
    def _sig_chld(self):
        try:
            ...
            self._loop.call_exception_handler({'message':
                'Unknown exception in SIGCHLD handler', 'exception': exc})
...
class FastChildWatcher(BaseChildWatcher):
    ...
    def __exit__(self, a, b, c):
        with self._lock:
            ...
            self._zombies.clear()
        logger.warning('Caught subprocesses termination from unknown pids: %s',
            collateral_victims)
...
class _UnixDefaultEventLoopPolicy(events.BaseDefaultEventLoopPolicy):
    ...
    def set_event_loop(self, loop):
        ...
        if self._watcher is not None and isinstance(threading.
            current_thread(), threading._MainThread):
