...
class OrderedDict(dict):
    ...
    def __setitem__(self, key, value, dict_setitem=dict.__setitem__, proxy=
        _proxy, Link=_Link):
...
def namedtuple(typename, field_names, *, verbose=False, rename=False,
    ...
    if rename:
        ...
        for index, name in enumerate(field_names):
            if not name.isidentifier() or _iskeyword(name) or name.startswith(
                '_') or name in seen:
    ...
    for name in ([typename] + field_names):
        ...
        if not name.isidentifier():
            raise ValueError(
                'Type names and field names must be valid identifiers: %r' %
                name)
        if _iskeyword(name):
            raise ValueError(
    ...
    class_definition = _class_template.format(typename=typename,
        field_names=tuple(field_names), num_fields=len(field_names),
        arg_list=repr(tuple(field_names)).replace("'", '')[1:-1], repr_fmt=
        ', '.join(_repr_template.format(name=name) for name in field_names),
        field_defs='\n'.join(_field_template.format(index=index, name=name) for
        index, name in enumerate(field_names)))
...
class ChainMap(MutableMapping):
    ...
    @_recursive_repr()
    def __repr__(self):
        return '{0.__class__.__name__}({1})'.format(self, ', '.join(map(
            repr, self.maps)))
