...
import sys
import warnings
__all__ = ['NoSectionError', 'DuplicateOptionError',
    ...
    'ConfigParser', 'SafeConfigParser', 'RawConfigParser', 'Interpolation',
...
class InterpolationMissingOptionError(InterpolationError):
    ...
    def __init__(self, option, section, rawval, reference):
        msg = (
            'Bad value substitution: option {!r} in section {!r} contains an interpolation key {!r} which is not a valid option name. Raw value: {!r}'
            .format(option, section, reference, rawval))
...
class InterpolationDepthError(InterpolationError):
    ...
    def __init__(self, option, section, rawval):
        msg = (
            'Recursion limit exceeded in value substitution: option {!r} in section {!r} contains an interpolation key which cannot be substituted in {} steps. Raw value: {!r}'
        ...
        while rest:
            ...
            if c == '$':
                ...
                try:
                    ...
                    raise InterpolationMissingOptionError(option, section,
                        rawval, ':'.join(path)) from None
                if '$' in v:
                    self._interpolate_some(parser, opt, accum, v, sect,
                        dict(parser.items(sect, raw=True)), depth + 1)
                else:
                    accum.append(v)
            else:
                raise InterpolationSyntaxError(option, section, 
...
class RawConfigParser(MutableMapping):
    ...
    BOOLEAN_STATES = {'1': True, 'yes': True, 'true': True, 'on': True, '0':
    ...
    def getint(self, section, option, *, raw=False, vars=None, fallback=
        _UNSET, **kwargs):
        return self._get_conv(section, option, int, raw=raw, vars=vars,
            fallback=fallback, **kwargs)

    def getfloat(self, section, option, *, raw=False, vars=None, fallback=
        _UNSET, **kwargs):
        return self._get_conv(section, option, float, raw=raw, vars=vars,
            fallback=fallback, **kwargs)

    def getboolean(self, section, option, *, raw=False, vars=None, fallback
        =_UNSET, **kwargs):
    ...
    def write(self, fp, space_around_delimiters=True):
        ...
        if self._defaults:
            self._write_section(fp, self.default_section, self._defaults.
                items(), d)
        ...
        for lineno, line in enumerate(fp, start=1):
            ...
            if not value:
                if self._empty_lines_in_values:
                    if (comment_start is None and cursect is not None and
                        optname and cursect[optname] is not None):
            ...
            if (cursect is not None and optname and cur_indent_level >
                indent_level):
                ...
                if mo:
                    ...
                    if mo:
                        ...
                        if self._strict and (sectname, optname
                            ) in elements_added:
...
class SectionProxy(MutableMapping):
    ...
    def __delitem__(self, key):
        if not (self._parser.has_option(self._name, key) and self._parser.
            remove_option(self._name, key)):
            raise KeyError(key)
    ...
    def get(self, option, fallback=None, *, raw=False, vars=None, _impl=
        None, **kwargs):
        """Get an option value.
        ...
        return _impl(self._name, option, raw=raw, vars=vars, fallback=
            fallback, **kwargs)
...
class ConverterMapping(MutableMapping):
    ...
    def __setitem__(self, key, value):
        try:
            ...
            raise ValueError('Incompatible key: {} (type: {})'.format(key,
                type(key)))
        if k == 'get':
