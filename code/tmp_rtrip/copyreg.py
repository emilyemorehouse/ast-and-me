...
def add_extension(module, name, code):
    ...
    key = module, name
    if _extension_registry.get(key) == code and _inverted_registry.get(code
        ) == key:
        return
    if key in _extension_registry:
        raise ValueError('key %s is already registered with code %s' % (key,
            _extension_registry[key]))
    if code in _inverted_registry:
        raise ValueError('code %s is already in use for key %s' % (code,
...
def remove_extension(module, name, code):
    ...
    key = module, name
    if _extension_registry.get(key) != code or _inverted_registry.get(code
        ) != key:
