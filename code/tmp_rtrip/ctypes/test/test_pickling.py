...
class PickleTest:
    ...
    def test_simple(self):
        for src in [c_int(42), c_double(3.14)]:
            ...
            self.assertEqual(memoryview(src).tobytes(), memoryview(dst).
                tobytes())
    ...
    def test_unpickable(self):
        ...
        for item in [c_char_p(), c_wchar_p(), c_void_p(), pointer(c_int(42)
            ), dll._testfunc_p_p, prototype(lambda : 42)]:
...
for proto in range(pickle.HIGHEST_PROTOCOL + 1):
    ...
    globals()[name] = type(name, (PickleTest, unittest.TestCase), {'proto':
        proto})
