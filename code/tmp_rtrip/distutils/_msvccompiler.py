...
def _find_vcvarsall(plat_spec):
    try:
        key = winreg.OpenKeyEx(winreg.HKEY_LOCAL_MACHINE,
            'Software\\Microsoft\\VisualStudio\\SxS\\VC7', access=winreg.
            KEY_READ | winreg.KEY_WOW64_32KEY)
    except OSError:
    ...
    with key:
        ...
        if vcruntime_spec:
            vcruntime = os.path.join(best_dir, vcruntime_spec.format(
                best_version))
...
def _get_vc_env(plat_spec):
    ...
    try:
        out = subprocess.check_output('cmd /u /c "{}" {} && set'.format(
            vcvarsall, plat_spec), stderr=subprocess.STDOUT).decode('utf-16le',
            errors='replace')
    except subprocess.CalledProcessError as exc:
        log.error(exc.output)
        raise DistutilsPlatformError('Error executing {}'.format(exc.cmd))
    env = {key.lower(): value for key, _, value in (line.partition('=') for
        line in out.splitlines()) if key and value}
    if vcruntime:
...
_VCVARS_PLAT_TO_VCRUNTIME_REDIST = {'x86':
    'redist\\x86\\Microsoft.VC{0}0.CRT\\vcruntime{0}0.dll', 'amd64':
    'redist\\x64\\Microsoft.VC{0}0.CRT\\vcruntime{0}0.dll', 'x86_amd64':
    'redist\\x64\\Microsoft.VC{0}0.CRT\\vcruntime{0}0.dll'}
...
class MSVCCompiler(CCompiler):
    ...
    def initialize(self, plat_name=None):
        ...
        self.compile_options_debug = ['/nologo', '/Od', '/MDd', '/Zi',
            '/W3', '/D_DEBUG']
        ...
        self._ldflags = {(CCompiler.EXECUTABLE, None): self.ldflags_exe, (
            CCompiler.EXECUTABLE, False): self.ldflags_exe, (CCompiler.
            EXECUTABLE, True): self.ldflags_exe_debug, (CCompiler.
            SHARED_OBJECT, None): self.ldflags_shared, (CCompiler.
            SHARED_OBJECT, False): self.ldflags_shared, (CCompiler.
            SHARED_OBJECT, True): self.ldflags_shared_debug, (CCompiler.
            SHARED_LIBRARY, None): self.ldflags_static, (CCompiler.
            SHARED_LIBRARY, False): self.ldflags_static, (CCompiler.
            SHARED_LIBRARY, True): self.ldflags_static_debug}
        self.initialized = True

    def object_filenames(self, source_filenames, strip_dir=0, output_dir=''):
        ext_map = {**{ext: self.obj_extension for ext in self.
            src_extensions}, **{ext: self.res_extension for ext in self.
            _rc_extensions + self._mc_extensions}}
    ...
    def compile(self, sources, output_dir=None, macros=None, include_dirs=
        None, debug=0, extra_preargs=None, extra_postargs=None, depends=None):
    ...
    def create_static_lib(self, objects, output_libname, output_dir=None,
        ...
        output_filename = self.library_filename(output_libname, output_dir=
            output_dir)
        ...
        if self._need_link(objects, output_filename):
            ...
            if export_symbols is not None:
                dll_name, dll_ext = os.path.splitext(os.path.basename(
                    output_filename))
                implib_file = os.path.join(build_temp, self.
