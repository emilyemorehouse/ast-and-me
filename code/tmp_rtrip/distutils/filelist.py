...
class FileList:
    ...
    def process_template_line(self, line):
        ...
        if action == 'include':
            ...
            self.debug_print('recursive-include %s %s' % (dir, ' '.join(
                patterns)))
            for pattern in patterns:
            ...
            self.debug_print('recursive-exclude %s %s' % (dir, ' '.join(
                patterns)))
            for pattern in patterns:
...
def _find_all_simple(path):
    ...
    """
    results = (os.path.join(base, file) for base, dirs, files in os.walk(
        path, followlinks=True) for file in files)
...
def translate_pattern(pattern, anchor=1, prefix=None, is_regex=0):
    ...
    if prefix is not None:
        ...
        pattern_re = '%s\\A%s%s.*%s%s' % (start, prefix_re, sep, pattern_re,
            end)
    elif anchor:
