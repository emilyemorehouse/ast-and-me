...
HKEYS = (winreg.HKEY_USERS, winreg.HKEY_CURRENT_USER, winreg.
    HKEY_LOCAL_MACHINE, winreg.HKEY_CLASSES_ROOT)
...
PLAT_TO_VCVARS = {'win32': 'x86', 'win-amd64': 'amd64', 'win-ia64': 'ia64'}
...
class MacroExpander:
    ...
    def load_macros(self, version):
        self.set_macro('VCInstallDir', self.vsbase + '\\Setup\\VC',
            'productdir')
        self.set_macro('VSInstallDir', self.vsbase + '\\Setup\\VS',
            'productdir')
...
def query_vcvarsall(version, arch='x86'):
    ...
    popen = subprocess.Popen('"%s" %s & set' % (vcvarsall, arch), stdout=
...
class MSVCCompiler(CCompiler):
    ...
    def initialize(self, plat_name=None):
        ...
        if ('DISTUTILS_USE_SDK' in os.environ and 'MSSdk' in os.environ and
            ...
            if plat_name == get_platform() or plat_name == 'win32':
                ...
                plat_spec = PLAT_TO_VCVARS[get_platform()
                    ] + '_' + PLAT_TO_VCVARS[plat_name]
    ...
    def manifest_setup_ldargs(self, output_filename, build_temp, ld_args):
        temp_manifest = os.path.join(build_temp, os.path.basename(
            output_filename) + '.manifest')
