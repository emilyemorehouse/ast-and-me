...
class ArchiveUtilTestCase(support.TempdirManager, support.LoggingSilencer,
    ...
    def _tarinfo(self, path):
        ...
        try:
            ...
            tar.close()
    _created_files = ('dist', 'dist/file1', 'dist/file2', 'dist/sub',
        'dist/sub/file3', 'dist/sub2')
    ...
    @unittest.skipUnless(find_executable('tar') and find_executable('gzip') and
        ZLIB_SUPPORT, 'Need the tar, gzip and zlib command to run')
    ...
    def test_make_zipfile(self):
        ...
        with zipfile.ZipFile(tarball) as zf:
            self.assertEqual(sorted(zf.namelist()), ['dist/file1',
                'dist/file2', 'dist/sub/file3'])
    ...
    def test_make_zipfile_no_zlib(self):
        ...
        with change_cwd(tmpdir):
            make_zipfile(base_name, 'dist')
        tarball = base_name + '.zip'
        self.assertEqual(called, [((tarball, 'w'), {'compression': zipfile.
            ZIP_STORED})])
        self.assertTrue(os.path.exists(tarball))
        with zipfile.ZipFile(tarball) as zf:
            self.assertEqual(sorted(zf.namelist()), ['dist/file1',
