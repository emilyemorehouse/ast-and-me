...
class BuildExtTestCase(TempdirManager, LoggingSilencer, unittest.TestCase):
    ...
    def test_check_extensions_list(self):
        ...
        exts = [('foo.bar', '')]
        self.assertRaises(DistutilsSetupError, cmd.check_extensions_list, exts)
        exts = [('foo.bar', {'sources': [''], 'libraries': 'foo', 'some':
            'bar'})]
    ...
    def test_deployment_target_too_low(self):
        self.assertRaises(DistutilsPlatformError, self.
            _try_compile_deployment_target, '>', '10.1')
    ...
    def _try_compile_deployment_target(self, operator, target):
        ...
        deptarget_ext = Extension('deptarget', [deptarget_c],
            extra_compile_args=['-DTARGET=%s' % (target,)])
        dist = Distribution({'name': 'deptarget', 'ext_modules': [
            deptarget_ext]})
        dist.package_dir = self.tmp_dir
        cmd = self.build_ext(dist)
        cmd.build_lib = self.tmp_dir
