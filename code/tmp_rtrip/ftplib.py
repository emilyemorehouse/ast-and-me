...
class FTP:
    ...
    def connect(self, host='', port=0, timeout=-999, source_address=None):
        ...
        if source_address is not None:
            self.source_address = source_address
        self.sock = socket.create_connection((self.host, self.port), self.
            timeout, source_address=self.source_address)
    ...
    def makeport(self):
        ...
        err = None
        sock = None
        for res in socket.getaddrinfo(None, 0, self.af, socket.SOCK_STREAM,
            0, socket.AI_PASSIVE):
    ...
    def retrlines(self, cmd, callback=None):
        ...
        with self.transfercmd(cmd) as conn, conn.makefile('r', encoding=
            self.encoding) as fp:
            while 1:
...
try:
    ...
    class FTP_TLS(FTP):
        ...
        def __init__(self, host='', user='', passwd='', acct='', keyfile=
            None, certfile=None, context=None, timeout=
            _GLOBAL_DEFAULT_TIMEOUT, source_address=None):
        ...
        def auth(self):
            ...
            self.sock = self.context.wrap_socket(self.sock, server_hostname
                =self.host)
