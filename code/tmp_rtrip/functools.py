...
__all__ = ['update_wrapper', 'wraps', 'WRAPPER_ASSIGNMENTS',
    'WRAPPER_UPDATES', 'total_ordering', 'cmp_to_key', 'lru_cache',
...
class partialmethod(object):
    ...
    def __repr__(self):
        ...
        keywords = ', '.join('{}={!r}'.format(k, v) for k, v in self.
            keywords.items())
        format_string = '{module}.{cls}({func}, {args}, {keywords})'
        return format_string.format(module=self.__class__.__module__, cls=
            self.__class__.__qualname__, func=self.func, args=args,
            keywords=keywords)
...
def _make_key(args, kwds, typed, kwd_mark=(object(),), fasttypes={int, str,
    frozenset, type(None)}, tuple=tuple, type=type, len=len):
...
def _c3_mro(cls, abcs=None):
    ...
    for base in abcs:
        if issubclass(cls, base) and not any(issubclass(b, base) for b in
            cls.__bases__):
    ...
    return _c3_merge([[cls]] + explicit_c3_mros + abstract_c3_mros +
        other_c3_mros + [explicit_bases] + [abstract_bases] + [other_bases])
...
def _compose_mro(cls, types):
    ...
    def is_related(typ):
        return typ not in bases and hasattr(typ, '__mro__') and issubclass(cls,
            typ)
    ...
    *object* type, this function may return None.
    ...
    for t in mro:
        if match is not None:
            if (t in registry and t not in cls.__mro__ and match not in cls
                .__mro__ and not issubclass(match, t)):
