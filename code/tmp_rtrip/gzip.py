...
class _PaddedFile:
    ...
    def read(self, size):
        ...
        if self._read + size <= self._length:
            ...
            read = self._read
            self._read = None
            return self._buffer[read:] + self.file.read(size - self._length +
                read)
...
class GzipFile(_compression.BaseStream):
    ...
    def __init__(self, filename=None, mode=None, compresslevel=9, fileobj=
        None, mtime=None):
...
class _GzipReader(_compression.DecompressReader):
    ...
    def __init__(self, fp):
        super().__init__(_PaddedFile(fp), zlib.decompressobj, wbits=-zlib.
            MAX_WBITS)
    ...
    def _read_gzip_header(self):
        ...
        method, flag, self._last_mtime = struct.unpack('<BBIxx', self.
            _read_exact(8))
        if method != 8:
    ...
    def _read_eof(self):
        ...
        if crc32 != self._crc:
            raise OSError('CRC check failed %s != %s' % (hex(crc32), hex(
                self._crc)))
