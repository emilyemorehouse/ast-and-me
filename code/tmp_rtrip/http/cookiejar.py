...
def _timegm(tt):
    ...
    if (year >= EPOCH_YEAR and 1 <= month <= 12 and 1 <= mday <= 31 and 0 <=
        hour <= 24 and 0 <= min <= 59 and 0 <= sec <= 61):
        return timegm(tt)
    else:
        return None
...
def time2isoz(t=None):
    ...
    return '%04d-%02d-%02d %02d:%02d:%02dZ' % (dt.year, dt.month, dt.day,
...
def time2netscape(t=None):
    ...
    if t is None:
        ...
        dt = datetime.datetime.utcfromtimestamp(t)
    return '%s, %02d-%s-%04d %02d:%02d:%02d GMT' % (DAYS[dt.weekday()], dt.
...
STRICT_DATE_RE = re.compile(
    ...
    , re.ASCII)
WEEKDAY_RE = re.compile('^(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat)[a-z]*,?\\s*', re.
    I | re.ASCII)
...
class DefaultCookiePolicy(CookiePolicy):
    ...
    def __init__(self, blocked_domains=None, allowed_domains=None, netscape
        =True, rfc2965=False, rfc2109_as_netscape=None, hide_cookie2=False,
        strict_domain=False, strict_rfc2965_unverifiable=True,
        strict_ns_unverifiable=False, strict_ns_domain=DomainLiberal,
    ...
    def set_ok_path(self, cookie, request):
        if cookie.path_specified:
            ...
            if (cookie.version > 0 or cookie.version == 0 and self.
                strict_ns_set_path) and not req_path.startswith(cookie.path):
                _debug(
                    '   path attribute %s is not a prefix of request path %s',
                    cookie.path, req_path)
                return False
        return True
        ...
        if cookie.domain_specified:
            ...
            if self.strict_domain and domain.count('.') >= 2:
                ...
                if j == 0:
                    tld = domain[i + 1:]
                    sld = domain[j + 1:i]
                    if sld.lower() in ('co', 'ac', 'com', 'edu', 'org',
                        'net', 'gov', 'mil', 'int', 'aero', 'biz', 'cat',
            ...
            if not embedded_dots and domain != '.local':
                ...
                return False
            if cookie.version == 0:
                if not erhn.endswith(domain) and (not erhn.startswith('.') and
                    not ('.' + erhn).endswith(domain)):
                    _debug(
                        '   effective request-host %s (even with added initial dot) does not end with %s'
                        , erhn, domain)
                    return False
            if (cookie.version > 0 or self.strict_ns_domain & self.
                DomainRFC2965Match):
    ...
    def set_ok_port(self, cookie, request):
        if cookie.port_specified:
            ...
            for p in cookie.port.split(','):
                ...
                _debug('   request port (%s) not found in %s', req_port,
                    cookie.port)
                return False
        return True
    ...
    def return_ok_domain(self, cookie, request):
        req_host, erhn = eff_request_host(request)
        domain = cookie.domain
        if (cookie.version == 0 and self.strict_ns_domain & self.
            DomainStrictNonDomain and not cookie.domain_specified and 
            domain != erhn):
            _debug(
...
class CookieJar:
    ...
    def _cookie_attrs(self, cookies):
        ...
        for cookie in cookies:
            ...
            if cookie.value is not None and self.non_word_re.search(cookie.
                value) and version > 0:
    ...
    def _cookie_from_cookie_tuple(self, tup, request):
        ...
        if expires is Absent:
            ...
            _debug("Expiring cookie, domain='%s', path='%s', name='%s'",
                domain, path, name)
            return None
        return Cookie(version, name, value, port, port_specified, domain,
    ...
    def make_cookies(self, response, request):
        ...
        if (not rfc2965_hdrs and not ns_hdrs or not ns_hdrs and not rfc2965 or
            not rfc2965_hdrs and not netscape or not netscape and not rfc2965):
            return []
        try:
            cookies = self._cookies_from_attrs_set(split_header_words(
                rfc2965_hdrs), request)
        except Exception:
            _warn_unhandled_exception()
            cookies = []
        if ns_hdrs and netscape:
            try:
                ns_cookies = self._cookies_from_attrs_set(parse_ns_headers(
                    ns_hdrs), request)
            except Exception:
                _warn_unhandled_exception()
                ns_cookies = []
...
def lwp_cookie_str(cookie):
    ...
    """
    h = [(cookie.name, cookie.value), ('path', cookie.path), ('domain',
        cookie.domain)]
    if cookie.port is not None:
...
class LWPCookieJar(FileCookieJar):
    ...
    def _really_load(self, f, filename, ignore_discard, ignore_expires):
        ...
        if not self.magic_re.search(magic):
            msg = ('%r does not look like a Set-Cookie3 (LWP) format file' %
                filename)
            raise LoadError(msg)
        now = time.time()
        header = 'Set-Cookie3:'
        ...
        try:
            while 1:
                ...
                for data in split_header_words([line]):
                    ...
                    c = Cookie(h('version'), name, value, h('port'), h(
                        'port_spec'), domain, domain_specified, h(
                        'domain_dot'), h('path'), h('path_spec'), h(
                        'secure'), expires, discard, h('comment'), h(
                        'commenturl'), rest)
