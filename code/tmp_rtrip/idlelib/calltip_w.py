...
class CallTip:
    ...
    def showtip(self, text, parenleft, parenright):
        ...
        self.parenline, self.parencol = map(int, self.widget.index(
            parenleft).split('.'))
        ...
        try:
            ...
            pass
        self.label = Label(tw, text=self.text, justify=LEFT, background=
            '#ffffe0', relief=SOLID, borderwidth=1, font=self.widget['font'])
        self.label.pack()
        tw.lift()
    ...
    def checkhide_event(self, event=None):
        ...
        if (curline < self.parenline or curline == self.parenline and 
            curcol <= self.parencol or self.widget.compare('insert', '>',
            MARK_RIGHT)):
            self.hidetip()
        else:
            self.position_window()
            ...
            self.checkhide_after_id = self.widget.after(CHECKHIDE_TIME,
                self.checkhide_event)
