...
class EditorWindow(object):
    ...
    def __init__(self, flist=None, filename=None, key=None, root=None):
        ...
        self.width = idleConf.GetOption('main', 'EditorWindow', 'width',
            type='int')
        text_options = {'name': 'text', 'padx': 5, 'wrap': 'none',
            'highlightthickness': 0, 'width': self.width, 'tabstyle':
            'wordprocessor', 'height': idleConf.GetOption('main',
            'EditorWindow', 'height', type='int')}
        ...
        text.focus_set()
        usespaces = idleConf.GetOption('main', 'Indent', 'use-spaces', type
            ='bool')
    ...
    def _filename_to_unicode(self, filename):
        ...
        return re.sub('[𐀀-\U0010ffff]', '�', filename)
    ...
    def home_callback(self, event):
        if event.state & 4 != 0 and event.keysym == 'Home':
            return
        if self.text.index('iomark') and self.text.compare('iomark', '<=',
            'insert lineend') and self.text.compare('insert linestart',
            '<=', 'iomark'):
        ...
        if event.state & 1 == 0:
            ...
            elif self.text.compare(self.text.index('sel.first'), '<', self.
                text.index('insert')):
    ...
    menu_specs = [('file', '_File'), ('edit', '_Edit'), ('format',
        'F_ormat'), ('run', '_Run'), ('options', '_Options'), ('windows',
        '_Window'), ('help', '_Help')]
    ...
    def open_module(self, event=None):
        ...
        try:
            ...
            name = ''
        file_path = query.ModuleName(self.text, 'Open Module',
...
to search on sys.path and open:"""
    ...
    def set_notabs_indentwidth(self):
        ...
        if not self.usetabs:
            self.indentwidth = idleConf.GetOption('main', 'Indent',
                'num-spaces', type='int')
    ...
    def update_recent_files_list(self, new_file=None):
        ...
        if os.path.exists(self.recent_files_path):
            with open(self.recent_files_path, 'r', encoding='utf_8', errors
                ='replace') as rf_list_file:
        ...
        try:
            with open(self.recent_files_path, 'w', encoding='utf_8', errors
                ='replace') as rf_file:
                rf_file.writelines(rf_list)
        except OSError as err:
            if not getattr(self.root, 'recentfilelist_error_displayed', False):
                self.root.recentfilelist_error_displayed = True
                tkMessageBox.showwarning(title='IDLE Warning', message=
                    """Cannot update File menu Recent Files list. Your operating system says:
%s
Select OK and IDLE will continue without updating."""
                     % self._filename_to_unicode(str(err)), parent=self.text)
    ...
    extfiles = {'AutoComplete': 'autocomplete', 'AutoExpand': 'autoexpand',
        'CallTips': 'calltips', 'CodeContext': 'codecontext',
        'FormatParagraph': 'paragraph', 'ParenMatch': 'parenmatch',
        'RstripExtension': 'rstrip', 'ScriptBinding': 'runscript',
            ...
            for entry in entrylist:
                if not entry:
                    ...
                    if checkbutton:
                        ...
                        menu.add_checkbutton(label=label, underline=
                            underline, command=command, accelerator=
                            accelerator, variable=var)
                    else:
    ...
    def toggle_tabs_event(self, event):
        if self.askyesno('Toggle tabs', 'Turn tabs ' + ('on', 'off')[self.
            usetabs] + '?\nIndent width ' + ('will be', 'remains at')[self.
            usetabs] + ' 8.' + """
 Note: a tab is always 8 columns""",
    ...
    def change_indentwidth_event(self, event):
        new = self.askinteger('Indent width',
            """New indent width (2-16)
(Always use 8 when using tabs)""",
            parent=self.text, initialvalue=self.indentwidth, minvalue=2,
            maxvalue=16)
    ...
    def _asktabwidth(self):
        return self.askinteger('Tab width', 'Columns per tab? (2-16)',
            parent=self.text, initialvalue=self.indentwidth, minvalue=2,
            maxvalue=16)
...
class IndentSearcher(object):
    ...
    def tokeneater(self, type, token, start, end, line, INDENT=tokenize.
        INDENT, NAME=tokenize.NAME, OPENERS=('class', 'def', 'for', 'if',
        'try', 'while')):
        if self.finished:
            pass
...
def get_accelerator(keydefs, eventname):
    ...
    if not keylist or macosx.isCocoaTk() and eventname in {'<<open-module>>',
        '<<goto-line>>', '<<change-indentwidth>>'}:
        return ''
    s = keylist[0]
