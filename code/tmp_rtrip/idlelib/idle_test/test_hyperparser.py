...
cliché = "this is a string with unicode, what a cliché\""""
    ...
    def test_eat_identifier_various_lengths(self):
        ...
        for length in range(1, 21):
            ...
            self.assertEqual(eat_id('a' + '2' * (length - 1), 0, length),
                length)
            self.assertEqual(eat_id('é' + '2' * (length - 1), 0, length),
                length)
            self.assertEqual(eat_id('é' + 'a' * (length - 1), 0, length),
                length)
            self.assertEqual(eat_id('é' * (length - 1) + 'a', 0, length),
                length)
