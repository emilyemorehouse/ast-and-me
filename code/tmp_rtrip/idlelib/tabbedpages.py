...
class TabSet(Frame):
    ...
    def __init__(self, page_set, select_command, tabs=None, n_rows=1,
        max_tabs_per_row=5, expand_tabs=False, **kw):
        ...
        self._tab_rows = []
        self.padding_frame = Frame(self, height=2, borderwidth=0, relief=
            FLAT, background=self.cget('background'))
    ...
    class TabButton(Frame):
        ...
        def __init__(self, name, select_command, tab_row, tab_set):
            ...
            self.button = Radiobutton(self, text=name, command=self.
                _select_event, padx=5, pady=1, takefocus=FALSE, indicatoron
                =FALSE, highlightthickness=0, selectcolor='', borderwidth=0)
        ...
        def _init_masks(self):
            ...
            self.mskl.ml.place(x=0, y=-self.bw, width=2 * self.bw, height=
                self.bw * 4)
        ...
        def _place_masks(self, selected=False):
            ...
            if selected:
                height += self.bw
            self.mask.place(in_=self, relx=0.0, x=0, rely=1.0, y=0,
                relwidth=1.0, width=0, relheight=0.0, height=height)
            self.mskl.place(in_=self, relx=0.0, x=-self.bw, rely=1.0, y=0,
                relwidth=0.0, width=self.bw, relheight=0.0, height=height)
            page_set = self.tab_set.page_set
            if selected and (not self.is_last_in_row or self.winfo_rootx() +
                self.winfo_width() < page_set.winfo_rootx() + page_set.
                winfo_width()):
                height -= self.bw
            self.mskr.place(in_=self, relx=1.0, x=0, rely=1.0, y=0,
                relwidth=0.0, width=self.bw, relheight=0.0, height=height)
            self.mskr.mr.place(x=-self.bw, y=-self.bw, width=2 * self.bw,
                height=height + self.bw * 2)
            self.tab_set.lower()
...
class TabbedPageSet(Frame):
    ...
    def __init__(self, parent, page_names=None, page_class=PageLift, n_rows
        =1, max_tabs_per_row=5, expand_tabs=False, **kw):
        """Constructor arguments:
        ...
        * PagePackForget - This mechanism uses the pack placement manager.
        ...
        self._tab_set = TabSet(self, self.change_page, n_rows=n_rows,
            max_tabs_per_row=max_tabs_per_row, expand_tabs=expand_tabs)
        if page_names:
    ...
    def add_page(self, page_name):
        ...
        if page_name in self.pages:
            raise AlreadyExistsError("TabPage named '%s' already exists" %
                page_name)
...
def _tabbed_pages(parent):
    ...
    buttonAdd = Button(top, text='Add Page', command=lambda : tabPage.
        add_page(entryPgName.get()))
    buttonRemove = Button(top, text='Remove Page', command=lambda : tabPage
        .remove_page(entryPgName.get()))
