...
def _path_join(*path_parts):
    ...
    return path_sep.join([part.rstrip(path_separators) for part in
        path_parts if part])
...
def source_from_cache(path):
    ...
    if pycache != _PYCACHE:
        raise ValueError('{} not bottom-level directory in {!r}'.format(
            _PYCACHE, path))
    dot_count = pycache_filename.count('.')
    if dot_count not in {2, 3}:
        raise ValueError('expected only 2 or 3 dots in {!r}'.format(
            pycache_filename))
    elif dot_count == 3:
...
class WindowsRegistryFinder:
    ...
    def _search_registry(cls, fullname):
        ...
        key = registry_key.format(fullname=fullname, sys_version='%d.%d' %
            sys.version_info[:2])
        try:
        ...
        for loader, suffixes in _get_supported_file_loaders():
            if filepath.endswith(tuple(suffixes)):
                spec = _bootstrap.spec_from_loader(fullname, loader(
                    fullname, filepath), origin=filepath)
                return spec
...
class SourceLoader(_LoaderBasics):
    ...
    def get_code(self, fullname):
                ...
                try:
                    ...
                    try:
                        bytes_data = _validate_bytecode_header(data,
                            source_stats=st, name=fullname, path=bytecode_path)
                        ...
                        return _compile_bytecode(bytes_data, name=fullname,
                            bytecode_path=bytecode_path, source_path=
                            source_path)
        ...
        if (not sys.dont_write_bytecode and bytecode_path is not None and 
            ...
            data = _code_to_bytecode(code_object, source_mtime, len(
                source_bytes))
            try:
...
class ExtensionFileLoader(FileLoader, _LoaderBasics):
    ...
    def is_package(self, fullname):
        ...
        return any(file_name == '__init__' + suffix for suffix in
            EXTENSION_SUFFIXES)
