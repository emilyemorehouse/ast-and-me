...
class Test_map(FixerTestCase):
    ...
    def test_None_with_multiple_arguments(self):
        ...
        self.warns_unchanged(s,
            'cannot convert map(None, ...) with multiple arguments')
...
class Test_asserts(FixerTestCase):
    ...
    def test_deprecated_names(self):
        tests = [('self.assert_(True)', 'self.assertTrue(True)'), (
            ...
            'self.assertNotAlmostEqual(2, 8)'), ('self.failUnless(True)',
            'self.assertTrue(True)'), ('self.failUnlessRaises(foo)',
            'self.assertRaises(foo)'), ('self.failIf(False)',
