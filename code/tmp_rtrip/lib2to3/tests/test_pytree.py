...
class TestNodes(support.TestCase):
    ...
    def test_node_repr(self):
        ...
        n1 = pytree.Node(1000, [l1, l2])
        self.assertEqual(repr(n1), 'Node(1000, [%s, %s])' % (repr(l1), repr
            (l2)))
...
class TestPatterns(support.TestCase):
    ...
    def test_generate_matches(self):
        ...
        pw = pytree.WildcardPattern([[pa, pb, pc], [pd, pe], [pa, pb], [pc,
            pd], [pe, pf]], min=1, max=4, name='pw')
        self.assertEqual([x[0] for x in pw.generate_matches(leaves)], [3, 5,
            2, 4, 6])
    ...
    def test_has_key_example(self):
        pattern = pytree.NodePattern(331, (pytree.LeafPattern(7), pytree.
            WildcardPattern(name='args'), pytree.LeafPattern(8)))
        l1 = pytree.Leaf(7, '(')
        l2 = pytree.Leaf(3, 'x')
        l3 = pytree.Leaf(8, ')')
