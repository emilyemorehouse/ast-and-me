...
class TestRefactoringTool(unittest.TestCase):
    ...
    def test_print_function_option(self):
        ...
        self.assertIs(rt.driver.grammar, pygram.
    ...
    def test_fixer_loading_helpers(self):
        ...
        self.assertEqual(full_names, [('myfixes.fix_' + name) for name in
            contents])
    ...
    def test_detect_future_features(self):
        ...
        self.assertEqual(run('from __future__ import print_function'), fs((
            'print_function',)))
        self.assertEqual(run('from __future__ import generators'), fs((
            'generators',)))
        ...
        self.assertEqual(run(inp), fs(('generators', 'print_function')))
        invalid = ('from', 'from 4', 'from x', 'from x 5', 'from x im',
            'from x import', 'from x import 4')
        for inp in invalid:
    ...
    def check_file_refactoring(self, test_file, fixers=_2TO3_FIXERS,
        options=None, mock_log_debug=None, actually_write=True):
    ...
    def test_refactor_file_write_unchanged_file(self):
        ...
        self.check_file_refactoring(test_file, fixers=(), options={
            'write_unchanged_files': True}, mock_log_debug=
            recording_log_debug, actually_write=False)
        message_regex = 'Not writing changes to .*%s' % re.escape(os.sep +
    ...
    def test_refactor_dir(self):
        ...
        def check(structure, expected):
            ...
            try:
                ...
                shutil.rmtree(dir)
            self.assertEqual(got, [os.path.join(dir, path) for path in
                expected])
        check([], [])
