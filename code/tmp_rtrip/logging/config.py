        ...
        if m is None:
            ...
            while rest:
                ...
                if m:
                    ...
                    raise ValueError('Unable to convert %r at %r' % (value,
                        rest))
        return d
                ...
                for name in handlers:
                    if name not in logging._handlers:
                        ...
                        try:
                            ...
                            raise ValueError(
                                'Unable to configure handler %r: %s' % (
                                name, e))
...
def listen(port=DEFAULT_LOGGING_CONFIG_PORT, verify=None):
    ...
    class ConfigSocketReceiver(ThreadingTCPServer):
        ...
        def __init__(self, host='localhost', port=
            DEFAULT_LOGGING_CONFIG_PORT, handler=None, ready=None, verify=None
            ):
    ...
    class Server(threading.Thread):
        ...
        def run(self):
            server = self.rcvr(port=self.port, handler=self.hdlr, ready=
                self.ready, verify=self.verify)
            if self.port == 0:
