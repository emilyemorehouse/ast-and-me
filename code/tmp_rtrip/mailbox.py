...
try:
    ...
    fcntl = None
__all__ = ['Mailbox', 'Maildir', 'mbox', 'MH', 'Babyl', 'MMDF', 'Message',
...
class Maildir(Mailbox):
    ...
    def __init__(self, dirname, factory=None, create=True):
        ...
        self._paths = {'tmp': os.path.join(self._path, 'tmp'), 'new': os.
            path.join(self._path, 'new'), 'cur': os.path.join(self._path,
            'cur')}
    ...
    def add(self, message):
        ...
        if isinstance(message, MaildirMessage):
            os.utime(tmp_file.name, (os.path.getatime(tmp_file.name),
                message.get_date()))
        try:
            try:
    ...
    def list_folders(self):
        ...
        for entry in os.listdir(self._path):
            if len(entry) > 1 and entry[0] == '.' and os.path.isdir(os.path
                .join(self._path, entry)):
                result.append(entry[1:])
        return result

    def get_folder(self, folder):
        """Return a Maildir instance for the named folder."""
        return Maildir(os.path.join(self._path, '.' + folder), factory=self
            ._factory, create=False)
    ...
    def remove_folder(self, folder):
        ...
        path = os.path.join(self._path, '.' + folder)
        for entry in (os.listdir(os.path.join(path, 'new')) + os.listdir(os
            .path.join(path, 'cur'))):
            if len(entry) < 1 or entry[0] != '.':
                raise NotEmptyError('Folder contains message(s): %s' % folder)
        for entry in os.listdir(path):
            if (entry != 'new' and entry != 'cur' and entry != 'tmp' and os
                .path.isdir(os.path.join(path, entry))):
    ...
    def _create_tmp(self):
        ...
        uniq = '%s.M%sP%sQ%s.%s' % (int(now), int(now % 1 * 1000000.0), os.
            getpid(), Maildir._count, hostname)
    ...
    def flush(self):
        ...
        try:
            ...
            for key in sorted(self._toc.keys()):
                ...
                while True:
                    buffer = self._file.read(min(4096, stop - self._file.
                        tell()))
                    if not buffer:
                        break
...
class _mboxMMDF(_singlefileMailbox):
    ...
    def get_string(self, key, from_=False):
        """Return a string representation or raise a KeyError."""
        return email.message_from_bytes(self.get_bytes(key)).as_string(unixfrom
            =from_)
...
class MH(Mailbox):
        ...
        if not os.path.exists(self._path):
            if create:
                ...
                os.close(os.open(os.path.join(self._path, '.mh_sequences'),
                    os.O_CREAT | os.O_EXCL | os.O_WRONLY, 384))
            else:
                raise NoSuchMailboxError(self._path)
        self._locked = False
    ...
    def iterkeys(self):
        """Return an iterator over keys."""
        return iter(sorted(int(entry) for entry in os.listdir(self._path) if
            entry.isdigit()))
    ...
    def get_sequences(self):
        ...
        results = {}
        with open(os.path.join(self._path, '.mh_sequences'), 'r', encoding=
            'ASCII') as f:
            ...
            for line in f:
                try:
                    ...
                    results[name] = [key for key in sorted(keys) if key in
                        all_keys]
    ...
    def set_sequences(self, sequences):
        ...
        f = open(os.path.join(self._path, '.mh_sequences'), 'r+', encoding=
            'ASCII')
        try:
    ...
    def pack(self):
        ...
        for key in self.iterkeys():
            if key - 1 != prev:
                ...
                try:
                    os.link(os.path.join(self._path, str(key)), os.path.
                        join(self._path, str(prev + 1)))
                except (AttributeError, PermissionError):
                    os.rename(os.path.join(self._path, str(key)), os.path.
                        join(self._path, str(prev + 1)))
                else:
        ...
        while True:
            ...
            if line == b'\x1f\x0c' + linesep:
                if len(stops) < len(starts):
                    stops.append(line_pos - len(linesep))
                starts.append(next_pos)
                labels = [label.strip() for label in self._file.readline()[
                    1:].split(b',') if label.strip()]
...
class MaildirMessage(Message):
    ...
    def _explain_to(self, message):
        ...
        if isinstance(message, MaildirMessage):
            ...
            raise TypeError('Cannot convert to specified type: %s' % type(
                message))
