...
__all__ = ['normcase', 'isabs', 'join', 'splitdrive', 'split', 'splitext',
    ...
    'relpath', 'samefile', 'sameopenfile', 'samestat', 'commonpath']
curdir = '.'
pardir = '..'
extsep = '.'
sep = '\\'
...
def normcase(s):
    ...
    try:
        ...
        if not isinstance(s, (bytes, str)):
            raise TypeError(
                'normcase() argument must be str or bytes, not %r' % s.
                __class__.__name__) from None
        raise
...
def join(path, *paths):
    ...
    try:
        ...
        if result_path and result_path[0
            ] not in seps and result_drive and result_drive[-1:] != colon:
...
def commonpath(paths):
    ...
    if isinstance(paths[0], bytes):
        ...
        curdir = '.'
    try:
        drivesplits = [splitdrive(p.replace(altsep, sep).lower()) for p in
            paths]
