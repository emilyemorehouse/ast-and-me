...
def length_hint(obj, default=0):
    ...
    if not isinstance(default, int):
        msg = "'%s' object cannot be interpreted as an integer" % type(default
            ).__name__
        raise TypeError(msg)
    try:
        return len(obj)
...
class attrgetter:
    ...
    def __repr__(self):
        return '%s.%s(%s)' % (self.__class__.__module__, self.__class__.
            __qualname__, ', '.join(map(repr, self._attrs)))
...
class itemgetter:
    ...
    def __repr__(self):
        return '%s.%s(%s)' % (self.__class__.__module__, self.__class__.
            __name__, ', '.join(map(repr, self._items)))
...
class methodcaller:
    ...
    def __reduce__(self):
        if not self._kwargs:
            ...
            return partial(self.__class__, self._name, **self._kwargs
                ), self._args
