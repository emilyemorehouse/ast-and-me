...
__all__ = ['altsep', 'curdir', 'pardir', 'sep', 'pathsep', 'linesep',
    ...
    'SEEK_END', 'fsencode', 'fsdecode', 'get_exec_path', 'fdopen', 'popen',
    'extsep']
    ...
    def _fwalk(topfd, toppath, topdown, onerror, follow_symlinks):
        ...
        for name in names:
            try:
                ...
                try:
                    if st.S_ISLNK(stat(name, dir_fd=topfd, follow_symlinks=
                        False).st_mode):
                        nondirs.append(name)
                except OSError:
                    continue
        ...
        for name in dirs:
            try:
                orig_st = stat(name, dir_fd=topfd, follow_symlinks=
                    follow_symlinks)
...
class _Environ(MutableMapping):
    ...
    def __repr__(self):
        return 'environ({{{}}})'.format(', '.join('{!r}: {!r}'.format(self.
            decodekey(key), self.decodevalue(value)) for key, value in self
            ._data.items()))
