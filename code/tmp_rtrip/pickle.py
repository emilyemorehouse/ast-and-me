...
class _Pickler:
        ...
        if reduce is not None:
            ...
            if reduce is not None:
                ...
                if reduce is not None:
                    ...
                    raise PicklingError("Can't pickle %r object: %r" % (t.
                        __name__, obj))
    ...
    def save_reduce(self, func, args, state=None, listitems=None, dictitems
        =None, obj=None):
    ...
    def save_bytes(self, obj):
        if self.proto < 3:
            if not obj:
                ...
                self.save_reduce(codecs.encode, (str(obj, 'latin1'),
                    'latin1'), obj=obj)
            return
        n = len(obj)
        if n <= 255:
...
class _Unpickler:
    ...
    def __init__(self, file, *, fix_imports=True, encoding='ASCII', errors=
        'strict'):
    ...
    def load_binunicode8(self):
        ...
        if len > maxsize:
            raise UnpicklingError(
                "BINUNICODE8 exceeds system's maximum size of %d bytes" %
                maxsize)
        self.append(str(self.read(len), 'utf-8', 'surrogatepass'))
    ...
    def _instantiate(self, klass, args):
        if args or not isinstance(klass, type) or hasattr(klass,
            '__getinitargs__'):
            try:
                value = klass(*args)
            except TypeError as err:
                raise TypeError('in constructor for %s: %s' % (klass.
                    __name__, str(err)), sys.exc_info()[2])
        else:
