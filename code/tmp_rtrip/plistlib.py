...
Parse Plist example:
    ...
    print(pl["aKey"])
"""
__all__ = ['readPlist', 'writePlist', 'readPlistFromBytes',
    'writePlistToBytes', 'Plist', 'Data', 'Dict', 'InvalidFileException',
    'FMT_XML', 'FMT_BINARY', 'load', 'dump', 'loads', 'dumps']
import binascii
import codecs
import contextlib
import datetime
...
def readPlist(pathOrFile):
    ...
    warn('The readPlist function is deprecated, use load() instead',
        DeprecationWarning, 2)
    with _maybe_open(pathOrFile, 'rb') as fp:
        return load(fp, fmt=None, use_builtin_types=False, dict_type=
            _InternalDict)
...
def readPlistFromBytes(data):
    ...
    warn('The readPlistFromBytes function is deprecated, use loads() instead',
        DeprecationWarning, 2)
    return load(BytesIO(data), fmt=None, use_builtin_types=False, dict_type
        =_InternalDict)
...
def _date_to_string(d):
    return '%04d-%02d-%02dT%02d:%02d:%02dZ' % (d.year, d.month, d.day, d.
        hour, d.minute, d.second)
...
class _PlistParser:
    ...
    def add_object(self, value):
        if self.current_key is not None:
            if not isinstance(self.stack[-1], type({})):
                raise ValueError('unexpected element at line %d' % self.
    ...
    def end_key(self):
        if self.current_key or not isinstance(self.stack[-1], type({})):
            raise ValueError('unexpected key at line %d' % self.parser.
                CurrentLineNumber)
...
class _PlistWriter(_DumbXMLWriter):
    ...
    def __init__(self, file, indent_level=0, indent=b'\t', writeHeader=1,
    ...
    def write_bytes(self, data):
        ...
        self._indent_level -= 1
        maxlinelength = max(16, 76 - len(self.indent.replace(b'\t', b' ' * 
            8) * self._indent_level))
...
def _is_fmt_xml(header):
    ...
    for pfx in prefixes:
        if header.startswith(pfx):
            return True
    for bom, encoding in ((codecs.BOM_UTF8, 'utf-8'), (codecs.BOM_UTF16_BE,
        'utf-16-be'), (codecs.BOM_UTF16_LE, 'utf-16-le')):
...
class _BinaryPlistParser:
    ...
    def _read_ints(self, n, size):
        ...
        if size in _BINARY_FORMAT:
            ...
            return tuple(int.from_bytes(data[i:i + size], 'big') for i in
                range(0, size * n, size))
    ...
    def _read_object(self, offset):
        ...
        if token == 0:
            ...
            return int.from_bytes(self._fp.read(1 << tokenL), 'big', signed
                =tokenL >= 3)
        elif token == 34:
            ...
            return [self._read_object(self._object_offsets[x]) for x in
                obj_refs]
        elif tokenH == 208:
            s = self._get_size(tokenL)
...
_FORMATS = {FMT_XML: dict(detect=_is_fmt_xml, parser=_PlistParser, writer=
    _PlistWriter), FMT_BINARY: dict(detect=_is_fmt_binary, parser=
    _BinaryPlistParser, writer=_BinaryPlistWriter)}
...
def loads(value, *, fmt=None, use_builtin_types=True, dict_type=dict):
    ...
    """
    fp = BytesIO(value)
    return load(fp, fmt=fmt, use_builtin_types=use_builtin_types, dict_type
        =dict_type)
