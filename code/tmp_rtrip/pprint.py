...
def pprint(object, stream=None, indent=1, width=80, depth=None, *, compact=
    False):
    """Pretty-print a Python object to a stream [default is sys.stdout]."""
...
def pformat(object, indent=1, width=80, depth=None, *, compact=False):
    """Format a Python object into a pretty-printed representation."""
    return PrettyPrinter(indent=indent, width=width, depth=depth, compact=
        compact).pformat(object)
...
class _safe_key:
    ...
    def __lt__(self, other):
        try:
            ...
            return (str(type(self.obj)), id(self.obj)) < (str(type(other.
                obj)), id(other.obj))
...
class PrettyPrinter:
    ...
    def _pprint_ordered_dict(self, object, stream, indent, allowance,
        ...
        self._format(list(object.items()), stream, indent + len(cls.
            __name__) + 1, allowance + 1, context, level)
        stream.write(')')
    ...
    def _pprint_bytearray(self, object, stream, indent, allowance, context,
        ...
        write = stream.write
        write('bytearray(')
        self._pprint_bytes(bytes(object), stream, indent + 10, allowance + 
            1, context, level + 1)
        write(')')
    ...
    def _format_dict_items(self, items, stream, indent, allowance, context,
        ...
        for i, (key, ent) in enumerate(items):
            ...
            write(rep)
            write(': ')
            self._format(ent, stream, indent + len(rep) + 2, allowance if
                last else 1, context, level)
            if not last:
                write(delimnl)
    ...
    def _pprint_counter(self, object, stream, indent, allowance, context, level
        ...
        self._format_dict_items(items, stream, indent + len(cls.__name__) +
            1, allowance + 2, context, level)
        stream.write('})')
