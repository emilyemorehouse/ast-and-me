...
class Stats:
    ...
    def load_stats(self, arg):
        ...
        if not self.stats:
            raise TypeError(
                'Cannot create or construct a %r object from %r' % (self.
                __class__, arg))
        return
    ...
    sort_arg_dict_default = {'calls': (((1, -1),), 'call count'), 'ncalls':
        (((1, -1),), 'call count'), 'cumtime': (((3, -1),),
        'cumulative time'), 'cumulative': (((3, -1),), 'cumulative time'),
        'file': (((4, 1),), 'file name'), 'filename': (((4, 1),),
        'file name'), 'line': (((5, 1),), 'line number'), 'module': (((4, 1
        ),), 'file name'), 'name': (((6, 1),), 'function name'), 'nfl': (((
        6, 1), (4, 1), (5, 1)), 'name/file/line'), 'pcalls': (((0, -1),),
        'primitive call count'), 'stdname': (((7, 1),), 'standard name'),
        'time': (((2, -1),), 'internal time'), 'tottime': (((2, -1),),
        'internal time')}
    ...
    def sort_stats(self, *field):
        ...
        if len(field) == 1 and isinstance(field[0], int):
            field = [{(-1): 'stdname', (0): 'calls', (1): 'time', (2):
                'cumulative'}[field[0]]]
        ...
        stats_list = []
        for func, (cc, nc, tt, ct, callers) in self.stats.items():
            stats_list.append((cc, nc, tt, ct) + func + (func_std_string(
                func), func))
    ...
    def print_stats(self, *amount):
        ...
        print(indent, self.total_calls, 'function calls', end=' ', file=
            self.stream)
        if self.total_calls != self.prim_calls:
            print('(%d primitive calls)' % self.prim_calls, end=' ', file=
                self.stream)
    ...
    def print_call_heading(self, name_size, column_title):
        ...
        for cc, nc, tt, ct, callers in self.stats.values():
            if callers:
                ...
                break
        if subheader:
            print(' ' * name_size + '    ncalls  tottime  cumtime', file=
                self.stream)

    def print_call_line(self, name_size, source, call_dict, arrow='->'):
        print(func_std_string(source).ljust(name_size) + arrow, end=' ',
            file=self.stream)
        if not call_dict:
            print(file=self.stream)
            return
        ...
        for func in clist:
            ...
            if isinstance(value, tuple):
                ...
                if nc != cc:
                    ...
                    substats = '%d' % (nc,)
                substats = '%s %s %s  %s' % (substats.rjust(7 + 2 * len(
                    indent)), f8(tt), f8(ct), name)
...
def add_func_stats(target, source):
    ...
    return cc + t_cc, nc + t_nc, tt + t_tt, ct + t_ct, add_callers(t_callers,
        callers)
    ...
    for func, caller in source.items():
        if func in new_callers:
            if isinstance(caller, tuple):
                new_callers[func] = tuple([(i[0] + i[1]) for i in zip(
                    caller, new_callers[func])])
            else:
                new_callers[func] += caller
        else:
            new_callers[func] = caller
...
if __name__ == '__main__':
    ...
    class ProfileBrowser(cmd.Cmd):
        ...
        def generic(self, fn, line):
            ...
            for term in args:
                ...
                try:
                    frac = float(term)
                    if frac > 1 or frac < 0:
                        print('Fraction argument must be in [0, 1]', file=
                            self.stream)
                        continue
        ...
        def generic_help(self):
            ...
            print('* An integer maximum number of entries to print.', file=
                self.stream)
            print('* A decimal fractional number between 0 and 1, controlling',
                file=self.stream)
            print('  what fraction of selected entries to print.', file=
                self.stream)
        ...
        def help_read(self):
            print('Read in profile data from a specified file.', file=self.
                stream)
            print('Without argument, reload the current file.', file=self.
                stream)
        ...
        def help_reverse(self):
            print('Reverse the sort order of the profiling report.', file=
                self.stream)
        ...
        def help_sort(self):
            print('Sort profile data according to specified keys.', file=
                self.stream)
            print("(Typing `sort' without arguments lists valid keys.)",
                file=self.stream)

        def complete_sort(self, text, *args):
            return [a for a in Stats.sort_arg_dict_default if a.startswith(
                text)]
        ...
        def help_stats(self):
            print('Print statistics from the current stat object.', file=
                self.stream)
            self.generic_help()
