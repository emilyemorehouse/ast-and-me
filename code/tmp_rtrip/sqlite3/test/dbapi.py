...
class ModuleTests(unittest.TestCase):
    ...
    def CheckIntegrityError(self):
        self.assertTrue(issubclass(sqlite.IntegrityError, sqlite.
            DatabaseError), 'IntegrityError is not a subclass of DatabaseError'
            )

    def CheckInternalError(self):
        self.assertTrue(issubclass(sqlite.InternalError, sqlite.
            DatabaseError), 'InternalError is not a subclass of DatabaseError')
    ...
    def CheckNotSupportedError(self):
        self.assertTrue(issubclass(sqlite.NotSupportedError, sqlite.
            DatabaseError),
...
class CursorTests(unittest.TestCase):
    ...
    def CheckExecuteArgStringWithZeroByte(self):
        ...
        self.cu.execute('select name from test where id=?', (self.cu.
            lastrowid,))
    ...
    def CheckExecuteDictMappingTooLittleArgs(self):
        self.cu.execute("insert into test(name) values ('foo')")
        with self.assertRaises(sqlite.ProgrammingError):
            self.cu.execute('select name from test where name=:name and id=:id'
                , {'name': 'foo'})
    ...
    def CheckRowcountExecutemany(self):
        self.cu.execute('delete from test')
        self.cu.executemany('insert into test(name) values (?)', [(1,), (2,
            ), (3,)])
    ...
    def CheckTotalChanges(self):
        self.cu.execute("insert into test(name) values ('foo')")
        self.cu.execute("insert into test(name) values ('foo')")
        self.assertLess(2, self.cx.total_changes, msg=
            'total changes reported wrong value')

    def CheckExecuteManySequence(self):
        self.cu.executemany('insert into test(income) values (?)', [(x,) for
            x in range(100, 110)])
    ...
    def CheckLastRowIDOnIgnore(self):
        self.cu.execute('insert or ignore into test(unique_test) values (?)',
            ('test',))
        ...
        for statement in ('FAIL', 'ABORT', 'ROLLBACK'):
            ...
            with self.subTest(statement='INSERT OR {}'.format(statement)):
                ...
                with self.assertRaises(sqlite.IntegrityError):
                    self.cu.execute(sql.format(statement), (statement,))
                results.append((statement, self.cu.lastrowid))
        expected = [('FAIL', 2), ('FAIL', 2), ('ABORT', 3), ('ABORT', 3), (
...
class ThreadTests(unittest.TestCase):
    ...
    def CheckConCursor(self):
        ...
        errors = []
        t = threading.Thread(target=run, kwargs={'con': self.con, 'errors':
            errors})
        t.start()
        t.join()
        if len(errors) > 0:
    ...
    def CheckCurImplicitBegin(self):
        ...
        def run(cur, errors):
            try:
                ...
                errors.append('raised wrong exception')
        errors = []
        t = threading.Thread(target=run, kwargs={'cur': self.cur, 'errors':
            errors})
        t.start()
        t.join()
        if len(errors) > 0:
...
class SqliteOnConflictTests(unittest.TestCase):
    ...
    def CheckOnConflictRollbackWithExplicitTransaction(self):
        ...
        self.cu.execute(
            "INSERT OR ROLLBACK INTO test(unique_name) VALUES ('foo')")
        with self.assertRaises(sqlite.IntegrityError):
            self.cu.execute(
                "INSERT OR ROLLBACK INTO test(unique_name) VALUES ('foo')")
        self.cx.commit()
    ...
    def CheckOnConflictAbortRaisesWithExplicitTransactions(self):
        ...
        with self.assertRaises(sqlite.IntegrityError):
            self.cu.execute(
                "INSERT OR ABORT INTO test(unique_name) VALUES ('foo')")
        self.cx.commit()
        self.cu.execute('SELECT name, unique_name FROM test')
        self.assertEqual(self.cu.fetchall(), [('abort_test', None), (None,
            'foo')])
    ...
    def CheckOnConflictFail(self):
        ...
        with self.assertRaises(sqlite.IntegrityError):
            self.cu.execute(
                "INSERT OR FAIL INTO test(unique_name) VALUES ('foo')")
        self.assertEqual(self.cu.fetchall(), [])

    def CheckOnConflictIgnore(self):
        self.cu.execute(
            "INSERT OR IGNORE INTO test(unique_name) VALUES ('foo')")
        self.cu.execute(
            "INSERT OR IGNORE INTO test(unique_name) VALUES ('foo')")
...
def suite():
    ...
    return unittest.TestSuite((module_suite, connection_suite, cursor_suite,
        thread_suite, constructor_suite, ext_suite, closed_con_suite,
        closed_cur_suite, on_conflict_suite))
