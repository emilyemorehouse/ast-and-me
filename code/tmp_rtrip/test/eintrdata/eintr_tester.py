...
class EINTRBaseTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        signal.setitimer(signal.ITIMER_REAL, cls.signal_delay, cls.
            signal_period)
...
class OSEINTRTest(EINTRBaseTest):
    ...
    def test_read(self):
        ...
        code = '\n'.join(('import os, sys, time', '',
            'wr = int(sys.argv[1])', 'datas = %r' % datas, 
            'sleep_time = %r' % self.sleep_time, '', 'for data in datas:',
            '    # let the parent block on read()',
            '    time.sleep(sleep_time)', '    os.write(wr, data)'))
    ...
    def test_write(self):
        ...
        code = '\n'.join(('import io, os, sys, time', '',
            'rd = int(sys.argv[1])', 'sleep_time = %r' % self.sleep_time, 
            'data = b"x" * %s' % support.PIPE_MAX_SIZE,
            'data_len = len(data)', '', '# let the parent block on write()',
            'time.sleep(sleep_time)', '', 'read_data = io.BytesIO()',
            'while len(read_data.getvalue()) < data_len:',
            '    chunk = os.read(rd, 2 * data_len)',
            '    read_data.write(chunk)', '',
            'value = read_data.getvalue()', 'if value != data:',
...
class SocketEINTRTest(EINTRBaseTest):
    ...
    def _test_recv(self, recv_func):
        ...
        datas = [b'x', b'y', b'z']
        code = '\n'.join(('import os, socket, sys, time', '',
            'fd = int(sys.argv[1])', 'family = %s' % int(wr.family), 
            'sock_type = %s' % int(wr.type), 'datas = %r' % datas, 
            'sleep_time = %r' % self.sleep_time, '',
            'wr = socket.fromfd(fd, family, sock_type)', 'os.close(fd)', '',
            'with wr:', '    for data in datas:',
            '        # let the parent block on recv()',
            '        time.sleep(sleep_time)', '        wr.sendall(data)'))
        fd = wr.fileno()
    ...
    def _test_send(self, send_func):
        ...
        code = '\n'.join(('import os, socket, sys, time', '',
            'fd = int(sys.argv[1])', 'family = %s' % int(rd.family), 
            'sock_type = %s' % int(rd.type), 'sleep_time = %r' % self.
            sleep_time, 'data = b"xyz" * %s' % (support.SOCK_MAX_SIZE // 3),
            'data_len = len(data)', '',
            'rd = socket.fromfd(fd, family, sock_type)', 'os.close(fd)', '',
            'with rd:', '    # let the parent block on send()',
            '    time.sleep(sleep_time)', '',
            '    received_data = bytearray(data_len)', '    n = 0',
            '    while n < data_len:',
    ...
    def test_accept(self):
        ...
        sock.listen()
        code = '\n'.join(('import socket, time', '', 'host = %r' % support.
            HOST, 'port = %s' % port, 'sleep_time = %r' % self.sleep_time,
            '', '# let parent block on accept()', 'time.sleep(sleep_time)',
    ...
    def _test_open(self, do_open_close_reader, do_open_close_writer):
        ...
        code = '\n'.join(('import os, time', '', 'path = %a' % filename, 
            'sleep_time = %r' % self.sleep_time, '',
            '# let the parent block', 'time.sleep(sleep_time)', '',
...
class SignalEINTRTest(EINTRBaseTest):
    ...
    def check_sigwait(self, wait_func):
        ...
        code = '\n'.join(('import os, time', 'pid = %s' % os.getpid(), 
            'signum = %s' % int(signum), 'sleep_time = %r' % self.
            sleep_time, 'time.sleep(sleep_time)', 'os.kill(pid, signum)'))
...
def test_main():
    support.run_unittest(OSEINTRTest, SocketEINTRTest, TimeEINTRTest,
        SignalEINTRTest, SelectEINTRTest)
