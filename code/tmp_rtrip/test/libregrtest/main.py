...
from test.libregrtest.runtest import findtests, runtest, get_abs_module, STDTESTS, NOTTESTS, PASSED, FAILED, ENV_CHANGED, SKIPPED, RESOURCE_DENIED, INTERRUPTED, CHILD_ERROR, PROGRESS_MIN_TIME, format_test_result
...
class Regrtest:
    ...
    def parse_args(self, kwargs):
        ...
        if ns.findleaks:
            if gc is not None:
                ...
                print('No GC available, disabling --findleaks', file=sys.stderr
                    )
                ns.findleaks = False
        removepy(ns.args)
        return ns
    ...
    def display_result(self):
        ...
        if self.good and not self.ns.quiet:
            if (not self.bad and not self.skipped and not self.interrupted and
                len(self.good) > 1):
                print('All', end=' ')
        ...
        if self.environment_changed:
            print()
            print('{} altered the execution environment:'.format(count(len(
                self.environment_changed), 'test')))
    ...
    def display_header(self):
        ...
        print('==', platform.platform(aliased=True), '%s-endian' % sys.
            byteorder)
        print('== hash algorithm:', sys.hash_info.algorithm, '64bit' if sys
            .maxsize > 2 ** 32 else '32bit')
        print('== cwd:', os.getcwd())
        cpu_count = os.cpu_count()
        if cpu_count:
            print('== CPU count:', cpu_count)
        print('== encodings: locale=%s, FS=%s' % (locale.
            getpreferredencoding(False), sys.getfilesystemencoding()))
        print('Testing with flags:', sys.flags)

    def run_tests(self):
        if self.ns.header or not (self.ns.pgo or self.ns.quiet or self.ns.
            single or self.tests or self.ns.args):
            self.display_header()
        if self.ns.randomize:
    ...
    def finalize(self):
        ...
        if self.tracer:
            ...
            r.write_results(show_missing=True, summary=True, coverdir=self.
                ns.coverdir)
        print()
