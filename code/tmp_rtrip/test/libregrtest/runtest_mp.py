...
try:
    ...
    sys.exit(2)
from test.libregrtest.runtest import runtest, INTERRUPTED, CHILD_ERROR, PROGRESS_MIN_TIME, format_test_result
...
def run_test_in_subprocess(testname, ns):
    ...
    cmd = [sys.executable, *support.args_from_interpreter_flags(), '-u',
        '-m', 'test.regrtest', '--slaveargs', slaveargs]
    if ns.pgo:
        cmd += ['--pgo']
    popen = Popen(cmd, stdout=PIPE, stderr=PIPE, universal_newlines=True,
        close_fds=os.name != 'nt', cwd=support.SAVEDCWD)
    with popen:
...
def run_tests_multiprocess(regrtest):
    ...
    workers = [MultiprocessThread(pending, output, regrtest.ns) for i in
        range(regrtest.ns.use_mp)]
    ...
    try:
        while finished < regrtest.ns.use_mp:
            ...
            if ok not in (CHILD_ERROR, INTERRUPTED
                ) and test_time >= PROGRESS_MIN_TIME and not regrtest.ns.pgo:
