...
class TestBase:
            ...
            if expected:
                ...
                if func is self.decode:
                    ...
                    self.assertEqual(result, expected, 
                        '%a.decode(%r, %r)=%a != %a' % (source, self.
                        encoding, scheme, result, expected))
                else:
                    self.assertTrue(type(result) is bytes, type(result))
                    self.assertEqual(result, expected, 
                        '%a.encode(%r, %r)=%a != %a' % (source, self.
                        encoding, scheme, result, expected))
            else:
    ...
    def test_xmlcharrefreplace(self):
        ...
        s = 'ଓଣୠ nd eggs'
        self.assertEqual(self.encode(s, 'xmlcharrefreplace')[0],
            b'&#2835;&#2851;&#2912; nd eggs')
    ...
    def test_callback_returns_bytes(self):
        ...
        enc = self.encode('abc' + self.unmappedunicode + 'def', 'test.cjktest'
            )[0]
    ...
    def test_incrementalencoder(self):
        ...
        for sizehint in ([None] + list(range(1, 33)) + [64, 128, 256, 512, 
            1024]):
    ...
    def test_incrementaldecoder(self):
        ...
        for sizehint in ([None, -1] + list(range(1, 33)) + [64, 128, 256, 
            512, 1024]):
    ...
    def test_streamreader(self):
        ...
        for name in ['read', 'readline', 'readlines']:
            for sizehint in ([None, -1] + list(range(1, 33)) + [64, 128, 
                256, 512, 1024]):
    ...
    def test_streamwriter(self):
        ...
        for name in readfuncs:
            for sizehint in ([None] + list(range(1, 33)) + [64, 128, 256, 
                512, 1024]):
                istream = UTF8Reader(BytesIO(self.tstring[1]))
        ...
        with self.open_mapping_file() as f:
            for line in f:
                ...
                if csetval <= 127:
                    ...
                    csetch = bytes([csetval >> 24, csetval >> 16 & 255, 
                        csetval >> 8 & 255, csetval & 255])
                elif csetval >= 65536:
                    csetch = bytes([csetval >> 16, csetval >> 8 & 255, 
                        csetval & 255])
                elif csetval >= 256:
