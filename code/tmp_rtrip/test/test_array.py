...
class ArrayReconstructorTest(unittest.TestCase):
    ...
    def test_error(self):
        ...
        self.assertRaises(TypeError, array_reconstructor, array.array, 'b',
            '', b'')
        self.assertRaises(TypeError, array_reconstructor, array.array, 'b',
            0, '')
        self.assertRaises(ValueError, array_reconstructor, array.array, '?',
            0, b'')
        self.assertRaises(ValueError, array_reconstructor, array.array, 'b',
            UNKNOWN_FORMAT, b'')
        self.assertRaises(ValueError, array_reconstructor, array.array, 'b',
            22, b'')
        self.assertRaises(ValueError, array_reconstructor, array.array, 'd',
            16, b'a')
...
class BaseTest:
    ...
    def test_insert(self):
        ...
        a = array.array(self.typecode, self.example)
        a.insert(1000, self.example[0])
        self.assertEqual(a, array.array(self.typecode, self.example + self.
            example[:1]))
    ...
    def test_tofromstring(self):
        ...
        with warnings.catch_warnings(record=True) as r:
            warnings.filterwarnings('always', message=
                '(to|from)string\\(\\) is deprecated', category=
                DeprecationWarning)
            a = array.array(self.typecode, 2 * self.example)
            b = array.array(self.typecode)
    ...
    def test_add(self):
        a = array.array(self.typecode, self.example) + array.array(self.
            typecode, self.example[::-1])
        self.assertEqual(a, array.array(self.typecode, self.example + self.
            example[::-1]))
    ...
    def test_setitem(self):
        ...
        self.assertRaises(IndexError, a.__setitem__, len(self.example),
            self.example[0])
        self.assertRaises(IndexError, a.__setitem__, -len(self.example) - 1,
            self.example[0])
        ...
        for start in indices:
            for stop in indices:
                for step in indices[1:]:
                    self.assertEqual(list(a[start:stop:step]), list(a)[
                        start:stop:step])
    ...
    def test_pop(self):
        ...
        self.assertEqual(a, array.array(self.typecode, self.example[1:2] +
            self.example[3:] + self.example))
        self.assertEntryEqual(a.pop(0), self.example[1])
        self.assertEntryEqual(a.pop(), self.example[-1])
        self.assertEqual(a, array.array(self.typecode, self.example[3:] +
            self.example[:-1]))
    ...
    def test_free_after_iterating(self):
        support.check_free_after_iterating(self, iter, array.array, (self.
            typecode,))
...
class NumberTest(BaseTest):
    ...
    def test_delslice(self):
        ...
        del a[::1000]
        self.assertEqual(a, array.array(self.typecode, [1, 2, 3, 4, 5, 6, 7,
            8, 9]))
        a = array.array(self.typecode, range(10))
        del a[9::1 << 333]
    ...
    def test_assignment(self):
        ...
        self.assertEqual(a, array.array(self.typecode, [42, 1, 42, 3, 42, 5,
            42, 7, 42, 9]))
        a = array.array(self.typecode, range(10))
        a[::-4] = array.array(self.typecode, [10] * 3)
        self.assertEqual(a, array.array(self.typecode, [0, 10, 2, 3, 4, 10,
            6, 7, 8, 10]))
    ...
    def check_overflow(self, lower, upper):
        ...
        a[0] = lower
        self.assertRaises(OverflowError, array.array, self.typecode, [lower -
            1])
        self.assertRaises(OverflowError, a.__setitem__, 0, lower - 1)
        a = array.array(self.typecode, [upper])
        a[0] = upper
        self.assertRaises(OverflowError, array.array, self.typecode, [upper +
            1])
