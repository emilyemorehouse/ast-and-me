...
class DuckFuture:
    ...
    def done(self):
        return (self.__cancelled or self.__result is not None or self.
            __exception is not None)
...
class BaseFutureTests:
    ...
    def test_exception(self):
        ...
        self.assertRaisesRegex(TypeError,
            'StopIteration .* cannot be raised', f.set_exception, StopIteration
            )
        f.set_exception(exc)
        self.assertFalse(f.cancelled())
        self.assertTrue(f.done())
    ...
    def test_future_repr(self):
        ...
        self.assertRegex(repr(f_two_callbacks), 
            '<Future pending cb=\\[%s, %s\\]>' % (first_repr, last_repr))
    ...
    def test_future_source_traceback(self):
        ...
        self.assertEqual(future._source_traceback[-2][:3], (__file__,
            lineno, 'test_future_source_traceback'))
    ...
    def check_future_exception_never_retrieved(self, debug, m_log):
        ...
        if sys.version_info >= (3, 4):
            regex = '^Future exception was never retrieved\\n'
    ...
    def test_future_iter_throw(self):
        ...
        self.assertRaises(TypeError, fi.throw, Exception, Exception(
            'elephant'), 32)
        self.assertRaises(TypeError, fi.throw, Exception('elephant'),
            Exception('elephant'))
...
class PyFutureDoneCallbackTests(BaseFutureDoneCallbackTests, test_utils.
    TestCase):
