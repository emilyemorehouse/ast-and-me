...
STR_RGX_REPR = (
    '^<(?P<class>.*?) object at (?P<address>.*?)\\[(?P<extras>(set|unset|locked|unlocked)(,value:\\d)?(,waiters:\\d+)?)\\]>\\Z'
    )
...
class ConditionTests(test_utils.TestCase):
    ...
    def test_wait_unacquired(self):
        cond = asyncio.Condition(loop=self.loop)
        self.assertRaises(RuntimeError, self.loop.run_until_complete, cond.
            wait())
...
class SemaphoreTests(test_utils.TestCase):
    ...
    def test_acquire_cancel(self):
        ...
        self.assertRaises(asyncio.CancelledError, self.loop.
            run_until_complete, acquire)
        self.assertTrue(not sem._waiters or all(waiter.done() for waiter in
            sem._waiters))
