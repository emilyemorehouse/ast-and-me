...
class LockTests(BaseTest):
    ...
    def test_context_manager_async_with(self):
        primitives = [asyncio.Lock(loop=self.loop), asyncio.Condition(loop=
            self.loop), asyncio.Semaphore(loop=self.loop), asyncio.
            BoundedSemaphore(loop=self.loop)]
...
class CoroutineTests(BaseTest):
    ...
    def test_async_def_wrapped(self, m_log):
        ...
        async def start():
            foo_coro = foo()
            self.assertRegex(repr(foo_coro),
                '<CoroWrapper .*\\.foo\\(\\) running at .*pep492.*>')
            with support.check_warnings(('.*foo.*was never', RuntimeWarning)):
                foo_coro = None
                support.gc_collect()
                self.assertTrue(m_log.error.called)
                message = m_log.error.call_args[0][0]
                self.assertRegex(message, 'CoroWrapper.*foo.*was never')
        self.loop.set_debug(True)
        ...
        async def start():
            ...
            self.assertRegex(repr(task), 'Task.*foo.*running')
        self.loop.run_until_complete(start())
    ...
    def test_double_await(self):
        ...
        with self.assertRaisesRegex(RuntimeError,
            'Cannot await.*test_double_await.*\\bafunc\\b.*while.*\\bsleep\\b'
            ):
            self.loop.run_until_complete(runner())
