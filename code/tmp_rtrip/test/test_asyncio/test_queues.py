...
class QueueGetTests(_QueueTestBase):
    ...
    def test_why_are_getters_waiting(self):
        ...
        self.loop.run_until_complete(asyncio.gather(producer(q,
            producer_num_items), consumer(q, producer_num_items), loop=self
            .loop))
...
class QueuePutTests(_QueueTestBase):
    ...
    def test_why_are_putters_waiting(self):
        ...
        t2 = putter(2)
        t3 = putter(3)
        self.loop.run_until_complete(asyncio.gather(getter(), t0, t1, t2,
            t3, loop=self.loop))
...
class _QueueJoinTestMixin:
    ...
    def test_task_done(self):
        ...
        @asyncio.coroutine
        def test():
            tasks = [asyncio.Task(worker(), loop=self.loop) for index in
                range(2)]
            yield from q.join()
            return tasks
