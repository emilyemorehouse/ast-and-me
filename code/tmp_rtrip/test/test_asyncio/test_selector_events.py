...
class BaseSelectorEventLoopTests(test_utils.TestCase):
    ...
    def test_make_ssl_transport(self):
        ...
        with test_utils.disable_logger():
            transport = self.loop._make_ssl_transport(m, asyncio.Protocol(),
                m, waiter)
    ...
    def test_sock_sendall(self):
        ...
        self.assertIsInstance(f, asyncio.Future)
        self.assertEqual((f, False, sock, b'data'), self.loop._sock_sendall
            .call_args[0])
    ...
    def test__sock_sendall_tryagain(self):
        ...
        self.loop.add_writer = mock.Mock()
        self.loop._sock_sendall(f, False, sock, b'data')
        self.assertEqual((10, self.loop._sock_sendall, f, True, sock,
            b'data'), self.loop.add_writer.call_args[0])
    ...
    def test__sock_sendall_partial(self):
        ...
        self.assertEqual((10, self.loop._sock_sendall, f, True, sock, b'ta'
    ...
    def test_sock_connect_timeout(self):
        ...
        fut = self.loop.create_task(self.loop.sock_connect(sock, (
            '127.0.0.1', 80)))
        self.loop._run_once()
    ...
    def test_sock_connect_resolve_using_socket_params(self, m_gai):
        ...
        while not m_gai.called:
            self.loop._run_once()
        m_gai.assert_called_with(addr[0], addr[1], sock.family, sock.type,
            sock.proto, 0)
        con.cancel()
        with self.assertRaises(asyncio.CancelledError):
    ...
    def test__sock_connect(self):
        ...
        resolved.set_result([(socket.AF_INET, socket.SOCK_STREAM, socket.
            IPPROTO_TCP, '', ('127.0.0.1', 8080))])
    ...
    def test__sock_accept_tryagain(self):
        ...
        self.loop.add_reader = mock.Mock()
        self.loop._sock_accept(f, False, sock)
        self.assertEqual((10, self.loop._sock_accept, f, True, sock), self.
            loop.add_reader.call_args[0])
    ...
    def test_add_reader_existing(self):
        reader = mock.Mock()
        writer = mock.Mock()
        self.loop._selector.get_key.return_value = selectors.SelectorKey(1,
            1, selectors.EVENT_WRITE, (reader, writer))
        cb = lambda : True
        self.loop.add_reader(1, cb)
    ...
    def test_remove_reader(self):
        self.loop._selector.get_key.return_value = selectors.SelectorKey(1,
            1, selectors.EVENT_READ, (None, None))
    ...
    def test_remove_reader_read_write(self):
        ...
        writer = mock.Mock()
        self.loop._selector.get_key.return_value = selectors.SelectorKey(1,
            1, selectors.EVENT_READ | selectors.EVENT_WRITE, (reader, writer))
        self.assertTrue(self.loop.remove_reader(1))
        self.assertFalse(self.loop._selector.unregister.called)
        self.assertEqual((1, selectors.EVENT_WRITE, (None, writer)), self.
            loop._selector.modify.call_args[0])
    ...
    def test_add_writer_existing(self):
        ...
        writer = mock.Mock()
        self.loop._selector.get_key.return_value = selectors.SelectorKey(1,
            1, selectors.EVENT_READ, (reader, writer))
        cb = lambda : True
        self.loop.add_writer(1, cb)
    ...
    def test_remove_writer(self):
        self.loop._selector.get_key.return_value = selectors.SelectorKey(1,
            1, selectors.EVENT_WRITE, (None, None))
    ...
    def test_remove_writer_read_write(self):
        ...
        self.assertFalse(self.loop._selector.unregister.called)
        self.assertEqual((1, selectors.EVENT_READ, (reader, None)), self.
            loop._selector.modify.call_args[0])
    ...
    def test_process_events_read(self):
        ...
        self.loop._process_events([(selectors.SelectorKey(1, 1, selectors.
            EVENT_READ, (reader, None)), selectors.EVENT_READ)])
    ...
    def test_process_events_write(self):
        ...
        self.loop._add_callback = mock.Mock()
        self.loop._process_events([(selectors.SelectorKey(1, 1, selectors.
            EVENT_WRITE, (None, writer)), selectors.EVENT_WRITE)])
...
class SelectorSocketTransportTests(test_utils.TestCase):
    ...
    def socket_transport(self, waiter=None):
        transport = _SelectorSocketTransport(self.loop, self.sock, self.
            protocol, waiter=waiter)
        self.addCleanup(close_transport, transport)
        return transport
    ...
    def test_write_buffer(self):
        ...
        self.assertFalse(self.sock.send.called)
        self.assertEqual(list_to_buffer([b'data1', b'data2']), transport.
            _buffer)
...
class SelectorSslTransportTests(test_utils.TestCase):
    ...
    def ssl_transport(self, waiter=None, server_hostname=None):
        transport = _SelectorSslTransport(self.loop, self.sock, self.
            protocol, self.sslcontext, waiter=waiter, server_hostname=
            server_hostname)
        self.addCleanup(close_transport, transport)
        return transport
    ...
    def test_write_bytearray(self):
        ...
        self.assertEqual(list_to_buffer([b'data']), transport._buffer)
        self.assertEqual(data, bytearray(b'data'))
        self.assertIsNot(data, transport._buffer)
    ...
    def test_read_ready_write_wants_read(self):
        ...
        self.loop._add_writer.assert_called_with(transport._sock_fd,
            transport._write_ready)
    ...
    def test_write_ready_read_wants_write(self):
        ...
        self.loop._add_reader.assert_called_with(transport._sock_fd,
            transport._read_ready)
    ...
    def test_server_hostname(self):
        ...
        self.sslcontext.wrap_socket.assert_called_with(self.sock,
            do_handshake_on_connect=False, server_side=False,
...
class SelectorDatagramTransportTests(test_utils.TestCase):
    ...
    def datagram_transport(self, address=None):
        transport = _SelectorDatagramTransport(self.loop, self.sock, self.
    ...
    def test_sendto(self):
        ...
        self.assertEqual(self.sock.sendto.call_args[0], (data, ('0.0.0.0', 
            1234)))
    ...
    def test_sendto_no_data(self):
        ...
        self.assertEqual([(b'data', ('0.0.0.0', 12345))], list(transport.
            _buffer))
    ...
    def test_sendto_buffer(self):
        ...
        self.assertFalse(self.sock.sendto.called)
        self.assertEqual([(b'data1', ('0.0.0.0', 12345)), (b'data2', (
            '0.0.0.0', 12345))], list(transport._buffer))
    ...
    def test_sendto_ready_tryagain(self):
        ...
        transport._sendto_ready()
        self.loop.assert_writer(7, transport._sendto_ready)
        self.assertEqual([(b'data1', ()), (b'data2', ())], list(transport.
            _buffer))
