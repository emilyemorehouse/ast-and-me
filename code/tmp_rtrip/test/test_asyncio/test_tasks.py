...
def format_coroutine(qualname, state, src, source_traceback, generator=False):
    ...
    if source_traceback is not None:
        ...
        return 'coro=<%s() %s at %s> created at %s:%s' % (qualname, state,
            src, frame[0], frame[1])
    else:
...
class BaseTaskTests:
    ...
    def test_task_repr(self):
        ...
        if PY35:
            self.assertRegex(notmuch.__qualname__,
                '\\w+.test_task_repr.<locals>.notmuch')
        ...
        coro = format_coroutine(coro_qualname, 'running', src, t.
            _source_traceback, generator=True)
        self.assertEqual(repr(t), '<Task pending %s cb=[<Dummy>()]>' % coro)
        t.cancel()
        self.assertEqual(repr(t), '<Task cancelling %s cb=[<Dummy>()]>' % coro)
        self.assertRaises(asyncio.CancelledError, self.loop.
    ...
    def test_task_repr_coro_decorator(self):
        ...
        if PY35:
            self.assertRegex(notmuch.__qualname__,
                '\\w+.test_task_repr_coro_decorator\\.<locals>\\.notmuch')
        self.assertEqual(notmuch.__module__, __name__)
        gen = notmuch()
        if coroutines._DEBUG or PY35:
            coro_name = 'notmuch'
        ...
        if coroutines._DEBUG:
            if coroutines._DEBUG:
                ...
                code = gen.gi_code
                coro = '%s() running at %s:%s' % (coro_qualname, code.
        ...
        coro = format_coroutine(coro_qualname, 'running', src, t.
            _source_traceback, generator=not coroutines._DEBUG)
        self.assertEqual(repr(t), '<Task pending %s cb=[<Dummy>()]>' % coro)
        self.loop.run_until_complete(t)
    ...
    def test_task_repr_wait_for(self):
        ...
        self.assertRegex(repr(task), '<Task .* wait_for=%s>' % re.escape(
            repr(fut)))
        fut.set_result(None)
        self.loop.run_until_complete(task)
    ...
    def test_task_repr_partial_corowrapper(self):
        ...
        expected = (
            '<CoroWrapper \\w+.test_task_repr_partial_corowrapper\\.<locals>\\.func\\(1\\)\\(\\) running, '
            )
    ...
    def test_wait_errors(self):
        self.assertRaises(ValueError, self.loop.run_until_complete, asyncio
            .wait(set(), loop=self.loop))
    ...
    def test_wait_first_completed(self):
        ...
        task = self.new_task(loop, asyncio.wait([b, a], return_when=asyncio
            .FIRST_COMPLETED, loop=loop))
    ...
    def test_wait_really_done(self):
        ...
        task = self.new_task(self.loop, asyncio.wait([b, a], return_when=
            asyncio.FIRST_COMPLETED, loop=self.loop))
        done, pending = self.loop.run_until_complete(task)
    ...
    def test_wait_concurrent_complete(self):
        ...
        a = self.new_task(loop, asyncio.sleep(0.1, loop=loop))
        b = self.new_task(loop, asyncio.sleep(0.15, loop=loop))
        done, pending = loop.run_until_complete(asyncio.wait([b, a],
    ...
    def test_as_completed_reverse_wait(self):
        ...
        b = asyncio.sleep(0.1, 'b', loop=loop)
        fs = {a, b}
    ...
    def test_current_task_with_interleaving_tasks(self):
        ...
        self.loop.run_until_complete(asyncio.wait((task1, task2), loop=self
            .loop))
        self.assertIsNone(Task.current_task(loop=self.loop))
    ...
    def test_as_completed_invalid_args(self):
        fut = self.new_future(self.loop)
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.
            as_completed(fut, loop=self.loop))
    ...
    def test_wait_invalid_args(self):
        fut = self.new_future(self.loop)
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.
            wait(fut, loop=self.loop))
        coro = coroutine_function()
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.
            wait(coro, loop=self.loop))
        coro.close()
        self.assertRaises(ValueError, self.loop.run_until_complete, asyncio
            .wait([], loop=self.loop))
    ...
    def test_log_destroyed_pending_task(self):
        ...
        mock_handler.assert_called_with(self.loop, {'message':
            'Task was destroyed but it is pending!', 'task': mock.ANY,
    ...
    def test_coroutine_never_yielded(self, m_log):
        ...
        regex = (
            '^<CoroWrapper %s\\(?\\)? .* at %s:%s, .*> was never yielded from\\nCoroutine object created at \\(most recent call last\\):\\n.*\\n  File "%s", line %s, in test_coroutine_never_yielded\\n    coro_noop\\(\\)$'
             % (re.escape(coro_noop.__qualname__), re.escape(func_filename),
            func_lineno, re.escape(tb_filename), tb_lineno))
    ...
    def test_task_source_traceback(self):
        ...
        self.assertEqual(task._source_traceback[-2][:3], (__file__, lineno,
...
def add_subclass_tests(cls):
    ...
    def test_subclasses_ctask_cfuture(self):
        ...
        self.assertEqual(dict(task.calls), {'_step': 2, '_wakeup': 1,
...
class GatherTestsBase:
    ...
    def test_env_var_debug(self):
        ...
        code = '\n'.join(('import asyncio.coroutines',
            'print(asyncio.coroutines._DEBUG)'))
        sts, stdout, stderr = assert_python_ok('-E', '-c', code, PYTHONPATH
            =aio_path)
        self.assertEqual(stdout.rstrip(), b'False')
        sts, stdout, stderr = assert_python_ok('-c', code,
            PYTHONASYNCIODEBUG='', PYTHONPATH=aio_path)
        self.assertEqual(stdout.rstrip(), b'False')
        sts, stdout, stderr = assert_python_ok('-c', code,
            PYTHONASYNCIODEBUG='1', PYTHONPATH=aio_path)
...
class FutureGatherTests(GatherTestsBase, test_utils.TestCase):
    ...
    def test_result_exception_one_cancellation(self):
        a, b, c, d, e, f = [asyncio.Future(loop=self.one_loop) for i in
            range(6)]
...
class RunCoroutineThreadsafeTests(test_utils.TestCase):
    ...
    def test_run_coroutine_threadsafe_task_factory_exception(self):
        ...
        future = self.loop.run_in_executor(None, lambda : self.target(
            advance_coro=True))
