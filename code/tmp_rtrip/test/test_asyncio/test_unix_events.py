...
class SelectorEventLoopSignalTests(test_utils.TestCase):
    ...
    def test_add_signal_handler_coroutine_error(self, m_signal):
        ...
        for func in (coro_func, coro_obj):
            self.assertRaisesRegex(TypeError,
                'coroutines cannot be used with add_signal_handler', self.
    ...
    def test_add_signal_handler_install_error(self, m_signal):
        ...
        self.assertRaises(Err, self.loop.add_signal_handler, signal.SIGINT,
            lambda : True)
    ...
    def test_remove_signal_handler(self, m_signal):
        ...
        self.assertEqual((signal.SIGHUP, m_signal.SIG_DFL), m_signal.signal
            .call_args[0])
    ...
    def test_remove_signal_handler_2(self, m_signal):
        ...
        self.assertTrue(m_signal.signal.called)
        self.assertEqual((signal.SIGINT, m_signal.default_int_handler),
    ...
    def test_remove_signal_handler_error(self, m_signal):
        ...
        self.assertRaises(OSError, self.loop.remove_signal_handler, signal.
            SIGHUP)
    ...
    def test_remove_signal_handler_error2(self, m_signal):
        ...
        class Err(OSError):
            errno = errno.EINVAL
        m_signal.signal.side_effect = Err
        self.assertRaises(RuntimeError, self.loop.remove_signal_handler,
            signal.SIGHUP)
...
class SelectorEventLoopUnixSocketTests(test_utils.TestCase):
    ...
    def test_create_unix_server_ssl_bool(self):
        coro = self.loop.create_unix_server(lambda : None, path='spam', ssl
            =True)
    ...
    def test_create_unix_server_path_stream_bittype(self):
        sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM | socket.
            SOCK_NONBLOCK)
        with tempfile.NamedTemporaryFile() as file:
            fn = file.name
        try:
    ...
    def test_create_unix_connection_path_sock(self):
        coro = self.loop.create_unix_connection(lambda : None, os.devnull,
            sock=object())
...
class UnixReadPipeTransportTests(test_utils.TestCase):
    ...
    def test__read_ready_error(self, m_read, m_logexc):
        ...
        m_logexc.assert_called_with(test_utils.MockPattern(
...
class UnixWritePipeTransportTests(test_utils.TestCase):
    ...
    def write_pipe_transport(self, waiter=None):
        transport = unix_events._UnixWritePipeTransport(self.loop, self.
            pipe, self.protocol, waiter=waiter)
        self.addCleanup(close_pipe_transport, transport)
        return transport
...
WaitPidMocks = collections.namedtuple('WaitPidMocks', ('waitpid',
...
class ChildWatcherTestsMixin:
    ...
    def test_create_watcher(self):
        self.m_add_signal_handler.assert_called_once_with(signal.SIGCHLD,
    ...
    def waitpid_mocks(func):
        ...
        def wrapped_func(self):
            ...
            with patch('os.WTERMSIG', self.WTERMSIG) as m_WTERMSIG, patch(
                'os.WEXITSTATUS', self.WEXITSTATUS) as m_WEXITSTATUS, patch(
                'os.WIFSIGNALED', self.WIFSIGNALED) as m_WIFSIGNALED, patch(
                'os.WIFEXITED', self.WIFEXITED) as m_WIFEXITED, patch(
                'os.waitpid', self.waitpid) as m_waitpid:
                func(self, WaitPidMocks(m_waitpid, m_WIFEXITED,
    ...
    def test_set_loop(self, m):
        ...
        with patch(old_loop, 'remove_signal_handler') as m_old_remove, patch(
            self.loop, 'add_signal_handler') as m_new_add:
            self.watcher.attach_loop(self.loop)
            m_old_remove.assert_called_once_with(signal.SIGCHLD)
            m_new_add.assert_called_once_with(signal.SIGCHLD, self.watcher.
                _sig_chld)
        self.running = False
    ...
    def test_set_loop_race_condition(self, m):
        ...
        old_loop = self.loop
        self.loop = None
        with mock.patch.object(old_loop, 'remove_signal_handler'
        ...
        self.loop = self.new_test_loop()
        with mock.patch.object(self.loop, 'add_signal_handler'
            ) as m_add_signal_handler:
            self.watcher.attach_loop(self.loop)
            m_add_signal_handler.assert_called_once_with(signal.SIGCHLD,
                self.watcher._sig_chld)
...
class PolicyTests(unittest.TestCase):
    ...
    def test_get_child_watcher_thread(self):
        ...
        def f():
            ...
            self.assertIsInstance(policy.get_event_loop(), asyncio.
                AbstractEventLoop)
            watcher = policy.get_child_watcher()
            self.assertIsInstance(watcher, asyncio.SafeChildWatcher)
            self.assertIsNone(watcher._loop)
