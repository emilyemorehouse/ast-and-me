...
class ProactorTests(test_utils.TestCase):
    ...
    def _test_pipe(self):
        ...
        with self.assertRaises(FileNotFoundError):
            yield from self.loop.create_pipe_connection(asyncio.Protocol,
                ADDRESS)
        ...
        for i in range(5):
            ...
            protocol = asyncio.StreamReaderProtocol(stream_reader, loop=
                self.loop)
            trans, proto = yield from self.loop.create_pipe_connection(lambda :
                protocol, ADDRESS)
