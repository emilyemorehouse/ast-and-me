...
class HelperFunctionTests(unittest.TestCase):
    ...
    def test_readwrite(self):
        ...
        expected = (select.POLLIN, 'read'), (select.POLLPRI, 'expt'), (select
            .POLLOUT, 'write'), (select.POLLERR, 'closed'), (select.POLLHUP,
            'closed'), (select.POLLNVAL, 'closed')
...
class BaseTestAPI:
    ...
    def test_create_socket(self):
        ...
        if hasattr(socket, 'SOCK_CLOEXEC'):
            self.assertIn(s.socket.type, (sock_type | socket.SOCK_CLOEXEC,
                sock_type))
        else:
    ...
    def test_quick_connect(self):
        if self.family not in (socket.AF_INET, getattr(socket, 'AF_INET6',
            object())):
        ...
        try:
            with socket.socket(self.family, socket.SOCK_STREAM) as s:
                s.settimeout(0.2)
                s.setsockopt(socket.SOL_SOCKET, socket.SO_LINGER, struct.
                    pack('ii', 1, 0))
                try:
