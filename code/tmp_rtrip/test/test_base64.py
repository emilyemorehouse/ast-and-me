...
class BaseXYTestCase(unittest.TestCase):
    ...
    def test_b64encode(self):
        ...
        eq(base64.b64encode(b'\xd3V\xbeo\xf7\x1d', altchars=memoryview(
            b'*$')), b'01a*b$cd')
        eq(base64.b64encode(b'\xd3V\xbeo\xf7\x1d', altchars=array('B',
            b'*$')), b'01a*b$cd')
        ...
        eq(base64.standard_b64encode(b'www.python.org'),
    ...
    def test_b64decode(self):
        eq = self.assertEqual
        tests = {b'd3d3LnB5dGhvbi5vcmc=': b'www.python.org', b'AA==':
            b'\x00', b'YQ==': b'a', b'YWI=': b'ab', b'YWJj': b'abc',
            b'YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXpBQkNERUZHSElKS0xNTk9QUVJTVFVWV1hZWjAxMjM0\nNTY3ODkhQCMwXiYqKCk7Ojw+LC4gW117fQ=='
            :
            b'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#0^&*();:<>,. []{}'
            , b'': b''}
    ...
    def test_b64decode_invalid_chars(self):
        tests = (b'%3d==', b'\xdd'), (b'$3d==', b'\xdd'), (b'[==', b''), (
            b'YW]3=', b'am'), (b'3{d==', b'\xdd'), (b'3d}==', b'\xdd'), (b'@@',
            b''), (b'!', b''), (b'YWJj\nYWI=', b'abcab')
        funcs = (base64.b64decode, base64.standard_b64decode, base64.
            urlsafe_b64decode)
    ...
    def test_b32decode(self):
        eq = self.assertEqual
        tests = {b'': b'', b'AA======': b'\x00', b'ME======': b'a',
            b'MFRA====': b'ab', b'MFRGG===': b'abc', b'MFRGGZA=': b'abcd',
            b'MFRGGZDF': b'abcde'}
        for data, res in tests.items():
    ...
    def test_b32decode_casefold(self):
        eq = self.assertEqual
        tests = {b'': b'', b'ME======': b'a', b'MFRA====': b'ab',
            b'MFRGG===': b'abc', b'MFRGGZA=': b'abcd', b'MFRGGZDF':
            b'abcde', b'me======': b'a', b'mfra====': b'ab', b'mfrgg===':
            b'abc', b'mfrggza=': b'abcd', b'mfrggzdf': b'abcde'}
        for data, res in tests.items():
    ...
    def test_b85encode(self):
        eq = self.assertEqual
        tests = {b'': b'', b'www.python.org': b'cXxL#aCvlSZ*DGca%T', bytes(
            range(255)):
            ...
            b'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#0^&*();:<>,. []{}'
            :
            b'VPa!sWoBn+X=-b1ZEkOHadLBXb#`}nd3r%YLqtVJM@UIZOH55pPf$@(Q&d$}S6EqEFflSSG&MFiI5{CeBQRbjDkv#CIy^osE+AW7dwl'
            , b'no padding..': b'Zf_uPVPs@!Zf7no',
            b'zero compression\x00\x00\x00\x00':
            b'dS!BNAY*TBaB^jHb7^mG00000', b'zero compression\x00\x00\x00':
            b'dS!BNAY*TBaB^jHb7^mG0000', b'Boundary:\x00\x00\x00\x00':
            b'LT`0$WMOi7IsgCw00', b'Space compr:    ':
            b'Q*dEpWgug3ZE$irARr(h', b'\xff': b'{{', (b'\xff' * 2): b'|Nj',
            (b'\xff' * 3): b'|Ns9', (b'\xff' * 4): b'|NsC0'}
        for data, res in tests.items():
    ...
    def test_a85decode(self):
        eq = self.assertEqual
        tests = {b'': b'', b'GB\\6`E-ZP=Df.1GEb>': b'www.python.org',
            b'! ! * -\'"\n\t\t9eu\r\n7#  RL\x0bhG$k3[W&.oNg\'GVB"(`=52*$$(B+<_pR,UFcb-n-Vr/1iJ-0JP==1c70M3&s#]4?Ykm5X@_(6q\'R884cEH9MJ8X:f1+h<)lt#=BSg3>[:ZC?t!MSA7]@cBPD3sCi+\'.E,fo>FEMbNG^4U^I!pHnJ:W<)KS>/9Ll%"IN/`jYOHG]iPa.Q$R$jD4S=Q7DTV8*TUnsrdW2ZetXKAY/Yd(L?[\'d?O\\@K2_]Y2%o^qmn*`5Ta:aN;TJbg"GZd*^:jeCE.%f\\,!5gtgiEi8N\\UjQ5OekiqBum-X60nF?)@o_%qPq"ad`r;HT'
            : bytes(range(255)),
            b'@:E_WAS,RgBkhF"D/O92EH6,BF`qtRH$VbC6UX@47n?3D92&&T:Jand;cHat=\'/U/0JP==1c70M3&r-I,;<FN.OZ`-3]oSW/g+A(H[P'
            :
            b'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#0^&*();:<>,. []{}'
            , b'DJpY:@:Wn_DJ(RS': b'no padding..', b'H=_,8+Cf>,E,oN2F(oQ1z':
            b'zero compression\x00\x00\x00\x00',
            b'H=_,8+Cf>,E,oN2F(oQ1!!!!': b'zero compression\x00\x00\x00',
            b'6>q!aA79M(3WK-[!!': b'Boundary:\x00\x00\x00\x00',
            b';fH/TAKYK$D/aMV+<VdL': b'Space compr:    ', b'rr': b'\xff',
            b's8N': b'\xff' * 2, b's8W*': b'\xff' * 3, b's8W-!': b'\xff' * 4}
        for data, res in tests.items():
    ...
    def test_b85decode(self):
        eq = self.assertEqual
        tests = {b'': b'', b'cXxL#aCvlSZ*DGca%T': b'www.python.org',
            b'009C61O)~M2nh-c3=Iws5D^j+6crX17#SKH9337XAR!_nBqb&%C@Cr{EG;fCFflSSG&MFiI5|2yJUu=?KtV!7L`6nNNJ&adOifNtP*GA-R8>}2SXo+ITwPvYU}0ioWMyV&XlZI|Y;A6DaB*^Tbai%jczJqze0_d@fPsR8goTEOh>41ejE#<ukdcy;l$Dm3n3<ZJoSmMZprN9pq@|{(sHv)}tgWuEu(7hUw6(UkxVgH!yuH4^z`?@9#Kp$P$jQpf%+1cv(9zP<)YaD4*xB0K+}+;a;Njxq<mKk)=;`X~?CtLF@bU8V^!4`l`1$(#{Qdp'
            : bytes(range(255)),
            b'VPa!sWoBn+X=-b1ZEkOHadLBXb#`}nd3r%YLqtVJM@UIZOH55pPf$@(Q&d$}S6EqEFflSSG&MFiI5{CeBQRbjDkv#CIy^osE+AW7dwl'
            :
            ...
            b'dS!BNAY*TBaB^jHb7^mG00000':
            b'zero compression\x00\x00\x00\x00',
            b'dS!BNAY*TBaB^jHb7^mG0000': b'zero compression\x00\x00\x00',
            b'LT`0$WMOi7IsgCw00': b'Boundary:\x00\x00\x00\x00',
            b'Q*dEpWgug3ZE$irARr(h': b'Space compr:    ', b'{{': b'\xff',
            b'|Nj': b'\xff' * 2, b'|Ns9': b'\xff' * 3, b'|NsC0': b'\xff' * 4}
        for data, res in tests.items():
    ...
    def test_a85decode_errors(self):
        ...
        self.assertRaises(ValueError, base64.a85decode, b'malformed', adobe
            =True)
    ...
    def test_decode_nonascii_str(self):
        decode_funcs = (base64.b64decode, base64.standard_b64decode, base64
            .urlsafe_b64decode, base64.b32decode, base64.b16decode, base64.
            b85decode, base64.a85decode)
