...
class ExceptionClassTests(unittest.TestCase):
    ...
    def verify_instance_interface(self, ins):
        for attr in ('args', '__str__', '__repr__'):
            self.assertTrue(hasattr(ins, attr), '%s missing %s attribute' %
    ...
    def test_inheritance(self):
        ...
        try:
            ...
            self.assertIn(superclass_name, exc_set, '%s not found' %
                superclass_name)
    ...
    def test_interface_single_arg(self):
        arg = 'spam'
        exc = Exception(arg)
        results = [len(exc.args), 1], [exc.args[0], arg], [str(exc), str(arg)
    ...
    def test_interface_multi_arg(self):
        ...
        exc = Exception(*args)
        results = [len(exc.args), arg_count], [exc.args, args], [str(exc),
            str(args)], [repr(exc), exc.__class__.__name__ + repr(exc.args)]
        self.interface_test_driver(results)

    def test_interface_no_arg(self):
        exc = Exception()
        results = [len(exc.args), 0], [exc.args, tuple()], [str(exc), ''], [
