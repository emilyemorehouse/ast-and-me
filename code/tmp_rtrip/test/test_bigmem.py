...
class BaseStrTest:
    ...
    def test_find(self, size):
        ...
        self.assertEqual(s.find(_('i'), sublen), sublen + size + SUBSTR.
            find(_('i')))
        self.assertEqual(s.find(_('i'), size), sublen + size + SUBSTR.find(
            _('i')))
    ...
    def test_index(self, size):
        ...
        self.assertEqual(s.index(_('i'), sublen), sublen + size + SUBSTR.
            index(_('i')))
        self.assertEqual(s.index(_('i'), size), sublen + size + SUBSTR.
            index(_('i')))
    ...
    def test_rfind(self, size):
        ...
        self.assertEqual(s.rfind(_('i'), 0, sublen + size), SUBSTR.rfind(_(
            'i')))
    ...
    def test_rindex(self, size):
        ...
        self.assertEqual(s.rindex(_(' '), 0, sublen + size - 1), SUBSTR.
            rindex(_(' ')))
        self.assertEqual(s.rindex(SUBSTR, 0, sublen + size), 0)
        self.assertEqual(s.rindex(_('i')), sublen + size + SUBSTR.rindex(_(
            'i')))
        self.assertEqual(s.rindex(_('i'), 0, sublen), SUBSTR.rindex(_('i')))
        self.assertEqual(s.rindex(_('i'), 0, sublen + size), SUBSTR.rindex(
            _('i')))
...
class StrTest(unittest.TestCase, BaseStrTest):
    ...
    def test_encode_utf32(self, size):
        try:
            return self.basic_encode_test(size, 'utf32', expectedsize=4 *
                size + 4)
        except MemoryError:
            pass
    ...
    @bigmemtest(size=_2G // 5 + 1, memuse=ucs2_char_size + ucs4_char_size +
        ascii_char_size * 6)
    ...
    @bigmemtest(size=_2G // 5 + 1, memuse=ucs4_char_size * 2 + 
        ascii_char_size * 10)
...
class TupleTest(unittest.TestCase):
    ...
    def test_from_2G_generator(self, size):
        self.skipTest(
            'test needs much more memory than advertised, see issue5438')
        try:
            t = tuple(range(size))
        except MemoryError:
            pass
