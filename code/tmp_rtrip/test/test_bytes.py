...
class BaseBytesTest:
    ...
    def test_compare_to_str(self):
        ...
        self.assertEqual(self.type2test(
            b'\x00\x00\x00a\x00\x00\x00b\x00\x00\x00c') == 'abc', False)
        self.assertEqual(self.type2test(b'a\x00b\x00c\x00') == 'abc', False)
        self.assertEqual(self.type2test(
            b'a\x00\x00\x00b\x00\x00\x00c\x00\x00\x00') == 'abc', False)
        ...
        for start in indices:
            for stop in indices:
                for step in indices[1:]:
                    self.assertEqual(b[start:stop:step], self.type2test(L[
                        start:stop:step]))
    ...
    def test_repeat_1char(self):
        self.assertEqual(self.type2test(b'x') * 100, self.type2test([ord(
            'x')] * 100))
    ...
    def test_fromhex(self):
        ...
        for data, pos in (('12 x4 56', 3), ('12 3x 56', 4), ('12 xy 56', 3),
            ('12 3ÿ 56', 4)):
    ...
    def test_find(self):
        ...
        for index in (-1, 256, sys.maxsize + 1):
            self.assertRaisesRegex(ValueError,
                'byte must be in range\\(0, 256\\)', b.find, index)
    ...
    def test_strip_bytearray(self):
        ...
        self.assertEqual(self.type2test(b'abc').lstrip(memoryview(b'ac')),
            b'bc')
        self.assertEqual(self.type2test(b'abc').rstrip(memoryview(b'ac')),
            b'ab')
    ...
    def test_center(self):
        ...
        for fill_type in (bytes, bytearray):
            self.assertEqual(b.center(7, fill_type(b'-')), self.type2test(
                b'--abc--'))

    def test_ljust(self):
        b = self.type2test(b'abc')
        for fill_type in (bytes, bytearray):
            self.assertEqual(b.ljust(7, fill_type(b'-')), self.type2test(
                b'abc----'))

    def test_rjust(self):
        b = self.type2test(b'abc')
        for fill_type in (bytes, bytearray):
            self.assertEqual(b.rjust(7, fill_type(b'-')), self.type2test(
                b'----abc'))

    def test_ord(self):
        b = self.type2test(b'\x00A\x7f\x80\xff')
        self.assertEqual([ord(b[i:i + 1]) for i in range(len(b))], [0, 65, 
            127, 128, 255])
    ...
    def test_maketrans(self):
        ...
        self.assertEqual(self.type2test.maketrans(b'\xfd\xfe\xff', b'xyz'),
            transtable)
    ...
    def test_find_etc_raise_correct_error_messages(self):
        b = self.type2test(b'hello')
        x = self.type2test(b'x')
        self.assertRaisesRegex(TypeError, '\\bfind\\b', b.find, x, None,
            None, None)
        self.assertRaisesRegex(TypeError, '\\brfind\\b', b.rfind, x, None,
            None, None)
        self.assertRaisesRegex(TypeError, '\\bindex\\b', b.index, x, None,
            None, None)
        self.assertRaisesRegex(TypeError, '\\brindex\\b', b.rindex, x, None,
            None, None)
        self.assertRaisesRegex(TypeError, '\\bcount\\b', b.count, x, None,
            None, None)
        self.assertRaisesRegex(TypeError, '\\bstartswith\\b', b.startswith,
            x, None, None, None)
        self.assertRaisesRegex(TypeError, '\\bendswith\\b', b.endswith, x,
            None, None, None)
...
class BytesTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_from_format(self):
        ...
        self.assertEqual(PyBytes_FromFormat(b'd=%d ld=%ld zd=%zd', c_int(1),
            c_long(2), c_size_t(3)), b'd=1 ld=2 zd=3')
        self.assertEqual(PyBytes_FromFormat(b'd=%d ld=%ld zd=%zd', c_int(-1
            ), c_long(-2), c_size_t(-3)), b'd=-1 ld=-2 zd=-3')
        self.assertEqual(PyBytes_FromFormat(b'u=%u lu=%lu zu=%zu', c_uint(
            123), c_ulong(456), c_size_t(789)), b'u=123 lu=456 zu=789')
        ...
        for formatstr, ctypes_type, value, py_formatter in ((b'%d', c_int,
            _testcapi.INT_MIN, str), (b'%d', c_int, _testcapi.INT_MAX, str),
            (b'%ld', c_long, _testcapi.LONG_MIN, str), (b'%ld', c_long,
            _testcapi.LONG_MAX, str), (b'%lu', c_ulong, _testcapi.ULONG_MAX,
            str), (b'%zd', c_ssize_t, _testcapi.PY_SSIZE_T_MIN, str), (
            b'%zd', c_ssize_t, _testcapi.PY_SSIZE_T_MAX, str), (b'%zu',
            c_size_t, size_max, str), (b'%p', c_char_p, size_max,
            ptr_formatter)):
            self.assertEqual(PyBytes_FromFormat(formatstr, ctypes_type(
                value)), py_formatter(value).encode('ascii')),
...
class ByteArrayTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_setslice(self):
        ...
        self.assertEqual(b, bytearray(range(10)))
        b[3:0] = [42, 42, 42]
        self.assertEqual(b, bytearray([0, 1, 2, 42, 42, 42, 3, 4, 5, 6, 7, 
            8, 9]))
        b[3:] = b'foo'
        ...
        for elem in [5, -5, 0, int(1e+21), 'str', 2.3, ['a', 'b'], [b'a',
            b'b'], [[]]]:
            with self.assertRaises(TypeError):
                b[3:4] = elem
    ...
    test_exhausted_iterator = (test.list_tests.CommonTest.
...
class AssortedBytesTest(unittest.TestCase):
    ...
    def test_literal(self):
        tests = [(b'Wonderful spam', 'Wonderful spam'), (
            b'Wonderful spam too', 'Wonderful spam too'), (
            b'\xaa\x00\x00\x80', 'ª\x00\x00\x80'), (b'\\xaa\\x00\\000\\200',
            '\\xaa\\x00\\000\\200')]
        for b, s in tests:
...
class BytearrayPEP3137Test(unittest.TestCase):
    ...
    def test_returns_new_copy(self):
        ...
        for methname in ('zfill', 'rjust', 'ljust', 'center'):
            ...
            self.assertTrue(val is not newval, methname +
        ...
        for expr in ('val.split()[0]', 'val.rsplit()[0]',
            ...
            newval = eval(expr)
            self.assertEqual(val, newval)
            self.assertTrue(val is not newval, expr +
