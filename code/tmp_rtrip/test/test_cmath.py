...
complex_infinities = [complex(x, y) for x, y in [(INF, 0.0), (INF, 2.3), (
    INF, INF), (2.3, INF), (0.0, INF), (-0.0, INF), (-2.3, INF), (-INF, INF
    ), (-INF, 2.3), (-INF, 0.0), (-INF, -0.0), (-INF, -2.3), (-INF, -INF),
    (-2.3, -INF), (-0.0, -INF), (0.0, -INF), (2.3, -INF), (INF, -INF), (INF,
    -2.3), (INF, -0.0)]]
complex_nans = [complex(x, y) for x, y in [(NAN, -INF), (NAN, -2.3), (NAN, 
    -0.0), (NAN, 0.0), (NAN, 2.3), (NAN, INF), (-INF, NAN), (-2.3, NAN), (-
    0.0, NAN), (0.0, NAN), (2.3, NAN), (INF, NAN)]]


class CMathTests(unittest.TestCase):
    test_functions = [getattr(cmath, fname) for fname in ['acos', 'acosh',
        'asin', 'asinh', 'atan', 'atanh', 'cos', 'cosh', 'exp', 'log',
    ...
    def rAssertAlmostEqual(self, a, b, rel_err=2e-15, abs_err=5e-323, msg=None
        ...
        if math.isinf(a):
            if a == b:
                return
            self.fail(msg or
                'finite result where infinity expected: expected {!r}, got {!r}'
                .format(a, b))
        if not a and not b:
            if math.copysign(1.0, a) != math.copysign(1.0, b):
                self.fail(msg or
                    'zero has wrong sign: expected {!r}, got {!r}'.format(a, b)
                    )
        try:
            ...
            if absolute_error <= max(abs_err, rel_err * abs(a)):
                return
        self.fail(msg or '{!r} and {!r} are not sufficiently close'.format(
            a, b))

    def test_constants(self):
        e_expected = 2.718281828459045
        pi_expected = 3.141592653589793
        self.assertAlmostEqual(cmath.pi, pi_expected, places=9, msg=
            'cmath.pi is {}; should be {}'.format(cmath.pi, pi_expected))
        self.assertAlmostEqual(cmath.e, e_expected, places=9, msg=
            'cmath.e is {}; should be {}'.format(cmath.e, e_expected))
    ...
    def test_user_object(self):
        ...
        flt_arg = -6.131677725
        non_complexes = ['not complex', 1, 5, 2.0, None, object(),
            NotImplemented]
    ...
    def test_cmath_matches_math(self):
        ...
        unit_interval = test_values + [(-x) for x in test_values] + [0.0, 
            1.0, -1.0]
        positive = test_values + [1.0] + [(1.0 / x) for x in test_values]
        nonnegative = [0.0] + positive
        real_line = [0.0] + positive + [(-x) for x in positive]
        test_functions = {'acos': unit_interval, 'asin': unit_interval,
            'atan': real_line, 'cos': real_line, 'cosh': real_line, 'exp':
            real_line, 'log': positive, 'log10': positive, 'sin': real_line,
            'sinh': real_line, 'sqrt': nonnegative, 'tan': real_line,
            'tanh': real_line}
...
Received value insufficiently close to expected value."""
                .format(id, fn, ar, ai, expected.real, expected.imag,
                actual.real, actual.imag))
            self.rAssertAlmostEqual(expected.real, actual.real, abs_err=
                real_abs_err, msg=error_message)
            self.rAssertAlmostEqual(expected.imag, actual.imag, msg=
                error_message)
    ...
    def test_isfinite(self):
        real_vals = [float('-inf'), -2.3, -0.0, 0.0, 2.3, float('inf'),
            float('nan')]
        for x in real_vals:
            for y in real_vals:
                z = complex(x, y)
                self.assertEqual(cmath.isfinite(z), math.isfinite(x) and
                    math.isfinite(y))
...
class IsCloseTests(test_math.IsCloseTests):
    ...
    def test_complex_values(self):
        complex_examples = [(1.0 + 1j, 1.000000000001 + 1j), (1.0 + 1j, 1.0 +
            1.000000000001j), (-1.0 + 1j, -1.000000000001 + 1j), (1.0 - 1j,
            1.0 - 0.999999999999j)]
    ...
    def test_complex_near_zero(self):
        near_zero_examples = [(0.001j, 0), (0.001, 0), (0.001 + 0.001j, 0),
            (-0.001 + 0.001j, 0), (0.001 - 0.001j, 0), (-0.001 - 0.001j, 0)]
