...
def _make_test_zip_pkg(zip_dir, zip_basename, pkg_name, script_basename,
    ...
    to_return = make_zip_pkg(zip_dir, zip_basename, pkg_name,
        script_basename, source, depth)
    importlib.invalidate_caches()
    return to_return
...
class CmdLineTest(unittest.TestCase):
    ...
    def _check_output(self, script_name, exit_code, data, expected_file,
        expected_argv0, expected_path0, expected_package, expected_loader):
        if verbose > 1:
    ...
    def _check_script(self, script_name, expected_file, expected_argv0,
        expected_path0, expected_package, expected_loader, *cmd_line_switches):
    ...
    def test_basic_script(self):
        with support.temp_dir() as script_dir:
            ...
            self._check_script(script_name, script_name, script_name,
                script_dir, None, importlib.machinery.SourceFileLoader)
    ...
    def test_script_compiled(self):
        with support.temp_dir() as script_dir:
            ...
            self._check_script(pyc_file, pyc_file, pyc_file, script_dir,
                None, importlib.machinery.SourcelessFileLoader)

    def test_directory(self):
        with support.temp_dir() as script_dir:
            script_name = _make_test_script(script_dir, '__main__')
            self._check_script(script_dir, script_name, script_dir,
                script_dir, '', importlib.machinery.SourceFileLoader)
    ...
    def test_module_in_package(self):
        with support.temp_dir() as script_dir:
            ...
            self._check_script(launch_name, script_name, script_name,
                script_dir, 'test_pkg', importlib.machinery.SourceFileLoader)
    ...
    def test_module_in_package_in_zipfile(self):
        with support.temp_dir() as script_dir:
            ...
            self._check_script(launch_name, run_name, run_name, zip_name,
                'test_pkg', zipimport.zipimporter)

    def test_module_in_subpackage_in_zipfile(self):
        with support.temp_dir() as script_dir:
            zip_name, run_name = _make_test_zip_pkg(script_dir, 'test_zip',
                'test_pkg', 'script', depth=2)
            launch_name = _make_launch_script(script_dir, 'launch',
                'test_pkg.test_pkg.script', zip_name)
            self._check_script(launch_name, run_name, run_name, zip_name,
                'test_pkg.test_pkg', zipimport.zipimporter)
    ...
    def test_package_compiled(self):
        with support.temp_dir() as script_dir:
            ...
            self._check_script(launch_name, pyc_file, pyc_file, script_dir,
                'test_pkg', importlib.machinery.SourcelessFileLoader)
        ...
        with support.temp_dir() as script_dir:
            with support.change_cwd(path=script_dir):
                ...
                rc, out, err = assert_python_ok('-m', 'test_pkg.script', *
                    example_args, __isolated=False)
                if verbose > 1:
                    print(repr(out))
                expected = 'init_argv0==%r' % '-m'
                self.assertIn(expected.encode('utf-8'), out)
                self._check_output(script_name, rc, out, script_name,
                    script_name, '', 'test_pkg', importlib.machinery.
                    SourceFileLoader)
            ...
            with support.change_cwd(path=script_dir):
                with open('-m', 'w') as f:
                    ...
                    rc, out, err = assert_python_ok('-m', 'other', *
                        example_args, __isolated=False)
                    self._check_output(script_name, rc, out, script_name,
                        script_name, '', '', importlib.machinery.
                        SourceFileLoader)

    @contextlib.contextmanager
    def setup_test_pkg(self, *args):
        with support.temp_dir() as script_dir, support.change_cwd(path=
            script_dir):
            pkg_dir = os.path.join(script_dir, 'test_pkg')
    ...
    def test_dash_m_errors(self):
        tests = ('builtins', b'No code object available'), ('builtins.x',
            b'Error while finding module specification.*AttributeError'), (
            'builtins.x.y',
            b'Error while finding module specification.*ModuleNotFoundError.*No module named.*not a package'
            ), ('os.path', b'loader.*cannot handle'), ('importlib',
            b'No module named.*is a package and cannot be directly executed'
            ), ('importlib.nonexistant', b'No module named'), ('.unittest',
            b'Relative module names not supported')
    ...
    def test_non_ascii(self):
        if support.TESTFN_UNDECODABLE and sys.platform not in ('win32',
            'darwin'):
        ...
        self.assertEqual(ascii(script_name), stdout.rstrip().decode('ascii'
