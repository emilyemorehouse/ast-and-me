...
class TestUserObjects(unittest.TestCase):
    ...
    def _superset_test(self, a, b):
        self.assertGreaterEqual(set(dir(a)), set(dir(b)),
            '{a} should have all the methods of {b}'.format(a=a.__name__, b
            =b.__name__))
...
class TestChainMap(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        for k, v in dict(a=1, b=2, c=30, z=100).items():
            self.assertEqual(d.get(k, 100), v)
        self.assertIn(repr(d), [type(d).__name__ +
            "({'c': 30}, {'a': 1, 'b': 2})", type(d).__name__ +
            "({'c': 30}, {'b': 2, 'a': 1})"])
...
class TestNamedTuple(unittest.TestCase):
    ...
    def test_name_fixer(self):
        for spec, renamed in [[('efg', 'g%hi'), ('efg', '_1')], [('abc',
            'class'), ('abc', '_1')], [('8efg', '9ghi'), ('_0', '_1')], [(
            'abc', '_efg'), ('abc', '_1')], [('abc', 'efg', 'efg', 'ghi'),
            ('abc', 'efg', '_2', 'ghi')], [('abc', '', 'x'), ('abc', '_1',
            'x')]]:
            self.assertEqual(namedtuple('NT', spec, rename=True)._fields,
                renamed)
    ...
    def test_odd_sizes(self):
        ...
        n = 254
        names = list(set(''.join([choice(string.ascii_letters) for j in
...
class ABCTestCase(unittest.TestCase):
    ...
    def validate_comparison(self, instance):
        ...
        for name, op in operators.items():
            ...
            other = Other()
            op(instance, other)
            self.assertTrue(other.right_side, 
...
class TestOneTrickPonyABCs(ABCTestCase):
    ...
    def test_Hashable(self):
        ...
        samples = [None, int(), float(), complex(), str(), tuple(),
            frozenset(), int, list, object, type, bytes()]
        for x in samples:
    ...
    def test_Iterable(self):
        ...
        samples = [bytes(), str(), tuple(), list(), set(), frozenset(),
            dict(), dict().keys(), dict().items(), dict().values(),
    ...
    def test_Reversible(self):
        ...
        non_reversibles = [dict().keys(), dict().items(), dict().values(),
            Counter(), Counter().keys(), Counter().items(), Counter().
            values(), _test_gen(), (x for x in []), iter([]), reversed([])]
        for x in non_reversibles:
            self.assertNotIsInstance(x, Reversible)
            self.assertFalse(issubclass(type(x), Reversible), repr(type(x)))
        samples = [bytes(), str(), tuple(), list(), OrderedDict(),
            OrderedDict().keys(), OrderedDict().items(), OrderedDict().values()
            ]
        for x in samples:
        ...
        self.assertFalse(issubclass(MutableMapping, Reversible), repr(
            MutableMapping))
    ...
    def test_Collection(self):
        ...
        non_col_iterables = [_test_gen(), iter(b''), iter(bytearray()), (x for
            x in []), dict().values()]
        ...
        self.assertTrue(issubclass(MutableMapping, Collection), repr(
            MutableMapping))
    ...
    def test_Iterator(self):
        ...
        samples = [iter(bytes()), iter(str()), iter(tuple()), iter(list()),
            iter(dict()), iter(set()), iter(frozenset()), iter(dict().keys(
            )), iter(dict().items()), iter(dict().values()), _test_gen(), (
            x for x in [])]
        for x in samples:
    ...
    def test_Generator(self):
        ...
        class NonGen3:
            ...
            def throw(self, typ, val=None, tb=None):
                pass
        non_samples = [None, 42, 3.14, 1j, b'', '', (), [], {}, set(), iter
            (()), iter([]), NonGen1(), NonGen2(), NonGen3()]
        for x in non_samples:
        ...
        self.assertRaisesRegex(ValueError, '^huhu$', mgen.throw, ValueError,
            ValueError('huhu'))
    ...
    def test_AsyncGenerator(self):
        ...
        class NonAGen3:
            ...
            def athrow(self, typ, val=None, tb=None):
                pass
        non_samples = [None, 42, 3.14, 1j, b'', '', (), [], {}, set(), iter
            (()), iter([]), NonAGen1(), NonAGen2(), NonAGen3()]
        for x in non_samples:
    ...
    def test_Callable(self):
        ...
        samples = [lambda : None, type, int, object, len, list.append, [].
            append]
        for x in samples:
    ...
    def test_direct_subclassing(self):
        for B in (Hashable, Iterable, Iterator, Reversible, Sized,
            Container, Callable):
...
class TestCollectionABCs(ABCTestCase):
    ...
    def test_MutableSet(self):
        ...
        self.validate_abstract_methods(MutableSet, '__contains__',
    ...
    def test_issue26915(self):
        ...
        containers = [seq, ItemsView({(1): nan, (2): obj}), ValuesView({(1):
            nan, (2): obj})]
    ...
    def test_Sequence_mixins(self):
        ...
        def assert_index_same(seq1, seq2, index_args):
            try:
                ...
                self.assertEqual(actual, expected, '%r.index%s' % (seq1,
                    index_args))
        for ty in (list, str):
...
class TestCounter(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        for i in range(5):
            self.assertEqual(c.most_common(i), [('a', 3), ('b', 2), ('c', 1
                )][:i])
    ...
    def test_multiset_operations(self):
        ...
        for i in range(1000):
            ...
            for counterop, numberop in [(Counter.__add__, lambda x, y: max(
                0, x + y)), (Counter.__sub__, lambda x, y: max(0, x - y)),
                (Counter.__or__, lambda x, y: max(0, x, y)), (Counter.
                __and__, lambda x, y: max(0, min(x, y)))]:
        ...
        for i in range(100):
            ...
            for counterop, setop in [(Counter.__sub__, set.__sub__), (
                Counter.__or__, set.__or__), (Counter.__and__, set.__and__)]:
    ...
    def test_inplace_operations(self):
        ...
        for i in range(1000):
            ...
            q = Counter(dict((elem, randrange(-2, 4)) for elem in elements))
            q.update(h=1, i=-1, j=0)
            for inplace_op, regular_op in [(Counter.__iadd__, Counter.
                __add__), (Counter.__isub__, Counter.__sub__), (Counter.
                __ior__, Counter.__or__), (Counter.__iand__, Counter.__and__)]:
                c = p.copy()
                c_id = id(c)
    ...
    def test_helper_function(self):
        ...
        self.assertEqual(m, OrderedDict([('a', 5), ('b', 2), ('r', 2), ('c',
            1), ('d', 1)]))
...
def test_main(verbose=None):
    ...
    test_classes = [TestNamedTuple, NamedTupleDocs, TestOneTrickPonyABCs,
        TestCollectionABCs, TestCounter, TestChainMap, TestUserObjects]
