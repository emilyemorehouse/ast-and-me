...
class CfgParserTestCaseClass:
    ...
    def newconfig(self, defaults=None):
        arguments = dict(defaults=defaults, allow_no_value=self.
            allow_no_value, delimiters=self.delimiters, comment_prefixes=
            self.comment_prefixes, inline_comment_prefixes=self.
            inline_comment_prefixes, empty_lines_in_values=self.
            empty_lines_in_values, dict_type=self.dict_type, strict=self.
            strict, default_section=self.default_section, interpolation=
            self.interpolation)
...
class BasicTestCase(CfgParserTestCaseClass):
    ...
    def basic_test(self, cf):
        E = ['Commented Bar', 'Foo Bar', 'Internationalized Stuff',
            'Long Line', 'Section\\with$weird%characters[\t', 'Spaces',
            'Spacey Bar', 'Spacey Bar From The Beginning', 'Types']
        ...
        self.assertAlmostEqual(cf.getfloat('Types', 'float', fallback=0.0),
            0.44)
        ...
        if self.allow_no_value:
            ...
            eq(cf.get('NoValue', 'no-such-option-without-value', fallback=
                False), False)
        ...
        self.assertAlmostEqual(cf['Types'].getfloat('float', fallback=0.0),
            0.44)
        ...
        if self.allow_no_value:
            ...
            eq(cf['NoValue'].get('no-such-option-without-value', fallback=
                False), False)
...
"""
    ...
    def test_basic_from_dict(self):
        config = {'Foo Bar': {'foo': 'bar1'}, 'Spacey Bar': {'foo': 'bar2'},
            'Spacey Bar From The Beginning': {'foo': 'bar3', 'baz': 'qwe'},
            'Commented Bar': {'foo': 'bar4', 'baz': 'qwe'}, 'Long Line': {
            'foo':
            """this line is much, much longer than my editor
likes it."""},
            'Section\\with$weird%characters[\t': {},
            'Internationalized Stuff': {'foo[bg]': 'Bulgarian', 'foo':
            'Default', 'foo[en]': 'English', 'foo[de]': 'Deutsch'},
            'Spaces': {'key with spaces': 'value', 'another with spaces':
            'splat!'}, 'Types': {'int': 42, 'float': 0.44, 'boolean': False,
        ...
        if self.strict:
            ...
            with self.assertRaises(configparser.DuplicateOptionError):
                cf.read_dict({'Duplicate Options Here': {'option':
                    'with a value', 'OPTION': 'with another value'}})
        else:
            cf.read_dict({'section': {'key': 'value'}, 'SECTION': {'key2':
                'value2'}})
            cf.read_dict({'Duplicate Options Here': {'option':
                'with a value', 'OPTION': 'with another value'}})
    ...
    def test_case_sensitivity(self):
        ...
        for opt in ('a-b', 'A-b', 'a-B', 'A-B'):
            self.assertTrue(cf.has_option('A', opt),
        ...
        cf = self.fromstring('[section]\nnekey{}nevalue\n'.format(self.
            delimiters[0]), defaults={'key': 'value'})
    ...
    def test_default_case_sensitivity(self):
        ...
        self.assertEqual(cf.get(self.default_section, 'Foo'), 'Bar',
            'could not locate option, expecting case-insensitive option names')
        cf = self.newconfig({'Foo': 'Bar'})
        self.assertEqual(cf.get(self.default_section, 'Foo'), 'Bar',
    ...
    def get_error(self, cf, exc, section, option):
        try:
            ...
            self.fail('expected exception type %s.%s' % (exc.__module__,
                exc.__qualname__))
...
class ConfigParserTestCase(BasicTestCase, unittest.TestCase):
    ...
    def test_interpolation(self):
        ...
        e = self.get_error(cf, configparser.InterpolationDepthError, 'Foo',
            'bar11')
    ...
    def test_items(self):
        self.check_items_config([('default', '<default>'), ('getdefault',
            '|<default>|'), ('key', '|value|'), ('name', 'value'), ('value',
            'value')])
...
class MultilineValuesTestCase(BasicTestCase, unittest.TestCase):
    ...
    def test_dominating_multiline_values(self):
        ...
        self.assertEqual(cf_from_file.get('section8', 'lovely_spam4'), self
            .wonderful_spam.replace('\t\n', '\n'))
...
class RawConfigParserTestCase(BasicTestCase, unittest.TestCase):
    ...
    def test_items(self):
        self.check_items_config([('default', '<default>'), ('getdefault',
            '|%(default)s|'), ('key', '|%(name)s|'), ('name', '%(value)s'),
            ('value', 'value')])
    ...
    def test_set_nonstring_types(self):
        ...
        self.assertEqual(cf.get('non-string', 'list'), [0, 1, 1, 2, 3, 5, 8,
            13])
...
class RawConfigParserTestSambaConf(CfgParserTestCaseClass, unittest.TestCase):
    ...
    def test_reading(self):
        ...
        sections = ['global', 'homes', 'printers', 'print$',
            'pdf-generator', 'tmp', 'Agustin']
...
class ConfigParserTestCaseExtendedInterpolation(BasicTestCase, unittest.
    TestCase):
    config_class = configparser.ConfigParser
...
class ConfigParserTestCaseTrickyFile(CfgParserTestCaseClass, unittest.TestCase
    ...
    def test_cfgparser_dot_3(self):
        ...
        self.assertEqual(cf.sections(), ['strange', 'corruption',
            'yeah, sections can be indented as well', 'another one!',
            'no values here', 'tricky interpolation', 'more interpolation'])
        self.assertEqual(cf.getint(self.default_section, 'go', vars={
            'interpolate': '-1'}), -1)
        with self.assertRaises(ValueError):
            cf.getint(self.default_section, 'go', raw=True, vars={
                'interpolate': '-1'})
        ...
        self.assertEqual(cf.get('tricky interpolation', 'lets'), cf.get(
            'tricky interpolation', 'go'))
...
class ReadFileTestCase(unittest.TestCase):
    ...
    def test_source_as_bytes(self):
        ...
        self.assertEqual(str(dse.exception),
...
class ExceptionPicklingTestCase(unittest.TestCase):
    ...
    def test_duplicateoptionerror(self):
        import pickle
        e1 = configparser.DuplicateOptionError('section', 'option',
            'source', 123)
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            pickled = pickle.dumps(e1, proto)
            e2 = pickle.loads(pickled)
    ...
    def test_interpolationmissingoptionerror(self):
        import pickle
        e1 = configparser.InterpolationMissingOptionError('option',
            'section', 'rawval', 'reference')
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            pickled = pickle.dumps(e1, proto)
            e2 = pickle.loads(pickled)
            self.assertEqual(e1.message, e2.message)
...
class ExceptionContextTestCase(unittest.TestCase):
    ...
    def test_get_extended_interpolation(self):
        parser = configparser.ConfigParser(interpolation=configparser.
...
class ConvertersTestCase(BasicTestCase, unittest.TestCase):
    ...
    def test_converters(self):
        ...
        self.assertEqual(cfg.getlist('s', 'list'), ['a', 'b', 'c', 'd', 'e',
            'f', 'g'])
    ...
    def test_inheritance(self):
        ...
        class StrangeConfigParser(configparser.ConfigParser):
            ...
            def getboolean(self, section, option, *, raw=False, vars=None,
                ...
                if section == option:
                    return True
                return super().getboolean(section, option, raw=raw, vars=
                    vars, fallback=fallback)

            def getlen(self, section, option, *, raw=False, vars=None,
                fallback=configparser._UNSET):
                return self._get_conv(section, option, len, raw=raw, vars=
                    vars, fallback=fallback)
