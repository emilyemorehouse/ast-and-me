    ...
    def test_badsyntax_2(self):
        ...
        for code in samples:
            with self.subTest(code=code), self.assertWarnsRegex(
                DeprecationWarning, "'await' will become reserved keywords"):
                compile(code, '<test>', 'exec')
...
class CoroutineTest(unittest.TestCase):
    ...
    def test_func_4(self):
        ...
        async def foo():
            raise StopIteration
        check = lambda : self.assertRaisesRegex(TypeError,
    ...
    def test_func_7(self):
        ...
        def foo():
            yield from bar()
        with silence_coro_gc(), self.assertRaisesRegex(TypeError,
    ...
    def test_cr_await(self):
        ...
        @types.coroutine
        def a():
            self.assertEqual(inspect.getcoroutinestate(coro_b), inspect.
                CORO_RUNNING)
    ...
    def test_with_1(self):
        ...
        class Manager:
            ...
            async def __aenter__(self):
                await AsyncYieldFrom(['enter-1-' + self.name, 'enter-2-' +
                    self.name])
                return self

            async def __aexit__(self, *args):
                await AsyncYieldFrom(['exit-1-' + self.name, 'exit-2-' +
                    self.name])
                if self.name == 'B':
                    return True
        ...
        self.assertEqual(result, ['enter-1-A', 'enter-2-A', 'enter-1-B',
            'enter-2-B', ('managers', 'A', 'B'), 'exit-1-B', 'exit-2-B',
            'exit-1-A', 'exit-2-A'])
    ...
    def test_comp_2(self):
        ...
        async def run_list():
            return [s for c in [f(''), f('abc'), f(''), f(['de', 'fg'])] for
                s in await c]
        self.assertEqual(run_async(run_list()), ([], ['a', 'b', 'c', 'de',
            'fg']))

        async def run_set():
            return {d for c in [f([f([10, 30]), f([20])])] for s in await c for
                d in await s}
    ...
    def test_comp_5(self):
        ...
        async def run_list():
            return [(i + 1) for pair in ([10, 20], [30, 40]) if pair[0] > 
                10 async for i in f(pair) if i > 30]
...
class SysSetCoroWrapperTest(unittest.TestCase):
    ...
    def test_set_wrapper_3(self):
        ...
        try:
            with silence_coro_gc(), self.assertRaisesRegex(RuntimeError,
                'coroutine wrapper.*\\.wrapper at 0x.*attempted to recursively wrap .* wrap .*'
                ):
                foo()
        finally:
            sys.set_coroutine_wrapper(None)
    ...
    def test_set_wrapper_4(self):
        ...
        try:
            ...
            self.assertIs(wrapped, None,
                'generator-based coroutine was wrapped via sys.set_coroutine_wrapper'
                )
        finally:
            sys.set_coroutine_wrapper(None)
...
class CAPITest(unittest.TestCase):
    ...
    def test_tp_await_3(self):
        ...
        async def foo():
            ...
            return await future
        with self.assertRaisesRegex(TypeError,
            "__await__.*returned non-iterator of type 'int'"):
            self.assertEqual(foo().send(None), 1)
