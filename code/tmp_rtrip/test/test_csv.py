...
class Test_Csv(unittest.TestCase):
    ...
    def _test_arg_valid(self, ctor, arg):
        ...
        self.assertRaises(TypeError, ctor, arg, quoting=csv.QUOTE_ALL,
            quotechar='')
        self.assertRaises(TypeError, ctor, arg, quoting=csv.QUOTE_ALL,
            quotechar=None)
    ...
    def _test_kw_attrs(self, ctor, *args):
        kwargs = dict(delimiter=':', doublequote=False, escapechar='\\',
            lineterminator='\r', quotechar='*', quoting=csv.QUOTE_NONE,
    ...
    def _write_test(self, fields, expect, **kwargs):
        with TemporaryFile('w+', newline='') as fileobj:
            ...
            fileobj.seek(0)
            self.assertEqual(fileobj.read(), expect + writer.dialect.
                lineterminator)
    ...
    def test_write_bigfield(self):
        bigstring = 'X' * 50000
        self._write_test([bigstring, bigstring], '%s,%s' % (bigstring,
            bigstring))

    def test_write_quoting(self):
        self._write_test(['a', 1, 'p,q'], 'a,1,"p,q"')
        self._write_error_test(csv.Error, ['a', 1, 'p,q'], quoting=csv.
            QUOTE_NONE)
        self._write_test(['a', 1, 'p,q'], 'a,1,"p,q"', quoting=csv.
            QUOTE_MINIMAL)
        self._write_test(['a', 1, 'p,q'], '"a",1,"p,q"', quoting=csv.
            QUOTE_NONNUMERIC)
        self._write_test(['a', 1, 'p,q'], '"a","1","p,q"', quoting=csv.
    ...
    def test_write_escape(self):
        ...
        self._write_error_test(csv.Error, ['a', 1, 'p,"q"'], escapechar=
            None, doublequote=False)
        self._write_test(['a', 1, 'p,"q"'], 'a,1,"p,\\"q\\""', escapechar=
            '\\', doublequote=False)
        self._write_test(['"'], '""""', escapechar='\\', quoting=csv.
            QUOTE_MINIMAL)
        self._write_test(['"'], '\\"', escapechar='\\', quoting=csv.
            QUOTE_MINIMAL, doublequote=False)
    ...
    def test_read_oddinputs(self):
        ...
        self.assertRaises(csv.Error, self._read_test, ['ab\x00c'], None,
            strict=1)
    ...
    def test_read_eof(self):
        ...
        self.assertRaises(csv.Error, self._read_test, ['"a'], [], strict=True)
        self.assertRaises(csv.Error, self._read_test, ['^'], [], escapechar
            ='^', strict=True)
    ...
    def test_read_quoting(self):
        ...
        self._read_test(['1,",3,",5'], [['1', '"', '3', '"', '5']], quoting
            =csv.QUOTE_NONE, escapechar='\\')
        self._read_test([',3,"5",7.3, 9'], [['', 3, '5', 7.3, 9]], quoting=
            csv.QUOTE_NONNUMERIC)
        self._read_test(['"a\nb", 7'], [['a\nb', ' 7']])
        self.assertRaises(ValueError, self._read_test, ['abc,3'], [[]],
    ...
    def test_roundtrip_escaped_unquoted_newlines(self):
        with TemporaryFile('w+', newline='') as fileobj:
            ...
            writer.writerows(rows)
            fileobj.seek(0)
            for i, row in enumerate(csv.reader(fileobj, quoting=csv.
                QUOTE_NONE, escapechar='\\')):
                self.assertEqual(row, rows[i])
...
hammer and saw"
9,8,7,6"""
        self.readerAssertEqual(input, [['1', '2', '3',
            '"I see,"\nsaid the blind man',
            """as he picked up his
...
class TestDictFields(unittest.TestCase):
    ...
    def test_write_multiple_dict_rows(self):
        ...
        writer.writerows([{'f1': 1, 'f2': 'abc', 'f3': 'f'}, {'f1': 2, 'f2':
            5, 'f3': 'xyz'}])
    ...
    def test_read_dict_fieldnames_from_file(self):
        with TemporaryFile('w+') as fileobj:
            fileobj.write('f1,f2,f3\r\n1,2,abc\r\n')
            fileobj.seek(0)
            reader = csv.DictReader(fileobj, fieldnames=next(csv.reader(
                fileobj)))
            self.assertEqual(reader.fieldnames, ['f1', 'f2', 'f3'])
            self.assertEqual(next(reader), {'f1': '1', 'f2': '2', 'f3': 'abc'})
    ...
    def test_read_long(self):
        with TemporaryFile('w+') as fileobj:
            ...
            self.assertEqual(next(reader), {'f1': '1', 'f2': '2', None: [
                'abc', '4', '5', '6']})
    ...
    def test_read_long_with_rest(self):
        with TemporaryFile('w+') as fileobj:
            ...
            reader = csv.DictReader(fileobj, fieldnames=['f1', 'f2'],
                restkey='_rest')
            self.assertEqual(next(reader), {'f1': '1', 'f2': '2', '_rest':
                ['abc', '4', '5', '6']})
    ...
    def test_read_short(self):
        with TemporaryFile('w+') as fileobj:
            fileobj.write('1,2,abc,4,5,6\r\n1,2,abc\r\n')
            fileobj.seek(0)
            reader = csv.DictReader(fileobj, fieldnames='1 2 3 4 5 6'.split
                (), restval='DEFAULT')
    ...
    def test_read_multi(self):
        ...
        self.assertEqual(next(reader), {'i1': '2147483648', 'float':
            '43.0e12', 'i2': '17', 's1': 'abc', 's2': 'def'})

    def test_read_with_blanks(self):
        reader = csv.DictReader(['1,2,abc,4,5,6\r\n', '\r\n',
            '1,2,abc,4,5,6\r\n'], fieldnames='1 2 3 4 5 6'.split())
        self.assertEqual(next(reader), {'1': '1', '2': '2', '3': 'abc', '4':
            '4', '5': '5', '6': '6'})
        self.assertEqual(next(reader), {'1': '1', '2': '2', '3': 'abc', '4':
            '4', '5': '5', '6': '6'})
...
class TestUnicode(unittest.TestCase):
    names = ['Martin von Löwis', 'Marc André Lemburg', 'Guido van Rossum',
        'François Pinard']
...
class KeyOrderingTest(unittest.TestCase):
    ...
    def test_ordering_for_the_dict_reader_and_writer(self):
        ...
        for keys in permutations('abcde'):
            with TemporaryFile('w+', newline='', encoding='utf-8'
                ) as fileobject:
                ...
                resultset.add(kt)
        self.assertEqual(len(resultset), 120,
    ...
    def test_ordered_dict_reader(self):
        ...
        self.assertEqual(list(csv.DictReader(data)), [OrderedDict([(
            'FirstName', 'Eric'), ('LastName', 'Idle')]), OrderedDict([(
            'FirstName', 'Graham'), ('LastName', 'Chapman'), (None, [
            'Over1', 'Over2'])]), OrderedDict([('FirstName', 'Under1'), (
            'LastName', None)]), OrderedDict([('FirstName', 'John'), (
            'LastName', 'Cleese')])])
        self.assertEqual(list(csv.DictReader(data, restkey='OtherInfo')), [
            ...
            ('OtherInfo', ['Over1', 'Over2'])]), OrderedDict([('FirstName',
            'Under1'), ('LastName', None)]), OrderedDict([('FirstName',
            'John'), ('LastName', 'Cleese')])])
        del data[0]
        self.assertEqual(list(csv.DictReader(data, fieldnames=['fname',
            'lname'])), [OrderedDict([('fname', 'Eric'), ('lname', 'Idle')]
            ), OrderedDict([('fname', 'Graham'), ('lname', 'Chapman'), (
            None, ['Over1', 'Over2'])]), OrderedDict([('fname', 'Under1'),
            ('lname', None)]), OrderedDict([('fname', 'John'), ('lname',
            'Cleese')])])
