...
import numbers
import locale
from test.support import run_unittest, run_doctest, is_resource_enabled, requires_IEEE_754, requires_docstrings
from test.support import check_warnings, import_fresh_module, TestFailed, run_with_locale, cpython_only
import random
import time
import warnings
import inspect
...
Signals = {C: tuple(C.getcontext().flags.keys()) if C else None, P: tuple(P
    .getcontext().flags.keys())}
OrderedSignals = {C: [C.Clamped, C.Rounded, C.Inexact, C.Subnormal, C.
    Underflow, C.Overflow, C.DivisionByZero, C.InvalidOperation, C.
    FloatOperation] if C else None, P: [P.Clamped, P.Rounded, P.Inexact, P.
    Subnormal, P.Underflow, P.Overflow, P.DivisionByZero, P.
    InvalidOperation, P.FloatOperation]}
...
RoundingModes = [ROUND_UP, ROUND_DOWN, ROUND_CEILING, ROUND_FLOOR,
    ROUND_HALF_UP, ROUND_HALF_DOWN, ROUND_HALF_EVEN, ROUND_05UP]
ORIGINAL_CONTEXT = {C: C.getcontext().copy() if C else None, P: P.
    getcontext().copy()}
...
def init(m):
    if not m:
        return
    DefaultTestContext = m.Context(prec=9, rounding=ROUND_HALF_EVEN, traps=
        dict.fromkeys(Signals[m], 0))
...
requires_extra_functionality = unittest.skipUnless(EXTRA_FUNCTIONALITY,
    'test requires build with -DEXTRA_FUNCTIONALITY')
skip_if_extra_functionality = unittest.skipIf(EXTRA_FUNCTIONALITY,
    'test requires regular build')
...
class IBMTestCases(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ignore_list = ['#']
        self.skipped_test_ids = set(['scbx164', 'scbx165', 'expx901',
            'expx902', 'expx903', 'expx905', 'lnx901', 'lnx902', 'lnx903',
            'lnx905', 'logx901', 'logx902', 'logx903', 'logx905',
            'powx1183', 'powx1184', 'powx4001', 'powx4002', 'powx4003',
            'powx4005', 'powx4008', 'powx4010', 'powx4012', 'powx4014'])
        if self.decimal == C:
        ...
        self.ChangeDict = {'precision': self.change_precision, 'rounding':
            self.change_rounding_method, 'maxexponent': self.
            change_max_exponent, 'minexponent': self.change_min_exponent,
            'clamp': self.change_clamp}
        self.NameAdapter = {'and': 'logical_and', 'apply': '_apply',
            'class': 'number_class', 'comparesig': 'compare_signal',
            'comparetotal': 'compare_total', 'comparetotmag':
            'compare_total_mag', 'copy': 'copy_decimal', 'copyabs':
            'copy_abs', 'copynegate': 'copy_negate', 'copysign':
            'copy_sign', 'divideint': 'divide_int', 'invert':
            'logical_invert', 'iscanonical': 'is_canonical', 'isfinite':
            'is_finite', 'isinfinite': 'is_infinite', 'isnan': 'is_nan',
            'isnormal': 'is_normal', 'isqnan': 'is_qnan', 'issigned':
            'is_signed', 'issnan': 'is_snan', 'issubnormal': 'is_subnormal',
            'iszero': 'is_zero', 'maxmag': 'max_mag', 'minmag': 'min_mag',
            'nextminus': 'next_minus', 'nextplus': 'next_plus',
            'nexttoward': 'next_toward', 'or': 'logical_or', 'reduce':
            'normalize', 'remaindernear': 'remainder_near', 'samequantum':
            'same_quantum', 'squareroot': 'sqrt', 'toeng': 'to_eng_string',
            'tointegral': 'to_integral_value', 'tointegralx':
            'to_integral_exact', 'tosci': 'to_sci_string', 'xor': 'logical_xor'
            }
        self.RoundingDict = {'ceiling': ROUND_CEILING, 'down': ROUND_DOWN,
            'floor': ROUND_FLOOR, 'half_down': ROUND_HALF_DOWN, 'half_even':
            ROUND_HALF_EVEN, 'half_up': ROUND_HALF_UP, 'up': ROUND_UP,
            '05up': ROUND_05UP}
        self.ErrorNames = {'clamped': self.decimal.Clamped,
            ...
            'division_undefined': self.decimal.InvalidOperation, 'inexact':
            self.decimal.Inexact, 'invalid_context': self.decimal.
            InvalidOperation, 'invalid_operation': self.decimal.
            InvalidOperation, 'overflow': self.decimal.Overflow, 'rounded':
            self.decimal.Rounded, 'subnormal': self.decimal.Subnormal,
            'underflow': self.decimal.Underflow}
        self.LogicalFunctions = ('is_canonical', 'is_finite', 'is_infinite',
            'is_nan', 'is_normal', 'is_qnan', 'is_signed', 'is_snan',
            'is_subnormal', 'is_zero', 'same_quantum')
        ...
        with open(file) as f:
            for line in f:
                ...
                try:
                    ...
                    self.fail('Exception "' + exception.__class__.__name__ +
                        '" raised on line ' + line)
    ...
    def eval_equation(self, s):
        ...
        if EXTENDEDERRORTEST and fname not in ('to_sci_string', 'to_eng_string'
            ...
            for error in theirexceptions:
                ...
                try:
                    ...
                    self.fail('Raised %s in %s when %s disabled' % (e, s,
                        error))
                else:
                    self.fail('Did not raise %s in %s' % (error, s))
                self.context.traps[error] = 0
            ordered_errors = [e for e in OrderedSignals[self.decimal] if e in
                theirexceptions]
            for error in ordered_errors:
                ...
                try:
                    ...
                    self.fail('Raised %s in %s; expected %s' % (type(e), s,
                        error))
                else:
                    self.fail('Did not raise %s in %s' % (error, s))
            for error in ordered_errors:
        ...
        self.assertEqual(result, ans, 'Incorrect answer for ' + s +
            ' -- got ' + result)
...
class ExplicitConstructionTest(unittest.TestCase):
    ...
    def test_explicit_context_create_decimal(self):
        ...
        for v in [-2 ** 63 - 1, -2 ** 63, -2 ** 31 - 1, -2 ** 31, 0, 2 ** 
            31 - 1, 2 ** 31, 2 ** 63 - 1, 2 ** 63]:
        ...
        self.assertRaises(InvalidOperation, nc.create_decimal, Decimal(
            'NaN12345'))
    ...
    def test_explicit_context_create_from_float(self):
        ...
        self.assertTrue(nc.create_decimal(float('-inf')).is_infinite())
        self.assertEqual(str(nc.create_decimal(float('nan'))), str(nc.
            create_decimal('NaN')))
        self.assertEqual(str(nc.create_decimal(float('inf'))), str(nc.
            create_decimal('Infinity')))
        self.assertEqual(str(nc.create_decimal(float('-inf'))), str(nc.
            create_decimal('-Infinity')))
        self.assertEqual(str(nc.create_decimal(float('-0.0'))), str(nc.
            create_decimal('-0')))
        nc.prec = 100
        for i in range(200):
            x = random.expovariate(0.01) * (random.random() * 2.0 - 1.0)
    ...
    def test_unicode_digits(self):
        Decimal = self.decimal.Decimal
        test_values = {'１': '1', '٠.٠٣٧٢e-٣': '0.0000372', '-nan౨౪౦౦':
            '-NaN2400'}
...
class ImplicitConstructionTest(unittest.TestCase):
    ...
    def test_rop(self):
        ...
        oplist = [('+', '__add__', '__radd__'), ('-', '__sub__', '__rsub__'
            ), ('*', '__mul__', '__rmul__'), ('/', '__truediv__',
            '__rtruediv__'), ('%', '__mod__', '__rmod__'), ('//',
            '__floordiv__', '__rfloordiv__'), ('**', '__pow__', '__rpow__')]
        for sym, lop, rop in oplist:
            setattr(E, lop, lambda self, other: 'str' + lop + str(other))
            setattr(E, rop, lambda self, other: str(other) + rop + 'str')
            self.assertEqual(eval('E()' + sym + 'Decimal(10)'), 'str' + lop +
                '10')
            self.assertEqual(eval('Decimal(10)' + sym + 'E()'), '10' + rop +
                'str')
...
class FormatTest(unittest.TestCase):
    ...
    def test_formatting(self):
        Decimal = self.decimal.Decimal
        test_values = [('e', '0E-15', '0e-15'), ('e', '2.3E-15', '2.3e-15'),
            ('e', '2.30E+2', '2.30e+2'), ('e', '2.30000E-15', '2.30000e-15'
            ), ('e', '1.23456789123456789e40', '1.23456789123456789e+40'),
            ('e', '1.5', '1.5e+0'), ('e', '0.15', '1.5e-1'), ('e', '0.015',
            '1.5e-2'), ('e', '0.0000000000015', '1.5e-12'), ('e', '15.0',
            '1.50e+1'), ('e', '-15', '-1.5e+1'), ('e', '0', '0e+0'), ('e',
            '0E1', '0e+1'), ('e', '0.0', '0e-1'), ('e', '0.00', '0e-2'), (
            '.6e', '0E-15', '0.000000e-9'), ('.6e', '0', '0.000000e+6'), (
            '.6e', '9.999999', '9.999999e+0'), ('.6e', '9.9999999',
            '1.000000e+1'), ('.6e', '-1.23e5', '-1.230000e+5'), ('.6e',
            '1.23456789e-3', '1.234568e-3'), ('f', '0', '0'), ('f', '0.0',
            '0.0'), ('f', '0E-2', '0.00'), ('f', '0.00E-8', '0.0000000000'),
            ('f', '0E1', '0'), ('f', '3.2E1', '32'), ('f', '3.2E2', '320'),
            ('f', '3.20E2', '320'), ('f', '3.200E2', '320.0'), ('f',
            '3.2E-6', '0.0000032'), ('.6f', '0E-15', '0.000000'), ('.6f',
            '0E1', '0.000000'), ('.6f', '0', '0.000000'), ('.0f', '0', '0'),
            ('.0f', '0e-2', '0'), ('.0f', '3.14159265', '3'), ('.1f',
            '3.14159265', '3.1'), ('.4f', '3.14159265', '3.1416'), ('.6f',
            '3.14159265', '3.141593'), ('.7f', '3.14159265', '3.1415926'),
            ('.8f', '3.14159265', '3.14159265'), ('.9f', '3.14159265',
            '3.141592650'), ('g', '0', '0'), ('g', '0.0', '0.0'), ('g',
            '0E1', '0e+1'), ('G', '0E1', '0E+1'), ('g', '0E-5', '0.00000'),
            ('g', '0E-6', '0.000000'), ('g', '0E-7', '0e-7'), ('g', '-0E2',
            '-0e+2'), ('.0g', '3.14159265', '3'), ('.0n', '3.14159265', '3'
            ), ('.1g', '3.14159265', '3'), ('.2g', '3.14159265', '3.1'), (
            '.5g', '3.14159265', '3.1416'), ('.7g', '3.14159265',
            '3.141593'), ('.8g', '3.14159265', '3.1415926'), ('.9g',
            '3.14159265', '3.14159265'), ('.10g', '3.14159265',
            '3.14159265'), ('%', '0E1', '0%'), ('%', '0E0', '0%'), ('%',
            '0E-1', '0%'), ('%', '0E-2', '0%'), ('%', '0E-3', '0.0%'), ('%',
            '0E-4', '0.00%'), ('.3%', '0', '0.000%'), ('.3%', '0E10',
            '0.000%'), ('.3%', '0E-10', '0.000%'), ('.3%', '2.34',
            '234.000%'), ('.3%', '1.234567', '123.457%'), ('.0%', '1.23',
            '123%'), ('e', 'NaN', 'NaN'), ('f', '-NaN123', '-NaN123'), (
            '+g', 'NaN456', '+NaN456'), ('.3e', 'Inf', 'Infinity'), ('.16f',
            '-Inf', '-Infinity'), ('.0g', '-sNaN', '-sNaN'), ('', '1.00',
            '1.00'), ('6', '123', '   123'), ('<6', '123', '123   '), ('>6',
            '123', '   123'), ('^6', '123', ' 123  '), ('=+6', '123',
            '+  123'), ('#<10', 'NaN', 'NaN#######'), ('#<10', '-4.3',
            '-4.3######'), ('#<+10', '0.0130', '+0.0130###'), ('#< 10',
            '0.0130', ' 0.0130###'), ('@>10', '-Inf', '@-Infinity'), ('#>5',
            '-Inf', '-Infinity'), ('?^5', '123', '?123?'), ('%^6', '123',
            '%123%%'), (' ^6', '-45.6', '-45.6 '), ('/=10', '-45.6',
            '-/////45.6'), ('/=+10', '45.6', '+/////45.6'), ('/= 10',
            '45.6', ' /////45.6'), ('\x00=10', '-inf', '-\x00Infinity'), (
            '\x00^16', '-inf', '\x00\x00\x00-Infinity\x00\x00\x00\x00'), (
            '\x00>10', '1.2345', '\x00\x00\x00\x001.2345'), ('\x00<10',
            '1.2345', '1.2345\x00\x00\x00\x00'), (',', '1234567',
            '1,234,567'), (',', '123456', '123,456'), (',', '12345',
            '12,345'), (',', '1234', '1,234'), (',', '123', '123'), (',',
            '12', '12'), (',', '1', '1'), (',', '0', '0'), (',', '-1234567',
            '-1,234,567'), (',', '-123456', '-123,456'), ('7,', '123456',
            '123,456'), ('8,', '123456', ' 123,456'), ('08,', '123456',
            '0,123,456'), ('+08,', '123456', '+123,456'), (' 08,', '123456',
            ' 123,456'), ('08,', '-123456', '-123,456'), ('+09,', '123456',
            '+0,123,456'), ('07,', '1234.56', '1,234.56'), ('08,',
            '1234.56', '1,234.56'), ('09,', '1234.56', '01,234.56'), (
            '010,', '1234.56', '001,234.56'), ('011,', '1234.56',
            '0,001,234.56'), ('012,', '1234.56', '0,001,234.56'), ('08,.1f',
            '1234.5', '01,234.5'), (',', '1.23456789', '1.23456789'), (',%',
            '123.456789', '12,345.6789%'), (',e', '123456', '1.23456e+5'),
            (',E', '123456', '1.23456E+5'), ('a=-7.0', '0.12345', 'aaaa0.1'
            ), ('<^+15.20%', 'inf', '<<+Infinity%<<<'), ('\x07>,%',
            'sNaN1234567', 'sNaN1234567%'), ('=10.10%', 'NaN123', '   NaN123%')
            ]
    ...
    def test_n_format(self):
        ...
        en_US = {'decimal_point': '.', 'grouping': make_grouping([3, 3, 0]),
            'thousands_sep': ','}
        fr_FR = {'decimal_point': ',', 'grouping': make_grouping([CHAR_MAX]
            ), 'thousands_sep': ''}
        ru_RU = {'decimal_point': ',', 'grouping': make_grouping([3, 3, 0]),
            'thousands_sep': ' '}
        crazy = {'decimal_point': '&', 'grouping': make_grouping([1, 4, 2,
            CHAR_MAX]), 'thousands_sep': '-'}
        dotsep_wide = {'decimal_point': b'\xc2\xbf'.decode('utf-8'),
            'grouping': make_grouping([3, 3, 0]), 'thousands_sep':
            b'\xc2\xb4'.decode('utf-8')}
    ...
    def test_wide_char_separator_decimal_point(self):
        ...
        if thousands_sep != '٬':
            self.skipTest('inappropriate thousands separator({!a} not {!a})'
        ...
        for x, y in (qnan_pairs + snan_pairs):
            for op in (order_ops + equality_ops):
                ...
                self.assertIs(expected, got,
                    ...
                    .format(expected, op.__name__, x, y, got))
...
class UsabilityTest(unittest.TestCase):
    ...
    def test_decimal_fraction_comparison(self):
        ...
        with localcontext(c):
            ...
            self.assertLess(F(0, 9999999999999999999999999), D('1e' + str(
                etiny)))
            self.assertLess(D('-1e' + str(etiny)), F(0, 
            ...
            self.assertGreaterEqual(D('9e' + str(emax)), F(120984237, 
                9999999999))
    ...
    def test_hash_method(self):
        ...
        hashit(Decimal('-NaN'))
        test_values = [Decimal(sign * (2 ** m + n)) for m in [0, 14, 15, 16,
            17, 30, 31, 32, 33, 61, 62, 63, 64, 65, 66] for n in range(-10,
            10) for sign in [-1, 1]]
        test_values.extend([Decimal('-1'), Decimal('-0'), Decimal('0.00'),
            Decimal('-0.000'), Decimal('0E10'), Decimal('-0E12'), Decimal(
            '10.0'), Decimal('-23.00000'), Decimal('1230E100'), Decimal(
            '-4.5678E50'), Decimal(2 ** 64 + 2 ** 32 - 1), Decimal(
            '1.634E100'), Decimal('90.697E100'), Decimal('188.83E100'),
            Decimal('1652.9E100'), Decimal('56531E100')])
        ...
        self.assertTrue(hashit(Decimal('-Inf')))
        test_strings = ['inf', '-Inf', '0.0', '-.0e1', '34.0', '2.5',
            '112390.625', '-0.515625']
    ...
    def test_tonum_methods(self):
        ...
        test_pairs = [('123.00', 123), ('3.2', 3), ('3.54', 3), ('3.899', 3
            ), ('-2.3', -3), ('-11.0', -11), ('0.0', 0), ('-0E3', 0), (
        ...
        self.assertRaises(OverflowError, math.floor, Decimal('-Inf'))
        test_pairs = [('123.00', 123), ('3.2', 4), ('3.54', 4), ('3.899', 4
            ), ('-2.3', -2), ('-11.0', -11), ('0.0', 0), ('-0E3', 0), (
        ...
        self.assertRaises(OverflowError, math.ceil, Decimal('-Inf'))
        test_pairs = [('123.00', 123), ('3.2', 3), ('3.54', 4), ('3.899', 4
            ), ('-2.3', -2), ('-11.0', -11), ('0.0', 0), ('-0E3', 0), (
        ...
        self.assertRaises(OverflowError, round, Decimal('-Inf'))
        test_triples = [('123.456', -4, '0E+4'), ('123.456', -3, '0E+3'), (
            '123.456', -2, '1E+2'), ('123.456', -1, '1.2E+2'), ('123.456', 
            0, '123'), ('123.456', 1, '123.5'), ('123.456', 2, '123.46'), (
            '123.456', 3, '123.456'), ('123.456', 4, '123.4560'), (
            '123.455', 2, '123.46'), ('123.445', 2, '123.44'), ('Inf', 4,
            'NaN'), ('-Inf', -23, 'NaN'), ('sNaN314', 3, 'NaN314')]
    ...
    def test_as_integer_ratio(self):
        Decimal = self.decimal.Decimal
        self.assertRaises(OverflowError, Decimal.as_integer_ratio, Decimal(
            'inf'))
        self.assertRaises(OverflowError, Decimal.as_integer_ratio, Decimal(
            '-inf'))
        self.assertRaises(ValueError, Decimal.as_integer_ratio, Decimal('-nan')
            )
        self.assertRaises(ValueError, Decimal.as_integer_ratio, Decimal(
            'snan123'))
    ...
    def test_none_args(self):
        ...
        with localcontext(Context()) as c:
            ...
            self.assertRaises(InvalidOperation, Decimal('sNaN').next_minus,
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            self.assertEqual(str(y.next_plus(context=None)), 'Infinity')
            self.assertRaises(InvalidOperation, Decimal('sNaN').next_plus,
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ...
            self.assertRaises(InvalidOperation, x.compare, Decimal('sNaN'),
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.compare_signal(8224, context=None))
            self.assertEqual(ans, '-1')
            self.assertRaises(InvalidOperation, x.compare_signal, Decimal(
                'NaN'), context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.logical_and(101, context=None))
            self.assertEqual(ans, '101')
            self.assertRaises(InvalidOperation, x.logical_and, 123, context
                =None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ...
            self.assertRaises(InvalidOperation, x.logical_xor, 123, context
                =None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.max(101, context=None))
            self.assertEqual(ans, '111')
            self.assertRaises(InvalidOperation, x.max, Decimal('sNaN'),
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.max_mag(101, context=None))
            self.assertEqual(ans, '111')
            self.assertRaises(InvalidOperation, x.max_mag, Decimal('sNaN'),
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.min(101, context=None))
            self.assertEqual(ans, '101')
            self.assertRaises(InvalidOperation, x.min, Decimal('sNaN'),
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.min_mag(101, context=None))
            self.assertEqual(ans, '101')
            self.assertRaises(InvalidOperation, x.min_mag, Decimal('sNaN'),
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ans = str(x.remainder_near(101, context=None))
            self.assertEqual(ans, '10')
            self.assertRaises(InvalidOperation, y.remainder_near, 101,
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.clear_flags()
            ...
            self.assertRaises(Overflow, x.fma, Decimal('1e9999'), 3,
                context=None)
            self.assertTrue(c.flags[Overflow])
            ...
            ans = str(Decimal('1.5').to_integral(rounding=None, context=None))
            self.assertEqual(ans, '1')
            ans = str(Decimal('1.5').to_integral(rounding=ROUND_UP, context
                =None))
            self.assertEqual(ans, '2')
            c.clear_flags()
            self.assertRaises(InvalidOperation, Decimal('sNaN').to_integral,
                context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.rounding = ROUND_HALF_EVEN
            ans = str(Decimal('1.5').to_integral_value(rounding=None,
                context=None))
            self.assertEqual(ans, '2')
            c.rounding = ROUND_DOWN
            ans = str(Decimal('1.5').to_integral_value(rounding=None,
                context=None))
            self.assertEqual(ans, '1')
            ans = str(Decimal('1.5').to_integral_value(rounding=ROUND_UP,
                context=None))
            self.assertEqual(ans, '2')
            c.clear_flags()
            self.assertRaises(InvalidOperation, Decimal('sNaN').
                to_integral_value, context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.rounding = ROUND_HALF_EVEN
            ans = str(Decimal('1.5').to_integral_exact(rounding=None,
                context=None))
            self.assertEqual(ans, '2')
            c.rounding = ROUND_DOWN
            ans = str(Decimal('1.5').to_integral_exact(rounding=None,
                context=None))
            self.assertEqual(ans, '1')
            ans = str(Decimal('1.5').to_integral_exact(rounding=ROUND_UP,
                context=None))
            self.assertEqual(ans, '2')
            c.clear_flags()
            self.assertRaises(InvalidOperation, Decimal('sNaN').
                to_integral_exact, context=None)
            self.assertTrue(c.flags[InvalidOperation])
            c.rounding = ROUND_UP
            ans = str(Decimal('1.50001').quantize(exp=Decimal('1e-3'),
            ...
            ans = str(Decimal('1.50001').quantize(exp=Decimal('1e-3'),
                rounding=ROUND_UP, context=None))
            self.assertEqual(ans, '1.501')
            c.clear_flags()
            self.assertRaises(InvalidOperation, y.quantize, Decimal('1e-10'
                ), rounding=ROUND_UP, context=None)
            self.assertTrue(c.flags[InvalidOperation])
    ...
    def test_conversions_from_int(self):
        ...
        self.assertEqual(Decimal(4).compare_signal(3), Decimal(4).
            compare_signal(Decimal(3)))
        self.assertEqual(Decimal(4).compare_total(3), Decimal(4).
            compare_total(Decimal(3)))
        self.assertEqual(Decimal(4).compare_total_mag(3), Decimal(4).
            compare_total_mag(Decimal(3)))
        self.assertEqual(Decimal(10101).logical_and(1001), Decimal(10101).
            logical_and(Decimal(1001)))
        self.assertEqual(Decimal(10101).logical_or(1001), Decimal(10101).
            logical_or(Decimal(1001)))
        self.assertEqual(Decimal(10101).logical_xor(1001), Decimal(10101).
            logical_xor(Decimal(1001)))
        self.assertEqual(Decimal(567).max(123), Decimal(567).max(Decimal(123)))
        self.assertEqual(Decimal(567).max_mag(123), Decimal(567).max_mag(
            Decimal(123)))
        self.assertEqual(Decimal(567).min(123), Decimal(567).min(Decimal(123)))
        self.assertEqual(Decimal(567).min_mag(123), Decimal(567).min_mag(
            Decimal(123)))
        self.assertEqual(Decimal(567).next_toward(123), Decimal(567).
            next_toward(Decimal(123)))
        self.assertEqual(Decimal(1234).quantize(100), Decimal(1234).
            quantize(Decimal(100)))
        self.assertEqual(Decimal(768).remainder_near(1234), Decimal(768).
            remainder_near(Decimal(1234)))
        self.assertEqual(Decimal(123).rotate(1), Decimal(123).rotate(
            Decimal(1)))
        self.assertEqual(Decimal(1234).same_quantum(1000), Decimal(1234).
            same_quantum(Decimal(1000)))
        self.assertEqual(Decimal('9.123').scaleb(-100), Decimal('9.123').
            scaleb(Decimal(-100)))
        self.assertEqual(Decimal(456).shift(-1), Decimal(456).shift(Decimal
            (-1)))
        self.assertEqual(Decimal(-12).fma(Decimal(45), 67), Decimal(-12).
            fma(Decimal(45), Decimal(67)))
        self.assertEqual(Decimal(-12).fma(45, 67), Decimal(-12).fma(Decimal
            (45), Decimal(67)))
        self.assertEqual(Decimal(-12).fma(45, Decimal(67)), Decimal(-12).
            fma(Decimal(45), Decimal(67)))
...
class PythonAPItests(unittest.TestCase):
    ...
    def test_from_float(self):
        ...
        self.assertTrue(MyDecimal.from_float(float('-inf')).is_infinite())
        self.assertEqual(str(MyDecimal.from_float(float('nan'))), str(
            Decimal('NaN')))
        self.assertEqual(str(MyDecimal.from_float(float('inf'))), str(
            Decimal('Infinity')))
        self.assertEqual(str(MyDecimal.from_float(float('-inf'))), str(
            Decimal('-Infinity')))
    ...
    def test_create_decimal_from_float(self):
        ...
        self.assertEqual(context.create_decimal_from_float(math.pi),
            Decimal('3.1415'))
    ...
    def test_quantize(self):
        ...
        self.assertEqual(Decimal('7.335').quantize(Decimal('.01')), Decimal
            ('7.34'))
        self.assertEqual(Decimal('7.335').quantize(Decimal('.01'), rounding
            =ROUND_DOWN), Decimal('7.33'))
        self.assertRaises(InvalidOperation, Decimal('10e99999').quantize,
            Decimal('1e100000'), context=c)
        c = Context()
    ...
    def test_named_parameters(self):
        ...
        with localcontext() as c:
            ...
            xc.clear_flags()
            self.assertRaises(InvalidOperation, D('0').compare_signal, D(
                'nan'), context=xc)
            self.assertTrue(xc.flags[InvalidOperation])
            self.assertFalse(c.flags[InvalidOperation])
            ...
            self.assertEqual(D('0.02').min_mag(D('-0.03'), context=xc), D(
                '0.0'))
            self.assertEqual(D('0.2').next_toward(D('-1'), context=xc), D(
                '0.1'))
            xc.clear_flags()
            self.assertRaises(InvalidOperation, D('0.2').quantize, D('1e10'
                ), context=xc)
            self.assertTrue(xc.flags[InvalidOperation])
            self.assertFalse(c.flags[InvalidOperation])
            self.assertEqual(D('9.99').remainder_near(D('1.5'), context=xc),
                D('-0.5'))
            self.assertEqual(D('9.9').fma(third=D('0.9'), context=xc, other
                =7), D('7E+1'))
...
class ContextAPItests(unittest.TestCase):
    ...
    def test_none_args(self):
        ...
        for c in [c1, c2]:
            ...
            assert_signals(self, c, 'traps', [InvalidOperation,
                ...
                for ri, _ in enumerate(RoundingModes):
                    for fi, _ in enumerate(OrderedSignals[dumper]):
                        for ti, _ in enumerate(OrderedSignals[dumper]):
                            ...
                            c = dumper.Context(prec=prec, Emin=emin, Emax=
                                emax, rounding=RoundingModes[ri], capitals=
                                caps, clamp=clamp, flags=OrderedSignals[
                                dumper][:fi], traps=OrderedSignals[dumper][:ti]
                                )
                            ...
                            assert_signals(self, d, 'flags', OrderedSignals
                                [loader][:fi])
                            assert_signals(self, d, 'traps', OrderedSignals
                                [loader][:ti])
            sys.modules['decimal'] = savedecimal
    ...
    def test_fma(self):
        ...
        self.assertRaises(TypeError, c.fma, 2, '3', 4)
        self.assertRaises(TypeError, c.fma, 2, 3, '4')
        self.assertRaises(TypeError, c.fma, Decimal('Infinity'), Decimal(0),
            'not a decimal')
        self.assertRaises(TypeError, c.fma, Decimal(1), Decimal('snan'), 1.222)
        self.assertRaises(TypeError, Decimal('Infinity').fma, Decimal(0),
            'not a decimal')
...
class ContextWithStatement(unittest.TestCase):
    ...
    def test_localcontextarg(self):
        ...
        final_ctx = getcontext()
        self.assertIs(orig_ctx, final_ctx, 'did not restore context correctly')
        self.assertEqual(set_ctx.prec, new_ctx.prec,
...
class ContextFlags(unittest.TestCase):
    ...
    def test_flags_irrelevant(self):
        ...
        context = Context(prec=9, Emin=-425000000, Emax=425000000, rounding
            =ROUND_HALF_EVEN, traps=[], flags=[])
        operations = [(context._apply, [Decimal('100E-425000010')]), (
            context.sqrt, [Decimal(2)]), (context.add, [Decimal(
            '1.23456789'), Decimal('9.87654321')]), (context.multiply, [
            Decimal('1.23456789'), Decimal('9.87654321')]), (context.
            subtract, [Decimal('1.23456789'), Decimal('9.87654321')])]
        flagsets = [[Inexact], [Rounded], [Underflow], [Clamped], [
            Subnormal], [Inexact, Rounded, Underflow, Clamped, Subnormal]]
...
class SpecialContexts(unittest.TestCase):
    ...
    def test_context_templates(self):
        ...
        Underflow = self.decimal.Underflow
        Clamped = self.decimal.Clamped
        assert_signals(self, BasicContext, 'traps', [InvalidOperation,
            DivisionByZero, Overflow, Underflow, Clamped])
    ...
    def test_default_context(self):
        ...
        assert_signals(self, DefaultContext, 'traps', [InvalidOperation,
            DivisionByZero, Overflow])
        savecontext = getcontext().copy()
...
class ContextInputValidation(unittest.TestCase):
    ...
    def test_invalid_context(self):
        ...
        self.assertRaises(KeyError, setattr, c, 'traps', {
            'InvalidOperation': 0})
        for attr in ['prec', 'Emax', 'Emin', 'rounding', 'capitals',
            'clamp', 'flags', 'traps']:
...
class ContextSubclassing(unittest.TestCase):
    ...
    def test_context_subclassing(self):
        ...
        class MyContext(Context):
            ...
            def __init__(self, prec=None, rounding=None, Emin=None, Emax=
                None, capitals=None, clamp=None, flags=None, traps=None):
                ...
                if flags is not None:
                    if isinstance(flags, list):
                        flags = {v: (v in flags) for v in OrderedSignals[
                            decimal] + flags}
                    self.flags = flags
                if traps is not None:
                    if isinstance(traps, list):
                        traps = {v: (v in traps) for v in OrderedSignals[
                            decimal] + traps}
                    self.traps = traps
        c = Context()
        d = MyContext()
        for attr in ('prec', 'rounding', 'Emin', 'Emax', 'capitals',
            'clamp', 'flags', 'traps'):
        ...
        self.assertRaises(Overflow, c.add, Decimal('1e99'), Decimal(
            '2.234e2000'))
        if self.decimal == C:
...
class Coverage(unittest.TestCase):
    ...
    def test_context_repr(self):
        ...
        s = c.__repr__()
        t = (
            'Context(prec=425000000, rounding=ROUND_HALF_DOWN, Emin=-425000000, Emax=425000000, capitals=0, clamp=1, flags=[], traps=[])'
            )
    ...
    def test_divmod(self):
        ...
        with localcontext() as c:
            ...
            self.assertTrue(q.is_infinite() and r.is_nan())
            self.assertTrue(c.flags[InvalidOperation] and c.flags[
                DivisionByZero])
...
class PyFunctionality(unittest.TestCase):
    ...
    def test_py_alternate_formatting(self):
        ...
        test_values = [('.0e', '1.0', '1e+0'), ('#.0e', '1.0', '1.e+0'), (
            '.0f', '1.0', '1'), ('#.0f', '1.0', '1.'), ('g', '1.1', '1.1'),
            ('#g', '1.1', '1.1'), ('.0g', '1', '1'), ('#.0g', '1', '1.'), (
            '.0%', '1.0', '100%'), ('#.0%', '1.0', '100.%')]
        for fmt, d, result in test_values:
            self.assertEqual(format(Decimal(d), fmt), result)
...
class CFunctionality(unittest.TestCase):
    ...
    def test_constants(self):
        cond = (C.DecClamped, C.DecConversionSyntax, C.DecDivisionByZero, C
            .DecDivisionImpossible, C.DecDivisionUndefined, C.DecFpuError,
            C.DecInexact, C.DecInvalidContext, C.DecInvalidOperation, C.
            DecMallocError, C.DecFloatOperation, C.DecOverflow, C.
            DecRounded, C.DecSubnormal, C.DecUnderflow)
        ...
        self.assertEqual(C.DecIEEEInvalidOperation, C.DecConversionSyntax |
            C.DecDivisionImpossible | C.DecDivisionUndefined | C.
            DecFpuError | C.DecInvalidContext | C.DecInvalidOperation | C.
            DecMallocError)
        self.assertEqual(C.DecErrors, C.DecIEEEInvalidOperation | C.
            DecDivisionByZero)
        self.assertEqual(C.DecTraps, C.DecErrors | C.DecOverflow | C.
            DecUnderflow)
...
class CWhitebox(unittest.TestCase):
    ...
    def test_c_context_repr(self):
        ...
        s = c.__repr__()
        t = (
            'Context(prec=425000000, rounding=ROUND_HALF_DOWN, Emin=-425000000, Emax=425000000, capitals=0, clamp=1, flags=[Clamped, InvalidOperation, DivisionByZero, Inexact, FloatOperation, Overflow, Rounded, Subnormal, Underflow], traps=[Clamped, InvalidOperation, DivisionByZero, Inexact, FloatOperation, Overflow, Rounded, Subnormal, Underflow])'
            )
        self.assertEqual(s, t)
    ...
    def test_c_context_errors(self):
        ...
        self.assertRaises(TypeError, exec,
            'with localcontext(context=getcontext()): pass', locals())
    ...
    def test_c_round(self):
        ...
        with localcontext() as c:
            ...
            self.assertRaises(InvalidOperation, Decimal('1.23').__round__,
                int_max)
            self.assertRaises(InvalidOperation, Decimal('1').__round__, int
                (MAX_EMAX + 1))
    ...
    def test_c_integral(self):
        ...
        self.assertRaises(TypeError, x.to_integral_exact, 10)
        with localcontext() as c:
            x = Decimal('99999999999999999999999999.9').to_integral_value(
                ROUND_UP)
            self.assertEqual(x, Decimal('100000000000000000000000000'))
            x = Decimal('99999999999999999999999999.9').to_integral_exact(
                ROUND_UP)
            self.assertEqual(x, Decimal('100000000000000000000000000'))
            c.traps[Inexact] = True
            self.assertRaises(Inexact, Decimal('999.9').to_integral_exact,
                ROUND_UP)
    ...
    def test_c_funcs(self):
        ...
        self.assertRaises(TypeError, Decimal('1.23456789').quantize,
            Decimal('1e-100000'), [])
        self.assertRaises(TypeError, Decimal('1.23456789').quantize,
            Decimal('1e-100000'), getcontext())
        self.assertRaises(TypeError, Decimal('1.23456789').quantize,
            Decimal('1e-100000'), 10)
        self.assertRaises(TypeError, Decimal('1.23456789').quantize,
            Decimal('1e-100000'), ROUND_UP, 1000)
        with localcontext() as c:
            c.clear_traps()
    ...
    def test_va_args_exceptions(self):
        ...
        self.assertRaises(TypeError, x.to_integral_value, rounding=None,
            context=[])
        self.assertRaises(TypeError, x.to_integral_value, rounding={},
            context=[])
        self.assertRaises(TypeError, x.to_integral_value, [], [])
        self.assertRaises(TypeError, x.to_integral_exact, rounding=None,
            context=[])
        self.assertRaises(TypeError, x.to_integral_exact, rounding={},
            context=[])
    ...
    def test_c_context_templates(self):
        self.assertEqual(C.BasicContext._traps, C.DecIEEEInvalidOperation |
            C.DecDivisionByZero | C.DecOverflow | C.DecUnderflow | C.DecClamped
            )
        self.assertEqual(C.DefaultContext._traps, C.DecIEEEInvalidOperation |
            C.DecDivisionByZero | C.DecOverflow)
    ...
    def test_c_signal_dict(self):
        ...
        IntSignals = {Clamped: C.DecClamped, Rounded: C.DecRounded, Inexact:
            C.DecInexact, Subnormal: C.DecSubnormal, Underflow: C.
            DecUnderflow, Overflow: C.DecOverflow, DivisionByZero: C.
            DecDivisionByZero, InvalidOperation: C.DecIEEEInvalidOperation}
        IntCond = [C.DecDivisionImpossible, C.DecDivisionUndefined, C.
            DecFpuError, C.DecInvalidContext, C.DecInvalidOperation, C.
            DecMallocError, C.DecConversionSyntax]
        ...
        for r in range(lim):
            for t in range(lim):
                for round in RoundingModes:
                    ...
                    c = Context(prec=prec, rounding=round, Emin=emin, Emax=
                        emax, capitals=caps, clamp=clamp, flags=list(flags),
                        traps=list(traps))
    ...
    def test_invalid_override(self):
        ...
        def get_fmt(x, override=None, fmt='n'):
            return Decimal(x).__format__(fmt, override)
        invalid_grouping = {'decimal_point': ',', 'grouping': make_grouping
            ([255, 255, 0]), 'thousands_sep': ','}
        invalid_dot = {'decimal_point': 'xxxxx', 'grouping': make_grouping(
            [3, 3, 0]), 'thousands_sep': ','}
        invalid_sep = {'decimal_point': '.', 'grouping': make_grouping([3, 
            3, 0]), 'thousands_sep': 'yyyyy'}
        if CHAR_MAX == 127:
    ...
    def test_internal_use_of_overridden_methods(self):
        ...
        class Z(float):
            ...
            def __abs__(self):
                return self
        for cls in (X, Y, Z):
            self.assertEqual(Decimal.from_float(cls(101.1)), Decimal.
                from_float(101.1))
...
class SignatureTest(unittest.TestCase):
    ...
    def test_inspect_module(self):
        for attr in dir(P):
            ...
            if attr == 'Decimal' or attr == 'Context' or inspect.isfunction(
                p_func):
                p_sig = inspect.signature(p_func)
                c_sig = inspect.signature(c_func)
                c_names = list(c_sig.parameters.keys())
                p_names = [x for x in p_sig.parameters.keys() if not x.
                    startswith('_')]
                self.assertEqual(c_names, p_names, msg=
                    'parameter name mismatch in %s' % p_func)
                c_kind = [x.kind for x in c_sig.parameters.values()]
                p_kind = [x[1].kind for x in p_sig.parameters.items() if 
                    not x[0].startswith('_')]
                if attr != 'setcontext':
                    self.assertEqual(c_kind, p_kind, msg=
                        'parameter kind mismatch in %s' % p_func)

    def test_inspect_types(self):
        POS = inspect._ParameterKind.POSITIONAL_ONLY
        POS_KWD = inspect._ParameterKind.POSITIONAL_OR_KEYWORD
        pdict = {C: {'other': C.Decimal(1), 'third': C.Decimal(1), 'x': C.
            Decimal(1), 'y': C.Decimal(1), 'z': C.Decimal(1), 'a': C.
            Decimal(1), 'b': C.Decimal(1), 'c': C.Decimal(1), 'exp': C.
            Decimal(1), 'modulo': C.Decimal(1), 'num': '1', 'f': 1.0,
            'rounding': C.ROUND_HALF_UP, 'context': C.getcontext()}, P: {
            'other': P.Decimal(1), 'third': P.Decimal(1), 'a': P.Decimal(1),
            'b': P.Decimal(1), 'c': P.Decimal(1), 'exp': P.Decimal(1),
            'modulo': P.Decimal(1), 'num': '1', 'f': 1.0, 'rounding': P.
            ROUND_HALF_UP, 'context': P.getcontext()}}
        ...
        def doit(ty):
            ...
            for attr in dir(p_type):
                ...
                if inspect.isfunction(p_func):
                    ...
                    if ty == 'Decimal':
                        self.assertEqual(c_kind[1:], p_kind[1:], msg=
                            'parameter kind mismatch in %s' % p_func)
                    else:
                        self.assertEqual(len(c_kind), len(p_kind), msg=
                            'parameter kind mismatch in %s' % p_func)
                    args, kwds = mkargs(C, c_sig)
                    try:
                        getattr(c_type(9), attr)(*args, **kwds)
                    except Exception as err:
                        raise TestFailed('invalid signature for %s: %s %s' %
                            (c_func, args, kwds))
...
all_tests = [CExplicitConstructionTest, PyExplicitConstructionTest,
    CImplicitConstructionTest, PyImplicitConstructionTest, CFormatTest,
    PyFormatTest, CArithmeticOperatorsTest, PyArithmeticOperatorsTest,
    CThreadingTest, PyThreadingTest, CUsabilityTest, PyUsabilityTest,
    CPythonAPItests, PyPythonAPItests, CContextAPItests, PyContextAPItests,
    CContextWithStatement, PyContextWithStatement, CContextFlags,
    PyContextFlags, CSpecialContexts, PySpecialContexts,
    CContextInputValidation, PyContextInputValidation, CContextSubclassing,
    PyContextSubclassing, CCoverage, PyCoverage, CFunctionality,
    PyFunctionality, CWhitebox, PyWhitebox, CIBMTestCases, PyIBMTestCases]
if not C:
...
if __name__ == '__main__':
    ...
    p.add_option('--debug', '-d', action='store_true', help=
        'shows the test number and context before each test')
    p.add_option('--skip', '-s', action='store_true', help=
        'skip over 90% of the arithmetic tests')
