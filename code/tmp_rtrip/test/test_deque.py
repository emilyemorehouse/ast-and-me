...
class TestBasic(unittest.TestCase):
    ...
    def test_count(self):
        for s in ('', 'abracadabra', 'simsalabim' * 500 + 'abc'):
            ...
            for letter in 'abcdefghijklmnopqrstuvwxyz':
                self.assertEqual(s.count(letter), d.count(letter), (s, d,
                    letter))
    ...
    def test_comparisons(self):
        ...
        for e in [d, deque('abc'), deque('ab'), deque(), list(d)]:
            ...
            self.assertEqual(d != e, not (type(d) == type(e) and list(d) ==
                list(e)))
        args = map(deque, ('', 'a', 'b', 'ab', 'ba', 'abc', 'xba', 'xabc',
            'cba'))
        for x in args:
            for y in args:
    ...
    def test_long_steadystate_queue_popright(self):
        for size in (0, 1, 2, 100, 1000):
            ...
            for i in range(size, BIG):
                ...
                if x != i - size:
                    self.assertEqual(x, i - size)
            self.assertEqual(list(reversed(list(d))), list(range(BIG - size,
                BIG)))
...
class TestVariousIteratorArgs(unittest.TestCase):
    ...
    def test_constructor(self):
        for s in ('123', '', range(1000), ('do', 1.2), range(2000, 2200, 5)):
            for g in (seq_tests.Sequence, seq_tests.IterFunc, seq_tests.
                IterGen, seq_tests.IterFuncStop, seq_tests.itermulti,
                seq_tests.iterfunc):
