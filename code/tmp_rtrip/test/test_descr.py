...
class OperatorsTest(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.binops = {'add': '+', 'sub': '-', 'mul': '*', 'matmul': '@',
            'truediv': '/', 'floordiv': '//', 'divmod': 'divmod', 'pow':
    ...
    def setsliceop_test(self, a, b, c, d, res, stmt='a[b:c]=d', meth=
        '__setitem__'):
    ...
    def test_lists(self):
        ...
        self.setsliceop_test([1, 2, 3, 4], 1, 3, [5, 6], [1, 5, 6, 4],
            'a[b:c]=d', '__setitem__')
    ...
    def test_spam_lists(self):
        ...
        self.sliceop_test(spamlist([1, 2, 3]), 0, 2, spamlist([1, 2]),
            'a[b:c]', '__getitem__')
...
class ClassPropertiesAndMethods(unittest.TestCase):
    ...
    def assertHasAttr(self, obj, name):
        self.assertTrue(hasattr(obj, name), '%r has no attribute %r' % (obj,
            name))
    ...
    def test_special_method_lookup(self):
        ...
        def format_impl(self, spec):
            return 'hello'
        specials = [('__bytes__', bytes, hello, set(), {}), ('__reversed__',
            reversed, empty_seq, set(), {}), ('__length_hint__', list, zero,
            set(), {'__iter__': iden, '__next__': stop}), ('__sizeof__',
            sys.getsizeof, zero, set(), {}), ('__instancecheck__',
            do_isinstance, return_true, set(), {}), ('__missing__',
            do_dict_missing, some_number, set(('__class__',)), {}), (
            ...
            '__floor__', math.floor, zero, set(), {}), ('__trunc__', math.
            trunc, zero, set(), {}), ('__trunc__', int, zero, set(), {}), (
            '__ceil__', math.ceil, zero, set(), {}), ('__dir__', dir,
            empty_seq, set(), {}), ('__round__', round, zero, set(), {})]
    ...
    def test_dir(self):
        ...
        del junk
        for arg in (2, 2, 2j, 2.0, [2], '2', b'2', (2,), {(2): 2}, type,
            self.test_dir):
            dir(arg)
    ...
    def test_classic_comparisons(self):
        ...
        for base in (classic, int, object):
            ...
            for x in (1, 2, 3):
                for y in (1, 2, 3):
                    for op in ('<', '<=', '==', '!=', '>', '>='):
                        self.assertEqual(eval('c[x] %s c[y]' % op), eval(
                            'x %s y' % op), 'x=%d, y=%d' % (x, y))
                        self.assertEqual(eval('c[x] %s y' % op), eval(
                            'x %s y' % op), 'x=%d, y=%d' % (x, y))
                        self.assertEqual(eval('x %s c[y]' % op), eval(
                            'x %s y' % op), 'x=%d, y=%d' % (x, y))
    ...
    def test_builtin_bases(self):
        builtin_types = [tp for tp in builtins.__dict__.values() if
            isinstance(tp, type)]
    ...
    def test_not_implemented(self):
        ...
        for name, expr, iexpr in [('__add__', 'x + y', 'x += y'), (
            '__sub__', 'x - y', 'x -= y'), ('__mul__', 'x * y', 'x *= y'),
            ('__matmul__', 'x @ y', 'x @= y'), ('__truediv__', 'x / y',
            'x /= y'), ('__floordiv__', 'x // y', 'x //= y'), ('__mod__',
            'x % y', 'x %= y'), ('__divmod__', 'divmod(x, y)', None), (
            '__pow__', 'x ** y', 'x **= y'), ('__lshift__', 'x << y',
            'x <<= y'), ('__rshift__', 'x >> y', 'x >>= y'), ('__and__',
            'x & y', 'x &= y'), ('__or__', 'x | y', 'x |= y'), ('__xor__',
            'x ^ y', 'x ^= y')]:
            rname = '__r' + name[2:]
    ...
    def test_bound_method_repr(self):
        ...
        self.assertRegex(repr(Foo().method),
            '<bound method .*Foo\\.method of <.*Foo object at .*>>')
        ...
        self.assertRegex(repr(base.method),
            '<bound method .*Base\\.method of <.*Base object at .*>>')
        ...
        self.assertRegex(repr(method),
            '<bound method \\? of <object object at .*>>')
        func.__name__ = 'name'
        self.assertRegex(repr(method),
            '<bound method name of <object object at .*>>')
        func.__qualname__ = 'qualname'
        self.assertRegex(repr(method),
            '<bound method qualname of <object object at .*>>')
...
class PicklingTests(unittest.TestCase):
    ...
    def _check_reduce(self, proto, obj, args=(), kwargs={}, state=None,
        ...
        if proto >= 2:
            ...
            reduce_value = copyreg._reconstructor, (type(obj), base_type, 
        ...
        class PickleCopier:
            ...
            def __repr__(self):
                return ('PickleCopier(proto={}, dumps={}.{}, loads={}.{})'.
                    format(self.proto, self.dumps.__module__, self.dumps.
                    __qualname__, self.loads.__module__, self.loads.
                    __qualname__))
        return (PickleCopier(*args) for args in itertools.product(range(
            pickle.HIGHEST_PROTOCOL + 1), {pickle.dumps, pickle._dumps}, {
    ...
    def test_reduce_copying(self):
        ...
        for cls, pickle_copier in itertools.product(test_classes,
            pickle_copiers):
...
def test_main():
    support.run_unittest(PTypesLongInitTest, OperatorsTest,
        ClassPropertiesAndMethods, DictProxyTests, MiscTests, PicklingTests,
        SharedKeyTests, MroTest)
