...
class DictTest(unittest.TestCase):
    ...
    def test_literal_constructor(self):
        for n in (0, 1, 6, 256, 400):
            items = [(''.join(random.sample(string.ascii_letters, 8)), i) for
                i in range(n)]
    ...
    def test_fromkeys(self):
        self.assertEqual(dict.fromkeys('abc'), {'a': None, 'b': None, 'c':
            None})
        d = {}
    ...
    def test_bad_key(self):
        ...
        x2 = BadDictKey()
        d[x1] = 1
        for stmt in ['d[x2] = 2', 'z = d[x2]', 'x2 in d', 'd.get(x2)',
            'd.setdefault(x2, 42)', 'd.pop(x2)', 'd.update({x2: 2})']:
