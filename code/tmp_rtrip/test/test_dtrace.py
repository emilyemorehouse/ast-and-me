...
def normalize_trace_output(output):
    ...
    try:
        result = [row.split('\t') for row in output.splitlines() if row and
            not row.startswith('#')]
...
class TraceBackend:
    ...
    def run_case(self, name, optimize_python=None):
        actual_output = normalize_trace_output(self.trace_python(
            script_file=abspath(name + self.EXTENSION), python_file=abspath
            (name + '.py'), optimize_python=optimize_python))
    ...
    def trace(self, script_file, subcommand=None):
        ...
        stdout, _ = subprocess.Popen(command, stdout=subprocess.PIPE,
    ...
    def assert_usable(self):
        ...
        if output != 'probe: success':
            raise unittest.SkipTest('{}(1) failed: {}'.format(self.COMMAND[
                0], output))
...
class TraceTests(unittest.TestCase):
    ...
    def run_case(self, name):
        actual_output, expected_output = self.backend.run_case(name,
    ...
    def test_verify_call_opcodes(self):
        ...
        def get_function_instructions(funcname):
            code = compile(source=code_string, filename='<string>', mode=
                'exec', optimize=self.optimize_python)
...
def test_main():
    run_unittest(DTraceNormalTests, DTraceOptimizedTests,
        SystemTapNormalTests, SystemTapOptimizedTests)
