...
class LockTests(unittest.TestCase):
    ...
    def test_LockType_context_manager(self):
        with _thread.LockType():
            pass
        self.assertFalse(self.lock.locked(), 'Acquired Lock was not released')
    ...
    def test_cond_acquire_fail(self):
        ...
        self.assertFalse(self.lock.acquire(0),
            'Conditional acquiring of a locked lock incorrectly succeeded.')
    ...
    def test_uncond_acquire_blocking(self):
        ...
        if support.verbose:
            print()
            print(
                '*** Waiting for thread to release the lock (approx. %s sec.) ***'
                 % DELAY)
        self.lock.acquire()
...
class MiscTests(unittest.TestCase):
    ...
    def test_set_sentinel(self):
        self.assertIsInstance(_thread._set_sentinel(), _thread.LockType,
            '_thread._set_sentinel() did not return a LockType instance.')
...
class ThreadTests(unittest.TestCase):
    ...
    def test_arg_passing(self):
        ...
        _thread.start_new_thread(arg_tester, tuple(), {'queue':
            testing_queue, 'arg1': True, 'arg2': True})
        result = testing_queue.get()
        self.assertTrue(result[0] and result[1],
            'Argument passing for thread creation using kwargs failed')
        _thread.start_new_thread(arg_tester, (testing_queue, True), {'arg2':
            True})
        result = testing_queue.get()
        self.assertTrue(result[0] and result[1],
            'Argument passing for thread creation using both tuple and kwargs failed'
            )
    ...
    def test_multi_thread_creation(self):
        ...
        if support.verbose:
            print()
            print(
                '*** Testing multiple thread creation (will take approx. %s to %s sec.) ***'
                 % (DELAY, thread_count))
