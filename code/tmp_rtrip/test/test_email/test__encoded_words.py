...
class TestDecode(TestEmailBase):
    ...
    def test_q_escaped_bytes_preserved(self):
        self._test(b'=?us-ascii?q?=20\xacfoo?='.decode('us-ascii',
            'surrogateescape'), ' \udcacfoo', defects=[errors.
            UndecodableBytesDefect])

    def test_b_undecodable_bytes_ignored_with_defect(self):
        self._test(b'=?us-ascii?b?dm\xack?='.decode('us-ascii',
            'surrogateescape'), 'vi', defects=[errors.
            InvalidBase64CharactersDefect, errors.InvalidBase64PaddingDefect])

    def test_b_invalid_bytes_ignored_with_defect(self):
        self._test('=?us-ascii?b?dm\x01k===?=', 'vi', defects=[errors.
    ...
    def test_b_padding_defect(self):
        self._test('=?us-ascii?b?dmk?=', 'vi', defects=[errors.
    ...
    def test_unknown_8bit_charset(self):
        self._test('=?unknown-8bit?q?foo=ACbar?=', b'foo\xacbar'.decode(
            'ascii', 'surrogateescape'), charset='unknown-8bit', defects=[])

    def test_unknown_charset(self):
        self._test('=?foobar?q?foo=ACbar?=', b'foo\xacbar'.decode('ascii',
            'surrogateescape'), charset='foobar', defects=[errors.CharsetError]
            )
