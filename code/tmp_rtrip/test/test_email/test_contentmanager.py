...
class TestContentManager(TestEmailBase):
    ...
    get_key_params = {'full_type': (1, 'text/plain'), 'maintype_only': (2,
        'text'), 'null_key': (3, '')}
    ...
    set_key_params = {'type': (0, Thing), 'full_path': (1,
        testobject_full_path), 'qualname': (2, 'TestContentManager.Thing'),
        'name': (3, 'Thing'), 'base_type': (4, BaseThing), 'base_full_path':
        (5, baseobject_full_path), 'base_qualname': (6,
        'TestContentManager.BaseThing'), 'base_name': (7, 'BaseThing'),
        'str_type': (8, str), 'str_full_path': (9, 'builtins.str'),
        'str_name': (10, 'str'), 'null_key': (11, None)}
...
class TestRawDataManager(TestEmailBase):
    policy = policy.default.clone(max_line_length=60, content_manager=
        raw_data_manager)
    message = EmailMessage
...
"""
    ...
    def test_set_headers_from_header_objects(self):
        ...
        raw_data_manager.set_content(m, content, headers=(header_factory(
            'To', 'foo@example.com'), header_factory('From',
            'foo@example.com'), header_factory('Subject',
    ...
    content_object_params = {'text_plain': ('content', ()), 'text_html': (
        'content', ('html',)), 'application_octet_stream': (b'content', (
        'application', 'octet_stream')), 'image_jpeg': (b'content', (
        'image', 'jpeg')), 'message_rfc822': (message(), ()),
        'message_external_body': (message(), ('external-body',))}

    def content_object_as_header_receiver(self, obj, mimetype):
        m = self._make_message()
        m.set_content(obj, *mimetype, headers=('To: foo@example.com',
            'From: bar@simple.net'))
