...
class TestGeneratorBase:
    ...
    refold_long_expected = {(0): textwrap.dedent(
...
"""
    ...
    def length_as_maxheaderlen_parm_overrides_policy(self, n):
        msg = self.msgmaker(self.typ(self.refold_long_expected[0]))
        s = self.ioclass()
        g = self.genclass(s, maxheaderlen=n, policy=self.policy.clone(
            max_line_length=10))
        g.flatten(msg)
        self.assertEqual(s.getvalue(), self.typ(self.refold_long_expected[n]))
    ...
    def test_set_mangle_from_via_policy(self):
        ...
        variants = (None, True), (policy.compat32, True), (policy.default,
            False), (policy.default.clone(mangle_from_=True), True)
...
class TestBytesGenerator(TestGeneratorBase, TestEmailBase):
    ...
    def test_cte_type_7bit_handles_unknown_8bit(self):
        source = ('Subject: Maintenant je vous présente mon collègue\n\n'.
            encode('utf-8'))
        expected = (
            """Subject: Maintenant je vous =?unknown-8bit?q?pr=C3=A9sente_mon_coll=C3=A8gue?=
