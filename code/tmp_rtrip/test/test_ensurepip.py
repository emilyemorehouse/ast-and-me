...
class TestBootstrap(EnsurepipMixin, unittest.TestCase):
    ...
    def test_basic_bootstrapping(self):
        ensurepip.bootstrap()
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, 'setuptools', 'pip'],
            unittest.mock.ANY)
    ...
    def test_bootstrapping_with_root(self):
        ...
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '--root', '/foo/bar/',
            'setuptools', 'pip'], unittest.mock.ANY)

    def test_bootstrapping_with_user(self):
        ensurepip.bootstrap(user=True)
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '--user', 'setuptools',
            'pip'], unittest.mock.ANY)

    def test_bootstrapping_with_upgrade(self):
        ensurepip.bootstrap(upgrade=True)
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '--upgrade', 'setuptools',
            'pip'], unittest.mock.ANY)

    def test_bootstrapping_with_verbosity_1(self):
        ensurepip.bootstrap(verbosity=1)
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '-v', 'setuptools', 'pip'],
            unittest.mock.ANY)

    def test_bootstrapping_with_verbosity_2(self):
        ensurepip.bootstrap(verbosity=2)
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '-vv', 'setuptools', 'pip'],
            unittest.mock.ANY)

    def test_bootstrapping_with_verbosity_3(self):
        ensurepip.bootstrap(verbosity=3)
        self.run_pip.assert_called_once_with(['install', '--no-index',
            '--find-links', unittest.mock.ANY, '-vvv', 'setuptools', 'pip'],
            unittest.mock.ANY)
...
class TestUninstall(EnsurepipMixin, unittest.TestCase):
    ...
    def test_uninstall(self):
        with fake_pip():
            ensurepip._uninstall_helper()
        self.run_pip.assert_called_once_with(['uninstall', '-y',
            '--disable-pip-version-check', 'pip', 'setuptools'])

    def test_uninstall_with_verbosity_1(self):
        with fake_pip():
            ensurepip._uninstall_helper(verbosity=1)
        self.run_pip.assert_called_once_with(['uninstall', '-y',
            '--disable-pip-version-check', '-v', 'pip', 'setuptools'])

    def test_uninstall_with_verbosity_2(self):
        with fake_pip():
            ensurepip._uninstall_helper(verbosity=2)
        self.run_pip.assert_called_once_with(['uninstall', '-y',
            '--disable-pip-version-check', '-vv', 'pip', 'setuptools'])

    def test_uninstall_with_verbosity_3(self):
        with fake_pip():
            ensurepip._uninstall_helper(verbosity=3)
        self.run_pip.assert_called_once_with(['uninstall', '-y',
            '--disable-pip-version-check', '-vvv', 'pip', 'setuptools'])
