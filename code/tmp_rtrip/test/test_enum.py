...
try:
    ...
    Answer = exc
try:
    Theory = Enum('Theory', 'rule law supposition', qualname=
        'spanish_inquisition')
except Exception as exc:
    Theory = exc
try:
...
class TestHelpers(unittest.TestCase):
    ...
    def test_is_sunder(self):
        ...
        for s in ('a', 'a_', '_a', '__a', 'a__', '__a__', '_a__', '__a_',
            '_', '__', '___', '____', '_____'):
...
class TestEnum(unittest.TestCase):
    ...
    def test_dir_on_class(self):
        Season = self.Season
        self.assertEqual(set(dir(Season)), set(['__class__', '__doc__',
            '__members__', '__module__', 'SPRING', 'SUMMER', 'AUTUMN',
            'WINTER']))

    def test_dir_on_item(self):
        Season = self.Season
        self.assertEqual(set(dir(Season.WINTER)), set(['__class__',
            '__doc__', '__module__', 'name', 'value']))
    ...
    def test_dir_with_added_behavior(self):
        ...
        self.assertEqual(set(dir(Test)), set(['__class__', '__doc__',
            '__members__', '__module__', 'this', 'these']))
        self.assertEqual(set(dir(Test.this)), set(['__class__', '__doc__',
            '__module__', 'name', 'value', 'wowser']))
    ...
    def test_dir_on_sub_with_behavior_on_super(self):
        ...
        self.assertEqual(set(dir(SubEnum.sample)), set(['__class__',
            '__doc__', '__module__', 'name', 'value', 'invisible']))
    ...
    def test_enum(self):
        ...
        self.assertEqual([Season.SPRING, Season.SUMMER, Season.AUTUMN,
            Season.WINTER], lst)
    ...
    def test_enum_duplicates(self):
        ...
        class Season(Enum):
            ...
            ANOTHER_SPRING = 1
        lst = list(Season)
        self.assertEqual(lst, [Season.SPRING, Season.SUMMER, Season.AUTUMN,
            Season.WINTER])
        ...
        self.assertEqual([k for k, v in Season.__members__.items() if v.
            name != k], ['FALL', 'ANOTHER_SPRING'])
    ...
    def test_format_enum(self):
        Season = self.Season
        self.assertEqual('{}'.format(Season.SPRING), '{}'.format(str(Season
            .SPRING)))
        self.assertEqual('{:}'.format(Season.SPRING), '{:}'.format(str(
            Season.SPRING)))
        self.assertEqual('{:20}'.format(Season.SPRING), '{:20}'.format(str(
            Season.SPRING)))
        self.assertEqual('{:^20}'.format(Season.SPRING), '{:^20}'.format(
            str(Season.SPRING)))
        self.assertEqual('{:>20}'.format(Season.SPRING), '{:>20}'.format(
            str(Season.SPRING)))
        self.assertEqual('{:<20}'.format(Season.SPRING), '{:<20}'.format(
    ...
    def test_intenum_duplicates(self):
        ...
        self.assertEqual([k for k, v in WeekDay.__members__.items() if v.
            name != k], ['TEUSDAY'])
    ...
    def test_class_nested_enum_and_pickle_protocol_four(self):
        ...
        self.NestedEnum.__qualname__ = ('%s.NestedEnum' % self.__class__.
            __name__)
    ...
    def test_exploding_pickle(self):
        BadPickle = Enum('BadPickle', 'dill sweet bread-n-butter', module=
            __name__)
    ...
    def test_iteration_order(self):
        ...
        class Season(Enum):
            ...
            AUTUMN = 3
            SPRING = 1
        self.assertEqual(list(Season), [Season.SUMMER, Season.WINTER,
            Season.AUTUMN, Season.SPRING])

    def test_reversed_iteration_order(self):
        self.assertEqual(list(reversed(self.Season)), [self.Season.WINTER,
            self.Season.AUTUMN, self.Season.SUMMER, self.Season.SPRING])
    ...
    def test_programmatic_function_string(self):
        ...
        self.assertEqual([SummerMonth.june, SummerMonth.july, SummerMonth.
            august], lst)
    ...
    def test_programmatic_function_iterable(self):
        SummerMonth = Enum('SummerMonth', (('june', 1), ('july', 2), (
            'august', 3)))
        lst = list(SummerMonth)
        self.assertEqual(len(lst), len(SummerMonth))
        self.assertEqual(len(SummerMonth), 3, SummerMonth)
    ...
    def test_programmatic_function_from_dict(self):
        SummerMonth = Enum('SummerMonth', OrderedDict((('june', 1), ('july',
            2), ('august', 3))))
        lst = list(SummerMonth)
        self.assertEqual(len(lst), len(SummerMonth))
        self.assertEqual(len(SummerMonth), 3, SummerMonth)
    ...
    def test_new_repr(self):
        ...
        self.assertEqual(repr(Color.blue),
        ...
        class auto_enum(type(Enum)):
            ...
            def __new__(metacls, cls, bases, classdict):
                ...
                for k, v in classdict.items():
                    if k not in names:
                        temp[k] = v
                return super(auto_enum, metacls).__new__(metacls, cls,
                    bases, temp)
        ...
        class NamedInt(int):
            ...
            def __add__(self, other):
                ...
                if isinstance(self, NamedInt) and isinstance(other, NamedInt):
                    return NamedInt('({0} + {1})'.format(self.__name__,
                        other.__name__), temp)
                else:
                    return temp
    ...
    def test_duplicate_values_give_unique_enum_items(self):
        ...
        self.assertEqual(list(AutoNumber), [AutoNumber.first, AutoNumber.
            second, AutoNumber.third])
    ...
    def test_nonhash_value(self):
        ...
        class ColorInAList(AutoNumberInAList):
            ...
            green = ()
            blue = ()
        self.assertEqual(list(ColorInAList), [ColorInAList.red,
            ColorInAList.green, ColorInAList.blue])
    ...
    def test_conflicting_types_resolved_in_new(self):
        ...
        self.assertEqual(list(LabelledList), [LabelledList.unprocessed,
            LabelledList.payment_complete])
...
class TestFlag(unittest.TestCase):
    ...
    def test_cascading_failure(self):
        ...
        class Bizarre(Flag):
            ...
            d = 4
            f = 6
        self.assertRaisesRegex(ValueError, '5 is not a valid Bizarre',
            Bizarre, 5)
        self.assertRaisesRegex(ValueError, '5 is not a valid Bizarre',
            Bizarre, 5)
        self.assertRaisesRegex(ValueError, '2 is not a valid Bizarre',
            Bizarre, 2)
        self.assertRaisesRegex(ValueError, '2 is not a valid Bizarre',
            Bizarre, 2)
        self.assertRaisesRegex(ValueError, '1 is not a valid Bizarre',
            Bizarre, 1)
        self.assertRaisesRegex(ValueError, '1 is not a valid Bizarre',
            Bizarre, 1)
...
class TestStdLib(unittest.TestCase):
    ...
    def test_inspect_getmembers(self):
        values = dict((('__class__', EnumMeta), ('__doc__',
            'An enumeration.'), ('__members__', self.Color.__members__), (
            '__module__', __name__), ('blue', self.Color.blue), ('green',
            self.Color.green), ('name', Enum.__dict__['name']), ('red',
            self.Color.red), ('value', Enum.__dict__['value'])))
    ...
    def test_inspect_classify_class_attrs(self):
        ...
        values = [Attribute(name='__class__', kind='data', defining_class=
            object, object=EnumMeta), Attribute(name='__doc__', kind='data',
            defining_class=self.Color, object='An enumeration.'), Attribute
            (name='__members__', kind='property', defining_class=EnumMeta,
            object=EnumMeta.__members__), Attribute(name='__module__', kind
            ='data', defining_class=self.Color, object=__name__), Attribute
            (name='blue', kind='data', defining_class=self.Color, object=
            self.Color.blue), Attribute(name='green', kind='data',
            defining_class=self.Color, object=self.Color.green), Attribute(
            name='red', kind='data', defining_class=self.Color, object=self
            .Color.red), Attribute(name='name', kind='data', defining_class
            =Enum, object=Enum.__dict__['name']), Attribute(name='value',
            kind='data', defining_class=Enum, object=Enum.__dict__['value'])]
...
class TestIntEnumConvert(unittest.TestCase):
    ...
    def test_convert_value_lookup_priority(self):
        test_type = enum.IntEnum._convert('UnittestConvert', (
            'test.test_enum', '__main__')[__name__ == '__main__'], filter=
            lambda x: x.startswith('CONVERT_TEST_'))
    ...
    def test_convert(self):
        test_type = enum.IntEnum._convert('UnittestConvert', (
            'test.test_enum', '__main__')[__name__ == '__main__'], filter=
            lambda x: x.startswith('CONVERT_TEST_'))
        self.assertEqual(test_type.CONVERT_TEST_NAME_F, test_type.
            CONVERT_TEST_NAME_A)
        self.assertEqual(test_type.CONVERT_TEST_NAME_B, 5)
        self.assertEqual(test_type.CONVERT_TEST_NAME_C, 5)
        self.assertEqual(test_type.CONVERT_TEST_NAME_D, 5)
        self.assertEqual(test_type.CONVERT_TEST_NAME_E, 5)
        self.assertEqual([name for name in dir(test_type) if name[0:2] not in
            ('CO', '__')], [], msg='Names other than CONVERT_TEST_* found.')
