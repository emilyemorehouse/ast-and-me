...
class EnumerateTestCase(unittest.TestCase, PickleTest):
    ...
    def test_tuple_reuse(self):
        self.assertEqual(len(set(map(id, list(enumerate(self.seq))))), len(
            self.seq))
        self.assertEqual(len(set(map(id, enumerate(self.seq)))), min(1, len
            (self.seq)))
...
class TestReversed(unittest.TestCase, PickleTest):
    ...
    def test_simple(self):
        ...
        class A:
            ...
            def __len__(self):
                return 5
        for data in ('abc', range(5), tuple(enumerate('abc')), A(), range(1,
            17, 5)):
...
class TestLongStart(EnumerateStartTestCase):
    ...
    seq, res = 'abc', [(sys.maxsize + 1, 'a'), (sys.maxsize + 2, 'b'), (sys
        .maxsize + 3, 'c')]
