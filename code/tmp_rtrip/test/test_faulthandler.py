...
def requires_raise(test):
    return test if not is_android else requires_android_level(24,
        'raise() is buggy')(test)
...
class FaultHandlerTests(unittest.TestCase):
    ...
    def check_error(self, code, line_number, fatal_error, *, filename=None,
        all_threads=True, other_regex=None, fd=None, know_current_thread=True):
        """
        ...
        regex = """
            ...
            """
        regex = dedent(regex.format(lineno=line_number, fatal_error=
            fatal_error, header=header)).strip()
        if other_regex:
            regex += '|' + other_regex
    ...
    def test_fatal_error_c_thread(self):
        self.check_fatal_error(
            ...
            """
            , 3, 'in new thread', know_current_thread=False)
    ...
    def test_stack_overflow(self):
        self.check_fatal_error(
            ...
            """
            , 3, '(?:Segmentation fault|Bus error)', other_regex=
    ...
    def test_enable_file(self):
        with temporary_filename() as filename:
            self.check_fatal_error(
                ...
                faulthandler._sigsegv()
                """
                .format(filename=repr(filename)), 4, 'Segmentation fault',
                filename=filename)
    ...
    def test_disable(self):
        ...
        stderr = '\n'.join(stderr)
        self.assertTrue(not_expected not in stderr, '%r is present in %r' %
            (not_expected, stderr))
        self.assertNotEqual(exitcode, 0)
    ...
    def test_disabled_by_default(self):
        ...
        args = filter(None, (sys.executable, '-E' if sys.flags.
            ignore_environment else '', '-c', code))
        env = os.environ.copy()
    ...
    def test_sys_xoptions(self):
        code = 'import faulthandler; print(faulthandler.is_enabled())'
        args = filter(None, (sys.executable, '-E' if sys.flags.
            ignore_environment else '', '-X', 'faulthandler', '-c', code))
        env = os.environ.copy()
        env.pop('PYTHONFAULTHANDLER', None)
        output = subprocess.check_output(args, env=env)
    ...
    def check_dump_traceback(self, *, filename=None, fd=None):
        ...
        if filename:
            ...
            lineno = 14
        expected = ['Stack (most recent call first):', 
    ...
    def check_dump_traceback_later(self, repeat=False, cancel=False, loops=
        1, *, filename=None, fd=None):
        """
        ...
        code = """
            ...
            if filename:
                file.close()
            """
        code = code.format(timeout=TIMEOUT, repeat=repeat, cancel=cancel,
            loops=loops, filename=filename, fd=fd)
    ...
    def check_register(self, filename=False, all_threads=False, unregister=
        False, chain=False, fd=None):
        """
        ...
        code = """
            ...
            sys.exit(exitcode)
            """
        code = code.format(all_threads=all_threads, signum=signum,
            unregister=unregister, chain=chain, filename=filename, fd=fd)
        trace, exitcode = self.get_output(code, filename)
        trace = '\n'.join(trace)
        if not unregister:
            if all_threads:
    ...
    def test_raise_exception(self):
        for exc, name in (('EXCEPTION_ACCESS_VIOLATION', 'access violation'
    ...
    def test_raise_nonfatal_exception(self):
        for exc in (0, 878082192, 1073741824, 1073745920, 1879048192, 
            2147483647):
