...
class Test_fileinput_input(BaseFileInputGlobalMethodsTest):
    ...
    def do_test_call_input(self):
        ...
        mode = object()
        openhook = object()
        result = fileinput.input(files=files, inplace=inplace, backup=
            backup, bufsize=bufsize, mode=mode, openhook=openhook)
...
class Test_hook_encoded(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        def check(errors, expected_lines):
            with FileInput(files=TESTFN, mode='r', openhook=hook_encoded(
                'utf-8', errors=errors)) as fi:
                lines = list(fi)
    ...
    def test_modes(self):
        ...
        def check(mode, expected_lines):
            with FileInput(files=TESTFN, mode=mode, openhook=hook_encoded(
                'utf-7')) as fi:
                lines = list(fi)
            self.assertEqual(lines, expected_lines)
        check('r', ['A\n', 'B\n', 'C\n', 'D€'])
