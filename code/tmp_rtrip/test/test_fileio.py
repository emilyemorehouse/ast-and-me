...
from test.support import TESTFN, TESTFN_UNICODE, check_warnings, run_unittest, make_bad_fd, cpython_only, swap_attr
...
class AutoFileTests:
    ...
    def testRepr(self):
        self.assertEqual(repr(self.f), 
            '<%s.FileIO name=%r mode=%r closefd=True>' % (self.modulename,
            self.f.name, self.f.mode))
        del self.f.name
        self.assertEqual(repr(self.f), 
            '<%s.FileIO fd=%r mode=%r closefd=True>' % (self.modulename,
            self.f.fileno(), self.f.mode))
        self.f.close()
        self.assertEqual(repr(self.f), '<%s.FileIO [closed]>' % (self.
            modulename,))
        ...
        try:
            with self.FileIO(fd, 'r', closefd=False) as f:
                self.assertEqual(repr(f), 
                    '<%s.FileIO name=%r mode=%r closefd=False>' % (self.
                    modulename, f.name, f.mode))
        finally:
            os.close(fd)
    ...
    def testMethods(self):
        methods = ['fileno', 'isatty', 'seekable', 'readable', 'writable',
            'read', 'readall', 'readline', 'readlines', 'tell', 'truncate',
            'flush']
        self.f.close()
...
class OtherFileTests:
        ...
        try:
            ...
            if sys.platform != 'win32':
                try:
                    ...
                    self.assertEqual(f.readable(), False)
                    self.assertEqual(f.writable(), True)
                    if (sys.platform != 'darwin' and 'bsd' not in sys.
                        platform and not sys.platform.startswith(('sunos',
                        'aix'))):
    ...
    def testModeStrings(self):
        try:
            for modes in [('w', 'wb'), ('wb', 'wb'), ('wb+', 'rb+'), ('w+b',
                'rb+'), ('a', 'ab'), ('ab', 'ab'), ('ab+', 'ab+'), ('a+b',
                'ab+'), ('r', 'rb'), ('rb', 'rb'), ('rb+', 'rb+'), ('r+b',
                'rb+')]:
    ...
    def testConstructorHandlesNULChars(self):
        ...
        self.assertRaises(ValueError, self.FileIO, bytes(fn_with_NUL,
            'ascii'), 'w')
...
def test_main():
    try:
        run_unittest(CAutoFileTests, PyAutoFileTests, COtherFileTests,
            PyOtherFileTests)
    finally:
        if os.path.exists(TESTFN):
            os.unlink(TESTFN)
