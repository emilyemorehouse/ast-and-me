...
class TestBase:
    ...
    def assert_survivors(self, ids):
        self.assertEqual(sorted(id(x) for x in SimpleBase.survivors),
            sorted(ids))
...
class CycleChainFinalizationTest(TestBase, unittest.TestCase):
    ...
    def check_resurrecting_chain(self, classes):
        ...
        with SimpleBase.test():
            ...
            N = len(nodes)
            ids = [id(s) for s in nodes]
            survivor_ids = [id(s) for s in nodes if isinstance(s,
                SimpleResurrector)]
            wrs = [weakref.ref(s) for s in nodes]
            del nodes
            gc.collect()
    ...
    def test_heterogenous_suicidal_two(self):
        self.check_non_resurrecting_chain([SuicidalChained] * 2 + [
            SimpleChained] * 2)
    ...
    def test_heterogenous_resurrect_two(self):
        self.check_resurrecting_chain([ChainedResurrector, SimpleChained,
            SuicidalChained] * 2)

    def test_heterogenous_resurrect_three(self):
        self.check_resurrecting_chain([ChainedResurrector] * 2 + [
            SimpleChained] * 2 + [SuicidalChained] * 2)
