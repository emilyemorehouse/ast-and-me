...
class DummyRational(object):
    ...
    def __eq__(self, other):
        if isinstance(other, fractions.Fraction):
            return (self.num == other._numerator and self.den == other.
                _denominator)
        else:
            return NotImplemented
...
class GcdTest(unittest.TestCase):
    ...
    def testMisc(self):
        with self.assertWarnsRegex(DeprecationWarning, 'fractions\\.gcd'):
            gcd(1, 1)
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', 'fractions\\.gcd',
                DeprecationWarning)
...
class FractionTest(unittest.TestCase):
    ...
    def testInitFromFloat(self):
        ...
        self.assertEqual((3602879701896397, 36028797018963968), _components
            (F(0.1)))
    ...
    def testFromString(self):
        ...
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '3 /2'", F, '3 /2')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '3/+2'", F, '3/+2')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '+ 3/2'", F, '+ 3/2')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '3a2'", F, '3a2')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '3/7.2'", F, '3/7.2')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '3.2/7'", F, '3.2/7')
        self.assertRaisesMessage(ValueError,
            "Invalid literal for Fraction: '.'", F, '.')
    ...
    def testFromFloat(self):
        ...
        self.assertEqual((99999999999999991611392, 1), _components(F.
            from_float(1e+23)))
        self.assertEqual(float(10 ** 23), float(F.from_float(1e+23)))
        self.assertEqual((3602879701896397, 1125899906842624), _components(
            F.from_float(3.2)))
        self.assertEqual(3.2, float(F.from_float(3.2)))
        inf = 1e1000
        nan = inf - inf
        self.assertRaisesMessage(OverflowError,
            'cannot convert Infinity to integer ratio', F.from_float, inf)
        self.assertRaisesMessage(OverflowError,
            'cannot convert Infinity to integer ratio', F.from_float, -inf)
        self.assertRaisesMessage(ValueError,
            'cannot convert NaN to integer ratio', F.from_float, nan)
    ...
    def testFromDecimal(self):
        ...
        self.assertEqual(1 - F(1, 10 ** 30), F.from_decimal(Decimal('0.' + 
            '9' * 30)))
        self.assertRaisesMessage(OverflowError,
            'cannot convert Infinity to integer ratio', F.from_decimal,
            Decimal('inf'))
        self.assertRaisesMessage(OverflowError,
            'cannot convert Infinity to integer ratio', F.from_decimal,
            Decimal('-inf'))
        self.assertRaisesMessage(ValueError,
            'cannot convert NaN to integer ratio', F.from_decimal, Decimal(
            'nan'))
        self.assertRaisesMessage(ValueError,
            'cannot convert NaN to integer ratio', F.from_decimal, Decimal(
            'snan'))
    ...
    def testLimitDenominator(self):
        ...
        for i in (0, -1):
            self.assertRaisesMessage(ValueError,
                'max_denominator should be at least 1', F(1).
                limit_denominator, i)
    ...
    def testConversions(self):
        ...
        self.assertAlmostEqual(2.0 / 3, float(F(int('2' * 400 + '7'), int(
            '3' * 400 + '1'))))
    ...
    def testMixingWithDecimal(self):
        self.assertRaises(TypeError, operator.add, F(3, 11), Decimal(
            '3.1415926'))
        self.assertRaises(TypeError, operator.add, Decimal('3.1415926'), F(
            3, 11))
    ...
    def testBigComplexComparisons(self):
        ...
        self.assertRaises(TypeError, operator.gt, F(10 ** 23), complex(10 **
            23))
        self.assertRaises(TypeError, operator.le, F(10 ** 23), complex(10 **
            23))
        x = F(3, 8)
        z = complex(0.375, 0.0)
        w = complex(0.375, 0.2)
    ...
    def testStringification(self):
        ...
        self.assertEqual('Fraction(6283185307, 2000000000)', repr(F(
            '3.1415926535')))
        self.assertEqual('Fraction(-1, 100000000000000000000)', repr(F(1, -
            10 ** 20)))
