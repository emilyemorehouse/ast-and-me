...
class GenericTest:
    ...
    def test_no_argument(self):
        for attr in (self.common_attributes + self.attributes):
            with self.assertRaises(TypeError):
                ...
                raise self.fail('{}.{}() did not raise a TypeError'.format(
                    self.pathmodule.__name__, attr))

    def test_commonprefix(self):
        commonprefix = self.pathmodule.commonprefix
        self.assertEqual(commonprefix([]), '')
        self.assertEqual(commonprefix(['/home/swenson/spam',
        ...
        self.assertEqual(commonprefix([b'/home/swenson/spam',
            b'/home/swen/spam']), b'/home/swen')
        self.assertEqual(commonprefix([b'/home/swen/spam',
            b'/home/swen/eggs']), b'/home/swen/')
        self.assertEqual(commonprefix([b'/home/swen/spam',
            b'/home/swen/spam']), b'/home/swen/spam')
        self.assertEqual(commonprefix([b'home:swenson:spam',
            b'home:swen:spam']), b'home:swen')
        self.assertEqual(commonprefix([b':home:swen:spam',
            b':home:swen:eggs']), b':home:swen:')
        self.assertEqual(commonprefix([b':home:swen:spam',
            b':home:swen:spam']), b':home:swen:spam')
        testlist = ['', 'abc', 'Xbcd', 'Xb', 'XY', 'abcd', 'aXc', 'abd',
            'ab', 'aX', 'abcX']
        for s1 in testlist:
            for s2 in testlist:
    ...
    def test_filetime(self):
        ...
        self.assertLessEqual(self.pathmodule.getctime(filename), self.
            pathmodule.getmtime(filename))
    ...
    def _test_samestat_on_link_func(self, func):
        ...
        create_file(test_fn1)
        func(test_fn1, test_fn2)
        self.assertTrue(self.pathmodule.samestat(os.stat(test_fn1), os.stat
            (test_fn2)))
        os.remove(test_fn2)
        create_file(test_fn2)
        self.assertFalse(self.pathmodule.samestat(os.stat(test_fn1), os.
            stat(test_fn2)))
...
class CommonTest(GenericTest):
    common_attributes = GenericTest.common_attributes + ['curdir', 'pardir',
    ...
    def test_expandvars_nonascii(self):
        ...
        with support.EnvironmentVarGuard() as env:
            ...
            check(os.fsencode('${%s}bar' % nonascii), os.fsencode(
                'ham%sbar' % nonascii))
            check(os.fsencode('$bar%s bar' % nonascii), os.fsencode(
                '$bar%s bar' % nonascii))
            check(b'$spam}bar', os.fsencode('%s}bar' % nonascii))
...
class PathLikeTests(unittest.TestCase):
    ...
    def test_path_commonprefix(self):
        self.assertEqual(os.path.commonprefix([self.file_path, self.
