...
class GetpassGetuserTest(unittest.TestCase):
    ...
    def test_username_priorities_of_env_values(self, environ):
        ...
        try:
            ...
            pass
        self.assertEqual(environ.get.call_args_list, [mock.call(x) for x in
    ...
    def test_username_falls_back_to_pwd(self, environ):
        ...
        environ.get.return_value = None
        if pwd:
            with mock.patch('os.getuid') as uid, mock.patch('pwd.getpwuid'
                ) as getpw:
                uid.return_value = 42
...
class UnixGetpassTest(unittest.TestCase):
    ...
    def test_uses_tty_directly(self):
        with mock.patch('os.open') as open, mock.patch('io.FileIO'
    ...
    def test_resets_termios(self):
        with mock.patch('os.open') as open, mock.patch('io.FileIO'
            ), mock.patch('io.TextIOWrapper'), mock.patch('termios.tcgetattr'
            ) as tcgetattr, mock.patch('termios.tcsetattr') as tcsetattr:
            open.return_value = 3
    ...
    def test_falls_back_to_fallback_if_termios_raises(self):
        with mock.patch('os.open') as open, mock.patch('io.FileIO'
            ...
            ) as tcsetattr, mock.patch('getpass.fallback_getpass') as fallback:
            open.return_value = 3
    ...
    def test_falls_back_to_stdin(self):
        with mock.patch('os.open') as os_open, mock.patch('sys.stdin', spec
            =StringIO) as stdin:
