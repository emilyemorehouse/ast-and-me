...
class CookieTests(unittest.TestCase):
    ...
    def test_basic(self):
        cases = [{'data': 'chips=ahoy; vienna=finger', 'dict': {'chips':
            'ahoy', 'vienna': 'finger'}, 'repr':
            "<SimpleCookie: chips='ahoy' vienna='finger'>", 'output':
            """Set-Cookie: chips=ahoy
Set-Cookie: vienna=finger"""}, {
            'data': 'keebler="E=mc2; L=\\"Loves\\"; fudge=\\012;"', 'dict':
            {'keebler': """E=mc2; L="Loves"; fudge=
;"""}, 'repr':
            '<SimpleCookie: keebler=\'E=mc2; L="Loves"; fudge=\\n;\'>',
            'output':
            'Set-Cookie: keebler="E=mc2; L=\\"Loves\\"; fudge=\\012;"'}, {
            'data': 'keebler=E=mc2', 'dict': {'keebler': 'E=mc2'}, 'repr':
            "<SimpleCookie: keebler='E=mc2'>", 'output':
            'Set-Cookie: keebler=E=mc2'}, {'data': 'key:term=value:term',
            'dict': {'key:term': 'value:term'}, 'repr':
            "<SimpleCookie: key:term='value:term'>", 'output':
            'Set-Cookie: key:term=value:term'}, {'data':
            'a=b; c=[; d=r; f=h', 'dict': {'a': 'b', 'c': '[', 'd': 'r',
            'f': 'h'}, 'repr': "<SimpleCookie: a='b' c='[' d='r' f='h'>",
    ...
    def test_invalid_cookies(self):
        ...
        for s in (']foo=x', '[foo=x', 'blah]foo=x', 'blah[foo=x',
            'Set-Cookie: foo=bar', 'Set-Cookie: foo', 'foo=bar; baz',
            'baz; foo=bar', 'secure;foo=bar', 'Version=1;foo=bar'):
            C.load(s)
...
class MorselTests(unittest.TestCase):
    ...
    def test_setter(self):
        ...
        for i in 'thou cast _the- !holy! ^hand| +*grenade~'.split():
            ...
            self.assertEqual(M.output(), 'Set-Cookie: %s=%s; Path=/foo' % (
                i, '%s_coded_val' % i))
            expected_js_output = (
                """
        <script type="text/javascript">
    ...
    def test_deprecation(self):
        morsel = cookies.Morsel()
        with self.assertWarnsRegex(DeprecationWarning, '\\bkey\\b'):
            morsel.key = ''
        with self.assertWarnsRegex(DeprecationWarning, '\\bvalue\\b'):
            morsel.value = ''
        with self.assertWarnsRegex(DeprecationWarning, '\\bcoded_value\\b'):
            morsel.coded_value = ''
        with self.assertWarnsRegex(DeprecationWarning, '\\bLegalChars\\b'):
            morsel.set('key', 'value', 'coded_value', LegalChars='.*')

    def test_eq(self):
        base_case = 'key', 'value', '"value"'
        attribs = {'path': '/', 'comment': 'foo', 'domain': 'example.com',
            'version': 2}
        ...
        cases = ('key', 'value', 'mismatch'), ('key', 'mismatch', '"value"'), (
    ...
    def test_repr(self):
        ...
        morsel.update({'path': '/', 'comment': 'foo', 'domain':
            'example.com', 'max-age': 0, 'secure': 0, 'version': 1})
