...
import calendar
import inspect
from test.support import reap_threads, verbose, transient_internet, run_with_tz, run_with_locale, cpython_only
import unittest
...
class TestImaplib(unittest.TestCase):
    ...
    def test_Internaldate2tuple_issue10941(self):
        self.assertNotEqual(imaplib.Internaldate2tuple(
            b'25 (INTERNALDATE "02-Apr-2000 02:30:00 +0000")'), imaplib.
            Internaldate2tuple(
            b'25 (INTERNALDATE "02-Apr-2000 03:30:00 +0000")'))

    def timevalues(self):
        return [2000000000, 2000000000.0, time.localtime(2000000000), (2033,
            5, 18, 5, 33, 20, -1, -1, -1), (2033, 5, 18, 5, 33, 20, -1, -1,
            1), datetime.fromtimestamp(2000000000, timezone(timedelta(0, 2 *
            60 * 60))), '"18-May-2033 05:33:20 +0200"']
...
class SimpleIMAPHandler(socketserver.StreamRequestHandler):
    ...
    def cmd_CAPABILITY(self, tag, args):
        caps = ('IMAP4rev1 ' + self.capabilities if self.capabilities else
            'IMAP4rev1')
...
class NewIMAPTestsMixin:
    ...
    def _setup(self, imap_handler, connect=True):
        ...
        self.thread = threading.Thread(name=self._testMethodName +
            '-server', target=self.server.serve_forever, kwargs={
            'poll_interval': 0.01})
    ...
    def test_EOF_without_complete_welcome_message(self):
        ...
        self.assertRaises(imaplib.IMAP4.abort, self.imap_class, *server.
            server_address)
...
class ThreadedNetworkedTests(unittest.TestCase):
    ...
    def make_server(self, addr, hdlr):
        ...
        t = threading.Thread(name='%s serving' % self.server_class, target=
    ...
    def test_linetoolong(self):
        ...
        with self.reaped_server(TooLongHandler) as server:
            self.assertRaises(imaplib.IMAP4.error, self.imap_class, *server
                .server_address)
...
class RemoteIMAP_SSLTest(RemoteIMAPTest):
    ...
    def test_ssl_context_certfile_exclusive(self):
        with transient_internet(self.host):
            self.assertRaises(ValueError, self.imap_class, self.host, self.
                port, certfile=CERTFILE, ssl_context=self.create_ssl_context())

    def test_ssl_context_keyfile_exclusive(self):
        with transient_internet(self.host):
            self.assertRaises(ValueError, self.imap_class, self.host, self.
                port, keyfile=CERTFILE, ssl_context=self.create_ssl_context())
