...
class LoaderTests(abc.LoaderTests):
    ...
    def setUp(self):
        self.loader = self.machinery.ExtensionFileLoader(util.EXTENSIONS.
            name, util.EXTENSIONS.file_path)
    ...
    def test_inequality(self):
        other = self.machinery.ExtensionFileLoader('_' + util.EXTENSIONS.
            name, util.EXTENSIONS.file_path)
    ...
    def test_module(self):
        with util.uncache(util.EXTENSIONS.name):
            ...
            for attr, value in [('__name__', util.EXTENSIONS.name), (
                '__file__', util.EXTENSIONS.file_path), ('__package__', '')]:
                self.assertEqual(getattr(module, attr), value)
            self.assertIn(util.EXTENSIONS.name, sys.modules)
            self.assertIsInstance(module.__loader__, self.machinery.
                ExtensionFileLoader)
    test_package = None
...
class MultiPhaseExtensionModuleTests(abc.LoaderTests):
    ...
    def setUp(self):
        ...
        assert self.spec
        self.loader = self.machinery.ExtensionFileLoader(self.name, self.
            spec.origin)
    test_package = None
    test_lacking_parent = None
    test_state_after_failure = None
    ...
    def test_module(self):
        ...
        with util.uncache(self.name):
            ...
            for attr, value in [('__name__', self.name), ('__file__', self.
                spec.origin), ('__package__', '')]:
                self.assertEqual(getattr(module, attr), value)
    ...
    def test_bad_modules(self):
        ...
        for name_base in ['bad_slot_large', 'bad_slot_negative',
            'create_int_with_state', 'negative_size', 'export_null',
            'export_uninitialized', 'export_raise',
            'export_unreported_exception', 'create_null', 'create_raise',
            'create_unreported_exception', 'nonmodule_with_exec_slots',
            'exec_err', 'exec_raise', 'exec_unreported_exception']:
    ...
    def test_nonascii(self):
        ...
        cases = [(self.name + '_zkouška_načtení', 'Czech'), ('＿インポートテスト',
            'Japanese')]
