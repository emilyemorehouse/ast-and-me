...
class ExecModuleTests(abc.LoaderTests):
    ...
    def exec_module(self, name):
        with util.uncache(name), captured_stdout() as stdout:
            spec = self.machinery.ModuleSpec(name, self.machinery.
                FrozenImporter, origin='frozen', is_package=self.machinery.
                FrozenImporter.is_package(name))
    ...
    def test_package(self):
        ...
        for attr, value in check.items():
            ...
            self.assertEqual(attr_value, value,
                'for {name}.{attr}, {given!r} != {expected!r}'.format(name=
                name, attr=attr, given=attr_value, expected=value))
        self.assertEqual(output, 'Hello world!\n')
...
class LoaderTests(abc.LoaderTests):
    ...
    def test_module(self):
        with util.uncache('__hello__'), captured_stdout() as stdout:
            ...
            check = {'__name__': '__hello__', '__package__': '',
    ...
    def test_package(self):
        with util.uncache('__phello__'), captured_stdout() as stdout:
            ...
            check = {'__name__': '__phello__', '__package__': '__phello__',
                '__path__': [], '__loader__': self.machinery.FrozenImporter}
            for attr, value in check.items():
                attr_value = getattr(module, attr)
                self.assertEqual(attr_value, value, 
    ...
    def test_lacking_parent(self):
        with util.uncache('__phello__', '__phello__.spam'), captured_stdout(
            ) as stdout:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore', DeprecationWarning)
                module = self.machinery.FrozenImporter.load_module(
                    '__phello__.spam')
            check = {'__name__': '__phello__.spam', '__package__':
                '__phello__', '__loader__': self.machinery.FrozenImporter}
            for attr, value in check.items():
                attr_value = getattr(module, attr)
                self.assertEqual(attr_value, value, 
                    'for __phello__.spam.%s, %r != %r' % (attr, attr_value,
                    value))
            self.assertEqual(stdout.getvalue(), 'Hello world!\n')
            self.assertFalse(hasattr(module, '__file__'))
