...
class CallingOrder:
    ...
    def test_continuing(self):
        mod_name = 'for_real'
        with util.mock_spec('nonexistent') as first, util.mock_spec(mod_name
            ) as second:
            first.find_spec = (lambda self, fullname, path=None, parent=
                None: None)
            with util.import_state(meta_path=[first, second]):
                self.assertIs(self.__import__(mod_name), second.modules[
                    mod_name])
...
Frozen_CallingOrder, Source_CallingOrder = util.test_both(CallingOrder,
...
class CallSignature:
    ...
    def test_no_path(self):
        ...
        with self.mock_modules(mod_name) as importer:
            ...
            setattr(importer, self.finder_name, MethodType(wrapped_call,
                importer))
