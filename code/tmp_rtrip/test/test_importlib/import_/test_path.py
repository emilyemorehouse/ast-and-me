...
class FinderTests:
    ...
    def test_sys_path(self):
        ...
        with util.import_state(path_importer_cache={path: importer}, path=[
            path]):
    ...
    def test_empty_path_hooks(self):
        ...
        with util.import_state(path_importer_cache={}, path_hooks=[], path=
            [path_entry]):
    ...
    def test_None_on_sys_path(self):
        ...
        new_path_hooks = [zipimport.zipimporter, self.machinery.FileFinder.
            path_hook(*self.importlib._bootstrap_external.
            _get_supported_file_loaders())]
        missing = object()
        email = sys.modules.pop('email', missing)
        try:
            with util.import_state(meta_path=sys.meta_path[:], path=
                new_path, path_importer_cache=new_path_importer_cache,
    ...
    def test_finder_with_find_module(self):
        ...
        with util.import_state(path_importer_cache={path: failing_finder}):
            self.assertIsNone(self.machinery.PathFinder.find_spec(
                'whatever', [path]))
...
Frozen_FindModuleTests, Source_FindModuleTests = util.test_both(FindModuleTests
    , importlib=importlib, machinery=machinery)
...
Frozen_PEFTests, Source_PEFTests = util.test_both(PathEntryFinderTests,
    machinery=machinery)
