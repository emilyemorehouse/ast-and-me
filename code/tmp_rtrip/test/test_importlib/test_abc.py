...
class InheritanceTests:
    ...
    def setUp(self):
        self.superclasses = [getattr(self.abc, class_name) for class_name in
            self.superclass_names]
        if hasattr(self, 'subclass_names'):
            machinery = self.abc.machinery
            self.subclasses = [getattr(machinery, class_name) for
                class_name in self.subclass_names]
    ...
    def test_superclasses(self):
        for superclass in self.superclasses:
            self.assertTrue(issubclass(self.__test, superclass),
                '{0} is not a superclass of {1}'.format(superclass, self.
                __test))
...
class InspectLoader(InheritanceTests):
    superclass_names = ['Loader']
    subclass_names = ['BuiltinImporter', 'FrozenImporter',
        'ExtensionFileLoader']
...
def make_abc_subclasses(base_class, name=None, inst=False, **kwargs):
    ...
    return {cls._KIND: (cls() if inst else cls) for cls in test_util.
        split_frozen(base_class, base, **kwargs)}
...
Frozen_LDefaultTests, SourceLDefaultTests = test_util.test_both(
    LoaderDefaultsTests)
...
class LoaderLoadModuleTests:
    ...
    def test_fresh(self):
        ...
        self.assertIsNotNone(module.__path__, module.__spec__.
...
Frozen_LoaderLoadModuleTests, Source_LoaderLoadModuleTests = (test_util.
    test_both(LoaderLoadModuleTests, abc=abc, util=util))
...
class InspectLoaderGetCodeTests:
    ...
    def test_get_code(self):
        module = types.ModuleType('blah')
        with mock.patch.object(self.InspectLoaderSubclass, 'get_source'
            ) as mocked:
...
Frozen_ILLoadModuleTests, Source_ILLoadModuleTests = test_util.test_both(
    InspectLoaderLoadModuleTests, InspectLoaderSubclass=SPLIT_IL, init=init,
    util=util)
...
class ExecutionLoaderGetCodeTests:
    ...
    def mock_methods(self, *, get_source=False, get_filename=False):
        ...
        if get_source:
            source_mock_context = mock.patch.object(self.
                ExecutionLoaderSubclass, 'get_source')
        if get_filename:
            filename_mock_context = mock.patch.object(self.
                ExecutionLoaderSubclass, 'get_filename')
...
class SourceOnlyLoader:
    source = (
        b"_ = '::'.join([__name__, __file__, __cached__, __package__, repr(__loader__)])"
        )
...
Frozen_SourceOnlyLoaderTests, Source_SourceOnlyLoaderTests = (test_util.
    test_both(SourceOnlyLoaderTests, util=util, loader_mock=SPLIT_SOL))
