    ...
    def test_success_path(self):
        ...
        with test_util.uncache(name):
            with test_util.import_state(meta_path=[self.FakeMetaFinder]):
                with warnings.catch_warnings():
                    warnings.simplefilter('ignore', DeprecationWarning)
                    self.assertEqual((name, path), self.init.find_loader(
                        name, path))

    def test_nothing(self):
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', DeprecationWarning)
            self.assertIsNone(self.init.find_loader(
...
Frozen_FindLoaderPEP451Tests, Source_FindLoaderPEP451Tests = (test_util.
    test_both(FindLoaderPEP451Tests, init=init))
...
Frozen_FindLoaderPEP302Tests, Source_FindLoaderPEP302Tests = (test_util.
    test_both(FindLoaderPEP302Tests, init=init))
...
class ReloadTests:
    ...
    def test_module_replaced(self):
        ...
        mock = test_util.mock_modules('top_level', module_code={'top_level':
            code})
        with mock:
            with test_util.import_state(meta_path=[mock]):
                module = self.init.import_module('top_level')
        ...
        with support.temp_cwd(None) as cwd:
            with test_util.uncache('spam'):
                with support.DirsOnSysPath(cwd):
                    ...
                    expected = {'__name__': name, '__package__': '',
                        '__file__': path, '__cached__': cached, '__doc__': None
                        }
                    ...
                    expected = {'__name__': name, '__package__': name,
                        '__file__': init_path, '__cached__': cached,
                        '__path__': [os.path.dirname(init_path)], '__doc__':
                        None}
                    os.mkdir(name)
        ...
        with support.temp_cwd(None) as cwd:
            with test_util.uncache('spam'):
                with support.DirsOnSysPath(cwd):
                    ...
                    self.assertEqual(set(path), set([os.path.dirname(
                        bad_path)]))
...
Frozen_ReloadTests, Source_ReloadTests = test_util.test_both(ReloadTests,
    init=init, util=util)
...
Frozen_StartupTests, Source_StartupTests = test_util.test_both(StartupTests,
    machinery=machinery)
