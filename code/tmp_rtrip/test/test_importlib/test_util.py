...
Frozen_SetPackageTests, Source_SetPackageTests = util.test_both(SetPackageTests
    , util=importlib_util)
...
Frozen_SetLoaderTests, Source_SetLoaderTests = util.test_both(SetLoaderTests,
    util=importlib_util)
...
class ResolveNameTests:
    ...
    def test_in_package(self):
        self.assertEqual('spam.eggs.bacon', self.util.resolve_name('.bacon',
            'spam.eggs'))

    def test_other_package(self):
        self.assertEqual('spam.bacon', self.util.resolve_name('..bacon',
            'spam.eggs'))
...
Frozen_FindSpecTests, Source_FindSpecTests = util.test_both(FindSpecTests,
    init=init, util=importlib_util, machinery=machinery)
...
class PEP3147Tests:
    ...
    def test_cache_from_source_no_dot(self):
        ...
        expect = os.path.join('foo.bar', '__pycache__', 'file{}.pyc'.format
            (self.tag))
        self.assertEqual(self.util.cache_from_source(path, optimization=''),
            expect)
    ...
    def test_cache_from_source_debug_override(self):
        ...
        with warnings.catch_warnings():
            ...
            self.assertEqual(self.util.cache_from_source(path, False), self
                .util.cache_from_source(path, optimization=1))
            self.assertEqual(self.util.cache_from_source(path, True), self.
                util.cache_from_source(path, optimization=''))
        with warnings.catch_warnings():
    ...
    def test_cache_from_source_override(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            self.assertEqual(self.util.cache_from_source(path, []), self.
                util.cache_from_source(path, optimization=1))
            self.assertEqual(self.util.cache_from_source(path, [17]), self.
                util.cache_from_source(path, optimization=''))
    ...
    def test_cache_from_source_optimization_None(self):
        ...
        if optimization_level == 0:
            ...
            msg = '{!r} is a non-standard optimization level'.format(
                optimization_level)
            self.skipTest(msg)
        self.assertEqual(self.util.cache_from_source(path, optimization=
            None), expect)
    ...
    def test_cache_from_source_optimization_set(self):
        ...
        self.assertEqual(got, almost_expect + '.opt-{}.pyc'.format(
            valid_characters))
    ...
    def test_sep_altsep_and_sep_cache_from_source(self):
        self.assertEqual(self.util.cache_from_source(
            '\\foo\\bar\\baz/qux.py', optimization=''),
    ...
    def test_source_from_cache_bad_path(self):
        self.assertRaises(ValueError, self.util.source_from_cache,
            '/foo/bar/bazqux.pyc')
...
Frozen_PEP3147Tests, Source_PEP3147Tests = util.test_both(PEP3147Tests,
    util=importlib_util)
...
class MagicNumberTests(unittest.TestCase):
    ...
    @unittest.skipUnless(sys.version_info.releaselevel in ('final',
        'release'), 'only applies to candidate or final python release levels')
