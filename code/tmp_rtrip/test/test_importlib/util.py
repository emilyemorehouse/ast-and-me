...
def import_importlib(module_name):
    ...
    source = support.import_fresh_module(module_name, fresh=fresh, blocked=
...
__import__ = {'Frozen': staticmethod(builtins.__import__), 'Source':
    staticmethod(source_importlib.__import__)}
...
def import_state(**kwargs):
    ...
    try:
        ...
        if len(kwargs):
            raise ValueError('unrecognized arguments: {0}'.format(kwargs.
                keys()))
        yield
    finally:
...
class mock_spec(_ImporterMock):
    ...
    def find_spec(self, fullname, path=None, parent=None):
        try:
            ...
            return None
        spec = util.spec_from_file_location(fullname, module.__file__,
            loader=self, submodule_search_locations=getattr(module,
            '__path__', None))
        return spec
...
class CASEOKTestBase:
    ...
    def caseok_env_changed(self, *, should_exist):
        ...
        if any(x in self.importlib._bootstrap_external._os.environ for x in
            possibilities) != should_exist:
