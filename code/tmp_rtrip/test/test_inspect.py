...
class IsTestBase(unittest.TestCase):
    predicates = set([inspect.isbuiltin, inspect.isclass, inspect.iscode,
        inspect.isframe, inspect.isfunction, inspect.ismethod, inspect.
        ismodule, inspect.istraceback, inspect.isgenerator, inspect.
        isgeneratorfunction, inspect.iscoroutine, inspect.
        iscoroutinefunction, inspect.isasyncgen, inspect.isasyncgenfunction])

    def istest(self, predicate, exp):
        obj = eval(exp)
        self.assertTrue(predicate(obj), '%s(%s)' % (predicate.__name__, exp))
        for other in (self.predicates - set([predicate])):
            if (predicate == inspect.isgeneratorfunction or predicate ==
                inspect.isasyncgenfunction or predicate == inspect.
                iscoroutinefunction) and other == inspect.isfunction:
                continue
...
class TestPredicates(IsTestBase):
    ...
    def test_excluding_predicates(self):
        ...
        try:
            ...
            if hasattr(types, 'GetSetDescriptorType'):
                ...
                self.assertFalse(inspect.isgetsetdescriptor(type(tb.
                    tb_frame).f_locals))
        finally:
            tb = None
        ...
        if hasattr(types, 'MemberDescriptorType'):
            ...
            self.assertFalse(inspect.ismemberdescriptor(datetime.timedelta.
                days))
    ...
    def test_iscoroutine(self):
        ...
        self.assertFalse(inspect.isgeneratorfunction(
            coroutine_function_example))
...
class TestInterpreterStack(IsTestBase):
    ...
    def test_stack(self):
        ...
        self.assertEqual(revise(*mod.st[0][1:]), (modfile, 16, 'eggs', [
            """    st = inspect.stack()
"""], 0))
        self.assertEqual(revise(*mod.st[1][1:]), (modfile, 9, 'spam', [
            '    eggs(b + d, c + f)\n'], 0))
        self.assertEqual(revise(*mod.st[2][1:]), (modfile, 43, 'argue', [
            """            spam(a, b, c)
"""], 0))
        self.assertEqual(revise(*mod.st[3][1:]), (modfile, 39, 'abuse', [
            """        self.argue(a, b, c)
"""], 0))
        record = mod.st[0]
...
"""], 0))
    ...
    def test_frame(self):
        ...
        self.assertEqual(inspect.formatargvalues(args, varargs, varkw,
            locals), '(x=11, y=14)')
    ...
    def test_previous_frame(self):
        ...
        self.assertEqual(inspect.formatargvalues(args, varargs, varkw,
            locals), '(a=7, b=8, c=9, d=3, e=4, f=5, *g=(), **h={})')
...
class TestRetrievingSourceCode(GetSourceBase):
    ...
    def test_getclasses(self):
        ...
        self.assertEqual(classes, [('FesteringGob', mod.FesteringGob), (
            'MalodorousPervert', mod.MalodorousPervert), ('ParrotDroppings',
            mod.ParrotDroppings), ('StupidGit', mod.StupidGit), ('Tit', mod
            .MalodorousPervert)])
        tree = inspect.getclasstree([cls[1] for cls in classes])
        self.assertEqual(tree, [(object, ()), [(mod.ParrotDroppings, (
            object,)), [(mod.FesteringGob, (mod.MalodorousPervert, mod.
            ParrotDroppings))], (mod.StupidGit, (object,)), [(mod.
            MalodorousPervert, (mod.StupidGit,)), [(mod.FesteringGob, (mod.
            MalodorousPervert, mod.ParrotDroppings))]]]])
        tree = inspect.getclasstree([cls[1] for cls in classes], True)
        self.assertEqual(tree, [(object, ()), [(mod.ParrotDroppings, (
            object,)), (mod.StupidGit, (object,)), [(mod.MalodorousPervert,
            (mod.StupidGit,)), [(mod.FesteringGob, (mod.MalodorousPervert,
            mod.ParrotDroppings))]]]])

    def test_getfunctions(self):
        functions = inspect.getmembers(mod, inspect.isfunction)
        self.assertEqual(functions, [('eggs', mod.eggs), ('lobbest', mod.
            lobbest), ('spam', mod.spam)])
...
class TestBuggyCases(GetSourceBase):
    ...
    @unittest.skipIf(not hasattr(unicodedata, '__file__') or unicodedata.
...
class TestNoEOL(GetSourceBase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.tempdir, 'inspect_fodder3%spy' % os.
            extsep), 'w') as f:
            f.write('class X:\n    pass # No EOL')
...
class TestClassesAndFunctions(unittest.TestCase):
    ...
    def assertArgSpecEquals(self, routine, args_e, varargs_e=None, varkw_e=
        None, defaults_e=None, formatted=None):
        ...
        if formatted is not None:
            self.assertEqual(inspect.formatargspec(args, varargs, varkw,
                defaults), formatted)

    def assertFullArgSpecEquals(self, routine, args_e, varargs_e=None,
        varkw_e=None, defaults_e=None, kwonlyargs_e=[], kwonlydefaults_e=
        None, ann_e={}, formatted=None):
        ...
        if formatted is not None:
            self.assertEqual(inspect.formatargspec(args, varargs, varkw,
                defaults, kwonlyargs, kwonlydefaults, ann), formatted)

    def test_getargspec(self):
        self.assertArgSpecEquals(mod.eggs, ['x', 'y'], formatted='(x, y)')
        self.assertArgSpecEquals(mod.spam, ['a', 'b', 'c', 'd', 'e', 'f'],
            'g', 'h', (3, 4, 5), '(a, b, c, d=3, e=4, f=5, *g, **h)')
        self.assertRaises(ValueError, self.assertArgSpecEquals, mod2.
            keyworded, [])
        self.assertRaises(ValueError, self.assertArgSpecEquals, mod2.
            annotated, [])
        self.assertRaises(ValueError, self.assertArgSpecEquals, mod2.
    ...
    def test_getfullargspec(self):
        self.assertFullArgSpecEquals(mod2.keyworded, [], varargs_e='arg1',
            kwonlyargs_e=['arg2'], kwonlydefaults_e={'arg2': 1}, formatted=
            '(*arg1, arg2=1)')
        self.assertFullArgSpecEquals(mod2.annotated, ['arg1'], ann_e={
            'arg1': list}, formatted='(arg1: list)')
        self.assertFullArgSpecEquals(mod2.keyword_only_arg, [],
            kwonlyargs_e=['arg'], formatted='(*, arg)')
    ...
    def test_argspec_api_ignores_wrapped(self):
        ...
        def check_method(f):
            self.assertArgSpecEquals(f, ['self', 'x', 'y'], formatted=
                '(self, x, y)')
    ...
    def test_getfullargspec_signature_attr(self):
        ...
        def test():
            pass
        spam_param = inspect.Parameter('spam', inspect.Parameter.
            POSITIONAL_ONLY)
    ...
    def test_getfullargspec_builtin_methods(self):
        self.assertFullArgSpecEquals(_pickle.Pickler.dump, args_e=['self',
            'obj'], formatted='(self, obj)')
        self.assertFullArgSpecEquals(_pickle.Pickler(io.BytesIO()).dump,
            args_e=['self', 'obj'], formatted='(self, obj)')
        self.assertFullArgSpecEquals(os.stat, args_e=['path'], kwonlyargs_e
            =['dir_fd', 'follow_symlinks'], kwonlydefaults_e={'dir_fd':
            None, 'follow_symlinks': True}, formatted=
    ...
    def test_classify_DynamicClassAttribute(self):
        ...
        class VA(metaclass=Meta):
            ...
            def ham(self):
                return 'eggs'
        should_find_dca = inspect.Attribute('ham', 'data', VA, VA.__dict__[
            'ham'])
    ...
    def test_getmembers_descriptors(self):
        ...
        self.assertEqual(inspect.getmembers(A, ismethoddescriptor), [('md',
            A.__dict__['md'])])
        self.assertEqual(inspect.getmembers(A, isdatadescriptor), [('dd', A
            .__dict__['dd'])])


        class B(A):
            pass
        self.assertEqual(inspect.getmembers(B, ismethoddescriptor), [('md',
            A.__dict__['md'])])
        self.assertEqual(inspect.getmembers(B, isdatadescriptor), [('dd', A
            .__dict__['dd'])])
...
class TestGetClosureVars(unittest.TestCase):
    ...
    def test_nonlocal_vars(self):
        ...
        self.assertEqual(_nonlocal_vars(greater_than_five), {'arg1': 5,
            'func': less_than})
...
class TestGetcallargsFunctions(unittest.TestCase):
    ...
    def assertEqualCallArgs(self, func, call_params_string, locs=None):
        ...
        r2 = eval('inspect.getcallargs(func, %s)' % call_params_string,
            None, locs)
    ...
    def test_multiple_features(self):
        ...
        self.assertEqualCallArgs(f,
            'x=8, *collections.UserList([2, 3, (4,[5,6])]), **{"y":9, "z":10}')
        self.assertEqualCallArgs(f,
...
class TestGetattrStatic(unittest.TestCase):
    ...
    def test_classVirtualAttribute(self):
        ...
        class Thing(object):
            ...
            def x(self):
                return self._x
            _x = object()
        self.assertEqual(inspect.getattr_static(Thing, 'x'), Thing.__dict__
            ['x'])
    ...
    def test_no_dict_no_slots_instance_member(self):
        with open(__file__) as handle:
            self.assertEqual(inspect.getattr_static(handle, 'name'), type(
                handle).name)
    ...
    def test_getgeneratorlocals(self):
        ...
        def each(lst, a=None):
            ...
            for v in lst:
                ...
                yield v
        numbers = each([1, 2, 3])
        self.assertEqual(inspect.getgeneratorlocals(numbers), {'a': None,
            'lst': [1, 2, 3]})
        next(numbers)
        self.assertEqual(inspect.getgeneratorlocals(numbers), {'a': None,
            'lst': [1, 2, 3], 'v': 1, 'b': (1, 2, 3)})
        next(numbers)
        self.assertEqual(inspect.getgeneratorlocals(numbers), {'a': None,
            'lst': [1, 2, 3], 'v': 2, 'b': (1, 2, 3)})
        next(numbers)
        self.assertEqual(inspect.getgeneratorlocals(numbers), {'a': None,
            'lst': [1, 2, 3], 'v': 3, 'b': (1, 2, 3), 'c': 12})
        try:
            next(numbers)
...
class TestGetCoroutineState(unittest.TestCase):
    ...
    def test_getcoroutinelocals(self):
        ...
        async def func(a=None):
            ...
            await gencoro
        coro = func()
        self.assertEqual(inspect.getcoroutinelocals(coro), {'a': None,
            'gencoro': gencoro})
        coro.send(None)
        self.assertEqual(inspect.getcoroutinelocals(coro), {'a': None,
            'gencoro': gencoro, 'b': 'spam'})
...
class TestSignatureObject(unittest.TestCase):
    ...
    def signature(func, **kw):
        ...
        return tuple((param.name, ... if param.default is param.empty else
            param.default, ... if param.annotation is param.empty else
            param.annotation, str(param.kind).lower()) for param in sig.
            parameters.values()
    ...
    def test_signature_on_wargs(self):
        ...
        def test(a, b: 'foo') ->(123):
            pass
        self.assertEqual(self.signature(test), ((('a', ..., ...,
            'positional_or_keyword'), ('b', ..., 'foo',
            'positional_or_keyword')), 123))

    def test_signature_on_wkwonly(self):

        def test(*, a: float, b: str) ->int:
            pass
        self.assertEqual(self.signature(test), ((('a', ..., float,
            'keyword_only'), ('b', ..., str, 'keyword_only')), int))

    def test_signature_on_complex_args(self):

        def test(a, b: 'foo'=10, *args: 'bar', spam: 'baz', ham=123, **
            kwargs: int):
            pass
        self.assertEqual(self.signature(test), ((('a', ..., ...,
            'positional_or_keyword'), ('b', 10, 'foo',
            'positional_or_keyword'), ('args', ..., 'bar', 'var_positional'
            ), ('spam', ..., 'baz', 'keyword_only'), ('ham', 123, ...,
            'keyword_only'), ('kwargs', ..., int, 'var_keyword')), ...))
    ...
    def test_signature_without_self(self):
        ...
        self.assertEqual(self.signature(test_args_only), ((('args', ...,
            ..., 'var_positional'),), ...))
        self.assertEqual(self.signature(test_args_kwargs_only), ((('args',
            ..., ..., 'var_positional'), ('kwargs', ..., ..., 'var_keyword'
            )), ...))
        self.assertEqual(self.signature(A.f1), ((('args', ..., ...,
            'var_positional'),), ...))
        self.assertEqual(self.signature(A.f2), ((('args', ..., ...,
            'var_positional'),), ...))
        self.assertEqual(self.signature(A.f3), ((('args', ..., ...,
            'var_positional'),), ...))
        self.assertEqual(self.signature(A.f4), ((('args', ..., ...,
            'var_positional'), ('kwargs', ..., ..., 'var_keyword')), ...))
    ...
    def test_signature_on_builtins(self):
        ...
        def test_unbound_method(o):
            ...
            self.assertEqual(list(signature.parameters.values())[0].name,
                'self')
            return signature
        ...
        def test_callable(o):
            ...
            if signature.parameters:
                self.assertNotEqual(list(signature.parameters.values())[0].
                    name, 'self')
            return signature
        signature = test_callable(_testcapi.
    ...
    def test_signature_on_decorated_builtins(self):
        ...
        self.assertEqual(inspect.signature(func), inspect.signature(
            decorated_func))

        def wrapper_like(*args, **kwargs) ->int:
            pass
        self.assertEqual(inspect.signature(decorated_func, follow_wrapped=
    ...
    def test_signature_from_functionlike_object(self):
        ...
        fl = funclike(func)
        del fl.__defaults__
        self.assertEqual(self.signature(fl), ((('args', ..., ...,
            'var_positional'), ('kwargs', ..., ..., 'var_keyword')), ...))
    ...
    def test_signature_on_method(self):
        ...
        class Test:
            ...
            def __call__(*, a):
                pass
        self.assertEqual(self.signature(Test().m1), ((('arg1', ..., ...,
            'positional_or_keyword'), ('arg2', 1, ...,
            'positional_or_keyword')), int))
        self.assertEqual(self.signature(Test().m2), ((('args', ..., ...,
            'var_positional'),), ...))
        self.assertEqual(self.signature(Test), ((('args', ..., ...,
            'var_positional'),), ...))
    ...
    def test_signature_wrapped_bound_method(self):
        ...
        def m1d(*args, **kwargs):
            pass
        self.assertEqual(self.signature(m1d), ((('arg1', ..., ...,
            'positional_or_keyword'), ('arg2', 1, ...,
            'positional_or_keyword')), int))
    ...
    def test_signature_on_classmethod(self):
        ...
        class Test:
            ...
            def foo(cls, arg1, *, arg2=1):
                pass
        meth = Test().foo
        self.assertEqual(self.signature(meth), ((('arg1', ..., ...,
            'positional_or_keyword'), ('arg2', 1, ..., 'keyword_only')), ...))
        meth = Test.foo
        self.assertEqual(self.signature(meth), ((('arg1', ..., ...,
            'positional_or_keyword'), ('arg2', 1, ..., 'keyword_only')), ...))
    ...
    def test_signature_on_staticmethod(self):
        ...
        class Test:
            ...
            def foo(cls, *, arg):
                pass
        meth = Test().foo
        self.assertEqual(self.signature(meth), ((('cls', ..., ...,
            'positional_or_keyword'), ('arg', ..., ..., 'keyword_only')), ...))
        meth = Test.foo
        self.assertEqual(self.signature(meth), ((('cls', ..., ...,
            'positional_or_keyword'), ('arg', ..., ..., 'keyword_only')), ...))
    ...
    def test_signature_on_partial(self):
        ...
        def test(a, b, *, c, d):
            pass
        self.assertEqual(self.signature(partial(test)), ((('a', ..., ...,
            'positional_or_keyword'), ('b', ..., ...,
            'positional_or_keyword'), ('c', ..., ..., 'keyword_only'), ('d',
            ..., ..., 'keyword_only')), ...))
        self.assertEqual(self.signature(partial(test, 1)), ((('b', ..., ...,
            'positional_or_keyword'), ('c', ..., ..., 'keyword_only'), ('d',
            ..., ..., 'keyword_only')), ...))
        self.assertEqual(self.signature(partial(test, 1, c=2)), ((('b', ...,
            ..., 'positional_or_keyword'), ('c', 2, ..., 'keyword_only'), (
            'd', ..., ..., 'keyword_only')), ...))
        self.assertEqual(self.signature(partial(test, b=1, c=2)), ((('a',
            ..., ..., 'positional_or_keyword'), ('b', 1, ...,
            'keyword_only'), ('c', 2, ..., 'keyword_only'), ('d', ..., ...,
            'keyword_only')), ...))
        self.assertEqual(self.signature(partial(test, 0, b=1, c=2)), ((('b',
            1, ..., 'keyword_only'), ('c', 2, ..., 'keyword_only'), ('d',
            ..., ..., 'keyword_only')), ...))
        self.assertEqual(self.signature(partial(test, a=1)), ((('a', 1, ...,
            'keyword_only'), ('b', ..., ..., 'keyword_only'), ('c', ...,
            ..., 'keyword_only'), ('d', ..., ..., 'keyword_only')), ...))

        def test(a, *args, b, **kwargs):
            pass
        self.assertEqual(self.signature(partial(test, 1)), ((('args', ...,
            ..., 'var_positional'), ('b', ..., ..., 'keyword_only'), (
            'kwargs', ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, a=1)), ((('a', 1, ...,
            'keyword_only'), ('b', ..., ..., 'keyword_only'), ('kwargs',
            ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, 1, 2, 3)), ((('args',
            ..., ..., 'var_positional'), ('b', ..., ..., 'keyword_only'), (
            'kwargs', ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, 1, 2, 3, test=True)),
            ((('args', ..., ..., 'var_positional'), ('b', ..., ...,
            'keyword_only'), ('kwargs', ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, 1, 2, 3, test=1, b=0)
            ), ((('args', ..., ..., 'var_positional'), ('b', 0, ...,
            'keyword_only'), ('kwargs', ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, b=0)), ((('a', ...,
            ..., 'positional_or_keyword'), ('args', ..., ...,
            'var_positional'), ('b', 0, ..., 'keyword_only'), ('kwargs',
            ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(test, b=0, test=1)), ((('a',
            ..., ..., 'positional_or_keyword'), ('args', ..., ...,
            'var_positional'), ('b', 0, ..., 'keyword_only'), ('kwargs',
            ..., ..., 'var_keyword')), ...))
        ...
        self.assertEqual(self.signature(partial(partial(test, 1))), ((('b',
            ..., ..., 'positional_or_keyword'), ('c', ..., int,
            'positional_or_keyword')), 42))
        ...
        self.assertEqual(self.signature(_foo), ((('a', 20, ...,
            'keyword_only'),), ...))
        ...
        self.assertEqual(self.signature(_foo), ((('b', 30, ...,
        ...
        self.assertEqual(self.signature(_foo), ((('a', ..., ...,
            'positional_or_keyword'), ('b', 10, ..., 'keyword_only'), ('c',
            20, ..., 'keyword_only'), ('d', 30, ..., 'keyword_only')), ...))
        ...
        self.assertEqual(self.signature(partial(foo, 1)), ((('b', ..., ...,
            'positional_only'), ('c', ..., ..., 'positional_or_keyword'), (
            'd', ..., ..., 'positional_or_keyword'), ('kwargs', ..., ...,
            'var_keyword')), ...))
        self.assertEqual(self.signature(partial(foo, 1, 2)), ((('c', ...,
            ..., 'positional_or_keyword'), ('d', ..., ...,
            'positional_or_keyword'), ('kwargs', ..., ..., 'var_keyword')),
            ...))
        self.assertEqual(self.signature(partial(foo, 1, 2, 3)), ((('d', ...,
            ..., 'positional_or_keyword'), ('kwargs', ..., ...,
            'var_keyword')), ...))
        self.assertEqual(self.signature(partial(foo, 1, 2, c=3)), ((('c', 3,
            ..., 'keyword_only'), ('d', ..., ..., 'keyword_only'), (
            'kwargs', ..., ..., 'var_keyword')), ...))
        self.assertEqual(self.signature(partial(foo, 1, c=3)), ((('b', ...,
            ..., 'positional_only'), ('c', 3, ..., 'keyword_only'), ('d',
            ..., ..., 'keyword_only'), ('kwargs', ..., ..., 'var_keyword')),
            ...))
    ...
    def test_signature_on_partialmethod(self):
        ...
        self.assertEqual(self.signature(Spam.ham), ((('it', ..., ...,
            'positional_or_keyword'), ('a', ..., ...,
            'positional_or_keyword'), ('c', 1, ..., 'keyword_only')), 'spam'))
        self.assertEqual(self.signature(Spam().ham), ((('a', ..., ...,
            'positional_or_keyword'), ('c', 1, ..., 'keyword_only')), 'spam'))
    ...
    def test_signature_on_decorated(self):
        ...
        class Foo:
            ...
            def bar(self, a, b):
                pass
        self.assertEqual(self.signature(Foo.bar), ((('self', ..., ...,
            'positional_or_keyword'), ('a', ..., ...,
            'positional_or_keyword'), ('b', ..., ...,
            'positional_or_keyword')), ...))
        self.assertEqual(self.signature(Foo().bar), ((('a', ..., ...,
            'positional_or_keyword'), ('b', ..., ...,
            'positional_or_keyword')), ...))
        self.assertEqual(self.signature(Foo.bar, follow_wrapped=False), (((
            'args', ..., ..., 'var_positional'), ('kwargs', ..., ...,
            'var_keyword')), ...))
        ...
        class Foo:
            ...
            def __call__(self, a, b):
                pass
        self.assertEqual(self.signature(Foo.__call__), ((('a', ..., ...,
            'positional_or_keyword'), ('b', ..., ...,
            'positional_or_keyword')), ...))
        self.assertEqual(self.signature(Foo().__call__), ((('b', ..., ...,
            'positional_or_keyword'),), ...))

        def wrapped_foo_call():
            pass
        wrapped_foo_call.__wrapped__ = Foo.__call__
        self.assertEqual(self.signature(wrapped_foo_call), ((('a', ..., ...,
            'positional_or_keyword'), ('b', ..., ...,
            'positional_or_keyword')), ...))
    ...
    def test_signature_on_class(self):
        ...
        class C:
            ...
            def __init__(self, a):
                pass
        self.assertEqual(self.signature(C), ((('a', ..., ...,
            'positional_or_keyword'),), ...))
        ...
        class C(metaclass=CM):
            ...
            def __init__(self, b):
                pass
        self.assertEqual(self.signature(C), ((('b', ..., ...,
            'positional_or_keyword'),), ...))
        self.assertEqual(self.signature(CM), ((('name', ..., ...,
            'positional_or_keyword'), ('bases', ..., ...,
            'positional_or_keyword'), ('dct', ..., ...,
            'positional_or_keyword'), ('foo', 1, ..., 'keyword_only')), ...))
        ...
        class C(metaclass=CM):
            ...
            def __init__(self, b):
                pass
        self.assertEqual(self.signature(CMM), ((('name', ..., ...,
            'positional_or_keyword'), ('bases', ..., ...,
            'positional_or_keyword'), ('dct', ..., ...,
            'positional_or_keyword'), ('foo', 1, ..., 'keyword_only')), ...))
        self.assertEqual(self.signature(CM), ((('nm', ..., ...,
            'positional_or_keyword'), ('bs', ..., ...,
            'positional_or_keyword'), ('dt', ..., ...,
            'positional_or_keyword')), ...))
        self.assertEqual(self.signature(C), ((('b', ..., ...,
            'positional_or_keyword'),), ...))
        ...
        self.assertEqual(self.signature(CM), ((('name', ..., ...,
            'positional_or_keyword'), ('bases', ..., ...,
            'positional_or_keyword'), ('dct', ..., ...,
            'positional_or_keyword'), ('bar', 2, ..., 'keyword_only')), ...))
    ...
    def test_signature_on_callable_objects(self):
        ...
        class Foo:
            ...
            def __call__(self, a):
                pass
        self.assertEqual(self.signature(Foo()), ((('a', ..., ...,
            'positional_or_keyword'),), ...))
        ...
        class Bar(Spam, Foo):
            pass
        self.assertEqual(self.signature(Bar()), ((('a', ..., ...,
            'positional_or_keyword'),), ...))
        ...
        self.assertEqual(self.signature(Wrapped), ((('a', ..., ...,
            'positional_or_keyword'),), ...))
    ...
    def test_signature_on_lambdas(self):
        self.assertEqual(self.signature(lambda a=10: a), ((('a', 10, ...,
            'positional_or_keyword'),), ...))
    ...
    def test_signature_equality(self):
        ...
        self.assertEqual(hash(inspect.signature(foo)), hash(inspect.
            signature(bar)))
        ...
        self.assertFalse(inspect.signature(foo) == inspect.signature(bar))
        self.assertTrue(inspect.signature(foo) != inspect.signature(bar))
        self.assertNotEqual(hash(inspect.signature(foo)), hash(inspect.
            signature(bar)))
        ...
        self.assertNotEqual(hash(inspect.signature(spam)), hash(inspect.
            signature(bar)))
    ...
    def test_signature_hashable(self):
        ...
        self.assertNotEqual(hash(foo_sig), hash(manual_sig.replace(
            return_annotation='spam')))
    ...
    def test_signature_str_positional_only(self):
        ...
        self.assertEqual(str(S(parameters=[P('foo', P.POSITIONAL_ONLY)])),
            '(foo, /)')
        self.assertEqual(str(S(parameters=[P('foo', P.POSITIONAL_ONLY), P(
            'bar', P.VAR_KEYWORD)])), '(foo, /, **bar)')
        self.assertEqual(str(S(parameters=[P('foo', P.POSITIONAL_ONLY), P(
            'bar', P.VAR_POSITIONAL)])), '(foo, /, *bar)')
    ...
    def test_signature_on_mangled_parameters(self):
        ...
        class Ham(Spam):
            pass
        self.assertEqual(self.signature(Spam.foo), ((('self', ..., ...,
            'positional_or_keyword'), ('_Spam__p1', 2, 1,
            'positional_or_keyword'), ('_Spam__p2', 3, 2, 'keyword_only')),
            ...))
...
class TestParameterObject(unittest.TestCase):
    ...
    def test_signature_parameter_kinds(self):
        P = inspect.Parameter
        self.assertTrue(P.POSITIONAL_ONLY < P.POSITIONAL_OR_KEYWORD < P.
            VAR_POSITIONAL < P.KEYWORD_ONLY < P.VAR_KEYWORD)
        self.assertEqual(str(P.POSITIONAL_ONLY), 'POSITIONAL_ONLY')
        self.assertTrue('POSITIONAL_ONLY' in repr(P.POSITIONAL_ONLY))

    def test_signature_parameter_object(self):
        p = inspect.Parameter('foo', default=10, kind=inspect.Parameter.
            POSITIONAL_ONLY)
        ...
        with self.assertRaisesRegex(ValueError, 'cannot have default values'):
            inspect.Parameter('a', default=42, kind=inspect.Parameter.
                VAR_KEYWORD)
        with self.assertRaisesRegex(ValueError, 'cannot have default values'):
            inspect.Parameter('a', default=42, kind=inspect.Parameter.
                VAR_POSITIONAL)
        p = inspect.Parameter('a', default=42, kind=inspect.Parameter.
            POSITIONAL_OR_KEYWORD)
        with self.assertRaisesRegex(ValueError, 'cannot have default values'):
    ...
    def test_signature_parameter_equality(self):
        ...
        self.assertTrue(p == P('foo', default=42, kind=inspect.Parameter.
            KEYWORD_ONLY))
        self.assertFalse(p != P('foo', default=42, kind=inspect.Parameter.
            KEYWORD_ONLY))

    def test_signature_parameter_replace(self):
        p = inspect.Parameter('foo', default=42, kind=inspect.Parameter.
            KEYWORD_ONLY)
    ...
    def test_signature_parameter_implicit(self):
        ...
        param = inspect.Parameter('.0', kind=inspect.Parameter.
            POSITIONAL_OR_KEYWORD)
...
class TestSignatureBind(unittest.TestCase):
    ...
    def test_signature_bind_var(self):
        ...
        self.assertEqual(self.call(test, 1, 2, foo='bar'), ((1, 2), {'foo':
            'bar'}))
    ...
    def test_signature_bind_just_kwargs(self):
        ...
        def test(**kwargs):
            return kwargs
        self.assertEqual(self.call(test), {})
        self.assertEqual(self.call(test, foo='bar', spam='ham'), {'foo':
            'bar', 'spam': 'ham'})
    ...
    def test_signature_bind_args_and_kwargs(self):
        ...
        def test(a, b, c=3, **kwargs):
            return a, b, c, kwargs
        self.assertEqual(self.call(test, 1, 2), (1, 2, 3, {}))
        self.assertEqual(self.call(test, 1, 2, foo='bar', spam='ham'), (1, 
            2, 3, {'foo': 'bar', 'spam': 'ham'}))
        ...
        self.assertEqual(self.call(test, a=1, b=2, foo='bar', spam='ham'),
            (1, 2, 3, {'foo': 'bar', 'spam': 'ham'}))
        self.assertEqual(self.call(test, 1, b=2, foo='bar', spam='ham'), (1,
            2, 3, {'foo': 'bar', 'spam': 'ham'}))
        self.assertEqual(self.call(test, 1, b=2, c=4, foo='bar', spam='ham'
            ), (1, 2, 4, {'foo': 'bar', 'spam': 'ham'}))
        self.assertEqual(self.call(test, 1, 2, 4, foo='bar'), (1, 2, 4, {
            'foo': 'bar'}))
    ...
    def test_signature_bind_kwonly(self):
        ...
        self.assertEqual(self.call(test, 1, bar=2, spam='ham'), (1, 2, {
            'spam': 'ham'}))
        self.assertEqual(self.call(test, spam='ham', foo=1, bar=2), (1, 2,
            {'spam': 'ham'}))
        with self.assertRaisesRegex(TypeError,
            "missing a required argument: 'foo'"):
            self.call(test, spam='ham', bar=2)
        self.assertEqual(self.call(test, 1, bar=2, bin=1, spam=10), (1, 2,
            {'bin': 1, 'spam': 10}))
    ...
    def test_signature_bind_arguments(self):
        ...
        self.assertEqual(tuple(ba.arguments.items()), (('a', 10), ('args',
            (20,)), ('b', 30), ('kwargs', {'c': 40, 'args': 50, 'kwargs': 60}))
            )
        self.assertEqual(ba.kwargs, {'b': 30, 'c': 40, 'args': 50, 'kwargs':
            60})
    ...
    def test_signature_bind_positional_only(self):
        ...
        self.assertEqual(self.call(test, 1, 2, foo=4, bar=5), (1, 2, 3, 4, 
            5, {}))
...
class TestBoundArguments(unittest.TestCase):
    ...
    def test_signature_bound_arguments_apply_defaults(self):
        ...
        ba = sig.bind(20)
        ba.apply_defaults()
        self.assertEqual(list(ba.arguments.items()), [('a', 20), ('b', 1),
            ('args', ()), ('c', {}), ('kw', {})])
        ba = sig.bind(10, 20, 30, d=1)
        ba.apply_defaults()
        self.assertEqual(list(ba.arguments.items()), [('a', 10), ('b', 20),
            ('args', (30,)), ('c', {}), ('kw', {'d': 1})])
...
class TestSignaturePrivateHelpers(unittest.TestCase):
    ...
    def _strip_non_python_syntax(self, input, clean_signature,
        self_parameter, last_positional_only):
    ...
    def test_signature_strip_non_python_syntax(self):
       ...
       follow_symlinks=True)""", 
            '(module, path, mode, *, dir_fd=None, ' +
            'effective_ids=False, follow_symlinks=True)', 0, 0)
        self._strip_non_python_syntax('($module, word, salt, /)',
            '(module, word, salt)', 0, 2)
        self._strip_non_python_syntax('(x, y=None, z=None, /)',
            '(x, y=None, z=None)', None, 2)
        self._strip_non_python_syntax('(x, y=None, z=None)',
            '(x, y=None, z=None)', None, None)
...
class TestSignatureDefinitions(unittest.TestCase):
    ...
    def test_builtins_have_signatures(self):
        no_signature = set()
        needs_groups = {'range', 'slice', 'dir', 'getattr', 'next', 'iter',
            'vars'}
...
class TestUnwrap(unittest.TestCase):
    ...
    def test_stop(self):
        ...
        def wrapper():
            pass
        func2.stop_here = 1
        unwrapped = inspect.unwrap(wrapper, stop=lambda f: hasattr(f,
            'stop_here'))
...
class TestMain(unittest.TestCase):
    ...
    def test_qualname_source(self):
        ...
        lines = out.decode().splitlines()
        self.assertEqual(lines[:-1], inspect.getsource(ThreadPoolExecutor).
            splitlines())
        self.assertEqual(err, b'')
    ...
    def test_details(self):
        ...
        rc, out, err = assert_python_ok(*args, '-m', 'inspect', 'unittest',
            '--details')
        output = out.decode()
...
class TestReload(unittest.TestCase):
    ...
    def assertInspectEqual(self, path, source):
        ...
        with open(path) as src:
            self.assertEqual(src.read().splitlines(True), inspected_src.
                splitlines(True))
...
def test_main():
    run_unittest(TestDecorators, TestRetrievingSourceCode, TestOneliners,
        TestBuggyCases, TestInterpreterStack, TestClassesAndFunctions,
        TestPredicates, TestGetcallargsFunctions, TestGetcallargsMethods,
        TestGetcallargsUnboundMethods, TestGetattrStatic,
        TestGetGeneratorState, TestNoEOL, TestSignatureObject,
        TestSignatureBind, TestParameterObject, TestBoundArguments,
