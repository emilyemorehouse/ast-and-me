...
class CCookedTest(BaseCookedTest):
    cooked_values = {'currency_symbol': '', 'decimal_point': '.',
        'frac_digits': 127, 'grouping': [], 'int_curr_symbol': '',
        'int_frac_digits': 127, 'mon_decimal_point': '', 'mon_grouping': [],
        'mon_thousands_sep': '', 'n_cs_precedes': 127, 'n_sep_by_space': 
        127, 'n_sign_posn': 127, 'negative_sign': '', 'p_cs_precedes': 127,
        'p_sep_by_space': 127, 'p_sign_posn': 127, 'positive_sign': '',
        'thousands_sep': ''}


class EnUSCookedTest(BaseCookedTest):
    cooked_values = {'currency_symbol': '$', 'decimal_point': '.',
        'frac_digits': 2, 'grouping': [3, 3, 0], 'int_curr_symbol': 'USD ',
        'int_frac_digits': 2, 'mon_decimal_point': '.', 'mon_grouping': [3,
        3, 0], 'mon_thousands_sep': ',', 'n_cs_precedes': 1,
        'n_sep_by_space': 0, 'n_sign_posn': 1, 'negative_sign': '-',
        'p_cs_precedes': 1, 'p_sep_by_space': 0, 'p_sign_posn': 1,
        'positive_sign': '', 'thousands_sep': ','}


class FrFRCookedTest(BaseCookedTest):
    cooked_values = {'currency_symbol': '€', 'decimal_point': ',',
        'frac_digits': 2, 'grouping': [3, 3, 0], 'int_curr_symbol': 'EUR ',
        'int_frac_digits': 2, 'mon_decimal_point': ',', 'mon_grouping': [3,
        3, 0], 'mon_thousands_sep': ' ', 'n_cs_precedes': 0,
        'n_sep_by_space': 1, 'n_sign_posn': 1, 'negative_sign': '-',
        'p_cs_precedes': 0, 'p_sep_by_space': 1, 'p_sign_posn': 1,
        'positive_sign': '', 'thousands_sep': ' '}
...
class BaseFormattingTest(object):
    ...
    def _test_format(self, format, value, out, **format_opts):
        self._test_formatfunc(format, value, out, func=locale.format, **
            format_opts)

    def _test_format_string(self, format, value, out, **format_opts):
        self._test_formatfunc(format, value, out, func=locale.format_string,
            **format_opts)
...
class EnUSNumberFormatting(BaseFormattingTest):
    ...
    def test_grouping(self):
        self._test_format('%f', 1024, grouping=1, out='1%s024.000000' %
            self.sep)
    ...
    def test_grouping_and_padding(self):
        ...
        if self.sep:
            self._test_format('%+10.f', -4200, grouping=1, out=('-4%s200' %
                self.sep).rjust(10))
            self._test_format('%-10.f', -4200, grouping=1, out=('-4%s200' %
                self.sep).ljust(10))
    ...
    def test_integer_grouping_and_padding(self):
        self._test_format('%10d', 4200, grouping=True, out=('4%s200' % self
            .sep).rjust(10))
        self._test_format('%-10d', -4200, grouping=True, out=('-4%s200' %
            self.sep).ljust(10))
    ...
    def test_complex_formatting(self):
        ...
        if self.sep:
            self._test_format_string('-->  %10.2f', 4200, grouping=1, out=
                '-->  ' + ('4%s200.00' % self.sep).rjust(10))
        self._test_format_string('%10.*f', (2, 1000), grouping=0, out=
            '1000.00'.rjust(10))
        if self.sep:
            self._test_format_string('%*.*f', (10, 2, 1000), grouping=1,
                out=('1%s000.00' % self.sep).rjust(10))
        if self.sep:
            self._test_format_string('int %i float %.2f str %s', (1000, 
                1000.0, 'str'), grouping=1, out=
                'int 1%s000 float 1%s000.00 str str' % (self.sep, self.sep))
...
class TestLocaleFormatString(unittest.TestCase):
    ...
    def test_mapping(self):
        self.assertEqual(locale.format_string('%(foo)s bing.', {'foo':
            'bar'}), '%(foo)s bing.' % {'foo': 'bar'})
...
class TestEnUSNumberFormatting(EnUSCookedTest, EnUSNumberFormatting):
    ...
    def test_currency(self):
        ...
        self._test_currency(50000, 'USD 50,000.00', grouping=True,
            international=True)
...
class TestFrFRNumberFormatting(FrFRCookedTest, BaseFormattingTest):
    ...
    def test_currency(self):
        ...
        self._test_currency(50000, '50 000,00 EUR ', grouping=True,
            international=True)
