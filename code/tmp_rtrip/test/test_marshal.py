...
class ContainerTestCase(unittest.TestCase, HelperMixin):
    d = {'astring': 'foo@bar.baz.spam', 'afloat': 7283.43, 'anint': 2 ** 20,
        'ashortlong': 2, 'alist': ['.zyx.41'], 'atuple': ('.zyx.41',) * 10,
        'aboolean': False, 'aunicode': 'Andrè Previn'}
...
class BugsTestCase(unittest.TestCase):
    ...
    def test_bad_reader(self):
        ...
        for value in (1.0, 1j, b'0123456789', '0123456789'):
            self.assertRaises(ValueError, marshal.load, BadReader(marshal.
                dumps(value)))
...
class LargeValuesTestCase(unittest.TestCase):
    ...
    @support.bigmemtest(size=LARGE_SIZE, memuse=pointer_size * 12 + sys.
        getsizeof(LARGE_SIZE - 1), dry_run=False)
...
class CAPI_TestCase(unittest.TestCase, HelperMixin):
    ...
    def test_write_long_to_file(self):
        for v in range(marshal.version + 1):
            _testcapi.pymarshal_write_long_to_file(305419896, support.TESTFN, v
                )
            with open(support.TESTFN, 'rb') as f:
                data = f.read()
            support.unlink(support.TESTFN)
    ...
    def test_read_last_object_from_file(self):
        ...
        for v in range(marshal.version + 1):
            ...
            with open(support.TESTFN, 'wb') as f:
                f.write(data + b'xxxx')
            r, p = _testcapi.pymarshal_read_last_object_from_file(support.
                TESTFN)
            support.unlink(support.TESTFN)
