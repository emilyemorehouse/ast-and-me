...
class MinidomTest(unittest.TestCase):
    ...
    def testGetElementsByTagName(self):
        dom = parse(tstfile)
        self.confirm(dom.getElementsByTagName('LI') == dom.documentElement.
    ...
    def testInsertBefore(self):
        ...
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2 and root.childNodes[0] is nelem and root.childNodes.item(0) is
            nelem and root.childNodes[1] is elem and root.childNodes.item(1
            ) is elem and root.firstChild is nelem and root.lastChild is
            elem and root.toxml() == '<doc><element/><foo/></doc>',
            'testInsertBefore -- node properly placed in tree')
        nelem = dom.createElement('element')
        root.insertBefore(nelem, None)
        self.confirm(len(root.childNodes) == 3 and root.childNodes.length ==
            3 and root.childNodes[1] is elem and root.childNodes.item(1) is
            elem and root.childNodes[2] is nelem and root.childNodes.item(2
            ) is nelem and root.lastChild is nelem and nelem.
            previousSibling is elem and root.toxml() ==
            '<doc><element/><foo/><element/></doc>',
            'testInsertBefore -- node properly placed in tree')
        nelem2 = dom.createElement('bar')
        root.insertBefore(nelem2, nelem)
        self.confirm(len(root.childNodes) == 4 and root.childNodes.length ==
            4 and root.childNodes[2] is nelem2 and root.childNodes.item(2) is
            nelem2 and root.childNodes[3] is nelem and root.childNodes.item
            (3) is nelem and nelem2.nextSibling is nelem and nelem.
            previousSibling is nelem2 and root.toxml() ==
    ...
    def testInsertBeforeFragment(self):
        ...
        self.confirm(tuple(dom.documentElement.childNodes) == (orig, c1, c2,
    ...
    def testLegalChildren(self):
        ...
        self.assertRaises(xml.dom.HierarchyRequestErr, dom.insertBefore,
            text, elem)
        self.assertRaises(xml.dom.HierarchyRequestErr, dom.replaceChild,
            text, elem)
        nodemap = elem.attributes
        self.assertRaises(xml.dom.HierarchyRequestErr, nodemap.setNamedItem,
            text)
        self.assertRaises(xml.dom.HierarchyRequestErr, nodemap.
            setNamedItemNS, text)
    ...
    def testRemoveAttrNS(self):
        dom = Document()
        child = dom.appendChild(dom.createElementNS('http://www.python.org',
            'python:abc'))
    ...
    def testRemoveAttributeNode(self):
        ...
        self.confirm(len(child.attributes) == 0 and child.getAttributeNode(
            'spam') is None)
        dom2 = Document()
    ...
    def testChangeAttr(self):
        ...
        self.confirm(len(el.attributes) == 1 and el.attributes['spam'].
            value == 'bam' and el.attributes['spam'].nodeValue == 'bam' and
            el.getAttribute('spam') == 'bam' and el.getAttributeNode('spam'
            ).isId)
        el.attributes['spam'] = 'ham'
        self.confirm(len(el.attributes) == 1 and el.attributes['spam'].
            value == 'ham' and el.attributes['spam'].nodeValue == 'ham' and
            el.getAttribute('spam') == 'ham' and el.attributes['spam'].isId)
        el.setAttribute('spam2', 'bam')
        self.confirm(len(el.attributes) == 2 and el.attributes['spam'].
            value == 'ham' and el.attributes['spam'].nodeValue == 'ham' and
            el.getAttribute('spam') == 'ham' and el.attributes['spam2'].
            value == 'bam' and el.attributes['spam2'].nodeValue == 'bam' and
            el.getAttribute('spam2') == 'bam')
        el.attributes['spam2'] = 'bam2'
        self.confirm(len(el.attributes) == 2 and el.attributes['spam'].
            value == 'ham' and el.attributes['spam'].nodeValue == 'ham' and
            el.getAttribute('spam') == 'ham' and el.attributes['spam2'].
            value == 'bam2' and el.attributes['spam2'].nodeValue == 'bam2' and
            el.getAttribute('spam2') == 'bam2')
        dom.unlink()
    ...
    def testGetElementsByTagNameNS(self):
        ...
        elems = dom.getElementsByTagNameNS('http://pyxml.sf.net/minidom',
            'myelem')
        self.confirm(len(elems) == 1 and elems[0].namespaceURI ==
            'http://pyxml.sf.net/minidom' and elems[0].localName ==
            'myelem' and elems[0].prefix == 'minidom' and elems[0].tagName ==
            'minidom:myelem' and elems[0].nodeName == 'minidom:myelem')
        dom.unlink()

    def get_empty_nodelist_from_elements_by_tagName_ns_helper(self, doc,
        nsuri, lname):
    ...
    def testGetEmptyNodeListFromElementsByTagNameNS(self):
        doc = parseString('<doc/>')
        self.get_empty_nodelist_from_elements_by_tagName_ns_helper(doc,
    ...
    def testElementReprAndStrUnicodeNS(self):
        dom = Document()
        el = dom.appendChild(dom.createElementNS('http://www.slashdot.org',
            'slash:abc'))
        string1 = repr(el)
        string2 = str(el)
        self.confirm(string1 == string2)
    ...
    def test_toprettyxml_with_text_nodes(self):
        ...
        self.assertEqual(parseString('<B>A</B>').toprettyxml(), decl +
            '<B>A</B>\n')
...
""")
        self.assertEqual(parseString('<C><B>A</B>A<B>A</B></C>').
            toprettyxml(), decl + """<C>
	<B>A</B>
	A
	<B>A</B>
...
""")
    ...
    def test_toprettyxml_preserves_content_of_text_node(self):
        for str in ('<B>A</B>', '<A><B>C</B></A>'):
            ...
            self.assertEqual(dom.getElementsByTagName('B')[0].childNodes[0]
                .toxml(), dom2.getElementsByTagName('B')[0].childNodes[0].
                toxml())
    ...
    def testProcessingInstruction(self):
        ...
        self.confirm(pi.target == 'mypi' and pi.data == 'data \t\n ' and pi
            .nodeName == 'mypi' and pi.nodeType == Node.
            PROCESSING_INSTRUCTION_NODE and pi.attributes is None and not
            pi.hasChildNodes() and len(pi.childNodes) == 0 and pi.
            firstChild is None and pi.lastChild is None and pi.localName is
            None and pi.namespaceURI == xml.dom.EMPTY_NAMESPACE)
    ...
    def _testCloneElementCopiesAttributes(self, e1, e2, test):
        ...
        for i in range(len(keys1)):
            a1 = attrs1.item(i)
            a2 = attrs2.item(i)
            self.confirm(a1 is not a2 and a1.value == a2.value and a1.
                nodeValue == a2.nodeValue and a1.namespaceURI == a2.
                namespaceURI and a1.localName == a2.localName,
    ...
    def _setupCloneElement(self, deep):
        ...
        self._testCloneElementCopiesAttributes(root, clone, 
            'testCloneElement' + (deep and 'Deep' or 'Shallow'))
    ...
    def testCloneElementShallow(self):
        ...
        self.confirm(len(clone.childNodes) == 0 and clone.childNodes.length ==
            0 and clone.parentNode is None and clone.toxml() ==
            '<doc attr="value"/>', 'testCloneElementShallow')
        dom.unlink()

    def testCloneElementDeep(self):
        dom, clone = self._setupCloneElement(1)
        self.confirm(len(clone.childNodes) == 1 and clone.childNodes.length ==
            1 and clone.parentNode is None and clone.toxml() ==
            '<doc attr="value"><foo/></doc>', 'testCloneElementDeep')
        dom.unlink()
...
<doc attr='value'/>"""
        ...
        self.confirm(not doc.documentElement.isSameNode(doc2.
            documentElement),
    ...
    def testCloneDocumentTypeDeepOk(self):
        ...
        self.confirm(clone is not None and clone.nodeName == doctype.
            nodeName and clone.name == doctype.name and clone.publicId ==
            doctype.publicId and clone.systemId == doctype.systemId and len
            (clone.entities) == len(doctype.entities) and clone.entities.
            item(len(clone.entities)) is None and len(clone.notations) ==
            len(doctype.notations) and clone.notations.item(len(clone.
            notations)) is None and len(clone.childNodes) == 0)
        for i in range(len(doctype.entities)):
            se = doctype.entities.item(i)
            ce = clone.entities.item(i)
            self.confirm(not se.isSameNode(ce) and not ce.isSameNode(se) and
                ce.nodeName == se.nodeName and ce.notationName == se.
                notationName and ce.publicId == se.publicId and ce.systemId ==
                se.systemId and ce.encoding == se.encoding and ce.
                actualEncoding == se.actualEncoding and ce.version == se.
                version)
        for i in range(len(doctype.notations)):
            sn = doctype.notations.item(i)
            cn = clone.notations.item(i)
            self.confirm(not sn.isSameNode(cn) and not cn.isSameNode(sn) and
                cn.nodeName == sn.nodeName and cn.publicId == sn.publicId and
                cn.systemId == sn.systemId)
    ...
    def testCloneDocumentTypeShallowOk(self):
        ...
        self.confirm(clone is not None and clone.nodeName == doctype.
            nodeName and clone.name == doctype.name and clone.publicId ==
            doctype.publicId and clone.systemId == doctype.systemId and len
            (clone.entities) == 0 and clone.entities.item(0) is None and 
            len(clone.notations) == 0 and clone.notations.item(0) is None and
    ...
    def testImportDocumentTypeShallow(self):
        ...
        self.assertRaises(xml.dom.NotSupportedErr, target.importNode, src.
            doctype, 0)
    ...
    def check_clone_attribute(self, deep, testName):
        ...
        self.confirm(clone.ownerElement is None, testName +
        ...
        self.confirm(clone.specified, testName +
    ...
    def testNormalize(self):
        ...
        doc.normalize()
        self.confirm(len(root.childNodes) == 1 and root.childNodes.length ==
            1 and root.firstChild is root.lastChild and root.firstChild.
            data == 'firstsecond', 'testNormalize -- result')
        doc.unlink()
        doc = parseString('<doc/>')
        root = doc.documentElement
        root.appendChild(doc.createTextNode(''))
        doc.normalize()
        self.confirm(len(root.childNodes) == 0 and root.childNodes.length ==
            0, 'testNormalize -- single empty node removed')
        doc.unlink()
    ...
    def testNormalizeCombineAndNextSibling(self):
        ...
        self.confirm(len(root.childNodes) == 3 and root.childNodes.length ==
            3, 'testNormalizeCombineAndNextSibling -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2 and root.firstChild.data == 'firstsecond' and root.firstChild
             is not root.lastChild and root.firstChild.nextSibling is root.
            lastChild and root.firstChild.previousSibling is None and root.
            lastChild.previousSibling is root.firstChild and root.lastChild
            .nextSibling is None,
    ...
    def testNormalizeDeleteWithPrevSibling(self):
        ...
        root.appendChild(doc.createTextNode(''))
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2, 'testNormalizeDeleteWithPrevSibling -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 1 and root.childNodes.length ==
            1 and root.firstChild.data == 'first' and root.firstChild is
            root.lastChild and root.firstChild.nextSibling is None and root
            .firstChild.previousSibling is None,
    ...
    def testNormalizeDeleteWithNextSibling(self):
        ...
        root.appendChild(doc.createTextNode('second'))
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2, 'testNormalizeDeleteWithNextSibling -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 1 and root.childNodes.length ==
            1 and root.firstChild.data == 'second' and root.firstChild is
            root.lastChild and root.firstChild.nextSibling is None and root
            .firstChild.previousSibling is None,
    ...
    def testNormalizeDeleteWithTwoNonTextSiblings(self):
        ...
        root.appendChild(doc.createElement('i'))
        self.confirm(len(root.childNodes) == 3 and root.childNodes.length ==
            3, 'testNormalizeDeleteWithTwoSiblings -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2 and root.firstChild is not root.lastChild and root.firstChild
            .nextSibling is root.lastChild and root.firstChild.
            previousSibling is None and root.lastChild.previousSibling is
            root.firstChild and root.lastChild.nextSibling is None,
    ...
    def testNormalizeDeleteAndCombine(self):
        ...
        root.appendChild(doc.createTextNode(''))
        self.confirm(len(root.childNodes) == 5 and root.childNodes.length ==
            5, 'testNormalizeDeleteAndCombine -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 1 and root.childNodes.length ==
            1 and root.firstChild is root.lastChild and root.firstChild.
            data == 'secondfourth' and root.firstChild.previousSibling is
            None and root.firstChild.nextSibling is None,
    ...
    def testNormalizeRecursion(self):
        ...
        root.appendChild(doc.createTextNode(''))
        self.confirm(len(root.childNodes) == 3 and root.childNodes.length ==
            3 and len(root.childNodes[0].childNodes) == 4 and root.
            childNodes[0].childNodes.length == 4 and len(root.childNodes[1]
            .childNodes) == 3 and root.childNodes[1].childNodes.length == 3 and
            len(root.childNodes[1].childNodes[0].childNodes) == 2 and root.
            childNodes[1].childNodes[0].childNodes.length == 2,
            'testNormalize2 -- preparation')
        doc.normalize()
        self.confirm(len(root.childNodes) == 2 and root.childNodes.length ==
            2 and len(root.childNodes[0].childNodes) == 2 and root.
            childNodes[0].childNodes.length == 2 and len(root.childNodes[1]
            .childNodes) == 2 and root.childNodes[1].childNodes.length == 2 and
            len(root.childNodes[1].childNodes[0].childNodes) == 1 and root.
            childNodes[1].childNodes[0].childNodes.length == 1,
            'testNormalize2 -- childNodes lengths')
        self.confirm(root.childNodes[0].childNodes[1].data == 'tx' and root
            .childNodes[1].childNodes[0].childNodes[0].data == 't2x2' and 
            root.childNodes[1].childNodes[1].data == 't3x3',
            'testNormalize2 -- joined text fields')
        self.confirm(root.childNodes[0].childNodes[1].nextSibling is None and
            root.childNodes[0].childNodes[1].previousSibling is root.
            childNodes[0].childNodes[0] and root.childNodes[0].childNodes[0
            ].previousSibling is None and root.childNodes[0].childNodes[0].
            nextSibling is root.childNodes[0].childNodes[1] and root.
            childNodes[1].childNodes[1].nextSibling is None and root.
            childNodes[1].childNodes[1].previousSibling is root.childNodes[
            1].childNodes[0] and root.childNodes[1].childNodes[0].
            previousSibling is None and root.childNodes[1].childNodes[0].
            nextSibling is root.childNodes[1].childNodes[1],
    ...
    def testSiblings(self):
        ...
        self.confirm(pi.nextSibling is text and pi.previousSibling is None and
            text.nextSibling is elm and text.previousSibling is pi and elm.
            nextSibling is None and elm.previousSibling is text, 'testSiblings'
            )
        doc.unlink()
    ...
    def testParents(self):
        ...
        self.confirm(root.parentNode is doc and elm1.parentNode is root and
            elm2a.parentNode is elm1 and elm2b.parentNode is elm1 and elm3.
            parentNode is elm2b, 'testParents')
        doc.unlink()
    ...
    def testNodeListItem(self):
        ...
        docelem = children[0]
        self.confirm(children[0] is children.item(0) and children.item(1) is
            None and docelem.childNodes.item(0) is docelem.childNodes[0] and
            docelem.childNodes.item(1) is docelem.childNodes[1] and docelem
            .childNodes.item(0).childNodes.item(0) is None,
            'test NodeList.item()')
        doc.unlink()
    ...
    def testEncodings(self):
        ...
        self.assertEqual(doc.toxml('utf-16'),
            '<?xml version="1.0" encoding="utf-16"?><foo>€</foo>'.encode(
            'utf-16'))
    ...
    def testUserData(self):
        ...
        c = n.cloneNode(1)
        self.confirm(handler.called and n.getUserData('bar') is None and c.
            getUserData('bar') == 13)
        n.unlink()
        c.unlink()
        dom.unlink()
    ...
    def testRenameAttribute(self):
        ...
        self.confirm(attr.name == 'b' and attr.nodeName == 'b' and attr.
            localName is None and attr.namespaceURI == xml.dom.
            EMPTY_NAMESPACE and attr.prefix is None and attr.value == 'v' and
            elem.getAttributeNode('a') is None and elem.getAttributeNode(
            'b').isSameNode(attr) and attrmap['b'].isSameNode(attr) and
            attr.ownerDocument.isSameNode(doc) and attr.ownerElement.
            isSameNode(elem))
        attr = doc.renameNode(attr, 'http://xml.python.org/ns', 'c')
        self.confirm(attr.name == 'c' and attr.nodeName == 'c' and attr.
            localName == 'c' and attr.namespaceURI ==
            'http://xml.python.org/ns' and attr.prefix is None and attr.
            value == 'v' and elem.getAttributeNode('a') is None and elem.
            getAttributeNode('b') is None and elem.getAttributeNode('c').
            isSameNode(attr) and elem.getAttributeNodeNS(
            'http://xml.python.org/ns', 'c').isSameNode(attr) and attrmap[
            'c'].isSameNode(attr) and attrmap['http://xml.python.org/ns',
            'c'].isSameNode(attr))
        attr = doc.renameNode(attr, 'http://xml.python.org/ns2', 'p:d')
        self.confirm(attr.name == 'p:d' and attr.nodeName == 'p:d' and attr
            .localName == 'd' and attr.namespaceURI ==
            'http://xml.python.org/ns2' and attr.prefix == 'p' and attr.
            value == 'v' and elem.getAttributeNode('a') is None and elem.
            getAttributeNode('b') is None and elem.getAttributeNode('c') is
            None and elem.getAttributeNodeNS('http://xml.python.org/ns',
            'c') is None and elem.getAttributeNode('p:d').isSameNode(attr) and
            elem.getAttributeNodeNS('http://xml.python.org/ns2', 'd').
            isSameNode(attr) and attrmap['p:d'].isSameNode(attr) and
            attrmap['http://xml.python.org/ns2', 'd'].isSameNode(attr))
        attr = doc.renameNode(attr, xml.dom.EMPTY_NAMESPACE, 'e')
        self.confirm(attr.name == 'e' and attr.nodeName == 'e' and attr.
            localName is None and attr.namespaceURI == xml.dom.
            EMPTY_NAMESPACE and attr.prefix is None and attr.value == 'v' and
            elem.getAttributeNode('a') is None and elem.getAttributeNode(
            'b') is None and elem.getAttributeNode('c') is None and elem.
            getAttributeNode('p:d') is None and elem.getAttributeNodeNS(
            'http://xml.python.org/ns', 'c') is None and elem.
            getAttributeNode('e').isSameNode(attr) and attrmap['e'].
            isSameNode(attr))
    ...
    def testRenameElement(self):
        ...
        self.confirm(elem.tagName == 'a' and elem.nodeName == 'a' and elem.
            localName is None and elem.namespaceURI == xml.dom.
            EMPTY_NAMESPACE and elem.prefix is None and elem.ownerDocument.
            isSameNode(doc))
        elem = doc.renameNode(elem, 'http://xml.python.org/ns', 'b')
        self.confirm(elem.tagName == 'b' and elem.nodeName == 'b' and elem.
            localName == 'b' and elem.namespaceURI ==
            'http://xml.python.org/ns' and elem.prefix is None and elem.
            ownerDocument.isSameNode(doc))
        elem = doc.renameNode(elem, 'http://xml.python.org/ns2', 'p:c')
        self.confirm(elem.tagName == 'p:c' and elem.nodeName == 'p:c' and 
            elem.localName == 'c' and elem.namespaceURI ==
            'http://xml.python.org/ns2' and elem.prefix == 'p' and elem.
            ownerDocument.isSameNode(doc))
        elem = doc.renameNode(elem, xml.dom.EMPTY_NAMESPACE, 'd')
        self.confirm(elem.tagName == 'd' and elem.nodeName == 'd' and elem.
            localName is None and elem.namespaceURI == xml.dom.
            EMPTY_NAMESPACE and elem.prefix is None and elem.ownerDocument.
            isSameNode(doc))
    ...
    def testRenameOther(self):
        doc = xml.dom.minidom.getDOMImplementation().createDocument(xml.dom
            .EMPTY_NAMESPACE, 'e', None)
  ...
  <!ATTLIST doc id   ID       #IMPLIED 
    ...
    >
        ...
        for name in names:
            ...
            t = a.schemaType
            self.confirm(hasattr(t, 'name') and t.namespace == xml.dom.
                EMPTY_NAMESPACE)
    ...
    def testSetIdAttribute(self):
        ...
        self.confirm(doc.getElementById('v') is None and not a1.isId and 
            not a2.isId)
        e.setIdAttribute('a1')
        self.confirm(e.isSameNode(doc.getElementById('v')) and a1.isId and 
            not a2.isId)
        e.setIdAttribute('a2')
        self.confirm(e.isSameNode(doc.getElementById('v')) and e.isSameNode
            (doc.getElementById('w')) and a1.isId and a2.isId)
        a3 = doc.createAttribute('a1')
        a3.value = 'v'
        e.setAttributeNode(a3)
        self.confirm(doc.getElementById('v') is None and e.isSameNode(doc.
            getElementById('w')) and not a1.isId and a2.isId and not a3.isId)
    ...
    def testSetIdAttributeNS(self):
        ...
        doc = parseString("<doc xmlns:ns1='" + NS1 + "' xmlns:ns2='" + NS2 +
            "' ns1:a1='v' ns2:a2='w'/>")
        e = doc.documentElement
