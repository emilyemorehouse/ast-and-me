...
def tester(fn, wantResult):
    ...
    if wantResult != gotResult:
        raise TestFailed('%s should return: %s but returned: %s' % (str(fn),
            str(wantResult), str(gotResult)))
    fn = fn.replace("('", "(b'")
    fn = fn.replace('("', '(b"')
    fn = fn.replace("['", "[b'")
    fn = fn.replace('["', '[b"')
...
class TestNtpath(unittest.TestCase):
    ...
    def test_join(self):
        ...
        tester("ntpath.join('//computer/share/', 'x/y')",
            '//computer/share/x/y')
        tester("ntpath.join('//computer/share/a/b', 'x/y')",
        ...
        tester("ntpath.join('//computer/share/', '/x/y')",
            '//computer/share/x/y')
        tester("ntpath.join('//computer/share/a', '/x/y')",
            '//computer/share/x/y')
        ...
        for x in ('', 'a/b', '/a/b', 'c:', 'c:a/b', 'c:/', 'c:/a/b',
            ...
            for y in ('d:', 'd:x/y', 'd:/', 'd:/x/y', '//machine/common',
                '//machine/common/', '//machine/common/x/y'):
                tester('ntpath.join(%r, %r)' % (x, y), y)
        tester("ntpath.join('\\\\computer\\share\\', 'a', 'b')",
            '\\\\computer\\share\\a\\b')
        tester("ntpath.join('\\\\computer\\share', 'a', 'b')",
            '\\\\computer\\share\\a\\b')
        tester("ntpath.join('\\\\computer\\share', 'a\\b')",
    ...
    def test_relpath(self):
        ...
        with support.temp_cwd(support.TESTFN) as cwd_dir:
            ...
            tester('ntpath.relpath("a/b", "../c")', '..\\' + currentdir +
                '\\a\\b')
    ...
    def test_commonpath(self):
        ...
        def check_error(exc, paths):
            ...
            self.assertRaises(exc, ntpath.commonpath, [os.fsencode(p) for p in
                paths])
        ...
        self.assertRaises(TypeError, ntpath.commonpath, [b'Program Files',
            'C:\\Program Files\\Foo'])
        self.assertRaises(TypeError, ntpath.commonpath, [
            'C:\\Program Files', b'C:\\Program Files\\Foo'])
        self.assertRaises(TypeError, ntpath.commonpath, [
            'C:\\Program Files', b'Program Files\\Foo'])
        self.assertRaises(TypeError, ntpath.commonpath, ['Program Files',
            b'C:\\Program Files\\Foo'])
