...
Args were %(args)s."""
    ...
    def assertRaises(self, func, args, kwargs, expected_exception,
        expected_message):
        """
...
"""
    ...
    def assertOutput(self, cmdline_args, expected_output, expected_status=0,
        expected_error=None):
        ...
        try:
            ...
            self.assertTrue(isinstance(output, str), 
                'expected output to be an ordinary string, not %r' % type(
                output))
...
class TestOptionChecks(BaseTest):
    ...
    def assertOptionError(self, expected_message, args=[], kwargs={}):
        self.assertRaises(make_option, args, kwargs, OptionError,
            expected_message)
    ...
    def test_attr_invalid(self):
        self.assertOptionError('option -b: invalid keyword arguments: bar, foo'
            , ['-b'], {'foo': None, 'bar': None})

    def test_action_invalid(self):
        self.assertOptionError("option -b: invalid action: 'foo'", ['-b'],
            {'action': 'foo'})

    def test_type_invalid(self):
        self.assertOptionError("option -b: invalid option type: 'foo'", [
            '-b'], {'type': 'foo'})
        self.assertOptionError("option -b: invalid option type: 'tuple'", [
            '-b'], {'type': tuple})

    def test_no_type_for_action(self):
        self.assertOptionError(
            "option -b: must not supply a type for action 'count'", ['-b'],
    ...
    def test_bad_choices_list(self):
        ...
        self.assertOptionError(
            "option -b/--bad: choices must be a list of strings ('%s' supplied)"
             % typename, ['-b', '--bad'], {'type': 'choice', 'choices':
            'bad choices'})

    def test_no_choices_for_type(self):
        self.assertOptionError(
            "option -b: must not supply choices for type 'int'", ['-b'], {
    ...
    def test_callback_not_callable(self):
        self.assertOptionError("option -b: callback not callable: 'foo'", [
    ...
    def test_callback_args_no_tuple(self):
        self.assertOptionError(
            ...
            , ['-b'], {'action': 'callback', 'callback': self.dummy,
            'callback_args': 'foo'})
...
class TestOptionParser(BaseTest):
    ...
    def setUp(self):
        ...
        self.parser.add_option('-v', '--verbose', '-n', '--noisy', action=
            'store_true', dest='verbose')
        self.parser.add_option('-q', '--quiet', '--silent', action=
    ...
    def test_add_option_invalid_arguments(self):
        self.assertTypeError(self.parser.add_option, 'invalid arguments',
            None, None)
...
class TestDefaultValues(BaseTest):
    ...
    def setUp(self):
        ...
        self.expected = {'verbose': True, 'n': 37, 'm': None, 's': 'foo',
            't': None, 'u': None}
...
"""
    ...
    def test_custom_progname(self):
        parser = OptionParser(prog='thingy', version='%prog 0.1', usage=
            '%prog arg arg')
...
"""
    ...
    def test_option_default(self):
        self.parser.add_option('-f', '--file', default='foo.txt', help=self
            .file_help)
    ...
    def test_default_none_1(self):
        self.parser.add_option('-f', '--file', default=None, help=self.
            file_help)
        self.assertHelp(self.parser, self.expected_help_none)
    ...
    def test_float_default(self):
        self.parser.add_option('-p', '--prob', help=
            'blow up with probability PROB [default: %default]')
...
"""
    ...
    def test_alt_expand(self):
        self.parser.add_option('-f', '--file', default='foo.txt', help=
            'read from FILE [default: *DEFAULT*]')
...
class TestStandard(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-a', type='string'), make_option('-b',
            '--boo', type='int', dest='boo'), make_option('--foo', action=
            'append')]
    ...
    def test_long_option_argument_split(self):
        self.assertParseOK(['--boo', '111'], {'a': None, 'boo': 111, 'foo':
            None}, [])

    def test_long_option_short_option(self):
        self.assertParseOK(['--foo=bar', '-axyz'], {'a': 'xyz', 'boo': None,
            'foo': ['bar']}, [])

    def test_abbrev_long_option(self):
        self.assertParseOK(['--f=bar', '-axyz'], {'a': 'xyz', 'boo': None,
            'foo': ['bar']}, [])
    ...
    def test_ambiguous_option(self):
        self.parser.add_option('--foz', action='store', type='string', dest
            ='foo')
        self.assertParseFail(['--f=bar'],
            'ambiguous option: --f (--foo, --foz?)')

    def test_short_and_long_option_split(self):
        self.assertParseOK(['-a', 'xyz', '--foo', 'bar'], {'a': 'xyz',
            'boo': None, 'foo': ['bar']}, [])

    def test_short_option_split_long_option_append(self):
        self.assertParseOK(['--foo=bar', '-b', '123', '--foo', 'baz'], {'a':
            None, 'boo': 123, 'foo': ['bar', 'baz']}, [])

    def test_short_option_split_one_positional_arg(self):
        self.assertParseOK(['-a', 'foo', 'bar'], {'a': 'foo', 'boo': None,
            'foo': None}, ['bar'])

    def test_short_option_consumes_separator(self):
        self.assertParseOK(['-a', '--', 'foo', 'bar'], {'a': '--', 'boo':
            None, 'foo': None}, ['foo', 'bar'])
        self.assertParseOK(['-a', '--', '--foo', 'bar'], {'a': '--', 'boo':
            None, 'foo': ['bar']}, [])

    def test_short_option_joined_and_separator(self):
        self.assertParseOK(['-ab', '--', '--foo', 'bar'], {'a': 'b', 'boo':
            None, 'foo': None}, ['--foo', 'bar']),
    ...
    def test_option_consumes_optionlike_string(self):
        self.assertParseOK(['-a', '-b3'], {'a': '-b3', 'boo': None, 'foo':
            None}, [])
...
class TestBool(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-v', '--verbose', action='store_true', dest
            ='verbose', default=''), make_option('-q', '--quiet', action=
            'store_false', dest='verbose')]
...
class TestChoice(BaseTest):
    ...
    def setUp(self):
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE)
        self.parser.add_option('-c', action='store', type='choice', dest=
    ...
    def test_add_choice_option(self):
        self.parser.add_option('-d', '--default', choices=['four', 'five',
            'six'])
...
class TestCount(BaseTest):
    ...
    def setUp(self):
        ...
        self.parser.add_option('-q', '--quiet', action='store_const', dest=
            'verbose', const=0)
    ...
    def test_count_overriding(self):
        self.assertParseOK(['-vvv', '--verbose=2', '-q', '-v'], {'verbose':
            1}, [])
...
class TestMultipleArgs(BaseTest):
    ...
    def test_nargs_with_positional_args(self):
        self.assertParseOK(['foo', '-p', '1', '2.5', '-4.3', 'xyz'], {
            'point': (1.0, 2.5, -4.3)}, ['foo', 'xyz'])

    def test_nargs_long_opt(self):
        self.assertParseOK(['--point', '-1', '2.5', '-0', 'xyz'], {'point':
            (-1.0, 2.5, -0.0)}, ['xyz'])
...
class TestMultipleArgsAppend(BaseTest):
    ...
    def setUp(self):
        ...
        self.parser.add_option('-f', '--foo', action='append', nargs=2,
            type='int', dest='foo')
        self.parser.add_option('-z', '--zero', action='append_const', dest=
            'foo', const=(0, 0))

    def test_nargs_append(self):
        self.assertParseOK(['-f', '4', '-3', 'blah', '--foo', '1', '666'],
            {'point': None, 'foo': [(4, -3), (1, 666)]}, ['blah'])
    ...
    def test_nargs_append_const(self):
        self.assertParseOK(['--zero', '--foo', '3', '4', '-z'], {'point':
            None, 'foo': [(0, 0), (3, 4), (0, 0)]}, [])
...
class TestConflictingDefaults(BaseTest):
    ...
    def setUp(self):
        self.parser = OptionParser(option_list=[make_option('-v', action=
            'store_true', dest='verbose', default=1)])
...
class TestOptionGroup(BaseTest):
    ...
    def test_option_group_create_instance(self):
        ...
        group.add_option('--spam', action='store_true', help=
            'spam spam spam spam')
    ...
    def test_group_manipulate(self):
        group = self.parser.add_option_group('Group 2', description=
            'Some more options')
...
class TestExtendAddTypes(BaseTest):
    ...
    def test_filetype_ok(self):
        ...
        self.assertParseOK(['--file', support.TESTFN, '-afoo'], {'file':
            support.TESTFN, 'a': 'foo'}, [])
...
class TestExtendAddActions(BaseTest):
    ...
    def setUp(self):
        options = [self.MyOption('-a', '--apple', action='extend', type=
            'string', dest='apple')]
        self.parser = OptionParser(option_list=options)
    ...
    def test_extend_add_action(self):
        self.assertParseOK(['-afoo,bar', '--apple=blah'], {'apple': ['foo',
            'bar', 'blah']}, [])

    def test_extend_add_action_normal(self):
        self.assertParseOK(['-a', 'foo', '-abar', '--apple=x,y'], {'apple':
            ['foo', 'bar', 'x', 'y']}, [])
...
class TestCallback(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-x', None, action='callback', callback=self
            .process_opt), make_option('-f', '--file', action='callback',
            callback=self.process_opt, type='string', dest='filename')]
        self.parser = OptionParser(option_list=options)
    ...
    def test_callback(self):
        self.assertParseOK(['-x', '--file=foo'], {'filename': 'foo', 'x': 
            42}, [])

    def test_callback_help(self):
        parser = OptionParser(usage=SUPPRESS_USAGE)
        parser.remove_option('-h')
        parser.add_option('-t', '--test', action='callback', callback=lambda :
            None, type='string', help='foo')
...
class TestCallbackExtraArgs(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-p', '--point', action='callback', callback
            =self.process_tuple, callback_args=(3, int), type='string',
            dest='points', default=[])]
        self.parser = OptionParser(option_list=options)
    ...
    def test_callback_extra_args(self):
        self.assertParseOK(['-p1,2,3', '--point', '4,5,6'], {'points': [(1,
            2, 3), (4, 5, 6)]}, [])
...
class TestCallbackMeddleArgs(BaseTest):
    ...
    def setUp(self):
        options = [make_option(str(x), action='callback', callback=self.
            process_n, dest='things') for x in range(-1, -6, -1)]
        self.parser = OptionParser(option_list=options)
    ...
    def test_callback_meddle_args(self):
        self.assertParseOK(['-1', 'foo', '-3', 'bar', 'baz', 'qux'], {
            'things': [('foo',), ('bar', 'baz', 'qux')]}, [1, 3])

    def test_callback_meddle_args_separator(self):
        self.assertParseOK(['-2', 'foo', '--'], {'things': [('foo', '--')]},
            [2])
...
class TestCallbackManyArgs(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-a', '--apple', action='callback', nargs=2,
            callback=self.process_many, type='string'), make_option('-b',
            '--bob', action='callback', nargs=3, callback=self.process_many,
            type='int')]
        self.parser = OptionParser(option_list=options)
...
class TestCallbackCheckAbbrev(BaseTest):
    ...
    def setUp(self):
        self.parser = OptionParser()
        self.parser.add_option('--foo-bar', action='callback', callback=
            self.check_abbrev)
...
class TestCallbackVarArgs(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-a', type='int', nargs=2, dest='a'),
            make_option('-b', action='store_true', dest='b'), make_option(
            '-c', '--callback', action='callback', callback=self.
            variable_args, dest='c')]
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE,
            option_list=options)
    ...
    def variable_args(self, option, opt, value, parser):
        ...
        while rargs:
            arg = rargs[0]
            if arg[:2] == '--' and len(arg) > 2 or arg[:1] == '-' and len(arg
                ) > 1 and arg[1] != '-':
                break
            else:
                value.append(arg)
    ...
    def test_variable_args(self):
        self.assertParseOK(['-a3', '-5', '--callback', 'foo', 'bar'], {'a':
            (3, -5), 'b': None, 'c': ['foo', 'bar']}, [])

    def test_consume_separator_stop_at_option(self):
        self.assertParseOK(['-c', '37', '--', 'xxx', '-b', 'hello'], {'a':
            None, 'b': True, 'c': ['37', '--', 'xxx']}, ['hello'])

    def test_positional_arg_and_variable_args(self):
        self.assertParseOK(['hello', '-c', 'foo', '-', 'bar'], {'a': None,
            'b': None, 'c': ['foo', '-', 'bar']}, ['hello'])

    def test_stop_at_option(self):
        self.assertParseOK(['-c', 'foo', '-b'], {'a': None, 'b': True, 'c':
            ['foo']}, [])
...
class ConflictBase(BaseTest):
    ...
    def setUp(self):
        options = [make_option('-v', '--verbose', action='count', dest=
...
class TestConflict(ConflictBase):
    ...
    def assertTrueconflict_error(self, func):
        err = self.assertRaises(func, ('-v', '--version'), {'action':
            'callback', 'callback': self.show_version, 'help':
            'show version'}, OptionConflictError,
    ...
    def test_no_such_conflict_handler(self):
        self.assertRaises(self.parser.set_conflict_handler, ('foo',), None,
...
class TestConflictOverride(BaseTest):
    ...
    def setUp(self):
        self.parser = InterceptingOptionParser(usage=SUPPRESS_USAGE)
        self.parser.set_conflict_handler('resolve')
        self.parser.add_option('-n', '--dry-run', action='store_true', dest
            ='dry_run', help="don't do anything")
        self.parser.add_option('--dry-run', '-n', action='store_const',
            const=42, dest='dry_run', help='dry run mode')
...
class TestHelp(BaseTest):
    ...
    def make_parser(self, columns):
        options = [make_option('-a', type='string', dest='a', metavar=
            'APPLE', help='throw APPLEs at basket'), make_option('-b',
            '--boo', type='int', dest='boo', metavar='NUM', help=
            'shout "boo!" NUM times (in order to frighten away all the evil spirits that cause trouble and mayhem)'
            ), make_option('--foo', action='append', type='string', dest=
            'foo', help='store FOO in the foo list for later fooing')]
    ...
    def test_help_description_groups(self):
        ...
        group = OptionGroup(self.parser, 'Dangerous Options',
...
class TestMatchAbbrev(BaseTest):
    ...
    def test_match_abbrev(self):
        self.assertEqual(_match_abbrev('--f', {'--foz': None, '--foo': None,
            '--fie': None, '--f': None}), '--f')

    def test_match_abbrev_error(self):
        s = '--f'
        wordmap = {'--foz': None, '--foo': None, '--fie': None}
        self.assertRaises(_match_abbrev, (s, wordmap), None, BadOptionError,
...
class TestParseNumber(BaseTest):
    ...
    def test_parse_num_fail(self):
        self.assertRaises(_parse_num, ('', int), {}, ValueError, re.compile
            ("invalid literal for int().*: '?'?"))
        self.assertRaises(_parse_num, ('0xOoops', int), {}, ValueError, re.
            compile("invalid literal for int().*: s?'?0xOoops'?"))
