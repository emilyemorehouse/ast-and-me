...
class PyUnpicklerTests(AbstractUnpickleTests):
    ...
    truncated_errors = (pickle.UnpicklingError, EOFError, AttributeError,
        ValueError, struct.error, IndexError, ImportError)
...
if has_c_implementation:
    ...
    class SizeofTests(unittest.TestCase):
        ...
        def test_pickler(self):
            ...
            check(p, basesize + MT_size + 8 * ME_size + sys.getsizeof(b'x' *
                4096))
            for i in range(6):
                p.dump(chr(i))
        ...
        def test_unpickler(self):
            ...
            for encoding in ('ASCII', 'UTF-16', 'latin-1'):
                for errors in ('strict', 'replace'):
                    u = unpickler(io.BytesIO(), encoding=encoding, errors=
                        errors)
                    self.assertEqual(object.__sizeof__(u), basesize)
                    check(u, basesize + 32 * P + len(encoding) + 1 + len(
                        errors) + 1)
            stdsize = basesize + len('ASCII') + 1 + len('strict') + 1

            def check_unpickler(data, memo_size, marks_size):
                dump = pickle.dumps(data)
                u = unpickler(io.BytesIO(dump), encoding='ASCII', errors=
                    'strict')
                u.load()
            ...
            u = unpickler(io.BytesIO(pickle.dumps('a', 0)), encoding=
                'ASCII', errors='strict')
            u.load()
            check(u, stdsize + 32 * P + 2 + 1)
ALT_IMPORT_MAPPING = {('_elementtree', 'xml.etree.ElementTree'), ('cPickle',
    'pickle'), ('StringIO', 'io'), ('cStringIO', 'io')}
ALT_NAME_MAPPING = {('__builtin__', 'basestring', 'builtins', 'str'), (
...
class CompatPickleTests(unittest.TestCase):
    ...
    def test_exceptions(self):
        self.assertEqual(mapping('exceptions', 'StandardError'), (
            'builtins', 'Exception'))
        self.assertEqual(mapping('exceptions', 'Exception'), ('builtins',
            'Exception'))
        self.assertEqual(reverse_mapping('builtins', 'Exception'), (
            'exceptions', 'Exception'))
        self.assertEqual(mapping('exceptions', 'OSError'), ('builtins',
            'OSError'))
    ...
    def test_multiprocessing_exceptions(self):
        ...
        for name, exc in get_exceptions(module):
            with self.subTest(name):
                self.assertEqual(reverse_mapping('multiprocessing.context',
...
def test_main():
    tests = [PickleTests, PyUnpicklerTests, PyPicklerTests,
        PyPersPicklerTests, PyIdPersPicklerTests, PyDispatchTableTests,
        PyChainDispatchTableTests, CompatPickleTests]
    if has_c_implementation:
        tests.extend([CUnpicklerTests, CPicklerTests, CPersPicklerTests,
            CIdPersPicklerTests, CDumpPickle_LoadPickle,
            DumpPickle_CLoadPickle, PyPicklerUnpicklerObjectTests,
            CPicklerUnpicklerObjectTests, CDispatchTableTests,
            CChainDispatchTableTests, InMemoryPickleTests, SizeofTests])
