...
TESTDATA = {plistlib.FMT_XML: binascii.a2b_base64(
...
class TestPlistlib(unittest.TestCase):
    ...
    def _create(self, fmt=None):
        pl = dict(aString='Doodah', aList=['A', 'B', 12, 32.5, [1, 2, 3]],
            aFloat=0.5, anInt=728, aBigInt=2 ** 63 - 44, aBigInt2=2 ** 63 +
            44, aNegativeInt=-5, aNegativeBigInt=-80000000000, aDict=dict(
            anotherString="<hello & 'hi' there!>", aUnicodeValue=
            'Mässig, Maß', aTrueValue=True, aFalseValue=False, deeperDict=
            dict(a=17, b=32.5, c=[1, 2, 'text'])), someData=
            b'<binary gunk>', someMoreData=
            b'<lots of binary gunk>\x00\x01\x02\x03' * 10, nestedData=[
            b'<lots of binary gunk>\x00\x01\x02\x03' * 10], aDate=datetime.
            datetime(2004, 10, 26, 10, 33, 33), anEmptyDict=dict(),
            anEmptyList=list())
    ...
    def test_int(self):
        for pl in [0, 2 ** 8 - 1, 2 ** 8, 2 ** 16 - 1, 2 ** 16, 2 ** 32 - 1,
            2 ** 32, 2 ** 63 - 1, 2 ** 64 - 1, 1, -2 ** 63]:
            for fmt in ALL_FORMATS:
        ...
        for fmt in ALL_FORMATS:
            for pl in (2 ** 64 + 1, 2 ** 127 - 1, -2 ** 64, -2 ** 127):
                with self.subTest(pl=pl, fmt=fmt):
                    self.assertRaises(OverflowError, plistlib.dumps, pl,
                        fmt=fmt)
        ...
        for use_builtin_types in (True, False):
            for fmt in ALL_FORMATS:
                with self.subTest(fmt=fmt, use_builtin_types=use_builtin_types
                    ...
                    pl = plistlib.loads(TESTDATA[fmt], use_builtin_types=
                        use_builtin_types)
                    data = plistlib.dumps(pl, fmt=fmt)
        ...
        for fmt in ALL_FORMATS:
            for sort_keys in (False, True):
                with self.subTest(fmt=fmt, sort_keys=sort_keys):
                    ...
                    pl2 = plistlib.load(BytesIO(b.getvalue()), dict_type=
        ...
        for fmt in ALL_FORMATS:
            for sort_keys in (False, True):
                with self.subTest(fmt=fmt, sort_keys=sort_keys):
                    ...
                    pl2 = plistlib.loads(data, dict_type=collections.
                        OrderedDict)
                    self.assertEqual(dict(pl), dict(pl2))
                    if sort_keys:
                        self.assertEqual(list(pl2.keys()), ['a', 'b', 'c'])
    ...
    def test_skipkeys(self):
        ...
        for fmt in ALL_FORMATS:
            with self.subTest(fmt=fmt):
                data = plistlib.dumps(pl, fmt=fmt, skipkeys=True, sort_keys
                    =False)
                pl2 = plistlib.loads(data)
    ...
    def test_tuple_members(self):
        ...
        for fmt in ALL_FORMATS:
            with self.subTest(fmt=fmt):
                data = plistlib.dumps(pl, fmt=fmt)
                pl2 = plistlib.loads(data)
                self.assertEqual(pl2, {'first': [1, 2], 'second': [1, 2],
                    'third': [3, 4]})
    ...
    def test_dict_members(self):
        ...
        for fmt in ALL_FORMATS:
            with self.subTest(fmt=fmt):
                data = plistlib.dumps(pl, fmt=fmt)
                pl2 = plistlib.loads(data)
                self.assertEqual(pl2, {'first': {'a': 1}, 'second': {'a': 1
                    }, 'third': {'b': 2}})
                self.assertIsNot(pl2['first'], pl2['second'])
    ...
    def test_controlcharacters(self):
        for i in range(128):
            ...
            if i >= 32 or c in '\r\n\t':
                ...
                self.assertRaises(ValueError, plistlib.dumps, testString)
    ...
    def test_xml_encodings(self):
        ...
        for xml_encoding, encoding, bom in [(b'utf-8', 'utf-8', codecs.
            BOM_UTF8), (b'utf-16', 'utf-16-le', codecs.BOM_UTF16_LE), (
    ...
    def test_nonstandard_refs_size(self):
        data = (
            b'bplist00\xd1\x00\x00\x01\x00\x00\x02QaQb\x00\x00\x08\x00\x00\x0f\x00\x00\x11\x00\x00\x00\x00\x00\x00\x03\x03\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x13'
            )
...
class TestPlistlibDeprecated(unittest.TestCase):
    ...
    def test_io_deprecated(self):
        pl_in = {'key': 42, 'sub': {'key': 9, 'alt': 'value', 'data':
            b'buffer'}}
        pl_out = plistlib._InternalDict({'key': 42, 'sub': plistlib.
            _InternalDict({'key': 9, 'alt': 'value', 'data': plistlib.Data(
            b'buffer')})})
    ...
    def test_bytes_deprecated(self):
        ...
        self.assertEqual(pl2, plistlib._InternalDict(key=42, sub=plistlib.
            _InternalDict(key=9, alt='value', data=plistlib.Data(b'buffer'))))
        with self.assertWarns(DeprecationWarning):
