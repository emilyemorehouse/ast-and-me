...
import unittest
import warnings
_DUMMY_SYMLINK = os.path.join(tempfile.gettempdir(), support.TESTFN +
    '-dummy-symlink')
...
class PosixTester(unittest.TestCase):
    ...
    def testNoArgFunctions(self):
        NO_ARG_FUNCTIONS = ['ctermid', 'getcwd', 'getcwdb', 'uname',
            'times', 'getloadavg', 'getegid', 'geteuid', 'getgid',
            'getgroups', 'getpid', 'getpgrp', 'getppid', 'getuid', 'sync']
    ...
    @unittest.skipUnless(hasattr(posix, 'truncate'),
    ...
    @unittest.skipUnless(getattr(os, 'execve', None) in os.supports_fd,
    ...
    def test_waitid(self):
        ...
        if pid == 0:
            os.chdir(os.path.split(sys.executable)[0])
            posix.execve(sys.executable, [sys.executable, '-c', 'pass'], os
                .environ)
        else:
    ...
    def test_posix_fallocate(self):
        ...
        try:
            ...
            if inst.errno != errno.EINVAL or not sys.platform.startswith(
                'sunos'):
                raise
        finally:
            os.close(fd)
    ...
    def test_utime_with_fd(self):
        ...
        try:
            ...
            self.assertRaises(ValueError, posix.utime, fd, (now, now), ns=(
                now, now))
            self.assertRaises(ValueError, posix.utime, fd, (now, 0), ns=(
                None, None))
            self.assertRaises(ValueError, posix.utime, fd, (None, None), ns
                =(now, 0))
    ...
    def test_utime_nofollow_symlinks(self):
        ...
        self.assertRaises(TypeError, posix.utime, support.TESTFN, (None,
            None), follow_symlinks=False)
        self.assertRaises(TypeError, posix.utime, support.TESTFN, (now,
            None), follow_symlinks=False)
        self.assertRaises(TypeError, posix.utime, support.TESTFN, (None,
    ...
    def test_osexlock(self):
        ...
        self.assertRaises(OSError, os.open, support.TESTFN, os.O_WRONLY |
    ...
    def test_osshlock(self):
        ...
        if hasattr(posix, 'O_EXLOCK'):
            ...
            self.assertRaises(OSError, os.open, support.TESTFN, os.O_RDONLY |
                os.O_EXLOCK | os.O_NONBLOCK)
            os.close(fd)
    ...
    def test_stat(self):
        ...
        self.assertWarnsRegex(DeprecationWarning,
            'should be string, bytes, os.PathLike or integer, not', posix.
            stat, bytearray(os.fsencode(support.TESTFN)))
        self.assertRaisesRegex(TypeError,
            'should be string, bytes, os.PathLike or integer, not', posix.
            stat, None)
        self.assertRaisesRegex(TypeError,
            'should be string, bytes, os.PathLike or integer, not', posix.
            stat, list(support.TESTFN))
        self.assertRaisesRegex(TypeError,
            'should be string, bytes, os.PathLike or integer, not', posix.
            stat, list(os.fsencode(support.TESTFN)))
    ...
    @unittest.skipUnless(hasattr(posix, 'mknod') and hasattr(stat,
        'S_IFIFO'), "don't have mknod()/S_IFIFO")
    ...
    @unittest.skipUnless(hasattr(posix, 'stat'), 'test needs posix.stat()')
    @unittest.skipUnless(hasattr(posix, 'makedev'),
    ...
    def test_chown(self):
        ...
        self._test_all_chown_common(posix.chown, support.TESTFN, getattr(
            posix, 'stat', None))
    ...
    def test_fchown(self):
        ...
        try:
            fd = test_file.fileno()
            self._test_all_chown_common(posix.fchown, fd, getattr(posix,
                'fstat', None))
        finally:
            test_file.close()
    ...
    def test_lchown(self):
        ...
        self._test_all_chown_common(posix.lchown, support.TESTFN, getattr(
            posix, 'lstat', None))
        ...
        for fn in (posix.lchflags, chflags_nofollow):
            ...
            try:
                ...
                self.assertEqual(dummy_symlink_st.st_flags | stat.
                    UF_IMMUTABLE, new_dummy_symlink_st.st_flags)
            finally:
    ...
    @unittest.skipUnless(hasattr(posix, 'getgrouplist'),
        'test needs posix.getgrouplist()')
    ...
    @unittest.skipUnless(os.access in os.supports_dir_fd,
        'test needs dir_fd support for os.access()')
    ...
    @unittest.skipUnless(os.chown in os.supports_dir_fd,
    ...
    @unittest.skipUnless(os.stat in os.supports_dir_fd,
        'test needs dir_fd support in os.stat()')
    def test_stat_dir_fd(self):
        ...
        try:
            ...
            self.assertEqual(s1, s2)
            self.assertRaisesRegex(TypeError,
                'should be integer or None, not', posix.stat, support.
                TESTFN, dir_fd=posix.getcwd())
            self.assertRaisesRegex(TypeError,
                'should be integer or None, not', posix.stat, support.
                TESTFN, dir_fd=float(f))
    ...
    @unittest.skipUnless(os.utime in os.supports_dir_fd,
        'test needs dir_fd support in os.utime()')
    def test_utime_dir_fd(self):
        ...
        try:
            ...
            posix.utime(support.TESTFN, (int(now), int((now - int(now)) * 
                1000000000.0)), dir_fd=f)
            posix.utime(support.TESTFN, dir_fd=f, times=(int(now), int((now -
                int(now)) * 1000000000.0)))
    ...
    @unittest.skipUnless(os.link in os.supports_dir_fd,
        'test needs dir_fd support in os.link()')
    @unittest.skipIf(android_not_root, 'hard link not allowed, non root user')
    def test_link_dir_fd(self):
        f = posix.open(posix.getcwd(), posix.O_RDONLY)
        try:
            posix.link(support.TESTFN, support.TESTFN + 'link', src_dir_fd=
                f, dst_dir_fd=f)
            self.assertEqual(posix.stat(support.TESTFN)[1], posix.stat(
                support.TESTFN + 'link')[1])
        finally:
            posix.close(f)
            support.unlink(support.TESTFN + 'link')

    @unittest.skipUnless(os.mkdir in os.supports_dir_fd,
        'test needs dir_fd support in os.mkdir()')
    ...
    @unittest.skipUnless(os.mknod in os.supports_dir_fd and hasattr(stat,
        'S_IFIFO'),
    ...
    @unittest.skipUnless(os.open in os.supports_dir_fd,
        'test needs dir_fd support in os.open()')
    ...
    @unittest.skipUnless(os.readlink in os.supports_dir_fd,
        'test needs dir_fd support in os.readlink()')
    def test_readlink_dir_fd(self):
        os.symlink(support.TESTFN, support.TESTFN + 'link')
        f = posix.open(posix.getcwd(), posix.O_RDONLY)
        try:
            self.assertEqual(posix.readlink(support.TESTFN + 'link'), posix
                .readlink(support.TESTFN + 'link', dir_fd=f))
        finally:
            support.unlink(support.TESTFN + 'link')
            posix.close(f)

    @unittest.skipUnless(os.rename in os.supports_dir_fd,
        'test needs dir_fd support in os.rename()')
    def test_rename_dir_fd(self):
        support.unlink(support.TESTFN)
        support.create_empty_file(support.TESTFN + 'ren')
        f = posix.open(posix.getcwd(), posix.O_RDONLY)
        try:
            posix.rename(support.TESTFN + 'ren', support.TESTFN, src_dir_fd
                =f, dst_dir_fd=f)
        except:
            posix.rename(support.TESTFN + 'ren', support.TESTFN)
            raise
        else:
            posix.stat(support.TESTFN)
        finally:
            posix.close(f)

    @unittest.skipUnless(os.symlink in os.supports_dir_fd,
    ...
    @unittest.skipUnless(os.unlink in os.supports_dir_fd,
        'test needs dir_fd support in os.unlink()')
    def test_unlink_dir_fd(self):
        f = posix.open(posix.getcwd(), posix.O_RDONLY)
        support.create_empty_file(support.TESTFN + 'del')
        posix.stat(support.TESTFN + 'del')
        try:
    ...
    @unittest.skipUnless(os.mkfifo in os.supports_dir_fd,
    ...
    requires_sched_affinity = unittest.skipUnless(hasattr(posix,
        'sched_setaffinity'), "don't have sched affinity support")
    ...
    @unittest.skipUnless(hasattr(posix, 'sched_setscheduler'),
        "can't change scheduler")
    def test_get_and_set_scheduler_and_param(self):
        possible_schedulers = [sched for name, sched in posix.__dict__.
            items() if name.startswith('SCHED_')]
        ...
        for name in ('rename', 'replace', 'link'):
            ...
            for dst in ('noodly2', support.TESTFN):
                try:
                    ...
                    self.assertIn("'doesnotexistfilename' -> '{}'".format(
                        dst), str(e))
                    break
            else:
