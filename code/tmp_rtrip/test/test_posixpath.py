...
class PosixPathTest(unittest.TestCase):
    ...
    def splitextTest(self, path, filename, ext):
        ...
        self.assertEqual(posixpath.splitext('/' + path), ('/' + filename, ext))
        self.assertEqual(posixpath.splitext('abc/' + path), ('abc/' +
            filename, ext))
        self.assertEqual(posixpath.splitext('abc.def/' + path), ('abc.def/' +
            filename, ext))
        ...
        ext = bytes(ext, 'ASCII')
        self.assertEqual(posixpath.splitext(path), (filename, ext))
        self.assertEqual(posixpath.splitext(b'/' + path), (b'/' + filename,
            ext))
        self.assertEqual(posixpath.splitext(b'abc/' + path), (b'abc/' +
            filename, ext))
        self.assertEqual(posixpath.splitext(b'abc.def/' + path), (
            b'abc.def/' + filename, ext))
        self.assertEqual(posixpath.splitext(b'/abc.def/' + path), (
            b'/abc.def/' + filename, ext))
        self.assertEqual(posixpath.splitext(path + b'/'), (filename + ext +
            b'/', b''))
    ...
    def test_expanduser(self):
        ...
        try:
            ...
            if posixpath.expanduser('~') != '/':
                self.assertEqual(posixpath.expanduser('~') + '/', posixpath
                    .expanduser('~/'))
                self.assertEqual(posixpath.expanduser(b'~') + b'/',
    ...
    def test_realpath_symlink_loops(self):
        try:
            ...
            self.assertEqual(realpath(ABSTFN + '1/../x'), dirname(ABSTFN) +
                '/x')
            os.symlink(ABSTFN + 'x', ABSTFN + 'y')
            self.assertEqual(realpath(ABSTFN + '1/../' + basename(ABSTFN) +
                'y'), ABSTFN + 'y')
            self.assertEqual(realpath(ABSTFN + '1/../' + basename(ABSTFN) +
                '1'), ABSTFN + '1')
            os.symlink(basename(ABSTFN) + 'a/b', ABSTFN + 'a')
            self.assertEqual(realpath(ABSTFN + 'a'), ABSTFN + 'a/b')
            os.symlink('../' + basename(dirname(ABSTFN)) + '/' + basename(
                ABSTFN) + 'c', ABSTFN + 'c')
    ...
    def test_relpath(self):
        ...
        try:
            ...
            self.assertEqual(posixpath.relpath('../a/b'), '../a/b')
            self.assertEqual(posixpath.relpath('a', '../b'), '../' + curdir +
                '/a')
    ...
    def test_relpath_bytes(self):
        ...
        try:
            ...
            self.assertEqual(posixpath.relpath(b'../a/b'), b'../a/b')
            self.assertEqual(posixpath.relpath(b'a', b'../b'), b'../' +
                curdir + b'/a')
            self.assertEqual(posixpath.relpath(b'a/b', b'../c'), b'../' +
                curdir + b'/a/b')
    ...
    def test_commonpath(self):
        ...
        def check(paths, expected):
            ...
            self.assertEqual(posixpath.commonpath([os.fsencode(p) for p in
                paths]), os.fsencode(expected))

        def check_error(exc, paths):
            self.assertRaises(exc, posixpath.commonpath, paths)
            self.assertRaises(exc, posixpath.commonpath, [os.fsencode(p) for
                p in paths])
        ...
        self.assertRaises(TypeError, posixpath.commonpath, [b'/usr/lib/',
            '/usr/lib/python3'])
        self.assertRaises(TypeError, posixpath.commonpath, [b'/usr/lib/',
            'usr/lib/python3'])
        self.assertRaises(TypeError, posixpath.commonpath, [b'usr/lib/',
            '/usr/lib/python3'])
        self.assertRaises(TypeError, posixpath.commonpath, ['/usr/lib/',
            b'/usr/lib/python3'])
        self.assertRaises(TypeError, posixpath.commonpath, ['/usr/lib/',
            b'usr/lib/python3'])
        self.assertRaises(TypeError, posixpath.commonpath, ['usr/lib/',
...
class PathLikeTests(unittest.TestCase):
    ...
    def test_path_join(self):
        self.assertEqual(self.path.join('a', self.PathLike('b'), 'c'), self
            .path.join('a', 'b', 'c'))
