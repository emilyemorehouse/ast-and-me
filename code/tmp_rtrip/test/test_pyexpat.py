...
class ParseTest(unittest.TestCase):
    ...
    class Outputter:
        ...
        def StartElementHandler(self, name, attrs):
            self.out.append('Start element: ' + repr(name) + ' ' + sortdict
                (attrs))
    ...
    def _verify_parse_output(self, operations):
        expected_operations = [('XML declaration', ('1.0', 'iso-8859-1', 0)
            ), 'PI: \'xml-stylesheet\' \'href="stylesheet.css"\'',
            "Comment: ' comment data '", 'Not standalone', ('Start doctype',
            ('quotations', 'quotations.dtd', None, 1)), (
            'Element declaration', ('root', (2, 0, None, ()))), (
            'Attribute list declaration', ('root', 'attr1', 'CDATA', None, 
            1)), ('Attribute list declaration', ('root', 'attr2', 'CDATA',
            None, 0)),
            ...
            "End of NS decl: 'myns'", "Start element: 'sub2' {}",
            'Start of CDATA section',
            "Character data: 'contents of CDATA section'",
            'End of CDATA section', "End element: 'sub2'",
            "External entity ref: (None, 'entity.file', None)", (
            'Skipped entity', ('skipped_entity', 0)), "Character data: 'µ'",
            "End element: 'root'"]
        for operation, expected_operation in zip(operations,
            expected_operations):
    ...
    def test_parse_again(self):
        ...
        with self.assertRaises(expat.error) as cm:
            parser.ParseFile(file)
        self.assertEqual(expat.ErrorString(cm.exception.code), expat.errors
            .XML_ERROR_FINISHED)
...
class BufferTextTest(unittest.TestCase):
    ...
    def check(self, expected, label):
        self.assertEqual(self.stuff, expected, 
            '%s\nstuff    = %r\nexpected = %r' % (label, self.stuff, map(
            str, expected)))
    ...
    def test4(self):
        ...
        self.parser.Parse(b'<a>1<b/>2<c/>3</a>', 1)
        self.assertEqual(self.stuff, ['<a>', '<b>', '</b>', '<c>', '</c>',
            '</a>'])

    def test5(self):
        self.setHandlers(['StartElementHandler', 'EndElementHandler'])
        self.parser.Parse(b'<a>1<b></b>2<c/>3</a>', 1)
        self.assertEqual(self.stuff, ['<a>', '1', '<b>', '</b>', '2', '<c>',
            '</c>', '3', '</a>'])
    ...
    def test6(self):
        ...
        self.assertEqual(self.stuff, ['<a>', '1', '<b>', '</b>', '2', '<c>',
            '</c>', '345', '</a>'], 'buffered text not properly split')
    ...
    def test_exception(self):
        ...
        try:
            ...
            self.assertEqual(e.args[0], 'a', 
                "Expected RuntimeError for element 'a', but" + ' found %r' %
                e.args[0])
            entries = traceback.extract_tb(e.__traceback__)
            self.assertEqual(len(entries), 3)
            self.check_traceback_entry(entries[0], 'test_pyexpat.py',
                'test_exception')
            self.check_traceback_entry(entries[1], 'pyexpat.c', 'StartElement')
            self.check_traceback_entry(entries[2], 'test_pyexpat.py',
                'StartElementHandler')
...
class PositionTest(unittest.TestCase):
    ...
    def check_pos(self, event):
        pos = (event, self.parser.CurrentByteIndex, self.parser.
            CurrentLineNumber, self.parser.CurrentColumnNumber)
    ...
    def test(self):
        ...
        self.upto = 0
        self.expected_list = [('s', 0, 1, 0), ('s', 5, 2, 1), ('s', 11, 3, 
            2), ('e', 15, 3, 6), ('e', 17, 4, 1), ('e', 22, 5, 0)]
        xml = b'<a>\n <b>\n  <c/>\n </b>\n</a>'
...
class ChardataBufferTest(unittest.TestCase):
    ...
    def small_buffer_test(self, buffer_len):
        xml = (b"<?xml version='1.0' encoding='iso8859'?><s>" + b'a' *
            buffer_len + b'</s>')
        parser = expat.ParserCreate()
        parser.CharacterDataHandler = self.counting_handler
        parser.buffer_size = 1024
...
class ErrorMessageTest(unittest.TestCase):
    ...
    def test_codes(self):
        self.assertEqual(errors.XML_ERROR_SYNTAX, errors.messages[errors.
            codes[errors.XML_ERROR_SYNTAX]])
    ...
    def test_expaterror(self):
        ...
        try:
            ...
            self.assertEqual(e.code, errors.codes[errors.
