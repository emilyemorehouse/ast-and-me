...
class RangeTest(unittest.TestCase):
    ...
    def assert_iterators_equal(self, xs, ys, test_id, limit=None):
        ...
        for i, (x, y) in enumerate(pairs):
            if x == y:
                ...
                self.fail(
                    '{}: iterator ended unexpectedly at position {}; expected {}'
                    .format(test_id, i, y))
            elif y == sentinel:
                self.fail('{}: unexpected excess element {} at position {}'
                    .format(test_id, x, i))
            else:
                self.fail(
                    '{}: wrong element at position {};expected {}, got {}'.
                    format(test_id, i, y, x))
        ...
        for proto in range(pickle.HIGHEST_PROTOCOL + 1):
            for t in testcases:
                with self.subTest(proto=proto, test=t):
                    r = range(*t)
                    self.assertEqual(list(pickle.loads(pickle.dumps(r,
                        proto))), list(r))
    ...
    def test_range_iterators(self):
        limits = [(base + jiggle) for M in (2 ** 32, 2 ** 64) for base in (
            -M, -M // 2, 0, M // 2, M) for jiggle in (-2, -1, 0, 1, 2)]
        test_ranges = [(start, end, step) for start in limits for end in
            limits for step in (-2 ** 63, -2 ** 31, -2, -1, 1, 2)]
    ...
    def test_range_iterator_invocation(self):
        ...
        with test.support.check_warnings(('', DeprecationWarning)):
            ...
            for bad_val in [_testcapi.LONG_MAX + 1, _testcapi.LONG_MIN - 1]:
                ...
                self.assertRaises(OverflowError, rangeiter_type, 1, bad_val, 1)
                self.assertRaises(OverflowError, rangeiter_type, 1, 1, bad_val)
    ...
    def test_issue11845(self):
        ...
        values = {None, 0, 1, -1, 2, -2, 5, -5, 19, -19, 20, -20, 21, -21, 
            30, -30, 99, -99}
        for i in values:
            for j in values:
                for k in (values - {0}):
    ...
    def test_comparison(self):
        test_ranges = [range(0), range(0, -1), range(1, 1, 3), range(1),
            range(5, 6), range(5, 6, 2), range(5, 7, 2), range(2), range(0,
            4, 2), range(0, 5, 2), range(0, 6, 2)]
        ...
        self.assertEqual(hash(range(0, 2 ** 100 - 1, 2)), hash(range(0, 2 **
            100, 2)))
        ...
        self.assertNotEqual(range(2 ** 200, 2 ** 201, 2 ** 100), range(2 **
            200, 2 ** 201 + 1, 2 ** 100))
        with self.assertRaises(TypeError):
            range(0) < range(0)
        with self.assertRaises(TypeError):
