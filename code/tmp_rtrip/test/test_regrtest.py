...
class ParseArgsTestCase(unittest.TestCase):
    ...
    def test_help(self):
        for opt in ('-h', '--help'):
            with self.subTest(opt=opt):
                with support.captured_stdout() as out, self.assertRaises(
                    SystemExit):
    ...
    def test_coverdir(self):
        for opt in ('-D', '--coverdir'):
            with self.subTest(opt=opt):
                ns = libregrtest._parse_args([opt, 'foo'])
                self.assertEqual(ns.coverdir, os.path.join(support.SAVEDCWD,
                    'foo'))
                self.checkError([opt], 'expected one argument')
...
class BaseTestCase(unittest.TestCase):
    ...
    def parse_executed_tests(self, output):
        regex = (
            '^[0-9]+:[0-9]+:[0-9]+ (?:load avg: [0-9]+\\.[0-9]{2} )?\\[ *[0-9]+(?:/ *[0-9]+)*\\] (%s)'
             % self.TESTNAME_REGEX)
    ...
    def run_command(self, args, input=None, exitcode=0, **kw):
        ...
        proc = subprocess.run(args, universal_newlines=True, input=input,
...
class ProgramsTestCase(BaseTestCase):
    ...
    def test_module_test(self):
        args = [*self.python_args, '-m', 'test', *self.regrtest_args, *self
            .tests]
        self.run_tests(args)

    def test_module_regrtest(self):
        args = [*self.python_args, '-m', 'test.regrtest', *self.
            regrtest_args, *self.tests]
        self.run_tests(args)

    def test_module_autotest(self):
        args = [*self.python_args, '-m', 'test.autotest', *self.
            regrtest_args, *self.tests]
        self.run_tests(args)
...
class ArgsTestCase(BaseTestCase):
    ...
    def test_fromfile(self):
        ...
        with open(filename, 'w') as fp:
            ...
            for index, name in enumerate(tests, 1):
                line = '00:00:%02i [%s/%s] %s' % (index, index, len(tests),
                    name)
                if previous:
    ...
    def test_huntrleaks_fd_leak(self):
        ...
        filename = 'reflog.txt'
        self.addCleanup(support.unlink, filename)
        output = self.run_tests('--huntrleaks', '3:3:', test, exitcode=1,
    ...
    def test_matchfile(self):
        ...
        all_methods = ['test_method1', 'test_method2', 'test_method3',
            'test_method4']
        testname = self.create_test(code=code)
