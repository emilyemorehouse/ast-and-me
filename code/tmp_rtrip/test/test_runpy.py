...
import py_compile
import warnings
from test.support import forget, make_legacy_pyc, unload, verbose, no_tracing, create_empty_file, temp_dir
from test.support.script_helper import make_pkg, make_script, make_zip_pkg, make_zip_script
import runpy
...
"""
implicit_namespace = {'__name__': None, '__file__': None, '__cached__':
    None, '__package__': None, '__doc__': None, '__spec__': None}
example_namespace = {'sys': sys, 'runpy': runpy, 'result': [
    'Top level assignment', 'Lower level reference'], 'run_argv0': sys.argv
    [0], 'run_name_in_sys_modules': False, 'module_in_sys_modules': False,
    ...
    def assertNamespaceMatches(self, result_ns, expected_ns):
        ...
        if expected_spec is None:
            ...
            if expected_spec.loader is not None:
                self.assertEqual(type(result_spec.loader), type(
                    expected_spec.loader))
...
class ExecutionLayerTestCase(unittest.TestCase, CodeExecutionMixin):
    ...
    def test_run_module_code(self):
        ...
        mod_package = ''
        mod_spec = importlib.machinery.ModuleSpec(mod_name, origin=
            mod_fname, loader=mod_loader)
        expected_ns = example_namespace.copy()
        expected_ns.update({'__name__': mod_name, '__file__': mod_fname,
            '__loader__': mod_loader, '__package__': mod_package,
            '__spec__': mod_spec, 'run_argv0': mod_fname,
            'run_name_in_sys_modules': True, 'module_in_sys_modules': True})

        def create_ns(init_globals):
            return _run_module_code(example_source, init_globals, mod_name,
                mod_spec)
        self.check_code_execution(create_ns, expected_ns)
...
class RunModuleTestCase(unittest.TestCase, CodeExecutionMixin):
    ...
    def _make_pkg(self, source, depth, mod_base='runpy_test', *, namespace=
        False, parent_namespaces=False):
        if (namespace or parent_namespaces) and not depth:
            raise RuntimeError(
                "Can't mark top level module as a namespace package")
    ...
    def _fix_ns_for_legacy_pyc(self, ns, alter_sys):
        ...
        spec = ns['__spec__']
        new_spec = importlib.util.spec_from_file_location(spec.name, ns[
            '__file__'])
        ns['__spec__'] = new_spec
        if alter_sys:
            ns['run_argv0'] += char_to_add

    def _check_module(self, depth, alter_sys=False, *, namespace=False,
        parent_namespaces=False):
        pkg_dir, mod_fname, mod_name, mod_spec = self._make_pkg(example_source,
            depth, namespace=namespace, parent_namespaces=parent_namespaces)
        forget(mod_name)
        expected_ns = example_namespace.copy()
        expected_ns.update({'__name__': mod_name, '__file__': mod_fname,
            '__cached__': mod_spec.cached, '__package__': mod_name.
            rpartition('.')[0], '__spec__': mod_spec})
        if alter_sys:
            expected_ns.update({'run_argv0': mod_fname,
                'run_name_in_sys_modules': True, 'module_in_sys_modules': True}
                )
    ...
    def _check_package(self, depth, alter_sys=False, *, namespace=False,
        parent_namespaces=False):
        pkg_dir, mod_fname, mod_name, mod_spec = self._make_pkg(example_source,
        ...
        expected_ns.update({'__name__': mod_name, '__file__': mod_fname,
            ...
            '__package__': pkg_name, '__spec__': mod_spec})
        if alter_sys:
            expected_ns.update({'run_argv0': mod_fname,
                'run_name_in_sys_modules': True, 'module_in_sys_modules': True}
                )
...
"""
    ...
    def test_run_name(self):
        ...
        forget(mod_name)
        expected_ns = example_namespace.copy()
        expected_ns.update({'__name__': run_name, '__file__': mod_fname,
            '__cached__': importlib.util.cache_from_source(mod_fname),
            '__package__': mod_name.rpartition('.')[0], '__spec__': mod_spec})
    ...
    def test_pkgutil_walk_packages(self):
        ...
        for moduleinfo in pkgutil.walk_packages([pkg_dir]):
            ...
            self.assertIsInstance(moduleinfo.module_finder, importlib.
...
class RunPathTestCase(unittest.TestCase, CodeExecutionMixin):
    ...
    def _check_script(self, script_name, expected_name, expected_file,
        expected_argv0, mod_name=None, expect_spec=True, check_loader=True):
        ...
        expected_ns.update({'__name__': expected_name, '__file__':
            expected_file, '__cached__': mod_cached, '__package__': '',
            '__spec__': mod_spec, 'run_argv0': expected_argv0,
            'run_name_in_sys_modules': True, 'module_in_sys_modules': True})
        self.check_code_execution(create_ns, expected_ns)
