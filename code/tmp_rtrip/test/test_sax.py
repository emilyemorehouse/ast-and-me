...
requires_nonascii_filenames = unittest.skipUnless(supports_nonascii_filenames,
...
class XmlgenTest:
    ...
    def test_xmlgen_encoding(self):
        encodings = ('iso-8859-15', 'utf-8', 'utf-8-sig', 'utf-16',
    ...
    def test_xmlgen_ns(self):
        ...
        gen.endDocument()
        self.assertEqual(result.getvalue(), self.xml(
            '<ns1:doc xmlns:ns1="%s"><udoc></udoc></ns1:doc>' % ns_uri))
    ...
    def test_xmlgen_ns_empty(self):
        ...
        gen.endDocument()
        self.assertEqual(result.getvalue(), self.xml(
            '<ns1:doc xmlns:ns1="%s"><udoc/></ns1:doc>' % ns_uri))
    ...
    def test_xmlgen_fragment(self):
        ...
        gen.endElement('bar')
        self.assertEqual(result.getvalue(), self.xml(
            '<foo a="1.0">Hello</foo><bar b="2.0"></bar>')[len(self.xml('')):])
...
class StreamReaderWriterXmlgenTest(XmlgenTest, unittest.TestCase):
    ...
    def ioclass(self):
        writer = codecs.open(self.fname, 'w', encoding='ascii', errors=
            'xmlcharrefreplace', buffering=0)
...
"""
        ...
        parser.feed('<doc></doc>')
        parser.close()
        self.assertEqual(handler._notations, [('GIF',
    ...
    def test_expat_nsattrs_wattr(self):
        ...
        self.assertTrue(attrs.getQNames() == [] or attrs.getQNames() == [
            'ns:attr'])
        self.assertEqual(len(attrs), 1)
...
class XmlReaderTest(XmlTestBase):
    ...
    def test_nsattrs_wattr(self):
        attrs = AttributesNSImpl({(ns_uri, 'attr'): 'val'}, {(ns_uri,
            'attr'): 'ns:attr'})
        self.assertEqual(attrs.getLength(), 1)
        self.assertEqual(attrs.getNames(), [(ns_uri, 'attr')])
...
def test_main():
    run_unittest(MakeParserTest, ParseTest, SaxutilsTest,
        PrepareInputSourceTest, StringXmlgenTest, BytesXmlgenTest,
        WriterXmlgenTest, StreamWriterXmlgenTest,
        StreamReaderWriterXmlgenTest, ExpatReaderTest, ErrorReportingTest,
        XmlReaderTest)
