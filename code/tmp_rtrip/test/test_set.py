...
class TestJointOps:
    ...
    def test_union(self):
        ...
        for C in (set, frozenset, dict.fromkeys, str, list, tuple):
            ...
            self.assertEqual(self.thetype('abcba').union(C('efgfe')), set(
                'abcefg'))
            self.assertEqual(self.thetype('abcba').union(C('ccb')), set('abc'))
            self.assertEqual(self.thetype('abcba').union(C('ef')), set('abcef')
                )
            self.assertEqual(self.thetype('abcba').union(C('ef'), C('fg')),
                set('abcefg'))
        x = self.thetype()
    ...
    def test_intersection(self):
        ...
        for C in (set, frozenset, dict.fromkeys, str, list, tuple):
            ...
            self.assertEqual(self.thetype('abcba').intersection(C('ef')),
                set(''))
            self.assertEqual(self.thetype('abcba').intersection(C('cbcf'),
                C('bag')), set('b'))
    ...
    def test_isdisjoint(self):
        ...
        for larg in ('', 'a', 'ab', 'abc', 'ababac', 'cdc', 'cc', 'efgfe',
            'ccb', 'ef'):
            s1 = self.thetype(larg)
            for rarg in ('', 'a', 'ab', 'abc', 'ababac', 'cdc', 'cc',
                'efgfe', 'ccb', 'ef'):
                for C in (set, frozenset, dict.fromkeys, str, list, tuple):
                    s2 = C(rarg)
    ...
    def test_difference(self):
        ...
        for C in (set, frozenset, dict.fromkeys, str, list, tuple):
            ...
            self.assertEqual(self.thetype('abcba').difference(C('ccb')),
                set('a'))
            self.assertEqual(self.thetype('abcba').difference(C('ef')), set
                ('abc'))
            self.assertEqual(self.thetype('abcba').difference(), set('abc'))
            self.assertEqual(self.thetype('abcba').difference(C('a'), C('b'
                )), set('c'))
    ...
    def test_symmetric_difference(self):
        ...
        for C in (set, frozenset, dict.fromkeys, str, list, tuple):
            self.assertEqual(self.thetype('abcba').symmetric_difference(C(
                'cdc')), set('abd'))
            self.assertEqual(self.thetype('abcba').symmetric_difference(C(
                'efgfe')), set('abcefg'))
            self.assertEqual(self.thetype('abcba').symmetric_difference(C(
...
class TestSet(TestJointOps, unittest.TestCase):
    ...
    def test_update(self):
        ...
        for p, q in (('cdc', 'abcd'), ('efgfe', 'abcefg'), ('ccb', 'abc'),
            ('ef', 'abcef')):
            for C in (set, frozenset, dict.fromkeys, str, list, tuple):
                s = self.thetype('abcba')
    ...
    def test_difference_update(self):
        ...
        for p, q in (('cdc', 'ab'), ('efgfe', 'abc'), ('ccb', 'a'), ('ef',
            'abc')):
            for C in (set, frozenset, dict.fromkeys, str, list, tuple):
                s = self.thetype('abcba')
    ...
    def test_symmetric_difference_update(self):
        ...
        self.assertRaises(PassThru, self.s.symmetric_difference_update,
            check_pass_thru())
        self.assertRaises(TypeError, self.s.symmetric_difference_update, [[]])
        for p, q in (('cdc', 'abd'), ('efgfe', 'abcefg'), ('ccb', 'a'), (
            'ef', 'abcef')):
            for C in (set, frozenset, dict.fromkeys, str, list, tuple):
                s = self.thetype('abcba')
...
class TestFrozenSet(TestJointOps, unittest.TestCase):
    ...
    def test_singleton_empty_frozenset(self):
        ...
        efs = [frozenset(), frozenset([]), frozenset(()), frozenset(''),
            ...
            frozenset(range(0)), frozenset(frozenset()), frozenset(f), f]
    ...
    def test_hash(self):
        self.assertEqual(hash(self.thetype('abcdeb')), hash(self.thetype(
            'ebecda')))
        n = 100
...
class TestFrozenSetSubclass(TestFrozenSet):
    ...
    def test_singleton_empty_frozenset(self):
        ...
        efs = [Frozenset(), Frozenset([]), Frozenset(()), Frozenset(''),
            Frozenset(), Frozenset([]), Frozenset(()), Frozenset(''),
            Frozenset(range(0)), Frozenset(Frozenset()), Frozenset(
            frozenset()), f, F, Frozenset(f), Frozenset(F)]
...
class TestOnlySetsInBinaryOps:
    ...
    def test_intersection_update(self):
        if self.otherIsIterable:
            ...
            self.assertRaises(TypeError, self.set.intersection_update, self
                .other)
    ...
    def test_sym_difference_update(self):
        if self.otherIsIterable:
            ...
            self.assertRaises(TypeError, self.set.
...
class TestVariousIteratorArgs(unittest.TestCase):
        ...
        for cons in (set, frozenset):
            for s in ('123', '', range(1000), ('do', 1.2), range(2000, 2200, 5)
                ...
                for g in (G, I, Ig, S, L, R):
                    self.assertEqual(sorted(cons(g(s)), key=repr), sorted(g
                        (s), key=repr))
    ...
    def test_inline_methods(self):
        s = set('november')
        for data in ('123', '', range(1000), ('do', 1.2), range(2000, 2200,
            5), 'december'):
            for meth in (s.union, s.intersection, s.difference, s.
                symmetric_difference, s.isdisjoint):
                for g in (G, I, Ig, L, R):
                    expected = meth(data)
                    actual = meth(g(data))
                    if isinstance(expected, bool):
                        ...
                        self.assertEqual(sorted(actual, key=repr), sorted(
                            expected, key=repr))
    ...
    def test_inplace_methods(self):
        for data in ('123', '', range(1000), ('do', 1.2), range(2000, 2200,
            ...
            for methname in ('update', 'intersection_update',
                ...
                self.assertRaises(TypeError, getattr(set('january'),
                    methname), X(data))
                self.assertRaises(TypeError, getattr(set('january'),
                    methname), N(data))
                self.assertRaises(ZeroDivisionError, getattr(set('january'),
                    methname), E(data))
...
def cube(n):
    ...
    return dict([(x, frozenset([(x ^ s) for s in singletons])) for x in
        powerset(range(n))])
...
class TestGraphs(unittest.TestCase):
    ...
    def test_cuboctahedron(self):
        ...
        othervertices = set(edge for edges in cuboctahedron.values() for
            edge in edges)
