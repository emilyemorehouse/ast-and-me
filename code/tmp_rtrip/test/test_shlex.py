...
class ShlexTest(unittest.TestCase):
    ...
    def splitTest(self, data, comments):
        for i in range(len(data)):
            ...
            self.assertEqual(l, data[i][1:], '%s: %s != %s' % (data[i][0],
                l, data[i][1:]))
    ...
    def testCompat(self):
        ...
        for i in range(len(self.data)):
            ...
            self.assertEqual(l, self.data[i][1:], '%s: %s != %s' % (self.
                data[i][0], l, self.data[i][1:]))

    def testSyntaxSplitAmpersandAndPipe(self):
        """Test handling of syntax splitting of &, |"""
        for delimiter in ('&&', '&', '|&', ';&', ';;&', '||', '|', '&|',
            ';|', ';;|'):
            src = ['echo hi %s echo bye' % delimiter, 'echo hi%secho bye' %
                delimiter]
    ...
    def testSyntaxSplitSemicolon(self):
        """Test handling of syntax splitting of ;"""
        for delimiter in (';', ';;', ';&', ';;&'):
            src = ['echo hi %s echo bye' % delimiter, 'echo hi%s echo bye' %
                delimiter, 'echo hi%secho bye' % delimiter]
            ref = ['echo', 'hi', delimiter, 'echo', 'bye']
            for ss in src:
                s = shlex.shlex(ss, punctuation_chars=True)
                result = list(s)
    ...
    def testSyntaxSplitRedirect(self):
        """Test handling of syntax splitting of >"""
        for delimiter in ('<', '|'):
            src = ['echo hi %s out' % delimiter, 'echo hi%s out' %
                delimiter, 'echo hi%sout' % delimiter]
    ...
    def testTokenTypes(self):
        ...
        for source, expected in (('a && b || c', [('a', 'a'), ('&&', 'c'),
            ('b', 'a'), ('||', 'c'), ('c', 'a')]),):
