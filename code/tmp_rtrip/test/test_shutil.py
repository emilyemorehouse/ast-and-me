...
from shutil import make_archive, register_archive_format, unregister_archive_format, get_archive_formats, Error, unpack_archive, register_unpack_format, RegistryError, unregister_unpack_format, get_unpack_formats, SameFileError
import tarfile
import zipfile
import warnings
from test import support
from test.support import TESTFN, check_warnings, captured_stdout, android_not_root
TESTFN2 = TESTFN + '2'
try:
    import grp
    import pwd
...
class TestShutil(unittest.TestCase):
    ...
    def test_copystat_symlinks(self):
        ...
        if os.utime in os.supports_follow_symlinks:
            for attr in ('st_atime', 'st_mtime'):
                self.assertLessEqual(getattr(src_link_stat, attr), getattr(
                    dst_link_stat, attr) + 1)
        if hasattr(os, 'lchmod'):
    ...
    @unittest.skipUnless(hasattr(os, 'chflags') and hasattr(errno,
        'EOPNOTSUPP') and hasattr(errno, 'ENOTSUP'),
    ...
    def test_copyxattr(self):
        ...
        self.assertEqual(os.getxattr(src, 'user.foo'), os.getxattr(dst,
            'user.foo'))
        os.remove(dst)
        write_file(dst, 'bar')
    ...
    def test_rmtree_uses_safe_fd_version_if_available(self):
        _use_fd_functions = ({os.open, os.stat, os.unlink, os.rmdir} <= os.
            supports_dir_fd and os.listdir in os.supports_fd and os.stat in
            ...
            try:
                ...
                def _filter(src, names):
                    ...
                    for name in names:
                        ...
                        if os.path.isdir(path) and path.split()[-1
                            ] == 'subdir':
                            res.append(name)
    ...
    def test_copytree_retains_permissions(self):
        ...
        self.assertEqual(os.stat(os.path.join(src_dir, 'permissive.txt')).
            st_mode, os.stat(os.path.join(dst_dir, 'permissive.txt')).st_mode)
        self.assertEqual(os.stat(os.path.join(src_dir, 'restrictive.txt')).
            st_mode, os.stat(os.path.join(dst_dir, 'restrictive.txt')).st_mode)
        restrictive_subdir_dst = os.path.join(dst_dir, os.path.split(
            restrictive_subdir)[1])
        self.assertEqual(os.stat(restrictive_subdir).st_mode, os.stat(
            restrictive_subdir_dst).st_mode)
    ...
    def test_copytree_symlink_dir(self):
        ...
        with open(os.path.join(src_dir, 'real_dir', 'test.txt'), 'w'):
            pass
        os.symlink(os.path.join(src_dir, 'real_dir'), os.path.join(src_dir,
            'link_to_dir'), target_is_directory=True)
        shutil.copytree(src_dir, dst_dir, symlinks=False)
        self.assertFalse(os.path.islink(os.path.join(dst_dir, 'link_to_dir')))
        self.assertIn('test.txt', os.listdir(os.path.join(dst_dir,
            'link_to_dir')))
        dst_dir = os.path.join(self.mkdtemp(), 'destination2')
        shutil.copytree(src_dir, dst_dir, symlinks=True)
    ...
    def test_copy2(self):
        ...
        for attr in ('st_atime', 'st_mtime'):
            self.assertLessEqual(getattr(file1_stat, attr), getattr(
                file2_stat, attr) + 1)
        if hasattr(os, 'chflags') and hasattr(file1_stat, 'st_flags'):
            self.assertEqual(getattr(file1_stat, 'st_flags'), getattr(
                file2_stat, 'st_flags'))
    ...
    def test_make_tarball(self):
        ...
        with tarfile.open(tarball, 'r:gz') as tf:
            self.assertCountEqual(tf.getnames(), ['.', './sub', './sub2',
    ...
    def test_make_zipfile(self):
        ...
        with zipfile.ZipFile(res) as zf:
            self.assertCountEqual(zf.namelist(), ['dist/', 'dist/sub/',
                'dist/sub2/', 'dist/file1', 'dist/file2', 'dist/sub/file3',
                'outer'])
        with support.change_cwd(work_dir):
            base_name = os.path.abspath(rel_base_name)
    ...
    def test_make_archive_owner_group(self):
        ...
        self.assertTrue(os.path.isfile(res))
        res = make_archive(base_name, 'tar', root_dir, base_dir, owner=
            owner, group=group)
        self.assertTrue(os.path.isfile(res))
        res = make_archive(base_name, 'tar', root_dir, base_dir, owner=
    ...
    def test_tarfile_root_owner(self):
        ...
        owner = pwd.getpwuid(0)[0]
        with support.change_cwd(root_dir):
            archive_name = make_archive(base_name, 'gztar', root_dir,
                'dist', owner=owner, group=group)
    ...
    def test_register_archive_format(self):
        ...
        self.assertRaises(TypeError, register_archive_format, 'xxx', lambda :
            x, 1)
        self.assertRaises(TypeError, register_archive_format, 'xxx', lambda :
            x, [(1, 2), (1, 2, 3)])
...
class TestWhich(unittest.TestCase):
    ...
    def test_empty_path(self):
        base_dir = os.path.dirname(self.dir)
        with support.change_cwd(path=self.dir), support.EnvironmentVarGuard(
            ) as env:
            env['PATH'] = self.dir
...
class TestMove(unittest.TestCase):
    ...
    def test_move_dir_to_dir(self):
        self._check_move_dir(self.src_dir, self.dst_dir, os.path.join(self.
            dst_dir, os.path.basename(self.src_dir)))
    ...
    def test_move_dir_sep_to_dir(self):
        self._check_move_dir(self.src_dir + os.path.sep, self.dst_dir, os.
            path.join(self.dst_dir, os.path.basename(self.src_dir)))
    ...
    def test_existing_file_inside_dest_dir(self):
        with open(self.dst_file, 'wb'):
            pass
        self.assertRaises(shutil.Error, shutil.move, self.src_file, self.
            dst_dir)
    ...
    def test_destinsrc_false_negative(self):
        ...
        try:
            for src, dst in [('srcdir', 'srcdir/dest')]:
                ...
                self.assertTrue(shutil._destinsrc(src, dst), msg=
    ...
    def test_destinsrc_false_positive(self):
        ...
        try:
            for src, dst in [('srcdir', 'src/dest'), ('srcdir', 'srcdir.new')]:
                src = os.path.join(TESTFN, src)
                dst = os.path.join(TESTFN, dst)
                self.assertFalse(shutil._destinsrc(src, dst), msg=
    ...
    def test_move_return_value(self):
        ...
        self.assertEqual(rv, os.path.join(self.dst_dir, os.path.basename(
            self.src_file)))
...
class TestCopyFile(unittest.TestCase):
    ...
    def test_move_dir_caseinsensitive(self):
        ...
        dst_dir = os.path.join(os.path.dirname(self.src_dir), os.path.
            basename(self.src_dir).upper())
...
class TermsizeTests(unittest.TestCase):
    ...
    def test_stty_match(self):
        ...
        except (FileNotFoundError, PermissionError, subprocess.
            CalledProcessError):
    ...
    def test_fallback(self):
        with support.EnvironmentVarGuard() as env:
            ...
            with open(os.devnull, 'w') as f, support.swap_attr(sys,
                '__stdout__', f):
