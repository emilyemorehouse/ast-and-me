    ...
    if site.ENABLE_USER_SITE and not os.path.isdir(site.USER_SITE):
        try:
            ...
            raise unittest.SkipTest(
                'unable to create user site directory (%r): %s' % (site.
                USER_SITE, exc))
...
class HelperFunctionsTests(unittest.TestCase):
    ...
    def test_init_pathinfo(self):
        ...
        for entry in [site.makepath(path)[1] for path in sys.path if path and
            os.path.exists(path)]:
    ...
    def test_addpackage_import_bad_syntax(self):
        ...
        self.assertRegex(err_out.getvalue(), re.escape(os.path.join(pth_dir,
            pth_fn)))
    ...
    @unittest.skipUnless(site.ENABLE_USER_SITE,
    ...
    def test_getuserbase(self):
        ...
        with EnvironmentVarGuard() as environ:
            ...
            self.assertTrue(site.getuserbase().startswith('xoxo'), site.
                getuserbase())
    ...
    def test_getsitepackages(self):
        ...
        if sys.platform == 'darwin' and sysconfig.get_config_var(
            ...
            wanted = os.path.join('/Library', sysconfig.get_config_var(
                'PYTHONFRAMEWORK'), '%d.%d' % sys.version_info[:2],
                'site-packages')
            self.assertEqual(dirs[1], wanted)
        elif os.sep == '/':
            self.assertEqual(len(dirs), 1)
            wanted = os.path.join('xoxo', 'lib', 'python%d.%d' % sys.
                version_info[:2], 'site-packages')
...
class PthFile(object):
    ...
    def __init__(self, filename_base=TESTFN, imported='time', good_dirname=
        '__testdir__', bad_dirname='__bad'):
...
class ImportSideEffectTests(unittest.TestCase):
    ...
    def test_abs_paths(self):
        ...
        code = ('import os, sys',
            ...
            'sys.stdout.buffer.write(os_file + b"\\n")',
            'os_cached = os.__cached__.encode("ascii", "backslashreplace")',
            'sys.stdout.buffer.write(os_cached + b"\\n")')
        command = '\n'.join(code)
        proc = subprocess.Popen([sys.executable, '-S', '-c', command], env=
        ...
        proc = subprocess.Popen([sys.executable, '-c', command], env=env,
            stdout=subprocess.PIPE)
        stdout, stderr = proc.communicate()
        self.assertEqual(proc.returncode, 0)
        os__file__, os__cached__ = stdout.splitlines()[:2]
        self.assertTrue(os.path.isabs(os__file__),
            'expected absolute path, got {}'.format(os__file__.decode('ascii'))
            )
        self.assertTrue(os.path.isabs(os__cached__),
            'expected absolute path, got {}'.format(os__cached__.decode(
            'ascii')))
...
class StartupImportTests(unittest.TestCase):
    ...
    def test_startup_imports(self):
        popen = subprocess.Popen([sys.executable, '-I', '-v', '-c',
            'import sys; print(set(sys.modules))'], stdout=subprocess.PIPE,
        ...
        collection_mods = {'_collections', 'collections', 'functools',
            'heapq', 'itertools', 'keyword', 'operator', 'reprlib', 'types',
            'weakref'}.difference(sys.builtin_module_names)
        if sys.platform != 'darwin':
    ...
    def _create_underpth_exe(self, lines):
        exe_file = os.path.join(os.getenv('TEMP'), os.path.split(sys.
            executable)[1])
    ...
    def test_underpth_nosite_file(self):
        ...
        pth_lines = ['fake-path-name', *[libpath for _ in range(200)], '',
            '# comment']
        exe_file = self._create_underpth_exe(pth_lines)
        sys_path = self._calc_sys_path_for_underpth_nosite(os.path.dirname(
            exe_file), pth_lines)
        try:
            env = os.environ.copy()
            ...
            rc = subprocess.call([exe_file, '-c', 
                'import sys; sys.exit(sys.flags.no_site and len(sys.path) > 200 and sys.path == %r)'
                 % sys_path], env=env)
        finally:
    ...
    def test_underpth_file(self):
        libpath = os.path.dirname(os.path.dirname(encodings.__file__))
        exe_prefix = os.path.dirname(sys.executable)
        exe_file = self._create_underpth_exe(['fake-path-name', *[libpath for
            _ in range(200)], '', '# comment', 'import site'])
        ...
        try:
            ...
            rc = subprocess.call([exe_file, '-c', 
                'import sys; sys.exit(not sys.flags.no_site and %r in sys.path and %r in sys.path and %r not in sys.path and all("\\r" not in p and "\\n" not in p for p in sys.path))'
                 % (os.path.join(sys_prefix, 'fake-path-name'), libpath, os
                .path.join(sys_prefix, 'from-env'))], env=env)
        finally:
            self._cleanup_underpth_exe(exe_file)
        self.assertTrue(rc, 'sys.path is incorrect')
