...
class SMTPDServerTest(unittest.TestCase):
    ...
    def test_process_message_unimplemented(self):
        server = smtpd.SMTPServer((support.HOST, 0), ('b', 0), decode_data=True
            )
    ...
    def test_decode_data_and_enable_SMTPUTF8_raises(self):
        self.assertRaises(ValueError, smtpd.SMTPServer, (support.HOST, 0),
...
class TestMailOptionParsing(unittest.TestCase):
    ...
    def test_with_decode_data_true(self):
        ...
        channel = smtpd.SMTPChannel(server, conn, addr, decode_data=True)
        self.write_line(channel, b'EHLO example')
        for line in [b'MAIL from: <foo@example.com> size=20 SMTPUTF8',
    ...
    def test_with_decode_data_false(self):
        ...
        self.assertEqual(channel.socket.last,
...
class SMTPDChannelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.old_debugstream = smtpd.DEBUGSTREAM
        self.debug = smtpd.DEBUGSTREAM = io.StringIO()
        self.server = DummyServer((support.HOST, 0), ('b', 0), decode_data=True
            )
    ...
    def test_broken_connect(self):
        self.assertRaises(DummyDispatcherBroken, BrokenDummyServer, (
            support.HOST, 0), ('b', 0), decode_data=True)

    def test_decode_data_and_enable_SMTPUTF8_raises(self):
        self.assertRaises(ValueError, smtpd.SMTPChannel, self.server, self.
            channel.conn, self.channel.addr, enable_SMTPUTF8=True,
            decode_data=True)
    ...
    def test_command_too_long(self):
        self.write_line(b'HELO example')
        self.write_line(b'MAIL from: ' + b'a' * self.channel.
            command_size_limit + b'@example')
        self.assertEqual(self.channel.socket.last,
    ...
    def test_MAIL_command_limit_extended_with_SIZE(self):
        ...
        self.write_line(b'MAIL from:<' + b'a' * fill_len +
            b'@example> SIZE=1234')
        self.assertEqual(self.channel.socket.last, b'250 OK\r\n')
        self.write_line(b'MAIL from:<' + b'a' * (fill_len + 26) +
            b'@example> SIZE=1234')
        self.assertEqual(self.channel.socket.last,
            b'500 Error: line too long\r\n')
    ...
    def test_data_dialog(self):
        ...
        self.write_line(b'data\r\nmore\r\n.')
        self.assertEqual(self.channel.socket.last, b'250 OK\r\n')
        self.assertEqual(self.server.messages, [(('peer-address',
            'peer-port'), 'eggs@example', ['spam@example'], 'data\nmore')])
    ...
    def test_multiple_RCPT(self):
        ...
        self.write_line(b'data\r\n.')
        self.assertEqual(self.server.messages, [(('peer-address',
            'peer-port'), 'eggs@example', ['spam@example', 'ham@example'],
            'data')])
    ...
    def test_RSET(self):
        ...
        self.write_line(b'data\r\n.')
        self.assertEqual(self.server.messages, [(('peer-address',
            'peer-port'), 'foo@example', ['eggs@example'], 'data')])
...
class SMTPDChannelTestWithEnableSMTPUTF8True(unittest.TestCase):
    ...
    def test_utf8_data(self):
        self.write_line(b'EHLO example')
        self.write_line('MAIL From: naïve@examplé BODY=8BITMIME SMTPUTF8'.
            encode('utf-8'))
        self.assertEqual(self.channel.socket.last[0:3], b'250')
...
class MiscTestCase(unittest.TestCase):
    ...
    def test__all__(self):
        blacklist = {'program', 'Devnull', 'DEBUGSTREAM', 'NEWLINE',
            'COMMASPACE', 'DATA_SIZE_DEFAULT', 'usage', 'Options', 'parseargs'}
