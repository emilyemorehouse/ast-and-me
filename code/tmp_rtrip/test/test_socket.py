...
class SocketCANTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.skipTest('network interface `%s` does not exist' % self.
                interface)
...
def requireAttrs(obj, *attributes):
    ...
    return skipWithClientIf(missing, "don't have " + ', '.join(name for
        name in missing))
...
def requireSocket(*args):
    ...
    """
    err = None
    missing = [obj for obj in args if isinstance(obj, str) and not hasattr(
        socket, obj)]
    if missing:
        err = "don't have " + ', '.join(name for name in missing)
    else:
        callargs = [(getattr(socket, obj) if isinstance(obj, str) else obj) for
            obj in args]
        try:
            ...
            s.close()
    return skipWithClientIf(err is not None,
        "can't create socket({0}): {1}".format(', '.join(str(o) for o in
        args), err))
...
class GeneralModuleTests(unittest.TestCase):
    ...
    def test_csocket_repr(self):
        ...
        try:
            expected = (
                '<socket object, fd=%s, family=%s, type=%s, proto=%s>' % (s
                .fileno(), s.family, s.type, s.proto))
            self.assertEqual(repr(s), expected)
        finally:
            s.close()
        expected = '<socket object, fd=-1, family=%s, type=%s, proto=%s>' % (s
            .family, s.type, s.proto)
        self.assertEqual(repr(s), expected)
    ...
    def test_host_resolution_bad_address(self):
        explanation = (
            'resolving an invalid IP address did not raise OSError; can be caused by a broken DNS server'
            )
    ...
    @unittest.skipUnless(hasattr(socket, 'sethostname'),
    ...
    def testNtoH(self):
        sizes = {socket.htonl: 32, socket.ntohl: 32, socket.htons: 16,
            socket.ntohs: 16}
    ...
    def testIPv4toString(self):
        ...
        assertInvalid = lambda func, a: self.assertRaises((OSError,
            ValueError), func, a)
    ...
    def testIPv6toString(self):
        ...
        self.assertEqual(
            b'E\xefv\xcb\x00\x1aV\xef\xaf\xeb\x0b\xac\x19$\xae\xae', f(
            '45ef:76cb:1a:56ef:afeb:bac:1924:aeae'))
        self.assertEqual(b'\xadB\n\xbc' + b'\x00' * 4 +
            b"\x01'\x00\x00\x02T\x00\x02", f('ad42:abc::127:0:254:2'))
        ...
        self.assertEqual(b'\x00B' + b'\x00' * 8 + b'\xa2\x9b\xfe*\x17@', f(
            '42::a29b:254.42.23.64'))
        self.assertEqual(
            b'\x00B\xa8\xb9\x00\x00\x00\x02\xff\xff\xa2\x9b\xfe*\x17@', f(
            '42:a8b9:0:2:ffff:a29b:254.42.23.64'))
    ...
    def testStringToIPv6(self):
        ...
        self.assertEqual('aef:b01:506:1001:ffff:9997:55:170', f(
    ...
    def testNewAttributes(self):
        ...
        if hasattr(socket, 'SOCK_CLOEXEC'):
            self.assertIn(sock.type, (socket.SOCK_STREAM | socket.
                SOCK_CLOEXEC, socket.SOCK_STREAM))
        else:
    ...
    def testGetaddrinfo(self):
        ...
        self.assertEqual(a, b)
        a = socket.getaddrinfo(None, 0, socket.AF_UNSPEC, socket.
            SOCK_STREAM, 0, socket.AI_PASSIVE)
        b = socket.getaddrinfo(host=None, port=0, family=socket.AF_UNSPEC,
            type=socket.SOCK_STREAM, proto=0, flags=socket.AI_PASSIVE)
        self.assertEqual(a, b)
        self.assertRaises(UnicodeEncodeError, socket.getaddrinfo,
            'localhost', '\ud800')
        if hasattr(socket, 'AI_NUMERICSERV'):
            try:
                socket.getaddrinfo('localhost', None, 0, 0, 0, socket.
                    AI_NUMERICSERV)
            except socket.gaierror:
                pass

    def test_getnameinfo(self):
        self.assertRaises(OSError, socket.getnameinfo, ('mail.python.org', 
            0), 0)
    ...
    def check_sendall_interrupted(self, with_timeout):
        ...
        try:
            ...
            if with_timeout:
                ...
                signal.alarm(1)
                self.assertRaises(socket.timeout, c.sendall, b'x' * support
                    .SOCK_MAX_SIZE)
        finally:
            signal.alarm(0)
    ...
    def test_flowinfo(self):
        self.assertRaises(OverflowError, socket.getnameinfo, (support.
            HOSTv6, 0, 4294967295), 0)
...
class BasicCANTest(unittest.TestCase):
    ...
    def testTooLongInterfaceName(self):
        with socket.socket(socket.PF_CAN, socket.SOCK_RAW, socket.CAN_RAW
            ...
            self.assertRaisesRegex(OSError, 'interface name too long', s.
                bind, ('x' * 1024,))
        ...
        with socket.socket(socket.PF_CAN, socket.SOCK_RAW, socket.CAN_RAW
            ...
            for loopback in (0, 1):
                s.setsockopt(socket.SOL_CAN_RAW, socket.CAN_RAW_LOOPBACK,
                    loopback)
                self.assertEqual(loopback, s.getsockopt(socket.SOL_CAN_RAW,
    ...
    def testFilter(self):
        ...
        with socket.socket(socket.PF_CAN, socket.SOCK_RAW, socket.CAN_RAW
            ...
            self.assertEqual(can_filter, s.getsockopt(socket.SOL_CAN_RAW,
                socket.CAN_RAW_FILTER, 8))
            s.setsockopt(socket.SOL_CAN_RAW, socket.CAN_RAW_FILTER,
                bytearray(can_filter))
...
class CANTest(ThreadedCANSocketTest):
    ...
    def testBCM(self):
        ...
        header = struct.pack(self.bcm_cmd_msg_fmt, opcode, flags, count,
            ival1_seconds, ival1_usec, ival2_seconds, ival2_usec,
            bcm_can_id, nframes)
...
class BasicTCPTest(SocketConnectedTest):
    ...
    def _testShutdown_overflow(self):
        import _testcapi
        self.serv_conn.send(MSG)
        self.assertRaises(OverflowError, self.serv_conn.shutdown, _testcapi
            .INT_MAX + 1)
        self.assertRaises(OverflowError, self.serv_conn.shutdown, 2 + (
...
class SendrecvmsgBase(ThreadSafeCleanupTestCase):
    ...
    def sendmsgToServer(self, *args):
        return self.cli_sock.sendmsg(*(args + self.
            sendmsg_to_server_defaults[len(args):]))
    ...
    def checkFlags(self, flags, eor=None, checkset=0, checkunset=0, ignore=0):
        ...
        if inboth:
            raise Exception(
                'contradictory set, unset requirements for flags {0:#x}'.
                format(inboth))
...
class SendmsgTests(SendrecvmsgServerTimeoutBase):
    ...
    def _testSendmsgArray(self):
        self.assertEqual(self.sendmsgToServer([array.array('B', MSG)]), len
            (MSG))
    ...
    def _testSendmsgBadArgs(self):
        ...
        self.assertRaises(TypeError, self.sendmsgToServer, [MSG], object())
        self.assertRaises(TypeError, self.sendmsgToServer, [MSG], [], object())
        self.assertRaises(TypeError, self.sendmsgToServer, [MSG], [], 0,
            object())
...
class SendmsgStreamTests(SendmsgTests):
    ...
    def _testSendmsgDontWait(self):
        try:
            ...
            self.assertIn(cm.exception.errno, (errno.EAGAIN, errno.EWOULDBLOCK)
                )
        finally:
            self.misc_event.set()
...
class RecvmsgGenericTests(SendrecvmsgBase):
    ...
    def testRecvmsgTimeout(self):
        try:
            ...
            self.assertRaises(socket.timeout, self.doRecvmsg, self.
                serv_sock, len(MSG))
        finally:
            self.misc_event.set()
    ...
    def testRecvmsgPeek(self):
        ...
        self.checkRecvmsgAddress(addr, self.cli_addr)
        self.assertEqual(ancdata, [])
        self.checkFlags(flags, eor=False, ignore=getattr(socket,
            'MSG_TRUNC', 0))
        msg, ancdata, flags, addr = self.doRecvmsg(self.serv_sock, len(MSG),
            0, socket.MSG_PEEK)
        self.assertEqual(msg, MSG)
...
class RecvmsgGenericStreamTests(RecvmsgGenericTests):
    ...
    def testRecvmsgOverflow(self):
        seg1, ancdata, flags, addr = self.doRecvmsg(self.serv_sock, len(MSG
            ) - 3)
        self.checkRecvmsgAddress(addr, self.cli_addr)
        self.assertEqual(ancdata, [])
        self.checkFlags(flags, eor=False)
...
class RecvmsgTests(RecvmsgGenericTests):
    ...
    def testRecvmsgBadArgs(self):
        ...
        self.assertRaises(TypeError, self.serv_sock.recvmsg, [bytearray(10)
            ], 0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg, object(), 0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg, len(MSG),
            object(), 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg, len(MSG), 0,
            object())
...
class RecvmsgIntoTests(RecvmsgIntoMixin, RecvmsgGenericTests):
    ...
    def testRecvmsgIntoBadArgs(self):
        ...
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, len(MSG),
            0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, buf, 0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, [object()
            ], 0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, [
            b"I'm not writable"], 0, 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, [buf,
            object()], 0, 0)
        self.assertRaises(ValueError, self.serv_sock.recvmsg_into, [buf], -1, 0
            )
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, [buf],
            object(), 0)
        self.assertRaises(TypeError, self.serv_sock.recvmsg_into, [buf], 0,
            object())
    ...
    def testRecvmsgIntoGenerator(self):
        buf = bytearray(len(MSG))
        nbytes, ancdata, flags, addr = self.serv_sock.recvmsg_into(o for o in
            [buf])
        self.assertEqual(nbytes, len(MSG))
        self.assertEqual(buf, bytearray(MSG))
        self.checkRecvmsgAddress(addr, self.cli_addr)
    ...
    def testRecvmsgIntoScatter(self):
        ...
        b3 = bytearray(b'--------------')
        nbytes, ancdata, flags, addr = self.serv_sock.recvmsg_into([b1,
            memoryview(b2)[2:9], b3])
...
class SCMRightsTest(SendrecvmsgServerTimeoutBase):
    ...
    def createAndSendFDs(self, n):
        self.assertEqual(self.sendmsgToServer([MSG], [(socket.SOL_SOCKET,
            socket.SCM_RIGHTS, array.array('i', self.newFDs(n)))]), len(MSG))
    ...
    def _testFDPassSimple(self):
        self.assertEqual(self.sendmsgToServer([MSG], [(socket.SOL_SOCKET,
            socket.SCM_RIGHTS, array.array('i', self.newFDs(1)).tobytes())]
            ), len(MSG))
    ...
    def testFDPassCMSG_SPACE(self):
        self.checkRecvmsgFDs(4, self.doRecvmsg(self.serv_sock, len(MSG),
    ...
    def testFDPassCMSG_LEN(self):
        self.checkRecvmsgFDs(1, self.doRecvmsg(self.serv_sock, len(MSG),
            socket.CMSG_LEN(4 * SIZEOF_INT)), ignoreflags=socket.MSG_CTRUNC)
    ...
    def testFDPassSeparate(self):
        self.checkRecvmsgFDs(2, self.doRecvmsg(self.serv_sock, len(MSG), 
            10240), maxcmsgs=2)
    ...
    def _testFDPassSeparate(self):
        ...
        self.assertEqual(self.sendmsgToServer([MSG], [(socket.SOL_SOCKET,
            socket.SCM_RIGHTS, array.array('i', [fd0])), (socket.SOL_SOCKET,
            socket.SCM_RIGHTS, array.array('i', [fd1]))]), len(MSG))
    ...
    def testFDPassEmpty(self):
        self.checkRecvmsgFDs(0, self.doRecvmsg(self.serv_sock, len(MSG), 
            10240), ignoreflags=socket.MSG_CTRUNC)

    def _testFDPassEmpty(self):
        self.sendAncillaryIfPossible(MSG, [(socket.SOL_SOCKET, socket.
            SCM_RIGHTS, b'')])
    ...
    def _testFDPassPartialInt(self):
        self.sendAncillaryIfPossible(MSG, [(socket.SOL_SOCKET, socket.
            SCM_RIGHTS, array.array('i', [self.badfd]).tobytes()[:-1])])
    ...
    def _testFDPassPartialIntInMiddle(self):
        fd0, fd1 = self.newFDs(2)
        self.sendAncillaryIfPossible(MSG, [(socket.SOL_SOCKET, socket.
            SCM_RIGHTS, array.array('i', [fd0, self.badfd]).tobytes()[:-1]),
    ...
    def checkTruncatedHeader(self, result, ignoreflags=0):
        ...
        self.checkRecvmsgAddress(addr, self.cli_addr)
        self.assertEqual(ancdata, [])
        self.checkFlags(flags, eor=True, checkset=socket.MSG_CTRUNC, ignore
            =ignoreflags)
    ...
    def testCmsgTrunc0(self):
        self.checkTruncatedHeader(self.doRecvmsg(self.serv_sock, len(MSG), 
            0), ignoreflags=socket.MSG_CTRUNC)
...
class RFC3542AncillaryTest(SendrecvmsgServerTimeoutBase):
    ...
    def checkHopLimit(self, ancbufsize, maxhop=255, ignoreflags=0):
        self.serv_sock.setsockopt(socket.IPPROTO_IPV6, socket.
            IPV6_RECVHOPLIMIT, 1)
        self.misc_event.set()
        msg, ancdata, flags, addr = self.doRecvmsg(self.serv_sock, len(MSG),
            ancbufsize)
    ...
    def _testSetHopLimit(self):
        self.assertTrue(self.misc_event.wait(timeout=self.fail_timeout))
        self.assertEqual(self.sendmsgToServer([MSG], [(socket.IPPROTO_IPV6,
            socket.IPV6_HOPLIMIT, array.array('i', [self.hop_limit]))]),
            len(MSG))
    ...
    @requireAttrs(socket, 'CMSG_SPACE', 'IPV6_RECVHOPLIMIT',
        'IPV6_HOPLIMIT', 'IPV6_RECVTCLASS', 'IPV6_TCLASS')
    def testRecvTrafficClassAndHopLimitCMSG_SPACE(self):
        self.checkTrafficClassAndHopLimit(ancbufsize=socket.CMSG_SPACE(
            SIZEOF_INT) * 2)
    ...
    def testSetTrafficClassAndHopLimit(self):
        self.checkTrafficClassAndHopLimit(ancbufsize=10240, maxhop=self.
            hop_limit)
    ...
    def _testSetTrafficClassAndHopLimit(self):
        self.assertTrue(self.misc_event.wait(timeout=self.fail_timeout))
        self.assertEqual(self.sendmsgToServer([MSG], [(socket.IPPROTO_IPV6,
            socket.IPV6_TCLASS, array.array('i', [self.traffic_class])), (
            socket.IPPROTO_IPV6, socket.IPV6_HOPLIMIT, array.array('i', [
            self.hop_limit]))]), len(MSG))
    ...
    def _testOddCmsgSize(self):
        self.assertTrue(self.misc_event.wait(timeout=self.fail_timeout))
        try:
            nbytes = self.sendmsgToServer([MSG], [(socket.IPPROTO_IPV6,
                socket.IPV6_TCLASS, array.array('i', [self.traffic_class]).
                tobytes() + b'\x00'), (socket.IPPROTO_IPV6, socket.
                IPV6_HOPLIMIT, array.array('i', [self.hop_limit]))])
        except OSError as e:
            self.assertIsInstance(e.errno, int)
            nbytes = self.sendmsgToServer([MSG], [(socket.IPPROTO_IPV6,
                socket.IPV6_TCLASS, array.array('i', [self.traffic_class])),
    ...
    @requireAttrs(socket, 'IPV6_RECVHOPLIMIT', 'IPV6_HOPLIMIT')
    def testCmsgTruncNoBufSize(self):
        self.checkHopLimitTruncatedHeader(ancbufsize=None, ignoreflags=
            socket.MSG_CTRUNC)
    ...
    def testSingleCmsgTrunc0(self):
        self.checkHopLimitTruncatedHeader(ancbufsize=0, ignoreflags=socket.
            MSG_CTRUNC)
    ...
    def testSecondCmsgTrunc2Int(self):
        self.checkTruncatedSecondHeader(socket.CMSG_SPACE(SIZEOF_INT) + 2 *
            SIZEOF_INT)
...
class SendrecvmsgUDPTestBase(SendrecvmsgDgramFlagsBase,
    SendrecvmsgConnectionlessBase, ThreadedSocketTestMixin, UDPTestBase):
    pass
...
class SendrecvmsgUDP6TestBase(SendrecvmsgDgramFlagsBase,
    SendrecvmsgConnectionlessBase, ThreadedSocketTestMixin, UDP6TestBase):
...
class RecvmsgIntoRFC3542AncillaryUDP6Test(RecvmsgIntoMixin,
    RFC3542AncillaryTest, SendrecvmsgUDP6TestBase):
    pass
...
class SendrecvmsgSCTPStreamTestBase(SendrecvmsgSCTPFlagsBase,
    SendrecvmsgConnectedBase, ConnectedStreamTestMixin, SCTPStreamBase):
    pass
    ...
    def testRecvmsgEOF(self):
        try:
            ...
            if e.errno != errno.ENOTCONN:
                raise
            self.skipTest(
                'sporadic ENOTCONN (kernel issue?) - see issue #13876')
...
class InterruptedTimeoutBase(unittest.TestCase):
    ...
    def setUp(self):
        super().setUp()
        orig_alrm_handler = signal.signal(signal.SIGALRM, lambda signum,
            frame: 1 / 0)
...
@unittest.skipUnless(hasattr(signal, 'alarm') or hasattr(signal,
    'setitimer'), "Don't have signal.alarm or signal.setitimer")
...
class InterruptedSendTimeoutTest(InterruptedTimeoutBase,
    ThreadSafeCleanupTestCase, SocketListeningTestMixin, TCPTestBase):
    ...
    def testInterruptedSendtoTimeout(self):
        self.checkInterruptedSend(self.serv_conn.sendto, b'a' * 512, self.
            serv_addr)
...
class FileObjectClassTestCase(SocketConnectedTest):
    ...
    def setUp(self):
        ...
        self.read_file = self.cli_conn.makefile(self.read_mode, self.
            bufsize, encoding=self.encoding, errors=self.errors, newline=
            self.newline)
    ...
    def clientSetUp(self):
        ...
        self.write_file = self.serv_conn.makefile(self.write_mode, self.
            bufsize, encoding=self.encoding, errors=self.errors, newline=
            self.newline)
...
class TestExceptions(unittest.TestCase):
    ...
    def test_setblocking_invalidfd(self):
        ...
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM, 0, sock0.
            fileno())
        sock0.close()
...
class TestUnixDomain(unittest.TestCase):
    ...
    def encoded(self, path):
        try:
            ...
            self.skipTest(
                'Pathname {0!a} cannot be represented in file system encoding {1!r}'
                .format(path, sys.getfilesystemencoding()))
...
class TIPCTest(unittest.TestCase):
    ...
    def testRDM(self):
        ...
        srv.bind(srvaddr)
        sendaddr = socket.TIPC_ADDR_NAME, TIPC_STYPE, TIPC_LOWER + int((
            TIPC_UPPER - TIPC_LOWER) / 2), 0
...
class TIPCThreadableTest(unittest.TestCase, ThreadableTest):
    ...
    def clientSetUp(self):
        ...
        self.addCleanup(self.cli.close)
        addr = socket.TIPC_ADDR_NAME, TIPC_STYPE, TIPC_LOWER + int((
            TIPC_UPPER - TIPC_LOWER) / 2), 0
...
class InheritanceTest(unittest.TestCase):
    ...
    def test_SOCK_CLOEXEC(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM | socket.
            SOCK_CLOEXEC) as s:
    ...
    def test_set_inheritable_cloexec(self):
        ...
        with sock:
            fd = sock.fileno()
            self.assertEqual(fcntl.fcntl(fd, fcntl.F_GETFD) & fcntl.
...
class SendfileUsingSendTest(ThreadedTCPSocketTest):
    ...
    def setUpClass(cls):
        ...
        def chunks(total, step):
            ...
            if total:
                yield total
        chunk = b''.join([random.choice(string.ascii_letters).encode() for
            i in range(cls.BUFSIZE)])
    ...
    def _testNonRegularFile(self):
        ...
        with socket.create_connection(address) as sock, file as file:
            ...
            self.assertEqual(sent, self.FILESIZE)
            self.assertEqual(file.tell(), self.FILESIZE)
            self.assertRaises(socket._GiveupOnSendfile, sock.
                _sendfile_use_sendfile, file)
    ...
    def _testWithTimeoutTriggeredSend(self):
        address = self.serv.getsockname()
        file = open(support.TESTFN, 'rb')
        with socket.create_connection(address, timeout=0.01
            ) as sock, file as file:
            meth = self.meth_from_sock(sock)
    ...
    def test_errors(self):
        ...
        with open(support.TESTFN, 'rb') as file:
            with socket.socket() as s:
                meth = self.meth_from_sock(s)
                self.assertRaisesRegex(TypeError, 'positive integer', meth,
                    file, count='2')
                self.assertRaisesRegex(TypeError, 'positive integer', meth,
                    file, count=0.1)
                self.assertRaisesRegex(ValueError, 'positive integer', meth,
                    file, count=0)
                self.assertRaisesRegex(ValueError, 'positive integer', meth,
                    file, count=-1)
...
class LinuxKernelCryptoAPI(unittest.TestCase):
    ...
    def test_aes_cbc(self):
        ...
        with self.create_alg('skcipher', 'cbc(aes)') as algo:
            ...
            op, _ = algo.accept()
            with op:
                op.sendmsg_afalg(op=socket.ALG_OP_ENCRYPT, iv=iv, flags=
                    socket.MSG_MORE)
                op.sendall(msg)
    ...
    def test_aead_aes_gcm(self):
        ...
        with self.create_alg('aead', 'gcm(aes)') as algo:
            ...
            op, _ = algo.accept()
            with op:
                op.sendmsg_afalg(op=socket.ALG_OP_ENCRYPT, iv=iv, assoclen=
            ...
            with op:
                msg = assoc + plain
                op.sendmsg([msg], ([socket.SOL_ALG, socket.ALG_SET_OP,
                    pack_uint32(socket.ALG_OP_ENCRYPT)], [socket.SOL_ALG,
                    socket.ALG_SET_IV, pack_uint32(len(iv)) + iv], [socket.
                    SOL_ALG, socket.ALG_SET_AEAD_ASSOCLEN, pack_uint32(
                    assoclen)]))
...
def test_main():
    tests = [GeneralModuleTests, BasicTCPTest, TCPCloserTest,
        TCPTimeoutTest, TestExceptions, BufferIOTest, BasicTCPTest2,
        BasicUDPTest, UDPTimeoutTest]
    tests.extend([NonBlockingTCPTests, FileObjectClassTestCase,
        UnbufferedFileObjectClassTestCase,
        LineBufferedFileObjectClassTestCase,
        SmallBufferedFileObjectClassTestCase,
        UnicodeReadFileObjectClassTestCase,
        UnicodeWriteFileObjectClassTestCase,
        UnicodeReadWriteFileObjectClassTestCase, NetworkConnectionNoServer,
        NetworkConnectionAttributesTest, NetworkConnectionBehaviourTest,
        ContextManagersTest, InheritanceTest, NonblockConstantTest])
    ...
    tests.extend([CmsgMacroTests, SendmsgUDPTest, RecvmsgUDPTest,
        RecvmsgIntoUDPTest, SendmsgUDP6Test, RecvmsgUDP6Test,
        RecvmsgRFC3542AncillaryUDP6Test,
        RecvmsgIntoRFC3542AncillaryUDP6Test, RecvmsgIntoUDP6Test,
        SendmsgTCPTest, RecvmsgTCPTest, RecvmsgIntoTCPTest,
        SendmsgSCTPStreamTest, RecvmsgSCTPStreamTest,
        RecvmsgIntoSCTPStreamTest, SendmsgUnixStreamTest,
        RecvmsgUnixStreamTest, RecvmsgIntoUnixStreamTest,
        RecvmsgSCMRightsStreamTest, RecvmsgIntoSCMRightsStreamTest,
