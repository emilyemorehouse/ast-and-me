...
from test.support import run_unittest, import_module, TESTFN, unlink, check_warnings, captured_stdout, skip_unless_symlink, change_cwd
import sysconfig
...
class TestSysConfig(unittest.TestCase):
    ...
    def test_get_platform(self):
        ...
        os.name = 'nt'
        sys.version = (
            '2.4.4 (#71, Oct 18 2006, 08:34:43) [MSC v.1310 32 bit (Itanium)]')
        sys.platform = 'win32'
...
[GCC 4.1.2 20061115 (prerelease) (Debian 4.1.1-21)]"""
    ...
    def test_user_similar(self):
        ...
        for name in ('stdlib', 'platstdlib', 'purelib', 'platlib'):
            ...
            if adapt:
                global_path = global_path.replace(sys.exec_prefix, sys.
                    base_prefix)
    ...
    def test_platform_in_subprocess(self):
        ...
        p = subprocess.Popen([sys.executable, '-c',
            'import sysconfig; print(sysconfig.get_platform())'], stdout=
            subprocess.PIPE, stderr=subprocess.DEVNULL, env=env)
    ...
    @unittest.skipUnless(sys.platform == 'linux' and hasattr(sys.
        implementation, '_multiarch'), 'multiarch-specific test')
        ...
        if re.match('(i[3-6]86|x86_64)$', machine):
            if ctypes.sizeof(ctypes.c_char_p()) == 4:
                self.assertTrue(suffix.endswith('i386-linux-gnu.so') or
                    suffix.endswith('x86_64-linux-gnux32.so'), suffix)
            else:
