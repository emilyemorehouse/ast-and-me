...
class ReadTest(TarTest):
    ...
    def setUp(self):
        self.tar = tarfile.open(self.tarname, mode=self.mode, encoding=
            'iso8859-1')
...
"""
    ...
    def _test_fileobj_link(self, lnktype, regtype):
        with self.tar.extractfile(lnktype) as a, self.tar.extractfile(regtype
            ) as b:
...
class ListTest(ReadTest, unittest.TestCase):
    ...
    def test_list(self):
        ...
        self.assertIn(conv(
            b'misc/regtype-hpux-signed-chksum-\xc4\xd6\xdc\xe4\xf6\xfc\xdf'
            ), out)
        self.assertIn(conv(
            b'misc/regtype-old-v7-signed-chksum-\xc4\xd6\xdc\xe4\xf6\xfc\xdf'
            ), out)
...
class MiscReadTestBase(CommonReadTest):
    ...
    def test_fail_comp(self):
        ...
        with open(tarname, 'rb') as fobj:
            self.assertRaises(tarfile.ReadError, tarfile.open, fileobj=fobj,
                mode=self.mode)
    ...
    def test_check_members(self):
        for tarinfo in self.tar:
            self.assertEqual(int(tarinfo.mtime), 1041808783, 
                'wrong mtime for %s' % tarinfo.name)
            if not tarinfo.name.startswith('ustar/'):
                continue
            self.assertEqual(tarinfo.uname, 'tarfile', 'wrong uname for %s' %
                tarinfo.name)
    ...
    def test_extract_hardlink(self):
        with tarfile.open(tarname, errorlevel=1, encoding='iso8859-1') as tar:
            ...
            self.addCleanup(support.unlink, os.path.join(TEMPDIR,
                'ustar/regtype'))
        ...
        try:
            ...
            for tarinfo in directories:
                ...
                if sys.platform != 'win32':
                    self.assertEqual(tarinfo.mode & 511, os.stat(path).
                        st_mode & 511)
    ...
    def test_extractall_pathlike_name(self):
        ...
        with support.temp_dir(DIR), tarfile.open(tarname, encoding='iso8859-1'
            ) as tar:
            directories = [t for t in tar if t.isdir()]
            tar.extractall(DIR, directories)
            for tarinfo in directories:
...
class LongnameTest:
    ...
    def test_header_offset(self):
        ...
        with open(tarname, 'rb') as fobj:
            fobj.seek(offset)
            tarinfo = tarfile.TarInfo.frombuf(fobj.read(512), 'iso8859-1',
                'strict')
...
class GNUReadTest(LongnameTest, ReadTest, unittest.TestCase):
    ...
    def _test_sparse_file(self, name):
        ...
        with open(filename, 'rb') as fobj:
            data = fobj.read()
        self.assertEqual(md5sum(data), md5_sparse, 'wrong md5sum for %s' % name
            )
    ...
    def test_open_nonwritable_fileobj(self):
        for exctype in (OSError, EOFError, RuntimeError):
            ...
            with self.assertRaises(exctype):
                tar = tarfile.open(tmpname, self.mode, fileobj=f, format=
                    tarfile.PAX_FORMAT, pax_headers={'non': 'empty'})
    ...
    def test_stream_padding(self):
        ...
        if self.decompressor:
            ...
            with self.open(tmpname) as fobj:
                data = fobj.read()
        self.assertEqual(data.count(b'\x00'), tarfile.RECORDSIZE,
...
class GNUWriteTest(unittest.TestCase):
    ...
    def test_longnamelink_1025(self):
        self._test('longnam/' * 127 + 'longname_', 'longlnk/' * 127 +
            'longlink_')
...
class PaxWriteTest(GNUWriteTest):
    ...
    def test_pax_global_header(self):
        pax_headers = {'foo': 'bar', 'uid': '0', 'mtime': '1.23', 'test':
            'äöü', 'äöü': 'test'}
        tar = tarfile.open(tmpname, 'w', format=tarfile.PAX_FORMAT,
...
class UnicodeTest:
    ...
    def _test_unicode_filename(self, encoding):
        tar = tarfile.open(tmpname, 'w', format=self.format, encoding=
    ...
    def test_uname_unicode(self):
        ...
        t.uname = 'äöü'
        t.gname = 'äöü'
        tar = tarfile.open(tmpname, mode='w', format=self.format, encoding=
            'iso8859-1')
        try:
            tar.addfile(t)
        finally:
...
class UstarUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def test_unicode_longname1(self):
        ...
        self._test_ustar_name('0123456789' * 15 + '0123/4' + '0123456789' *
            10, ValueError)
        self._test_ustar_name('0123456789' * 15 + '012ÿ/' + '0123456789' * 10)
        self._test_ustar_name('0123456789' * 15 + '0123ÿ/' + '0123456789' *
            10, ValueError)

    def test_unicode_longname2(self):
        self._test_ustar_name('0123456789' * 15 + '01ÿ/2' + '0123456789' * 
            10, ValueError)
        self._test_ustar_name('0123456789' * 15 + '01ÿÿ/' + '0123456789' * 
            10, ValueError)

    def test_unicode_longname3(self):
        self._test_ustar_name('0123456789' * 15 + '01ÿÿ/2' + '0123456789' *
            10, ValueError)
        self._test_ustar_name('0123456789' * 15 + '01234/' + '0123456789' *
            9 + '01234567ÿ')
        self._test_ustar_name('0123456789' * 15 + '01234/' + '0123456789' *
            9 + '012345678ÿ', ValueError)

    def test_unicode_longname4(self):
        self._test_ustar_name('0123456789' * 15 + '01234/' + '0123456789' *
            9 + '012345ÿÿ')
        self._test_ustar_name('0123456789' * 15 + '01234/' + '0123456789' *
            9 + '0123456ÿÿ', ValueError)

    def _test_ustar_name(self, name, exc=None):
        with tarfile.open(tmpname, 'w', format=self.format, encoding='utf-8'
            ) as tar:
            t = tarfile.TarInfo(name)
            if exc is None:
                tar.addfile(t)
...
class GNUUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def test_bad_pax_header(self):
        for encoding, name in (('utf-8', 'pax/bad-pax-\udce4\udcf6\udcfc'),
            ...
            with tarfile.open(tarname, encoding=encoding, errors=
                'surrogateescape') as tar:
                try:
...
class PAXUnicodeTest(UnicodeTest, unittest.TestCase):
    ...
    def test_binary_header(self):
        for encoding, name in (('utf-8',
            'pax/hdrcharset-\udce4\udcf6\udcfc'), ('iso8859-1',
            'pax/hdrcharset-äöü')):
            with tarfile.open(tarname, encoding=encoding, errors=
                'surrogateescape') as tar:
                try:
...
class MiscTest(unittest.TestCase):
    ...
    def test_char_fields(self):
        ...
        self.assertEqual(tarfile.nts(b'foo\x00\x00\x00\x00\x00', 'ascii',
            'strict'), 'foo')
        self.assertEqual(tarfile.nts(b'foo\x00bar\x00', 'ascii', 'strict'),
            'foo')
    ...
    def test_write_number_fields(self):
        ...
        self.assertEqual(tarfile.itn(2097152), b'\x80\x00\x00\x00\x00 \x00\x00'
            )
        self.assertEqual(tarfile.itn(4294967295),
        ...
        self.assertEqual(tarfile.itn(-72057594037927936),
    ...
    def test__all__(self):
        blacklist = {'version', 'grp', 'pwd', 'symlink_exception', 'NUL',
            'BLOCKSIZE', 'RECORDSIZE', 'GNU_MAGIC', 'POSIX_MAGIC',
            'LENGTH_NAME', 'LENGTH_LINK', 'LENGTH_PREFIX', 'REGTYPE',
            'AREGTYPE', 'LNKTYPE', 'SYMTYPE', 'CHRTYPE', 'BLKTYPE',
            'DIRTYPE', 'FIFOTYPE', 'CONTTYPE', 'GNUTYPE_LONGNAME',
            'GNUTYPE_LONGLINK', 'GNUTYPE_SPARSE', 'XHDTYPE', 'XGLTYPE',
            'SOLARIS_XHDTYPE', 'SUPPORTED_TYPES', 'REGULAR_TYPES',
            'GNU_TYPES', 'PAX_FIELDS', 'PAX_NAME_FIELDS',
...
class CommandLineTest(unittest.TestCase):
    ...
    def tarfilecmd(self, *args, **kwargs):
        rc, out, err = script_helper.assert_python_ok('-m', 'tarfile', *
            args, **kwargs)
    ...
    def make_simple_tarfile(self, tar_name):
        files = [support.findfile('tokenize_tests.txt'), support.findfile(
    ...
    def test_list_command_verbose(self):
        for tar_name in testtarnames:
            ...
            expected = t.getvalue().encode('ascii', 'backslashreplace')
            for opt in ('-v', '--verbose'):
                out = self.tarfilecmd(opt, '-l', tar_name, PYTHONIOENCODING
                    ='ascii')
                self.assertEqual(out, expected)
...
class NumericOwnerTest(unittest.TestCase):
    ...
    def _make_test_archive(filename_1, dirname_1, filename_2):
        ...
        items = [(filename_1, 99, 98, tarfile.REGTYPE, fobj), (dirname_1, 
            77, 76, tarfile.DIRTYPE, None), (filename_2, 88, 87, tarfile.
            REGTYPE, fobj)]
    ...
    def test_extract_with_numeric_owner(self, mock_geteuid, mock_chmod,
        ...
        mock_chown.assert_has_calls([unittest.mock.call(f_filename_1, 99, 
            98), unittest.mock.call(f_filename_2, 88, 87)], any_order=True)
    ...
    def test_extractall_with_numeric_owner(self, mock_geteuid, mock_chmod,
        mock_chown):
        with self._setup_test(mock_geteuid) as (tarfl, filename_1,
            dirname_1, filename_2):
        ...
        f_filename_2 = os.path.join(TEMPDIR, filename_2)
        mock_chown.assert_has_calls([unittest.mock.call(f_filename_1, 99, 
            98), unittest.mock.call(f_dirname_1, 77, 76), unittest.mock.
            call(f_filename_2, 88, 87)], any_order=True)
    ...
    def test_keyword_only(self, mock_geteuid):
        with self._setup_test(mock_geteuid) as (tarfl, filename_1, _, _):
            self.assertRaises(TypeError, tarfl.extract, filename_1, TEMPDIR,
                False, True)
