...
class GeneralTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.thread = threading.Thread(target=server, args=(self.evt, self.
            sock))
...
class ReadTests(ExpectAndReadTestCase):
    ...
    def test_read_until(self):
        ...
        self.assertEqual(data, b'xxxmatch', msg=(telnet.cookedq, telnet.
            rawq, telnet.sock.reads))
...
class WriteTests(unittest.TestCase):
    ...
    def test_write(self):
        data_sample = [b'data sample without IAC', b'data sample with' + tl
            .IAC + b' one IAC', b'a few' + tl.IAC + tl.IAC + b' iacs' + tl.
            IAC, tl.IAC, b'']
...
class OptionTests(unittest.TestCase):
    ...
    def test_SB_commands(self):
        send = [tl.IAC + tl.SB + tl.IAC + tl.SE, tl.IAC + tl.SB + tl.IAC +
            tl.IAC + tl.IAC + tl.SE, tl.IAC + tl.SB + tl.IAC + tl.IAC +
            b'aa' + tl.IAC + tl.SE, tl.IAC + tl.SB + b'bb' + tl.IAC + tl.
            IAC + tl.IAC + tl.SE, tl.IAC + tl.SB + b'cc' + tl.IAC + tl.IAC +
            b'dd' + tl.IAC + tl.SE]
        ...
        want_sb_data = (tl.IAC + tl.IAC + b'aabb' + tl.IAC + b'cc' + tl.IAC +
            b'dd')
...
"""), (tl.IAC + tl.DO + bytes([1]),
            ': IAC DO 1\n'), (tl.IAC + tl.DONT + bytes([1]),
            ': IAC DONT 1\n'), (tl.IAC + tl.WILL + bytes([1]),
            ': IAC WILL 1\n'), (tl.IAC + tl.WONT + bytes([1]),
            ': IAC WONT 1\n')]
