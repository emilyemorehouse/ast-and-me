...
class BaseTestCase(unittest.TestCase):
    ...
    def nameCheck(self, name, dir, pre, suf):
        ...
        self.assertEqual(npre, pre, 'file %r does not begin with %r' % (
            nbase, pre))
        self.assertEqual(nsuf, suf, 'file %r does not end with %r' % (nbase,
            suf))
...
class TestExports(BaseTestCase):
    ...
    def test_exports(self):
        ...
        expected = {'NamedTemporaryFile': 1, 'TemporaryFile': 1, 'mkstemp':
            1, 'mkdtemp': 1, 'mktemp': 1, 'TMP_MAX': 1, 'gettempprefix': 1,
            'gettempprefixb': 1, 'gettempdir': 1, 'gettempdirb': 1,
            'tempdir': 1, 'template': 1, 'SpooledTemporaryFile': 1,
...
def _mock_candidate_names(*names):
    return support.swap_attr(tempfile, '_get_candidate_names', lambda :
        iter(names))
...
class TestMkstempInner(TestBadTempdir, BaseTestCase):
    ...
    class mkstemped:
        ...
        def __init__(self, dir, pre, suf, bin):
            ...
            self.fd, self.name = tempfile._mkstemp_inner(dir, pre, suf,
                flags, output_type)
    ...
    def test_noinherit(self):
        ...
        self.assertFalse(retval < 0, 'child process caught fatal signal %d' %
            -retval)
        self.assertFalse(retval > 0, 'child process reports failure %d' %
            retval)
    ...
    def make_temp(self):
        return tempfile._mkstemp_inner(tempfile.gettempdir(), tempfile.
            gettempprefix(), '', tempfile._bin_openflags, str)

    def test_collision_with_existing_file(self):
        with _inside_empty_temp_dir(), _mock_candidate_names('aaa', 'aaa',
            'bbb'):
...
class TestSpooledTemporaryFile(BaseTestCase):
    ...
    def do_create(self, max_size=0, dir=None, pre='', suf=''):
        if dir is None:
            dir = tempfile.gettempdir()
        file = tempfile.SpooledTemporaryFile(max_size=max_size, dir=dir,
    ...
    def test_del_on_close(self):
        ...
        try:
            ...
            filename = f.name
            f.close()
            self.assertFalse(isinstance(filename, str) and os.path.exists(
                filename), 'SpooledTemporaryFile %s exists after close' %
                filename)
        finally:
            os.rmdir(dir)
    ...
    def test_text_newline_and_encoding(self):
        f = tempfile.SpooledTemporaryFile(mode='w+', max_size=10, newline=
            '', encoding='utf-8')
        f.write('Λ\r\n')
        f.seek(0)
