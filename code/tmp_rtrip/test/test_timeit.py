...
class TestTimeit(unittest.TestCase):
    ...
    def test_timer_invalid_setup(self):
        ...
        self.assertRaises(SyntaxError, timeit.Timer, setup=
            'from timeit import *')
    ...
    def test_timeit_globals_args(self):
        ...
        timeit.timeit(stmt='local_timer.inc()', timer=local_timer, globals=
            locals(), number=3)
    ...
    def test_repeat_callable_stmt(self):
        self.repeat(self.fake_callable_stmt, self.fake_setup, repeat=3,
            number=5)

    def test_repeat_callable_setup(self):
        self.repeat(self.fake_stmt, self.fake_callable_setup, repeat=3,
            number=5)
    ...
    def test_repeat_function_zero_reps(self):
        delta_times = timeit.repeat(self.fake_stmt, self.fake_setup, repeat
            =0, timer=FakeTimer())
        self.assertEqual(delta_times, [])

    def test_repeat_function_zero_iters(self):
        delta_times = timeit.repeat(self.fake_stmt, self.fake_setup, number
            =0, timer=FakeTimer())
    ...
    def test_main_very_verbose(self):
        s = self.run_main(seconds_per_increment=5e-05, switches=['-vv'])
        self.assertEqual(s, dedent(
