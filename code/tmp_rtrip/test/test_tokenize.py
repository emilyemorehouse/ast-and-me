...
from tokenize import tokenize, _tokenize, untokenize, NUMBER, NAME, OP, STRING, ENDMARKER, ENCODING, tok_name, detect_encoding, open as tokenize_open, Untokenizer
...
class TokenizeTest(TestCase):
    ...
    def check_tokenize(self, s, expected):
        ...
        self.assertEqual(result, [
            "    ENCODING   'utf-8'       (0, 0) (0, 0)"] + expected.rstrip
            ().splitlines())
...
def decistmt(s):
    ...
    for toknum, tokval, _, _, _ in g:
        if toknum == NUMBER and '.' in tokval:
            result.extend([(NAME, 'Decimal'), (OP, '('), (STRING, repr(
                tokval)), (OP, ')')])
        else:
...
class TestMisc(TestCase):
    ...
    def test_decistmt(self):
        ...
        self.assertEqual(eval(decistmt(s)), Decimal(
...
class TestTokenize(TestCase):
    ...
    def test_tokenize(self):
        ...
        try:
            ...
            self.assertEqual(list(results), [b'first', b'second', b'1',
                b'2', b'3', b'4'])
        finally:
    ...
    def test_oneline_defs(self):
        ...
        self.assertEqual(toks[-2].string, 'OK')

    def assertExactTypeEqual(self, opstr, *optypes):
        tokens = list(tokenize(BytesIO(opstr.encode('utf-8')).readline))
        num_optypes = len(optypes)
        self.assertEqual(len(tokens), 2 + num_optypes)
        self.assertEqual(token.tok_name[tokens[0].exact_type], token.
            tok_name[ENCODING])
    ...
    def test_exact_type(self):
        ...
        self.assertExactTypeEqual('a**2+b**2==c**2', NAME, token.DOUBLESTAR,
            NUMBER, token.PLUS, NAME, token.DOUBLESTAR, NUMBER, token.
            EQEQUAL, NAME, token.DOUBLESTAR, NUMBER)
        self.assertExactTypeEqual('{1, 2, 3}', token.LBRACE, token.NUMBER,
            token.COMMA, token.NUMBER, token.COMMA, token.NUMBER, token.RBRACE)
        self.assertExactTypeEqual('^(x & 0x1)', token.CIRCUMFLEX, token.
            LPAR, token.NAME, token.AMPER, token.NUMBER, token.RPAR)
...
"""
    ...
    def test_random_files(self):
        ...
        for f in ('buffer', 'builtin', 'fileio', 'inspect', 'os',
            'platform', 'sys'):
