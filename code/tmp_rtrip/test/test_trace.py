...
class TestLineCounts(unittest.TestCase):
    ...
    def test_traced_func_loop(self):
        ...
        expected = {(self.my_py_filename, firstlineno + 1): 1, (self.
            my_py_filename, firstlineno + 2): 6, (self.my_py_filename, 
            firstlineno + 3): 5, (self.my_py_filename, firstlineno + 4): 1}
        self.assertEqual(self.tracer.results().counts, expected)
    ...
    def test_traced_func_importing(self):
        ...
        expected = {(self.my_py_filename, firstlineno + 1): 1, (fix_ext_py(
            testmod.__file__), 2): 1, (fix_ext_py(testmod.__file__), 3): 1}
        self.assertEqual(self.tracer.results().counts, expected)
    ...
    def test_trace_func_generator(self):
        ...
        expected = {(self.my_py_filename, firstlineno_calling + 1): 1, (
            self.my_py_filename, firstlineno_calling + 2): 11, (self.
            my_py_filename, firstlineno_calling + 3): 10, (self.
            my_py_filename, firstlineno_gen + 1): 1, (self.my_py_filename, 
            firstlineno_gen + 2): 11, (self.my_py_filename, firstlineno_gen +
            3): 10}
        self.assertEqual(self.tracer.results().counts, expected)
    ...
    def test_trace_list_comprehension(self):
        ...
        expected = {(self.my_py_filename, firstlineno_calling + 1): 1, (
            self.my_py_filename, firstlineno_calling + 2): 12, (self.
            my_py_filename, firstlineno_calling + 3): 1, (self.
            my_py_filename, firstlineno_called + 1): 10}
        self.assertEqual(self.tracer.results().counts, expected)
...
class TestFuncs(unittest.TestCase):
    ...
    def test_loop_caller_importing(self):
        ...
        expected = {(self.filemod + ('traced_func_simple_caller',)): 1, (
            self.filemod + ('traced_func_linear',)): 1, (self.filemod + (
            'traced_func_importing_caller',)): 1, (self.filemod + (
            'traced_func_importing',)): 1, (fix_ext_py(testmod.__file__),
            'testmod', 'func'): 1}
        self.assertEqual(self.tracer.results().calledfuncs, expected)
    ...
    def test_inst_method_calling(self):
        ...
        expected = {(self.filemod + ('TracedClass.inst_method_calling',)): 
            1, (self.filemod + ('TracedClass.inst_method_linear',)): 1, (
            self.filemod + ('traced_func_linear',)): 1}
        self.assertEqual(self.tracer.results().calledfuncs, expected)
...
class TestCallers(unittest.TestCase):
    ...
    def test_loop_caller_importing(self):
        self.tracer.runfunc(traced_func_importing_caller, 1)
        expected = {((os.path.splitext(trace.__file__)[0] + '.py', 'trace',
            'Trace.runfunc'), self.filemod + (
            'traced_func_importing_caller',)): 1, (self.filemod + (
            'traced_func_simple_caller',), self.filemod + (
            'traced_func_linear',)): 1, (self.filemod + (
            'traced_func_importing_caller',), self.filemod + (
            'traced_func_simple_caller',)): 1, (self.filemod + (
            'traced_func_importing_caller',), self.filemod + (
            'traced_func_importing',)): 1, (self.filemod + (
            'traced_func_importing',), (fix_ext_py(testmod.__file__),
            'testmod', 'func')): 1}
...
class TestCoverage(unittest.TestCase):
    ...
    def _coverage(self, tracer, cmd=
        'import test.support, test.test_pprint;test.support.run_unittest(test.test_pprint.QueryTestCase)'
        ):
        tracer.run(cmd)
        r = tracer.results()
    ...
    def test_coverage_ignore(self):
        ...
        tracer = trace.Trace(ignoredirs=[sys.base_prefix, sys.
            base_exec_prefix, libpath], trace=0, count=1)
        with captured_stdout() as stdout:
            self._coverage(tracer)
...
class TestCommandLine(unittest.TestCase):
    ...
    def test_failures(self):
        _errors = (b'filename is missing: required with the main options',
            '-l', '-T'), (
