...
class TracebackCases(unittest.TestCase):
    ...
    def test_caret(self):
        ...
        self.assertEqual(err[1].find('+'), err[2].find('^'))
        err = self.get_exception_format(self.
            syntax_error_with_caret_non_ascii, SyntaxError)
        self.assertIn('^', err[2])
        self.assertEqual(err[2].count('\n'), 1)
        self.assertEqual(err[1].find('+'), err[2].find('^'))
    ...
    def test_encoded_file(self):
        ...
        process = subprocess.Popen([sys.executable, '-c',
            'import sys; print(sys.stdout.encoding)'], stdout=subprocess.
        ...
        def do_test(firstlines, message, charset, lineno):
            try:
                ...
                process = subprocess.Popen([sys.executable, TESTFN], stdout
            ...
            self.assertIn('line %s' % lineno, stdout[1],
                'Invalid line number: {0!r} instead of {1}'.format(stdout[1
                ], lineno))
    ...
    def test_print_exception(self):
        output = StringIO()
        traceback.print_exception(Exception, Exception('projector'), None,
            file=output)
...
class TracebackFormatTests(unittest.TestCase):
    ...
    def test_print_stack(self):
        ...
        self.assertEqual(stderr.getvalue().splitlines()[-4:], [
            '  File "%s", line %d, in test_print_stack' % (__file__, lineno +
            3), '    prn()', '  File "%s", line %d, in prn' % (__file__, 
            lineno + 1), '    traceback.print_stack()'])
...
"""
        ...
        self.assertIn(int(re.search('\\d+', actual[-2]).group()), range(
            rec_limit - 60, rec_limit))
...
"""
boundaries = re.compile('(%s|%s)' % (re.escape(cause_message), re.escape(
    context_message)))
...
class LimitTests(unittest.TestCase):
    ...
    def test_extract_stack(self):
        ...
        with support.swap_attr(sys, 'tracebacklimit', 1000):
            ...
            self.assertEqual(extract(limit=-2), nolim[:2])
            assertEqualExcept(extract(limit=-100), nolim[:100], len(nolim) -
                5 - 1)
    ...
    def test_format_exception(self):
        ...
        def extract(**kwargs):
            return traceback.format_exception(exc_type, exc_value, tb, **kwargs
                )[1:-1]
        with support.swap_attr(sys, 'tracebacklimit', 1000):
            nolim = extract()
            self.assertEqual(len(nolim), 5 + 1)
...
class MiscTracebackCases(unittest.TestCase):
    ...
    def test_extract_stack(self):
        ...
        self.assertEqual(result[-2:], [(__file__, lineno + 2,
            'test_extract_stack', 'result = extract()'), (__file__, lineno +
            1, 'extract', 'return traceback.extract_stack()')])
...
class TestFrame(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        self.assertEqual(f, ('f', 1, 'dummy',
            '"""Test cases for traceback module"""'))
        self.assertEqual(tuple(f), ('f', 1, 'dummy',
            '"""Test cases for traceback module"""'))
...
class TestTracebackException(unittest.TestCase):
    ...
    def test_smoke(self):
        try:
            ...
            expected_stack = traceback.StackSummary.extract(traceback.
                walk_tb(exc_info[2]))
    ...
    def test_from_exception(self):
        ...
        try:
            ...
            exc_info = sys.exc_info()
            self.expected_stack = traceback.StackSummary.extract(traceback.
                walk_tb(exc_info[2]), limit=1, lookup_lines=False,
                capture_locals=True)
            self.exc = traceback.TracebackException.from_exception(e, limit
                =1, lookup_lines=False, capture_locals=True)
    ...
    def test_locals(self):
        ...
        tb = test_tb(f, 6, None)
        exc = traceback.TracebackException(Exception, e, tb, capture_locals
            =True)
        self.assertEqual(exc.stack[0].locals, {'something': '1', 'other':
            "'string'"})
