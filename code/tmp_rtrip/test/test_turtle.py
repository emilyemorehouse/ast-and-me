...
class TurtleConfigTest(unittest.TestCase):
    ...
    def test_config_dict(self):
        ...
        expected = {'width': 0.75, 'height': 0.8, 'canvwidth': 500,
            'canvheight': 200, 'leftright': 100, 'topbottom': 100, 'mode':
            'world', 'colormode': 255, 'delay': 100, 'undobuffersize': 
            10000, 'shape': 'circle', 'pencolor': 'red', 'fillcolor':
            'blue', 'resizemode': 'auto', 'visible': None, 'language':
            'english', 'exampleturtle': 'turtle', 'examplescreen': 'screen',
            'title': 'Python Turtle Graphics', 'using_IDLE': ''}
    ...
    def test_partial_config_dict_with_commments(self):
        ...
        parsed_cfg = turtle.config_dict(cfg_name)
        expected = {'pencolor': 'red', 'fillcolor': 'blue', 'visible':
            False, 'language': 'english', 'using_IDLE': False}
        self.assertEqual(parsed_cfg, expected)
...
class VectorComparisonMixin:
    ...
    def assertVectorsAlmostEqual(self, vec1, vec2):
        ...
        for idx, (i, j) in enumerate(zip(vec1, vec2)):
            self.assertAlmostEqual(i, j, msg=
                'values at index {} do not match'.format(idx))
...
class TestVec2D(VectorComparisonMixin, unittest.TestCase):
    ...
    def test_vector_addition(self):
        test_cases = [(((0, 0), (1, 1)), (1.0, 1.0)), (((-1, 0), (2, 2)), (
            1, 2)), (((1.5, 0), (1, 1)), (2.5, 1))]
        self._assert_arithmetic_cases(test_cases, lambda x, y: x + y)

    def test_vector_subtraction(self):
        test_cases = [(((0, 0), (1, 1)), (-1, -1)), (((10.625, 0.125), (10,
            0)), (0.625, 0.125))]
        self._assert_arithmetic_cases(test_cases, lambda x, y: x - y)
    ...
    def test_rotate(self):
        cases = [(((0, 0), 0), (0, 0)), (((0, 1), 90), (-1, 0)), (((0, 1), 
            -90), (1, 0)), (((1, 0), 180), (-1, 0)), (((1, 0), 360), (1, 0))]
        for case in cases:
...
class TestTNavigator(VectorComparisonMixin, unittest.TestCase):
    ...
    def test_towards(self):
        coordinates = [((100, 0), 0.0), ((100, 100), 45.0), ((0, 100), 90.0
            ), ((-100, 100), 135.0), ((-100, 0), 180.0), ((-100, -100), 
            225.0), ((0, -100), 270.0), ((100, -100), 315.0)]
