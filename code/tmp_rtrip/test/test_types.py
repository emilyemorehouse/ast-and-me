...
class TypesTests(unittest.TestCase):
    ...
    def test_normal_integers(self):
        ...
        for divisor in (1, 2, 4, 8, 16, 32):
            ...
            prod = divisor * j
            if type(prod) is not int:
                self.fail('expected type(%r) to be int, not %r' % (prod,
                    type(prod)))
        m = sys.maxsize
        for divisor in (1, 2, 4, 8, 16, 32):
            j = m // divisor + 1
    ...
    def test_int__format__(self):
        ...
        for format_spec in 'eEfFgG%':
            for value in [0, 1, -1, 100, -100, 1234567890, -1234567890]:
                self.assertEqual(value.__format__(format_spec), float(value
    ...
    def test_float__format__locale(self):
        for i in range(-10, 10):
            ...
            self.assertEqual(locale.format('%g', x, grouping=True), format(
                x, 'n'))
            self.assertEqual(locale.format('%.10g', x, grouping=True),
                format(x, '.10n'))
    ...
    def test_int__format__locale(self):
        ...
        for i in range(0, 30):
            self.assertEqual(locale.format('%d', x, grouping=True), format(
                x, 'n'))
            x = x // 10
        rfmt = '>20n'
        lfmt = '<20n'
        cfmt = '^20n'
        for x in (1234, 12345, 123456, 1234567, 12345678, 123456789, 
    ...
    def test_format_spec_errors(self):
        ...
        self.assertRaises(ValueError, format, 0, '.' + '1' * 10000 + 'd')
        self.assertRaises(ValueError, format, 0, '1' * 1000 + '.' + '1' * 
            10000 + 'd')
        for code in 'xXobns':
...
class MappingProxyTests(unittest.TestCase):
    ...
    def test_methods(self):
        ...
        self.assertEqual(attrs, {'__contains__', '__getitem__', '__iter__',
            '__len__', 'copy', 'get', 'items', 'keys', 'values'})
...
class ClassCreationTests(unittest.TestCase):
    ...
    def test_new_class_metaclass_keywords(self):
        ...
        res = types.new_class('X', (int, object), dict(metaclass=meta_func,
            x=0))
    ...
    def test_new_class_meta_with_base(self):
        ...
        def func(ns):
            ns['x'] = 0
        C = types.new_class(name='C', bases=(int,), kwds=dict(metaclass=
            Meta, z=2), exec_body=func)
        self.assertTrue(issubclass(C, int))
        self.assertIsInstance(C, Meta)
        self.assertEqual(C.x, 0)
        self.assertEqual(C.y, 1)
...
class CoroutineTests(unittest.TestCase):
    ...
    def test_gen(self):
        ...
        for name in ('__name__', '__qualname__', 'gi_code', 'gi_running',
            'gi_frame'):
    ...
    def test_wrapper_object(self):
        ...
        self.assertTrue(set(dir(wrapper)).issuperset({'__await__',
            '__iter__', '__next__', 'cr_code', 'cr_running', 'cr_frame',
            'gi_code', 'gi_frame', 'gi_running', 'send', 'close', 'throw'}))
