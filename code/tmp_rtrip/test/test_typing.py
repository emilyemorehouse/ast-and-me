...
class UnionTests(BaseTestCase):
    ...
    def test_union_generalization(self):
        ...
        self.assertFalse(Union[str, typing.Iterable[int]] == typing.
            Iterable[int])
...
class CallableTests(BaseTestCase):
    ...
    def test_callable_with_ellipsis(self):
        ...
        def foo(a: Callable[..., T]):
            pass
        self.assertEqual(get_type_hints(foo, globals(), locals()), {'a':
            Callable[..., T]})
...
class GenericTests(BaseTestCase):
    ...
    def test_repr(self):
        self.assertEqual(repr(SimpleMapping), __name__ + '.' + 'SimpleMapping')
        self.assertEqual(repr(MySimpleMapping), __name__ + '.' +
            'MySimpleMapping')
    ...
    def test_new_repr_complex(self):
        T = TypeVar('T')
        TS = TypeVar('TS')
        self.assertEqual(repr(typing.Mapping[T, TS][TS, T]),
            'typing.Mapping[~TS, ~T]')
    ...
    def test_naive_runtime_checks(self):
        ...
        def naive_dict_check(obj, tp):
            ...
            if tp.__args__:
                KT, VT = tp.__args__
                return all(isinstance(k, KT) and isinstance(v, VT) for k, v in
                    obj.items())
        ...
        def naive_list_base_check(obj, tp):
            return all(isinstance(x, tp.__orig_bases__[0].__args__[0]) for
                x in obj)
    ...
    def test_subscript_meta(self):
        ...
        self.assertEqual(Callable[..., GenericMeta].__args__, (Ellipsis,
            GenericMeta))
    ...
    def test_generic_hashes(self):
        ...
        self.assertNotEqual(Union[str, A[str]], Union[str,
            mod_generics_cache.A[str]])
        self.assertNotEqual(Union[A[str], A[str]], Union[A[str],
            mod_generics_cache.A[str]])
        self.assertNotEqual(typing.FrozenSet[A[str]], typing.FrozenSet[
            mod_generics_cache.B.A[str]])
        if sys.version_info[:2] > (3, 2):
            self.assertTrue(repr(Tuple[A[str]]).endswith('<locals>.A[str]]'))
            self.assertTrue(repr(Tuple[B.A[str]]).endswith(
                '<locals>.B.A[str]]'))
            self.assertTrue(repr(Tuple[mod_generics_cache.A[str]]).endswith
                ('mod_generics_cache.A[str]]'))
    ...
    def test_extended_generic_rules_eq(self):
        ...
        self.assertEqual(typing.Iterable[Tuple[T, T]][T], typing.Iterable[
            Tuple[T, T]])
        with self.assertRaises(TypeError):
            Tuple[T, int][()]
        with self.assertRaises(TypeError):
    ...
    def test_extended_generic_rules_repr(self):
        ...
        self.assertEqual(repr(Union[Tuple, Tuple[int]]).replace('typing.',
            ''), 'Tuple')
        self.assertEqual(repr(Callable[..., Optional[T]][int]).replace(
            'typing.', ''), 'Callable[..., Union[int, NoneType]]')
    ...
    def test_all_repr_eq_any(self):
        ...
        for obj in objs:
            ...
            if getattr(obj, '__parameters__', None) and len(obj.__parameters__
                ) == 1:
    ...
    def test_substitution_helper(self):
        ...
        new_annots = {k: typing._replace_arg(v, type(obj).__parameters__,
            new_args) for k, v in obj.meth.__annotations__.items()}
    ...
    def test_copy_and_deepcopy(self):
        ...
        class Node(Generic[T]):
            ...
        things = [Union[T, int], Tuple[T, int], Callable[..., T], Callable[
            [int], int], Tuple[Any, Any], Node[T], Node[int], Node[Any],
            typing.Iterable[T], typing.Iterable[Any], typing.Iterable[int],
            typing.Dict[int, str], typing.Dict[T, Any], ClassVar[int],
            ClassVar[List[T]], Tuple['T', 'T'], Union['T', int], List['T'],
    ...
    def test_weakref_all(self):
        ...
        things = [Any, Union[T, int], Callable[..., T], Tuple[Any, Any],
            Optional[List[int]], typing.Mapping[int, str], typing.re.Match[
            bytes], typing.Iterable['whatever']]
        for t in things:
    ...
    def test_parameterized_slots(self):
        ...
        self.assertEqual(get_type_hints(foo, globals(), locals())['x'].
            __slots__, C.__slots__)
    ...
    def test_type_erasure(self):
        ...
        class Node(Generic[T]):
            ...
            def __init__(self, label: T, left: 'Node[T]'=None, right:
                'Node[T]'=None):
                self.label = label
                self.left = left
                self.right = right
...
class ForwardRefTests(BaseTestCase):
    ...
    def test_basics(self):
        ...
        class Node(Generic[T]):
            ...
            def add_both(self, left: 'Optional[Node[T]]', right: 'Node[T]'=
                None, stuff: int=None, blah=None):
                self.left = left
                self.right = right
...
class GetTypeHintTests(BaseTestCase):
    ...
    def test_get_type_hints_modules(self):
        ann_module_type_hints = {(1): 2, 'f': Tuple[int, int], 'x': int,
            'y': str}
    ...
    def test_get_type_hints_classes(self):
        self.assertEqual(gth(ann_module.C, ann_module.__dict__), {'y':
            Optional[ann_module.C]})
        self.assertIsInstance(gth(ann_module.j_class), dict)
        self.assertEqual(gth(ann_module.M), {'123': 123, 'o': type})
        self.assertEqual(gth(ann_module.D), {'j': str, 'k': str, 'y':
            Optional[ann_module.C]})
        ...
        self.assertEqual(gth(NoneAndForward, globals()), {'parent':
            NoneAndForward, 'meaning': type(None)})
    ...
    def test_get_type_hints_ClassVar(self):
        self.assertEqual(gth(ann_module2.CV, ann_module2.__dict__), {'var':
            typing.ClassVar[ann_module2.CV]})
        self.assertEqual(gth(B, globals()), {'y': int, 'x': ClassVar[
            Optional[B]], 'b': int})
        self.assertEqual(gth(CSub, globals()), {'z': ClassVar[CSub], 'y':
            int, 'b': int, 'x': ClassVar[Optional[B]]})
...
class CollectionsAbcTests(BaseTestCase):
    ...
    def test_defaultdict_instantiation(self):
        ...
        self.assertIs(type(typing.DefaultDict[KT, VT]()), collections.
            defaultdict)
        self.assertIs(type(typing.DefaultDict[str, int]()), collections.
            defaultdict)
...
class NamedTupleTests(BaseTestCase):
    ...
    def test_basics(self):
        ...
        self.assertEqual(Emp.__annotations__, collections.OrderedDict([(
            'name', str), ('id', int)]))
    ...
    def test_annotation_usage(self):
        ...
        self.assertEqual(CoolEmployee.__annotations__, collections.
            OrderedDict(name=str, cool=int))
    ...
    def test_annotation_usage_with_default(self):
        ...
        self.assertEqual(CoolEmployeeWithDefault._field_types, dict(name=
            str, cool=int))
    ...
    def test_cannot_subclass(self):
        with self.assertRaises(TypeError) as ex:
            ...
            class A(typing.Match):
                pass
        self.assertEqual(str(ex.exception), 'Cannot subclass typing._TypeAlias'
            )
