...
class DigestAuthHandler:
    ...
    def _return_auth_challenge(self, request_handler):
        ...
        request_handler.send_header('Proxy-Authenticate', 
            'Digest realm="%s", qop="%s",nonce="%s", ' % (self._realm_name,
            self._qop, self._generate_nonce()))
    ...
    def handle_request(self, request_handler):
        ...
        if 'Proxy-Authorization' not in request_handler.headers:
            ...
            auth_dict = self._create_auth_dict(request_handler.headers[
                'Proxy-Authorization'])
            ...
            for path in [request_handler.path, request_handler.short_path]:
                if self._validate_auth(auth_dict, password, request_handler
                    .command, path):
                    auth_validated = True
...
class FakeProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        scm, netloc, path, params, query, fragment = urllib.parse.urlparse(self
            .path, 'http')
...
class BasicAuthTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = LoopbackHttpServerThread(
    ...
    def test_basic_auth_httperror(self):
        ah = urllib.request.HTTPBasicAuthHandler()
        ah.add_password(self.REALM, self.server_url, self.USER, self.
            INCORRECT_PASSWD)
        urllib.request.install_opener(urllib.request.build_opener(ah))
...
class ProxyAuthTests(unittest.TestCase):
    ...
    def test_proxy_with_bad_password_raises_httperror(self):
        self.proxy_digest_handler.add_password(self.REALM, self.URL, self.
            USER, self.PASSWD + 'bad')
...
class TestUrlopen(unittest.TestCase):
    ...
    def test_redirection(self):
        ...
        responses = [(302, [('Location',
            'http://localhost:%(port)s/somewhere_else')], ''), (200, [],
            expected_response)]
    ...
    def test_https_with_cafile(self):
        ...
        with support.check_warnings(('', DeprecationWarning)):
            data = self.urlopen('https://localhost:%s/bizarre' % handler.
                port, cafile=CERT_localhost)
            self.assertEqual(data, b'we care a bit')
    ...
    def test_https_sni(self):
        ...
        handler = self.start_https_server(context=context, certfile=
            CERT_localhost)
        context = ssl.create_default_context(cafile=CERT_localhost)
    ...
    def test_basic(self):
        ...
        for attr in ('read', 'close', 'info', 'geturl'):
            self.assertTrue(hasattr(open_url, attr), 
