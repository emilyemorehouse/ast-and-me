        ...
        with self.urlopen(self.url) as open_url:
            for attr in ('read', 'readline', 'readlines', 'fileno', 'close',
                ...
                self.assertTrue(hasattr(open_url, attr), 
                    'object returned from urlopen lacks the %s attribute' %
                    attr)
    ...
    def test_bad_address(self):
        ...
        try:
            ...
            self.skipTest('%r should not resolve for test to work' %
                bogus_domain)
        failure_explanation = (
            'opening an invalid URL did not raise OSError; can be caused by a broken DNS server (e.g. returns 404 or hijacks page)'
            )
...
class urlretrieveNetworkTests(unittest.TestCase):
    ...
    def test_specified_path(self):
        with self.urlretrieve(self.logo, support.TESTFN) as (file_location,
            info):
    ...
    def test_reporthook(self):
        ...
        self.assertGreater(len(records), 1, msg=
            'There should always be two calls; the first one before the transfer starts.'
            )
        self.assertEqual(records[0][0], 0)
        self.assertGreater(records[0][1], 0, msg=
            "block size can't be 0 in %s" % records_repr)
        ...
        self.assertEqual({records[0][1]}, block_sizes, msg=
            'block sizes in %s must be equal' % records_repr)
        self.assertGreaterEqual(records[-1][0] * records[0][1],
            expected_size, msg=
