...
class FilterTests(BaseTest):
    ...
    def test_once(self):
        with original_warnings.catch_warnings(record=True, module=self.module
            ...
            self.assertEqual(len(w), 0)
            self.module.warn_explicit(message, UserWarning,
                'test_warnings2.py', 42)
            self.assertEqual(len(w), 0)
    ...
    def test_filterwarnings_duplicate_filters(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            self.assertEqual(self.module.filters[0][0], 'error',
    ...
    def test_append_duplicate(self):
        with original_warnings.catch_warnings(module=self.module, record=True
            ) as w:
            self.module.resetwarnings()
...
class WarnTests(BaseTest):
    ...
    def test_warn_nonstandard_types(self):
        for ob in (Warning, None, 42):
            with original_warnings.catch_warnings(record=True, module=self.
                module) as w:
                self.module.simplefilter('once')
                self.module.warn(ob)
    ...
    def test_missing_filename_not_main(self):
        ...
        try:
            ...
            with warnings_state(self.module):
                with original_warnings.catch_warnings(record=True, module=
                    self.module) as w:
    ...
    def test_warn_explicit_type_errors(self):
        self.assertRaises(TypeError, self.module.warn_explicit, None,
            UserWarning, None, None)
        self.assertRaises(TypeError, self.module.warn_explicit, None, None,
            None, 1)
        self.assertRaises((TypeError, AttributeError), self.module.
            warn_explicit, None, Warning, None, 1, registry=42)
    ...
    def test_bad_str(self):
        ...
        class BadStrWarning(Warning):
            ...
            def __str__(self):
                return 'A bad formatted string %(err)' % {'err':
                    'there is no %(err)s'}
    ...
    def test_warning_classes(self):
        ...
        self.assertIn('category must be a Warning subclass, not ', str(cm.
            exception))
        with self.assertRaises(TypeError) as cm:
...
class WCmdLineTests(BaseTest):
    ...
    def test_improper_input(self):
        with original_warnings.catch_warnings(module=self.module):
            self.assertRaises(self.module._OptionError, self.module.
                _setoption, '1:2:3:4:5:6')
            self.assertRaises(self.module._OptionError, self.module.
                _setoption, 'bogus::Warning')
            self.assertRaises(self.module._OptionError, self.module.
                _setoption, 'ignore:2::4:-5')
...
class _WarningsTests(BaseTest, unittest.TestCase):
    ...
    def test_default_action(self):
        ...
        try:
            with original_warnings.catch_warnings(record=True, module=self.
                ...
                self.module.resetwarnings()
                registry = {}
                self.module.warn_explicit(message, UserWarning, '<test>', 
    ...
    def test_filename_none(self):
        ...
        try:
            catch = original_warnings.catch_warnings(record=True, module=
                self.module)
            with catch as w:
                self.module.filterwarnings('always', category=UserWarning)
...
class EnvironmentVariableTests(BaseTest):
    ...
    def test_envvar_and_command_line(self):
        rc, stdout, stderr = assert_python_ok('-Wignore::UnicodeWarning',
    ...
    def test_nonascii(self):
        rc, stdout, stderr = assert_python_ok('-c',
            'import sys; sys.stdout.write(str(sys.warnoptions))',
            PYTHONIOENCODING='utf-8', PYTHONWARNINGS=
            'ignore:DeprecaciónWarning')
        self.assertEqual(stdout, "['ignore:DeprecaciónWarning']".encode(
            'utf-8'))
