...
class ChromeCommandTest(CommandTestMixin, unittest.TestCase):
    ...
    def test_open_with_autoraise_false(self):
        self._test('open', kw=dict(autoraise=False), options=[], arguments=
            [URL])
...
class NetscapeCommandTest(CommandTestMixin, unittest.TestCase):
    ...
    def test_open(self):
        self._test('open', options=['-raise', '-remote'], arguments=[
            'openURL({})'.format(URL)])

    def test_open_with_autoraise_false(self):
        self._test('open', kw=dict(autoraise=False), options=['-noraise',
            '-remote'], arguments=['openURL({})'.format(URL)])

    def test_open_new(self):
        self._test('open_new', options=['-raise', '-remote'], arguments=[
            'openURL({},new-window)'.format(URL)])

    def test_open_new_tab(self):
        self._test('open_new_tab', options=['-raise', '-remote'], arguments
            =['openURL({},new-tab)'.format(URL)])
...
class OperaCommandTest(CommandTestMixin, unittest.TestCase):
    ...
    def test_open(self):
        self._test('open', options=['-remote'], arguments=['openURL({})'.
            format(URL)])

    def test_open_with_autoraise_false(self):
        self._test('open', kw=dict(autoraise=False), options=['-remote',
            '-noraise'], arguments=['openURL({})'.format(URL)])

    def test_open_new(self):
        self._test('open_new', options=['-remote'], arguments=[
            'openURL({},new-window)'.format(URL)])

    def test_open_new_tab(self):
        self._test('open_new_tab', options=['-remote'], arguments=[
            'openURL({},new-page)'.format(URL)])
