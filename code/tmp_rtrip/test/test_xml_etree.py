    ...
    def pickleRoundTrip(self, obj, name, dumper, loader, proto):
        ...
        try:
            ...
            raise support.TestFailed(
                'Failed to round-trip %r from %r to %r' % (obj, human.get(
                dumper, dumper), human.get(loader, loader))) from pe
        finally:
...
class ElementTreeTest(unittest.TestCase):
    ...
    def test_interface(self):
        ...
        def check_string(string):
            len(string)
            for char in string:
                self.assertEqual(len(char), 1, msg=
                    'expected one-character string, got %r' % char)
            new_string = string + ''
            new_string = string + ' '
            string[:0]
        ...
        def check_mapping(mapping):
            ...
            for key in keys:
                item = mapping[key]
            mapping['key'] = 'value'
            self.assertEqual(mapping['key'], 'value', msg=
                'expected value string, got %r' % mapping['key'])

        def check_element(element):
            self.assertTrue(ET.iselement(element), msg='not an element')
            direlem = dir(element)
            for attr in ('tag', 'attrib', 'text', 'tail'):
                self.assertTrue(hasattr(element, attr), msg='no %s member' %
                    attr)
        ...
        def check_method(method):
            self.assertTrue(hasattr(method, '__call__'), msg=
    ...
    def test_attrib(self):
        ...
        self.assertEqual(sorted(elem.items()), [('testa', 'testval'), (
            'testb', 'test2')])
   ...
   <empty-element />
    ...
    def test_iterparse(self):
        ...
        self.assertEqual([(action, elem.tag) for action, elem in context],
            ...
            ('end', '{namespace}empty-element'), ('end', '{namespace}root')])
        events = ()
        ...
        context = iterparse(SIMPLE_XMLFILE, events)
        self.assertEqual([(action, elem.tag) for action, elem in context],
            [('start', 'root'), ('start', 'element'), ('end', 'element'), (
            'start', 'element'), ('end', 'element'), ('start',
            'empty-element'), ('end', 'empty-element'), ('end', 'root')])
        events = 'start', 'end', 'start-ns', 'end-ns'
        context = iterparse(SIMPLE_NS_XMLFILE, events)
        self.assertEqual([((action, elem.tag) if action in ('start', 'end')
             else (action, elem)) for action, elem in context], [(
            'start-ns', ('', 'namespace')), ('start', '{namespace}root'), (
            'start', '{namespace}element'), ('end', '{namespace}element'),
            ('start', '{namespace}element'), ('end', '{namespace}element'),
            ('start', '{namespace}empty-element'), ('end',
            '{namespace}empty-element'), ('end', '{namespace}root'), (
            'end-ns', None)])
        ...
        self.assertEqual([(action, elem) for action, elem in context], [(
            'start-ns', ('', 'http://éffbot.org/ns')), ('start-ns', ('clé',
            'http://effbot.org/ns'))])
    ...
    def test_custom_builder(self):
        ...
        parser.feed(data)
        self.assertEqual(builder, [('start', 'root'), ('start', 'element'),
            ('end', 'element'), ('start', 'element'), ('end', 'element'), (
            'start', 'empty-element'), ('end', 'empty-element'), ('end',
            'root')])
        ...
        parser = ET.XMLParser(target=builder)
        parser.feed(data)
        self.assertEqual(builder, [('pi', 'pi', 'data'), ('comment',
            ' comment '), ('start', '{namespace}root'), ('start',
            '{namespace}element'), ('end', '{namespace}element'), ('start',
            '{namespace}element'), ('end', '{namespace}element'), ('start',
            '{namespace}empty-element'), ('end', '{namespace}empty-element'
            ), ('end', '{namespace}root')])

    def test_getchildren(self):
        with open(SIMPLE_XMLFILE, 'rb') as f:
            tree = ET.parse(f)
        self.assertEqual([summarize_list(elem.getchildren()) for elem in
            tree.getroot().iter()], [['element', 'element', 'empty-element'
            ], [], [], []])
        self.assertEqual([summarize_list(elem.getchildren()) for elem in
            tree.getiterator()], [['element', 'element', 'empty-element'],
            [], [], []])
        elem = ET.XML(SAMPLE_XML)
    ...
    def test_encoding(self):
        ...
        supported_encodings = ['ascii', 'utf-8', 'utf-8-sig', 'utf-16',
            'utf-16be', 'utf-16le', 'iso8859-1', 'iso8859-2', 'iso8859-3',
            'iso8859-4', 'iso8859-5', 'iso8859-6', 'iso8859-7', 'iso8859-8',
            'iso8859-9', 'iso8859-10', 'iso8859-13', 'iso8859-14',
            'iso8859-15', 'iso8859-16', 'cp437', 'cp720', 'cp737', 'cp775',
            'cp850', 'cp852', 'cp855', 'cp856', 'cp857', 'cp858', 'cp860',
            'cp861', 'cp862', 'cp863', 'cp865', 'cp866', 'cp869', 'cp874',
            'cp1006', 'cp1125', 'cp1250', 'cp1251', 'cp1252', 'cp1253',
            'cp1254', 'cp1255', 'cp1256', 'cp1257', 'cp1258',
            'mac-cyrillic', 'mac-greek', 'mac-iceland', 'mac-latin2',
            'mac-roman', 'mac-turkish', 'iso2022-jp', 'iso2022-jp-1',
            'iso2022-jp-2', 'iso2022-jp-2004', 'iso2022-jp-3',
            'iso2022-jp-ext', 'koi8-r', 'koi8-t', 'koi8-u', 'kz1048', 'hz',
            'ptcp154']
        for encoding in supported_encodings:
            self.assertEqual(ET.tostring(ET.XML(bxml(encoding))), b'<xml />')
        unsupported_ascii_compatible_encodings = ['big5', 'big5hkscs',
            'cp932', 'cp949', 'cp950', 'euc-jp', 'euc-jis-2004',
            'euc-jisx0213', 'euc-kr', 'gb2312', 'gbk', 'gb18030',
            'iso2022-kr', 'johab', 'shift-jis', 'shift-jis-2004',
            'shift-jisx0213', 'utf-7']
        for encoding in unsupported_ascii_compatible_encodings:
            self.assertRaises(ValueError, ET.XML, bxml(encoding))
        unsupported_ascii_incompatible_encodings = ['cp037', 'cp424',
            'cp500', 'cp864', 'cp875', 'cp1026', 'cp1140', 'utf_32',
            'utf_32_be', 'utf_32_le']
  ...
  <ns0:section>
    ...
    def test_xpath_tokenizer(self):
        ...
        def check(p, expected):
            self.assertEqual([(op or tag) for op, tag in ElementPath.
                xpath_tokenizer(p)], expected)
        check('*', ['*'])
        check('text()', ['text', '()'])
        check('@name', ['@', 'name'])
        check('@*', ['@', '*'])
        ...
        check('*/para', ['*', '/', 'para'])
        check('/doc/chapter[5]/section[2]', ['/', 'doc', '/', 'chapter',
            '[', '5', ']', '/', 'section', '[', '2', ']'])
    ...
    def test_processinginstruction(self):
        self.assertEqual(ET.tostring(ET.ProcessingInstruction('test',
            'instruction')), b'<?test instruction?>')
        self.assertEqual(ET.tostring(ET.PI('test', 'instruction')),
            b'<?test instruction?>')
        self.assertEqual(ET.tostring(ET.PI('test', '<testing&>')),
            b'<?test <testing&>?>')
        self.assertEqual(ET.tostring(ET.PI('test', '<testing&>ã'),
            'latin-1'),
            b"<?xml version='1.0' encoding='latin-1'?>\n<?test <testing&>\xe3?>"
            )

    def test_html_empty_elems_serialization(self):
        for element in ['AREA', 'BASE', 'BASEFONT', 'BR', 'COL', 'FRAME',
...
class XMLPullParserTest(unittest.TestCase):
    ...
    def test_simple_xml(self):
        for chunk_size in (None, 1, 5):
            with self.subTest(chunk_size=chunk_size):
                ...
                self.assert_event_tags(parser, [('end', 'element'), ('end',
                    'empty-element')])
    ...
    def test_ns_events(self):
        ...
        self._feed(parser, '<!-- comment -->\n')
        self._feed(parser, "<root xmlns='namespace'>\n")
        self.assertEqual(list(parser.read_events()), [('start-ns', ('',
            'namespace'))])
        self._feed(parser, "<element key='value'>text</element")
        self._feed(parser, '>\n')
        self._feed(parser, '<element>text</element>tail\n')
    ...
    def test_events(self):
        ...
        self.assert_event_tags(parser, [('start', '{foo}element'), ('start',
            '{foo}empty-element'), ('end', '{foo}empty-element'), ('end',
            '{foo}element')])
        self._feed(parser, '</root>')
        self.assertIsNone(parser.close())
        self.assert_event_tags(parser, [('end', 'root')])
...
class BugsTest(unittest.TestCase):
    ...
    def test_bug_xmltoolkit54(self):
        e = ET.XML(
            "<!DOCTYPE doc [<!ENTITY ldots '&#x8230;'>]><doc>&ldots;</doc>")
        self.assertEqual(serialize(e, encoding='us-ascii'),
            b'<doc>&#33328;</doc>')
    ...
    def test_bug_xmltoolkit55(self):
        with self.assertRaises(ET.ParseError) as cm:
            ET.XML(
                b"<!DOCTYPE doc SYSTEM 'doc.dtd'><doc>&ldots;&ndots;&rdots;</doc>"
                )
        self.assertEqual(str(cm.exception),
...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    def test_bug_200709_default_namespace(self):
        ...
        self.assertEqual(serialize(e, default_namespace='default'),
    ...
    def test_issue6233(self):
        e = ET.XML(
            b"<?xml version='1.0' encoding='utf-8'?><body>t\xc3\xa3g</body>")
        self.assertEqual(ET.tostring(e, 'ascii'),
            b"<?xml version='1.0' encoding='ascii'?>\n<body>t&#227;g</body>")
        e = ET.XML(
            b"<?xml version='1.0' encoding='iso-8859-1'?><body>t\xe3g</body>")
        self.assertEqual(ET.tostring(e, 'ascii'),
            b"<?xml version='1.0' encoding='ascii'?>\n<body>t&#227;g</body>")
    ...
    def test_pickle(self):
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            for dumper, loader in product(self.modules, repeat=2):
                ...
                dumper.SubElement(e, 'child').append(dumper.Element(
                    'grandchild'))
...
class BadElementTest(ElementTestCase, unittest.TestCase):
    ...
    def test_element_iter(self):
        state = {'tag': 'tag', '_children': [None], 'attrib': 'attr',
            'tail': 'tail', 'text': 'text'}
        e = ET.Element('tag')
        try:
...
class ElementTreeTypeTest(unittest.TestCase):
    ...
    def test_Element_subclass_constructor(self):
        ...
        self.assertEqual(sorted(mye.items()), [('a', 1), ('b', 2), ('c', 3),
            ('d', 4)])
...
class ElementFindTest(unittest.TestCase):
    ...
    def test_test_find_with_ns(self):
        ...
        self.assertEqual(summarize_list(e.findall(
            '{http://effbot.org/ns}tag')), ['{http://effbot.org/ns}tag'] * 2)
        self.assertEqual(summarize_list(e.findall(
            './/{http://effbot.org/ns}tag')), ['{http://effbot.org/ns}tag'] * 3
            )
...
class ElementIterTest(unittest.TestCase):
    ...
    def test_iter_by_tag(self):
        ...
        all_tags = ['document', 'house', 'room', 'room', 'shed', 'house',
            'room']
    ...
    def test_getiterator(self):
        ...
        self.assertEqual(summarize_list(doc.getiterator('room')), ['room'] * 3)
        self.assertEqual(summarize_list(doc.getiterator('house')), ['house'
            ] * 2)
        self.assertEqual(summarize_list(doc.getiterator(tag='room')), [
            'room'] * 3)
        all_tags = ['document', 'house', 'room', 'room', 'shed', 'house',
            'room']
...
class TreeBuilderTest(unittest.TestCase):
    sample1 = (
        '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html>text<div>subtext</div>tail</html>'
        )
    ...
    def test_doctype(self):
        ...
        parser.feed(self.sample1)
        self.assertEqual(parser.close(), ('html',
...
class XMLParserTest(unittest.TestCase):
    ...
    sample2 = (
        b'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html>text</html>'
        )
    sample3 = (
    ...
    def test_constructor_args(self):
        ...
        parser2 = ET.XMLParser(encoding='utf-8', html=[{}], target=ET.
            TreeBuilder())
    ...
    def test_subclass_doctype(self):
        ...
        with self.assertWarns(DeprecationWarning):
            parser.feed(self.sample2)
        parser.close()
        self.assertEqual(_doctype, ('html',
            '-//W3C//DTD XHTML 1.0 Transitional//EN',
            'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'))
        ...
        with warnings.catch_warnings():
            ...
            self.assertEqual(_doctype2, ('html',
                '-//W3C//DTD XHTML 1.0 Transitional//EN',
                'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'))
...
class ElementSlicingTest(unittest.TestCase):
    ...
    def test_setslice_steps(self):
        ...
        self.assertEqual(self._subelem_tags(e), ['a0', 'b0', 'a2', 'b1',
            'a4', 'a5'])
        e = self._make_elem_with_children(6)
        with self.assertRaises(ValueError):
...
<tag key="åöö&lt;&gt;" />"""
    ...
    def test_tostringlist_invariant(self):
        ...
        self.assertEqual(ET.tostring(root, 'unicode'), ''.join(ET.
            tostringlist(root, 'unicode')))
        self.assertEqual(ET.tostring(root, 'utf-16'), b''.join(ET.
            tostringlist(root, 'utf-16')))

    def test_short_empty_elements(self):
        root = ET.fromstring('<tag>a<x />b<y></y>c</tag>')
        self.assertEqual(ET.tostring(root, 'unicode'),
            '<tag>a<x />b<y />c</tag>')
        self.assertEqual(ET.tostring(root, 'unicode', short_empty_elements=
            True), '<tag>a<x />b<y />c</tag>')
        self.assertEqual(ET.tostring(root, 'unicode', short_empty_elements=
            False), '<tag>a<x></x>b<y></y>c</tag>')
...
class ParseErrorTest(unittest.TestCase):
    ...
    def test_error_code(self):
        ...
        self.assertEqual(self._get_error('foo').code, ERRORS.codes[ERRORS.
            XML_ERROR_SYNTAX])
...
class KeywordArgsTest(unittest.TestCase):
    ...
    def test_issue14818(self):
        ...
        self.assertEqual(x.findtext('a', None, None), x.findtext(path='a',
            default=None, namespaces=None))
        self.assertEqual(x.findall('a', None), x.findall(path='a',
            namespaces=None))
        self.assertEqual(list(x.iterfind('a', None)), list(x.iterfind(path=
            'a', namespaces=None)))
        self.assertEqual(ET.Element('a').attrib, {})
        elements = [ET.Element('a', dict(href='#', id='foo')), ET.Element(
            'a', attrib=dict(href='#', id='foo')), ET.Element('a', dict(
            href='#'), id='foo'), ET.Element('a', href='#', id='foo'), ET.
            Element('a', dict(href='#', id='foo'), href='#', id='foo')]
        for e in elements:
...
class CleanContext(object):
    ...
    def __init__(self, quiet=False):
        if sys.flags.optimize >= 2:
            quiet = True
        deprecations = (
            "This search is broken in 1.3 and earlier, and will be fixed in a future version.  If you rely on the current behaviour, change it to '.+'"
            , FutureWarning), (
            'This method will be removed in future versions.  Use .+ instead.',
            DeprecationWarning), (
            'This method will be removed in future versions.  Use .+ instead.',
...
def test_main(module=None):
    global pyET
    pyET = import_fresh_module('xml.etree.ElementTree', blocked=[
        '_elementtree'])
    if module is None:
        module = pyET
    global ET
    ET = module
    test_classes = [ModuleTest, ElementSlicingTest, BasicElementTest,
        BadElementTest, BadElementPathTest, ElementTreeTest, IOTest,
        ParseErrorTest, XIncludeTest, ElementTreeTypeTest, ElementFindTest,
        ElementIterTest, TreeBuilderTest, XMLParserTest, XMLPullParserTest,
        BugsTest]
    if pyET is not ET:
