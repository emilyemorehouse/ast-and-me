...
from test.support import TESTFN, findfile, unlink, rmtree, temp_dir, temp_cwd, requires_zlib, requires_bz2, requires_lzma, captured_stdout, check_warnings
TESTFN2 = TESTFN + '2'
TESTFNDIR = TESTFN + 'd'
FIXEDTEST_SIZE = 1000
DATAFILES_DIR = 'zipfile_datafiles'
SMALL_TEST_DATA = [('_ziptest1', '1q2w3e4r5t'), ('ziptest2dir/_ziptest2',
    'qawsedrftg'), ('ziptest2dir/ziptest3dir/_ziptest3', 'azsxdcfvgb'), (
...
class AbstractTestZip64InSmallFiles:
    ...
    @classmethod
    def setUpClass(cls):
        line_gen = (bytes('Test of zipfile line %d.' % i, 'ascii') for i in
            range(0, FIXEDTEST_SIZE))
        cls.data = b'\n'.join(line_gen)
...
class StoredTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.
    TestCase):
    compression = zipfile.ZIP_STORED

    def large_file_exception_test(self, f, compression):
        with zipfile.ZipFile(f, 'w', compression, allowZip64=False) as zipfp:
            self.assertRaises(zipfile.LargeZipFile, zipfp.write, TESTFN,
                'another.name')
    ...
    def test_absolute_arcnames(self):
        with zipfile.ZipFile(TESTFN2, 'w', zipfile.ZIP_STORED, allowZip64=True
            ) as zipfp:
            zipfp.write(TESTFN, '/absolute')
        with zipfile.ZipFile(TESTFN2, 'r', zipfile.ZIP_STORED) as zipfp:
            self.assertEqual(zipfp.namelist(), ['absolute'])
...
@requires_zlib
class DeflateTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.
    TestCase):
    compression = zipfile.ZIP_DEFLATED


@requires_bz2
class Bzip2TestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.
    TestCase):
    compression = zipfile.ZIP_BZIP2


@requires_lzma
class LzmaTestZip64InSmallFiles(AbstractTestZip64InSmallFiles, unittest.
    TestCase):
    compression = zipfile.ZIP_LZMA
...
class PyZipFileTests(unittest.TestCase):
    ...
    def requiresWriteAccess(self, path):
        if not os.access(path, os.W_OK, effective_ids=os.access in os.
    ...
    def test_write_with_optimization(self):
        ...
        ext = '.pyc'
        with TemporaryFile() as t, zipfile.PyZipFile(t, 'w', optimize=optlevel
            ) as zipfp:
            zipfp.writepy(packagedir)
            names = zipfp.namelist()
    ...
    def test_write_python_directory_filtered(self):
        ...
        try:
            ...
            with open(os.path.join(TESTFN2, 'mod2.py'), 'w') as fp:
                fp.write('print(42 * 42)\n')
            with TemporaryFile() as t, zipfile.PyZipFile(t, 'w') as zipfp:
                zipfp.writepy(TESTFN2, filterfunc=lambda fn: not fn.
                    endswith('mod2.py'))
                names = zipfp.namelist()
                self.assertCompiledIn('mod1.py', names)
...
class ExtractTests(unittest.TestCase):
    ...
    def _test_extract_with_target(self, target):
        ...
        with zipfile.ZipFile(TESTFN2, 'r') as zipfp:
            for fpath, fdata in SMALL_TEST_DATA:
                ...
                correctfile = os.path.normpath(correctfile)
                self.assertTrue(os.path.samefile(writtenfile, correctfile),
                    (writtenfile, target))
                with open(writtenfile, 'rb') as f:
                    self.assertEqual(fdata.encode(), f.read())
                unlink(writtenfile)
    ...
    def test_extract_hackers_arcnames_common_cases(self):
        common_hacknames = [('../foo/bar', 'foo/bar'), ('foo/../bar',
    ...
    def test_extract_hackers_arcnames_windows_only(self):
        ...
        windows_hacknames = [('..\\foo\\bar', 'foo/bar'), ('..\\/foo\\/bar',
            'foo/bar'), ('foo/\\..\\/bar', 'foo/bar'), ('foo\\/../\\bar',
            'foo/bar'), ('C:foo/bar', 'foo/bar'), ('C:/foo/bar', 'foo/bar'),
            ...
            'conky/mountpoint/foo/bar'), ('//?/C:/foo/bar', 'foo/bar'), (
            '\\\\?\\C:\\foo\\bar', 'foo/bar'), ('C:/../C:/foo/bar',
            'C_/foo/bar'), ('a:b\\c<d>e|f"g?h*i', 'b/c_d_e_f_g_h_i'), (
            '../../foo../../ba..r', 'foo/ba..r')]
        self._test_extract_hackers_arcnames(windows_hacknames)

    @unittest.skipIf(os.path.sep != '/', 'Requires / as path separator.')
    def test_extract_hackers_arcnames_posix_only(self):
        posix_hacknames = [('//foo/bar', 'foo/bar'), (
            '../../foo../../ba..r', 'foo../ba..r'), ('foo/..\\bar',
            'foo/..\\bar')]
    ...
    def _test_extract_hackers_arcnames(self, hacknames):
        for arcname, fixedname in hacknames:
            ...
            with zipfile.ZipFile(TESTFN2, 'r') as zipfp:
                ...
                self.assertEqual(writtenfile, correctfile, msg=
                    'extract %r: %r != %r' % (arcname, writtenfile,
                    correctfile))
            ...
            with zipfile.ZipFile(TESTFN2, 'r') as zipfp:
                ...
                self.assertEqual(writtenfile, correctfile, msg='extract %r' %
                    arcname)
            self.check_file(correctfile, content)
...
class OtherTests(unittest.TestCase):
    ...
    def test_unsupported_version(self):
        ...
        self.assertRaises(NotImplementedError, zipfile.ZipFile, io.BytesIO(
            data), 'r')
    ...
    def test_comments(self):
        ...
        comment2 = ''.join([('%d' % (i ** 3 % 10)) for i in range((1 << 16) -
            1)])
    ...
    def test_create_zipinfo_before_1980(self):
        self.assertRaises(ValueError, zipfile.ZipInfo, 'seventies', (1979, 
            1, 1, 0, 0, 0))

    def test_zipfile_with_short_extra_field(self):
        """If an extra field in the header is less than 4 bytes, skip it."""
        zipdata = (
            b'PK\x03\x04\x14\x00\x00\x00\x00\x00\x93\x9b\xad@\x8b\x9e\xd9\xd3\x01\x00\x00\x00\x01\x00\x00\x00\x03\x00\x03\x00abc\x00\x00\x00APK\x01\x02\x14\x03\x14\x00\x00\x00\x00\x00\x93\x9b\xad@\x8b\x9e\xd9\xd3\x01\x00\x00\x00\x01\x00\x00\x00\x03\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\xa4\x81\x00\x00\x00\x00abc\x00\x00PK\x05\x06\x00\x00\x00\x00\x01\x00\x01\x003\x00\x00\x00%\x00\x00\x00\x00\x00'
            )
...
class StoredBadCrcTests(AbstractBadCrcTests, unittest.TestCase):
    compression = zipfile.ZIP_STORED
    zip_with_bad_crc = (
        b'PK\x03\x04\x14\x00\x00\x00\x00\x00 \x8b\x8a;:r\xab\xff\x0c\x00\x00\x00\x0c\x00\x00\x00\x05\x00\x00\x00afilehello,AworldPK\x01\x02\x14\x03\x14\x00\x00\x00\x00\x00 \x8b\x8a;:r\xab\xff\x0c\x00\x00\x00\x0c\x00\x00\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\x01\x00\x00\x00\x00afilePK\x05\x06\x00\x00\x00\x00\x01\x00\x01\x003\x00\x00\x00/\x00\x00\x00\x00\x00'
        )
...
class LzmaBadCrcTests(AbstractBadCrcTests, unittest.TestCase):
    compression = zipfile.ZIP_LZMA
    zip_with_bad_crc = (
        b'PK\x03\x04\x14\x03\x00\x00\x0e\x00nu\x0c=FAKE\x1b\x00\x00\x00n\x00\x00\x00\x05\x00\x00\x00afile\t\x04\x05\x00]\x00\x00\x00\x04\x004\x19I\xee\x8d\xe9\x17\x89:3`\tq!.8\x00PK\x01\x02\x14\x03\x14\x03\x00\x00\x0e\x00nu\x0c=FAKE\x1b\x00\x00\x00n\x00\x00\x00\x05\x00\x00\x00\x00\x00\x00\x00\x00\x00 \x80\x80\x81\x00\x00\x00\x00afilePK\x05\x06\x00\x00\x00\x00\x01\x00\x01\x003\x00\x00\x00>\x00\x00\x00\x00\x00'
        )
...
class AbstractTestsWithRandomBinaryFiles:
    ...
    def setUpClass(cls):
        ...
        cls.data = b''.join(struct.pack('<f', random() * randint(-1000, 
            1000)) for i in range(datacount))
    ...
    def test_writestr(self):
        for wrapper in (lambda f: f, Tellable, Unseekable):
            with self.subTest(wrapper=wrapper):
                ...
                with zipfile.ZipFile(wrapper(bf), 'w', zipfile.ZIP_STORED
                    ) as zipfp:
...
class CommandLineTest(unittest.TestCase):
    ...
    def zipfilecmd(self, *args, **kwargs):
        rc, out, err = script_helper.assert_python_ok('-m', 'zipfile', *
            args, **kwargs)
    ...
    def test_list_command(self):
        ...
        out = self.zipfilecmd('-l', zip_name, PYTHONIOENCODING=
        ...
        with temp_dir() as extdir:
            ...
            with zipfile.ZipFile(zip_name) as zf:
                for zi in zf.infolist():
                    path = os.path.join(extdir, zi.filename.replace('/', os
                        .sep))
                    if zi.is_dir():
