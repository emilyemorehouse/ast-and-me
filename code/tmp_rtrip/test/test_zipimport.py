...
def make_pyc(co, mtime, size):
    ...
    pyc = importlib.util.MAGIC_NUMBER + struct.pack('<ii', int(mtime), size &
        4294967295) + data
    return pyc
...
class UncompressedZipImportTestCase(ImportHooksBaseTestCase):
    ...
    def doTest(self, expected_ext, files, *modules, **kw):
        ...
        if expected_ext:
            file = mod.get_file()
            self.assertEqual(file, os.path.join(TEMP_ZIP, *modules) +
                expected_ext)
    ...
    def testBoth(self):
        files = {(TESTMOD + '.py'): (NOW, test_src), (TESTMOD + pyc_ext): (
            NOW, test_pyc)}
        self.doTest(pyc_ext, files, TESTMOD)
    ...
    def testSubPackage(self):
        ...
        files = {(packdir + '__init__' + pyc_ext): (NOW, test_pyc), (
            packdir2 + '__init__' + pyc_ext): (NOW, test_pyc), (packdir2 +
    ...
    def testSubNamespacePackage(self):
        packdir = TESTPACK + os.sep
        packdir2 = packdir + TESTPACK2 + os.sep
        files = {packdir: (NOW, ''), packdir2: (NOW, ''), (packdir2 +
            TESTMOD + pyc_ext): (NOW, test_pyc)}
        self.doTest(pyc_ext, files, TESTPACK, TESTPACK2, TESTMOD)
    ...
    def testMixedNamespacePackage(self):
        ...
        files1 = {packdir: (NOW, ''), (packdir + TESTMOD + pyc_ext): (NOW,
            test_pyc), packdir2: (NOW, ''), packdir3: (NOW, ''), (packdir3 +
            TESTMOD + pyc_ext): (NOW, test_pyc), (packdir2 + TESTMOD + '3' +
            pyc_ext): (NOW, test_pyc), (packdir2 + TESTMOD + pyc_ext): (NOW,
            test_pyc)}
        files2 = {packdir: (NOW, ''), (packdir + TESTMOD + '2' + pyc_ext):
            (NOW, test_pyc), packdir2: (NOW, ''), (packdir2 + TESTMOD + '2' +
            pyc_ext): (NOW, test_pyc), (packdir2 + TESTMOD + pyc_ext): (NOW,
            test_pyc)}
        ...
        self.assertEqual(os.path.basename(TEMP_DIR), mod.__file__.split(os.
            sep)[-3])
    ...
    def testZipImporterMethods(self):
        ...
        files = {(packdir + '__init__' + pyc_ext): (NOW, test_pyc), (
            packdir2 + '__init__' + pyc_ext): (NOW, test_pyc), (packdir2 +
            TESTMOD + pyc_ext): (NOW, test_pyc), ('spam' + pyc_ext): (NOW,
            test_pyc)}
...
"""
        ...
        files = {(TESTMOD + pyc_ext): (NOW, pyc), 'some.data': (NOW,
            'some data')}
        self.doTest(pyc_ext, files, TESTMOD)
...
"""
    ...
    def runDoctest(self, callback):
        files = {(TESTMOD + '.py'): (NOW, test_src), 'xyz.txt': (NOW,
            '>>> log.append(True)\n')}
    ...
    def doDoctestFile(self, module):
        ...
        try:
            doctest.testfile('xyz.txt', package=module, module_relative=
                True, globs=locals())
        finally:
    ...
    def doDoctestSuite(self, module):
        log = []
        doctest.DocFileTest('xyz.txt', package=module, module_relative=True,
            globs=locals()).run()
        self.assertEqual(log, [True])
...
class BadFileZipImportTestCase(unittest.TestCase):
    ...
    def testBadArgs(self):
        ...
        self.assertRaises(TypeError, zipimport.zipimporter, list(os.
            fsencode(TESTMOD)))
...
def test_main():
    try:
        support.run_unittest(UncompressedZipImportTestCase,
