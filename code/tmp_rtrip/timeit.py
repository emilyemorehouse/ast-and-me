...
def timeit(stmt='pass', setup='pass', timer=default_timer, number=
    default_number, globals=None):
    """Convenience function to create Timer object and call timeit method."""
    return Timer(stmt, setup, timer, globals).timeit(number)


def repeat(stmt='pass', setup='pass', timer=default_timer, repeat=
    default_repeat, number=default_number, globals=None):
...
def main(args=None, *, _wrap_timer=None):
    ...
    import getopt
    try:
        opts, args = getopt.getopt(args, 'n:u:s:r:tcpvh', ['number=',
            'setup=', 'repeat=', 'time', 'clock', 'process', 'verbose',
            'unit=', 'help'])
    ...
    if time_unit is not None:
        ...
        for scale, time_unit in scales:
            if usec >= scale:
                break
    print('best of %d: %.*g %s per loop' % (repeat, precision, usec / scale,
        time_unit))
    best = min(r)
    usec = best * 1000000.0 / number
    worst = max(r)
