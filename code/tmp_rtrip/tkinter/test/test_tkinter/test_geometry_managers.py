...
class PackTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_pack_configure_anchor(self):
        ...
        def check(anchor, geom):
            a.pack_configure(side='top', ipadx=5, padx=10, ipady=15, pady=
                20, expand=True, anchor=anchor)
            self.root.update()
    ...
    def test_pack_configure_expand(self):
        ...
        check('100x200+0+0', '200x100+100+0', '160x100+140+100',
            '40x100+100+100')
    ...
    def test_pack_configure_in(self):
        ...
        with self.assertRaisesRegex(TclError, "can't pack %s inside itself" %
            (a,)):
    ...
    def test_pack_configure_padx_ipadx_fill(self):
        ...
        check('20x40+249+80', '240x200+0+0', side='right', padx=(9, 31),
            fill='x')
        check('60x40+240+80', '240x200+0+0', side='right', ipadx=20, fill='x')
        check('30x40+260+80', '250x200+0+0', side='right', ipadx=5, padx=10,
            fill='x')
        check('30x40+255+80', '250x200+0+0', side='right', ipadx=5, padx=(5,
            15), fill='x')
        ...
        check('260x40+25+0', '300x160+0+40', side='top', padx=(25, 15),
            fill='x')
        check('300x40+0+0', '300x160+0+40', side='top', ipadx=20, fill='x')
        check('280x40+10+0', '300x160+0+40', side='top', ipadx=5, padx=10,
            fill='x')
        check('280x40+5+0', '300x160+0+40', side='top', ipadx=5, padx=(5, 
            15), fill='x')
        a.pack_configure(padx='1c')
        self.assertEqual(a.pack_info()['padx'], self._str(pack.winfo_pixels
            ('1c')))
        a.pack_configure(ipadx='1c')
        self.assertEqual(a.pack_info()['ipadx'], self._str(pack.
            winfo_pixels('1c')))
    ...
    def test_pack_configure_pady_ipady_fill(self):
        ...
        check('20x40+280+69', '280x200+0+0', side='right', pady=(9, 31),
            fill='x')
        check('20x80+280+60', '280x200+0+0', side='right', ipady=20, fill='x')
        check('20x50+280+75', '280x200+0+0', side='right', ipady=5, pady=10,
            fill='x')
        check('20x50+280+70', '280x200+0+0', side='right', ipady=5, pady=(5,
            15), fill='x')
        ...
        check('300x40+0+25', '300x120+0+80', side='top', pady=(25, 15),
            fill='x')
        check('300x80+0+0', '300x120+0+80', side='top', ipady=20, fill='x')
        check('300x50+0+10', '300x130+0+70', side='top', ipady=5, pady=10,
            fill='x')
        check('300x50+0+5', '300x130+0+70', side='top', ipady=5, pady=(5, 
            15), fill='x')
        a.pack_configure(pady='1c')
        self.assertEqual(a.pack_info()['pady'], self._str(pack.winfo_pixels
            ('1c')))
        a.pack_configure(ipady='1c')
        self.assertEqual(a.pack_info()['ipady'], self._str(pack.
            winfo_pixels('1c')))
    ...
    def test_pack_info(self):
        ...
        with self.assertRaisesRegex(TclError, 'window "%s" isn\'t packed' % a):
            a.pack_info()
        a.pack_configure()
        b.pack_configure(side='right', in_=a, anchor='s', expand=True, fill
            ='x', ipadx=5, padx=10, ipady=2, pady=(5, 15))
        info = a.pack_info()
...
class PlaceTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def test_place_configure_in(self):
        ...
        with self.assertRaisesRegex(TclError, 
            "can't place %s relative to itself" % re.escape(str(f2))):
    ...
    def test_place_configure_relx(self):
        ...
        with self.assertRaisesRegex(TclError,
            'expected floating-point number but got "spam"'):
    ...
    def test_place_configure_relwidth(self):
        ...
        self.assertEqual(f2.winfo_width(), 30)
        with self.assertRaisesRegex(TclError,
            'expected floating-point number but got "abcd"'):
    ...
    def test_place_info(self):
        t, f, f2 = self.create2()
        f2.place_configure(in_=f, x=1, y=2, width=3, height=4, relx=0.1,
            rely=0.2, relwidth=0.3, relheight=0.4, anchor='se', bordermode=
            'outside')
        info = f2.place_info()
        self.assertIsInstance(info, dict)
...
class GridTest(AbstractWidgetTest, unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for i in range(cols + 1):
            self.root.grid_columnconfigure(i, weight=0, minsize=0, pad=0,
                uniform='')
        for i in range(rows + 1):
            self.root.grid_rowconfigure(i, weight=0, minsize=0, pad=0,
                uniform='')
    ...
    def test_grid_configure_column(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad column value "-1": must be a non-negative integer'):
    ...
    def test_grid_configure_columnspan(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad columnspan value "0": must be a positive integer'):
    ...
    def test_grid_configure_ipadx(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad ipadx value "-1": must be positive screen distance'):
            b.grid_configure(ipadx=-1)
        b.grid_configure(ipadx=1)
        self.assertEqual(b.grid_info()['ipadx'], self._str(1))
        b.grid_configure(ipadx='.5c')
        self.assertEqual(b.grid_info()['ipadx'], self._str(round(
            pixels_conv('.5c') * self.scaling)))

    def test_grid_configure_ipady(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad ipady value "-1": must be positive screen distance'):
            b.grid_configure(ipady=-1)
        b.grid_configure(ipady=1)
        self.assertEqual(b.grid_info()['ipady'], self._str(1))
        b.grid_configure(ipady='.5c')
        self.assertEqual(b.grid_info()['ipady'], self._str(round(
            pixels_conv('.5c') * self.scaling)))
    ...
    def test_grid_configure_padx(self):
        ...
        self.assertEqual(b.grid_info()['padx'], self._str(round(pixels_conv
            ('.5c') * self.scaling)))
    ...
    def test_grid_configure_pady(self):
        ...
        self.assertEqual(b.grid_info()['pady'], self._str(round(pixels_conv
            ('.5c') * self.scaling)))

    def test_grid_configure_row(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad (row|grid) value "-1": must be a non-negative integer'):
    ...
    def test_grid_configure_rownspan(self):
        b = tkinter.Button(self.root)
        with self.assertRaisesRegex(TclError,
            'bad rowspan value "0": must be a positive integer'):
    ...
    def test_grid_columnconfigure(self):
        ...
        self.assertEqual(self.root.grid_columnconfigure(0), {'minsize': 0,
            'pad': 0, 'uniform': None, 'weight': 0})
    ...
    def test_grid_columnconfigure_pad(self):
        ...
        with self.assertRaisesRegex(TclError,
            'invalid arg "-pad": should be non-negative'):
    ...
    def test_grid_rowconfigure(self):
        ...
        self.assertEqual(self.root.grid_rowconfigure(0), {'minsize': 0,
            'pad': 0, 'uniform': None, 'weight': 0})
        with self.assertRaisesRegex(TclError, 'bad option "-foo"'):
    ...
    def test_grid_forget(self):
        ...
        b.grid_configure(row=2, column=2, rowspan=2, columnspan=2, padx=3,
            pady=4, sticky='ns')
    ...
    def test_grid_location(self):
        ...
        t.wm_geometry('1x1+0+0')
        t.wm_geometry('')
        f = tkinter.Frame(t, width=200, height=100, highlightthickness=0,
            bg='red')
