...
class InternalFunctionsTest(unittest.TestCase):
    ...
    def test_format_optdict(self):
        ...
        check_against(ttk._format_optdict({'fg': 'blue', 'padding': [1, 2, 
            3, 4]}), {'-fg': 'blue', '-padding': '1 2 3 4'})
        check_against(ttk._format_optdict({'test': (1, 2, '', 0)}), {
            '-test': '1 2 {} 0'})
        check_against(ttk._format_optdict({'test': {'left': 'as is'}}), {
            '-test': {'left': 'as is'}})
        check_against(ttk._format_optdict({'test': [1, -1, '', '2m', 0],
            'test2': 3, 'test3': '', 'test4': 'abc def', 'test5': '"abc"',
            'test6': '{}', 'test7': '} -spam {'}, script=True), {'-test':
            '{1 -1 {} 2m 0}', '-test2': '3', '-test3': '{}', '-test4':
            '{abc def}', '-test5': '{"abc"}', '-test6': '\\{\\}', '-test7':
            '\\}\\ -spam\\ \\{'})
    ...
    def test_format_mapdict(self):
        ...
        self.assertEqual(ttk._format_mapdict(opts, script=True), ('-a',
        ...
        self.assertRaises(IndexError, ttk._format_mapdict, {'a': [(
            'invalid',)]})
    ...
    def test_format_elemcreate(self):
        ...
        self.assertEqual(ttk._format_elemcreate('image', True, 'test', ('a',
            'b', 'c', 'd'), x=[2, 3]), ('{test {a b c} d}', '-x {2 3}'))
    ...
    def test_format_layoutlist(self):
        ...
        def sample(indent=0, indent_size=2):
            return ttk._format_layoutlist([('a', {'other': [1, 2, 3],
                ...
                {'nice': 'opt'})], 'something': (1, 2)})]})]})], indent=
                indent, indent_size=indent_size)[0]
...
%s}
%s}"""
                 % (spaces(), spaces(indent_size), spaces(2 * indent_size),
                spaces(3 * indent_size), spaces(2 * indent_size), spaces(
                indent_size), spaces()))
        ...
        self.assertRaises(AttributeError, ttk._format_layoutlist, [('name',
            'bad')])
        self.assertRaises(ValueError, ttk._format_layoutlist, [('name', {
            'children': {'a': None}})])

    def test_script_from_settings(self):
        self.assertFalse(ttk._script_from_settings({'name': {'configure':
            None, 'map': None, 'element create': None}}))
        self.assertEqual(ttk._script_from_settings({'name': {'layout': None
            }}), """ttk::style layout name {
null
}""")
        configdict = {'αβγ': True, 'á': False}
        self.assertTrue(ttk._script_from_settings({'name': {'configure':
            configdict}}))
        mapdict = {'üñíćódè': [('á', 'vãl')]}
        self.assertTrue(ttk._script_from_settings({'name': {'map': mapdict}}))
        self.assertRaises(IndexError, ttk._script_from_settings, {'name': {
            'element create': ['image']}})
        self.assertTrue(ttk._script_from_settings({'name': {
            'element create': ['image', 'name']}}))
        image = {'thing': {'element create': ['image', 'name', ('state1',
            'state2', 'val')]}}
    ...
    def test_tclobj_to_py(self):
        ...
        self.assertEqual(ttk._tclobj_to_py([MockTclObj('1'), 2, MockTclObj(
            '3m')]), [1, 2, '3m'])

    def test_list_from_statespec(self):

        def test_it(sspec, value, res_value, states):
            self.assertEqual(ttk._list_from_statespec((sspec, value)), [
                states + (res_value,)])
    ...
    def test_list_from_layouttuple(self):
        ...
        self.assertEqual(ttk._list_from_layouttuple(tk, sample_ltuple), [(
            'name', {'option': 'value'})])
        self.assertEqual(ttk._list_from_layouttuple(tk, ('something',
            '-children', ())), [('something', {'children': []})])
        ltuple = 'name', '-option', 'niceone', '-children', ('otherone',
            '-children', ('child',), '-otheropt', 'othervalue')
        self.assertEqual(ttk._list_from_layouttuple(tk, ltuple), [('name',
            {'option': 'niceone', 'children': [('otherone', {'otheropt':
            'othervalue', 'children': [('child', {})]})]})])
...
class TclObjsToPyTest(unittest.TestCase):
    ...
    def test_multivalues(self):
        ...
        self.assertEqual(ttk.tclobjs_to_py(adict), {'opt': [('a', 'b',
            'válũè')]})
        self.assertEqual(ttk.tclobjs_to_py({'x': ['y z']}), {'x': ['y z']})

    def test_nosplit(self):
        self.assertEqual(ttk.tclobjs_to_py({'text': 'some text'}), {'text':
            'some text'})
