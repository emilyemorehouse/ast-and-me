...
__all__ = ['Button', 'Checkbutton', 'Combobox', 'Entry', 'Frame', 'Label',
    ...
    'Separator', 'Sizegrip', 'Style', 'Treeview', 'LabeledScale',
...
def _format_mapdict(mapdict, script=False):
    ...
    for opt, value in mapdict.items():
        opts.extend(('-%s' % opt, _format_optvalue(_mapdict_values(value),
            script)))
    return _flatten(opts)
...
class Style(object):
    ...
    def map(self, style, query_opt=None, **kw):
        ...
        if query_opt is not None:
            return _list_from_statespec(self.tk.splitlist(self.tk.call(self
                ._name, 'map', style, '-%s' % query_opt)))
        return _splitdict(self.tk, self.tk.call(self._name, 'map', style, *
            _format_mapdict(kw)), conv=_tclobj_to_py)
    ...
    def layout(self, style, layoutspec=None):
        ...
        if layoutspec:
            ...
            lspec = 'null'
        return _list_from_layouttuple(self.tk, self.tk.call(self._name,
            'layout', style, lspec))
    ...
    def element_names(self):
        ...
        return tuple(n.lstrip('-') for n in self.tk.splitlist(self.tk.call(
            self._name, 'element', 'names')))

    def element_options(self, elementname):
        """Return the list of elementname's options."""
        return tuple(o.lstrip('-') for o in self.tk.splitlist(self.tk.call(
            self._name, 'element', 'options', elementname)))
...
class Widget(tkinter.Widget):
    ...
    def instate(self, statespec, callback=None, *args, **kw):
        ...
        ret = self.tk.getboolean(self.tk.call(self._w, 'instate', ' '.join(
            statespec)))
        if ret and callback:
...
class Treeview(Widget, tkinter.XView, tkinter.YView):
    ...
    def get_children(self, item=None):
        ...
        return self.tk.splitlist(self.tk.call(self._w, 'children', item or
            '') or ())
    ...
    def selection(self, selop=_sentinel, items=None):
        ...
        if selop is _sentinel:
            ...
            import warnings
            warnings.warn(
                'The selop=None argument of selection() is deprecated and will be removed in Python 3.7'
                , DeprecationWarning, 3)
        elif selop in ('set', 'add', 'remove', 'toggle'):
            import warnings
            warnings.warn(
                'The selop argument of selection() is deprecated and will be removed in Python 3.7, use selection_%s() instead'
                 % (selop,), DeprecationWarning, 3)
        else:
            raise TypeError('Unsupported operation')
        return self.tk.splitlist(self.tk.call(self._w, 'selection', selop,
            items))
    ...
    def tag_has(self, tagname, item=None):
        ...
        * Availability: Tk 8.6"""
        if item is None:
            return self.tk.splitlist(self.tk.call(self._w, 'tag', 'has',
                tagname))
        else:
            return self.tk.getboolean(self.tk.call(self._w, 'tag', 'has',
                tagname, item))
...
class OptionMenu(Menubutton):
    ...
    def __init__(self, master, variable, default=None, *values, **kwargs):
        ...
        *values and additional keywords.
        ...
        if kwargs:
            raise tkinter.TclError('unknown option -%s' % next(iter(kwargs.
                keys())))
    ...
    def set_menu(self, default=None, *values):
        ...
        menu.delete(0, 'end')
        for val in values:
            menu.add_radiobutton(label=val, command=tkinter._setit(self.
                _variable, val, self._callback))
        if default:
