...
import token
__all__ = token.__all__ + ['COMMENT', 'tokenize', 'detect_encoding', 'NL',
    'untokenize', 'ENCODING', 'TokenInfo']
del token
COMMENT = N_TOKENS
...
N_TOKENS += 3
EXACT_TOKEN_TYPES = {'(': LPAR, ')': RPAR, '[': LSQB, ']': RSQB, ':': COLON,
    ',': COMMA, ';': SEMI, '+': PLUS, '-': MINUS, '*': STAR, '/': SLASH,
    '|': VBAR, '&': AMPER, '<': LESS, '>': GREATER, '=': EQUAL, '.': DOT,
    '%': PERCENT, '{': LBRACE, '}': RBRACE, '==': EQEQUAL, '!=': NOTEQUAL,
    ...
    while True:
        ...
        if contstr:
            ...
            if endmatch:
                ...
                yield TokenInfo(ERRORTOKEN, contstr + line, strstart, (lnum,
                    len(line)), contline)
                contstr = ''
                contline = None
                continue
            ...
            if line[pos] in '#\r\n':
                if line[pos] == '#':
                    ...
                    yield TokenInfo((NL, COMMENT)[line[pos] == '#'], line[
                        pos:], (lnum, pos), (lnum, len(line)), line)
                continue
            if column > indents[-1]:
        ...
        while pos < max:
            ...
            if pseudomatch:
                ...
                if (initial in numchars or initial == '.' and token != '.' and
                    ...
                    if token[-1] == '\n':
                        strstart = lnum, start
                        endprog = _compile(endpats.get(initial) or endpats.
                            get(token[1]) or endpats.get(token[2]))
                    ...
                    if token in ('async', 'await'):
                        if async_def:
                            yield TokenInfo(ASYNC if token == 'async' else
                                AWAIT, token, spos, epos, line)
                            continue
                    ...
                    if token == 'async' and not stashed:
                        ...
                        continue
                    if token == 'def':
                        if (stashed and stashed.type == NAME and stashed.
                            string == 'async'):
                            async_def = True
                            async_def_indent = indents[-1]
                            yield TokenInfo(ASYNC, stashed.string, stashed.
                                start, stashed.end, stashed.line)
                            stashed = None
                    if stashed:
                        yield stashed
                        stashed = None
                ...
                yield TokenInfo(ERRORTOKEN, line[pos], (lnum, pos), (lnum, 
                    pos + 1), line)
                pos += 1
    if stashed:
        yield stashed
...
def main():
    ...
    try:
        ...
        for token in tokens:
            ...
            print('%-20s%-15s%-15r' % (token_range, tok_name[token_type],
                token.string))
