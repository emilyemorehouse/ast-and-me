...
def getExampleEntries():
    return [entry[:-3] for entry in os.listdir(demo_dir) if entry.endswith(
        '.py') and entry[0] != '_']


help_entries = ('Turtledemo help', __doc__), ('About turtledemo',
    about_turtledemo), ('About turtle module', turtle.__doc__)
...
class DemoWindow(object):
    ...
    def __init__(self, filename=None):
        ...
        if darwin:
            ...
            p = subprocess.Popen(['osascript', '-e',
                ...
                .format(os.getpid()), '-e', 'end tell'], stderr=subprocess.
        ...
        self.mBar.add_cascade(menu=self.makeLoadDemoMenu(self.mBar), label=
            'Examples', underline=0)
        self.mBar.add_cascade(menu=self.makeFontMenu(self.mBar), label=
            'Fontsize', underline=0)
        self.mBar.add_cascade(menu=self.makeHelpMenu(self.mBar), label=
            'Help', underline=0)
        root['menu'] = self.mBar
        pane = PanedWindow(orient=HORIZONTAL, sashwidth=5, sashrelief=SOLID,
            bg='#ddd')
        pane.add(self.makeTextFrame(pane))
        pane.add(self.makeGraphFrame(pane))
        pane.grid(row=0, columnspan=4, sticky='news')
        self.output_lbl = Label(root, height=1, text=' --- ', bg='#ddf',
            font=('Arial', 16, 'normal'), borderwidth=2, relief=RIDGE)
        self.start_btn = Button(root, text=' START ', font=btnfont, fg=
            'white', disabledforeground='#fed', command=self.startDemo)
        self.stop_btn = Button(root, text=' STOP ', font=btnfont, fg=
            'white', disabledforeground='#fed', command=self.stopIt)
        self.clear_btn = Button(root, text=' CLEAR ', font=btnfont, fg=
            'white', disabledforeground='#fed', command=self.clearCanvas)
    ...
    def makeTextFrame(self, root):
        ...
        self.text = text = Text(text_frame, name='text', padx=5, wrap=
            'none', width=45)
        color_config(text)
    ...
    def configGUI(self, start, stop, clear, txt='', color='blue'):
        self.start_btn.config(state=start, bg='#d00' if start == NORMAL else
            '#fca')
        self.stop_btn.config(state=stop, bg='#d00' if stop == NORMAL else
            '#fca')
        self.clear_btn.config(state=clear, bg='#d00' if clear == NORMAL else
            '#fca')
    ...
    def makeLoadDemoMenu(self, master):
        ...
        for entry in getExampleEntries():
            ...
            def load(entry=entry):
                self.loadfile(entry)
            menu.add_command(label=entry, underline=0, font=menufont,
                command=load)
        return menu

    def makeFontMenu(self, master):
        menu = Menu(master)
        menu.add_command(label="Decrease (C-'-')", command=self.
            decrease_size, font=menufont)
        menu.add_command(label="Increase (C-'+')", command=self.
            increase_size, font=menufont)
        menu.add_separator()
        for size in font_sizes:

            def resize(size=size):
                self.set_txtsize(size)
            menu.add_command(label=str(size), underline=0, font=menufont,
                command=resize)
        return menu
