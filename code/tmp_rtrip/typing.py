...
__all__ = ['Any', 'Callable', 'ClassVar', 'Generic', 'Optional', 'Tuple',
    'Type', 'TypeVar', 'Union', 'AbstractSet', 'GenericMeta', 'ByteString',
...
class _ForwardRef(_TypingBase, _root=True):
    ...
    def __eq__(self, other):
        ...
        return (self.__forward_arg__ == other.__forward_arg__ and self.
            __forward_value__ == other.__forward_value__)
...
def _type_check(arg, msg):
    ...
    if isinstance(arg, str):
        arg = _ForwardRef(arg)
    if isinstance(arg, _TypingBase) and type(arg
        ).__name__ == '_ClassVar' or not isinstance(arg, (type, _TypingBase)
        ) and not callable(arg):
        raise TypeError(msg + ' Got %.100r.' % (arg,))
    if type(arg).__name__ in ('_Union', '_Optional') and not getattr(arg,
        '__origin__', None) or isinstance(arg, TypingMeta) and _gorg(arg) in (
        Generic, _Protocol):
...
class TypeVar(_TypingBase, _root=True):
    ...
    def __init__(self, name, *constraints, bound=None, covariant=False,
        contravariant=False):
        super().__init__(name, *constraints, bound=bound, covariant=
...
def _replace_arg(arg, tvars, args):
    ...
    if tvars is None:
        tvars = []
    if hasattr(arg, '_subs_tree') and isinstance(arg, (GenericMeta,
        _TypingBase)):
...
def _subs_tree(cls, tvars=None, args=None):
    ...
    for ocls in orig_chain:
        ...
        for arg in ocls.__args__:
            new_tree_args.append(_replace_arg(arg, ocls.__parameters__,
                tree_args))
...
def _remove_dups_flatten(parameters):
    ...
    for t1 in params:
        if not isinstance(t1, type):
            continue
        if any(isinstance(t2, type) and issubclass(t1, t2) for t2 in 
            all_params - {t1} if not (isinstance(t2, GenericMeta) and t2.
...
class GenericMeta(TypingMeta, abc.ABCMeta):
    ...
    * if Generic[...] is present, then it should list all type variables
    ...
    def __new__(cls, name, bases, namespace, tvars=None, args=None, origin=
        None, extra=None, orig_bases=None):
                ...
                if not tvarset <= gvarset:
                    raise TypeError(
                        'Some type variables (%s) are not listed in Generic[%s]'
                         % (', '.join(str(t) for t in tvars if t not in
                        gvarset), ', '.join(str(g) for g in gvars)))
                tvars = gvars
        initial_bases = bases
        if extra is not None and type(extra
            ) is abc.ABCMeta and extra not in bases:
            bases = (extra,) + bases
        bases = tuple(_gorg(b) if isinstance(b, GenericMeta) else b for b in
            bases)
        ...
        self.__args__ = tuple(... if a is _TypingEllipsis else () if a is
            _TypingEmpty else a for a in args) if args else None
        self.__next_in_mro__ = _next_in_mro(self)
        if orig_bases is None:
            self.__orig_bases__ = initial_bases
        if '__subclasshook__' not in namespace and extra or getattr(self.
            __subclasshook__, '__name__', '') == '__extrahook__':
    ...
    def _eval_type(self, globalns, localns):
        ...
        ev_args = tuple(_eval_type(a, globalns, localns) for a in self.__args__
            ) if self.__args__ else None
        if ev_origin == self.__origin__ and ev_args == self.__args__:
            return self
        return self.__class__(self.__name__, self.__bases__, _no_slots_copy
            (self.__dict__), tvars=_type_vars(ev_args) if ev_args else None,
            args=ev_args, origin=ev_origin, extra=self.__extra__,
    ...
    def __getitem__(self, params):
        ...
        if not params and not _gorg(self) is Tuple:
            raise TypeError('Parameter list to %s[...] cannot be empty' %
                _qualname(self))
        msg = 'Parameters to generic types must be types.'
        ...
        return self.__class__(self.__name__, prepend + self.__bases__,
            _no_slots_copy(self.__dict__), tvars=tvars, args=args, origin=
            self, extra=self.__extra__, orig_bases=self.__orig_bases__)
    ...
    def __copy__(self):
        return self.__class__(self.__name__, self.__bases__, _no_slots_copy
            (self.__dict__), self.__parameters__, self.__args__, self.
            __origin__, self.__extra__, self.__orig_bases__)

    def __setattr__(self, attr, value):
        if attr.startswith('__') and attr.endswith('__') or attr.startswith(
            '_abc_'):
...
class CallableMeta(GenericMeta):
    ...
    def _tree_repr(self, tree):
        ...
        return repr(tree[0]) + '[[%s], %s]' % (', '.join(arg_list[:-1]),
            arg_list[-1])
...
class _ClassVar(_FinalTypingBase, _root=True):
    ...
    def __getitem__(self, item):
        ...
        if self.__type__ is None:
            return cls(_type_check(item, '{} accepts only single type.'.
                format(cls.__name__[1:])), _root=True)
        raise TypeError('{} cannot be further subscripted'.format(cls.
            __name__[1:]))
...
_allowed_types = (types.FunctionType, types.BuiltinFunctionType, types.
    MethodType, types.ModuleType, WrapperDescriptorType, MethodWrapperType,
...
def get_type_hints(obj, globalns=None, localns=None):
    ...
    if hints is None:
        if isinstance(obj, _allowed_types):
            ...
            raise TypeError('{!r} is not a module, class, method, or function.'
                .format(obj))
            ...
            for attr in base.__dict__.keys():
                for c in self.__mro__:
                    ...
                    if (not attr.startswith('_abc_') and attr !=
                        '__abstractmethods__' and attr != '__annotations__' and
                        attr != '__weakref__' and attr != '_is_protocol' and
                        attr != '__dict__' and attr != '__args__' and attr !=
                        '__slots__' and attr != '_get_protocol_attrs' and 
                        attr != '__next_in_mro__' and attr !=
                        '__parameters__' and attr != '__origin__' and attr !=
                        '__orig_bases__' and attr != '__extra__' and attr !=
                        '__tree_hash__' and attr != '__module__'):
                        attrs.add(attr)
        return attrs
...
if hasattr(collections_abc, 'Coroutine'):
    ...
    class Coroutine(Awaitable[V_co], Generic[T_co, T_contra, V_co], extra=
        collections_abc.Coroutine):
        __slots__ = ()
...
if hasattr(collections_abc, 'AsyncIterable'):
    ...
    class AsyncIterator(AsyncIterable[T_co], extra=collections_abc.
        AsyncIterator):
        __slots__ = ()
...
if hasattr(collections_abc, 'Collection'):
    ...
    class Collection(Sized, Iterable[T_co], Container[T_co], extra=
...
if hasattr(collections_abc, 'Collection'):
    ...
    class Mapping(Collection[KT], Generic[KT, VT_co], extra=collections_abc
        .Mapping):
        __slots__ = ()
else:
...
if hasattr(collections_abc, 'Reversible'):
    if hasattr(collections_abc, 'Collection'):
        ...
        class Sequence(Reversible[T_co], Collection[T_co], extra=
            collections_abc.Sequence):
            __slots__ = ()
    else:


        class Sequence(Sized, Reversible[T_co], Container[T_co], extra=
            collections_abc.Sequence):
            __slots__ = ()
else:
...
class ItemsView(MappingView[Tuple[KT, VT_co]], AbstractSet[Tuple[KT, VT_co]
    ], Generic[KT, VT_co], extra=collections_abc.ItemsView):
    __slots__ = ()
...
if hasattr(contextlib, 'AbstractAsyncContextManager'):
    ...
    class AsyncContextManager(Generic[T_co], extra=contextlib.
...
class DefaultDict(collections.defaultdict, MutableMapping[KT, VT], extra=
...
if hasattr(collections, 'ChainMap'):
    ...
    class ChainMap(collections.ChainMap, MutableMapping[KT, VT], extra=
...
_prohibited = ('__new__', '__init__', '__slots__', '__getnewargs__',
    '_fields', '_field_defaults', '_field_types', '_make', '_replace',
    '_asdict', '_source')
        ...
        for field_name in types:
            if field_name in ns:
                ...
                raise TypeError(
                    ...
                    .format(field_name=field_name, default_names=', '.join(
                    defaults_dict.keys())))
