...
class PatchTest(unittest.TestCase):
    ...
    def test_patch(self):
        ...
        def test():
            self.assertEqual(PTModule.something, sentinel.Something2,
                'unpatched')
            self.assertEqual(PTModule.something_else, sentinel.
                SomethingElse, 'unpatched')
        self.assertEqual(PTModule.something, sentinel.Something,
            'patch not restored')
    ...
    def test_patch_class_attribute(self):
        ...
        @patch('%s.SomeClass.class_attribute' % __name__, sentinel.
            ClassAttribute)
        def test():
            self.assertEqual(PTModule.SomeClass.class_attribute, sentinel.
                ClassAttribute, 'unpatched')
        test()
    ...
    def test_patchobject_with_spec(self):
        ...
        def test(MockAttribute):
            ...
            self.assertTrue(is_instance(SomeClass.class_attribute.wibble,
                MagicMock))
            self.assertRaises(AttributeError, lambda : SomeClass.
                class_attribute.not_wibble)
        test()
    ...
    def test_new_callable_incompatible_with_new(self):
        self.assertRaises(ValueError, patch, foo_name, new=object(),
            new_callable=MagicMock)
        self.assertRaises(ValueError, patch.object, Foo, 'f', new=object(),
            new_callable=MagicMock)

    def test_new_callable_incompatible_with_autospec(self):
        self.assertRaises(ValueError, patch, foo_name, new_callable=
            MagicMock, autospec=True)
        self.assertRaises(ValueError, patch.object, Foo, 'f', new_callable=
            MagicMock, autospec=True)
    ...
    def test_new_callable_inherit_for_mocks(self):
        ...
        class MockSub(Mock):
            pass
        MockClasses = (NonCallableMock, NonCallableMagicMock, MagicMock,
            Mock, MockSub)
    ...
    def test_patch_with_spec_mock_repr(self):
        for arg in ('spec', 'autospec', 'spec_set'):
            ...
            try:
                ...
                self.assertIn(" name='SomeClass.class_attribute'", repr(m.
                    class_attribute))
                self.assertIn(" name='SomeClass()'", repr(m()))
                self.assertIn(" name='SomeClass().class_attribute'", repr(m
                    ().class_attribute))
    ...
    def test_patch_nested_autospec_repr(self):
        with patch('unittest.test.testmock.support', autospec=True) as m:
            self.assertIn(" name='support.SomeClass.wibble()'", repr(m.
                SomeClass.wibble()))
            self.assertIn(" name='support.SomeClass().wibble()'", repr(m.
                SomeClass().wibble()))
    ...
    def test_patch_propogrates_exc_on_exit(self):
        ...
        def with_custom_patch(target):
            ...
            return custom_patch(getter, attribute, DEFAULT, None, False,
                None, None, None, {})
    ...
    def test_create_and_specs(self):
        for kwarg in ('spec', 'spec_set', 'autospec'):
            p = patch('%s.doesnotexist' % __name__, create=True, **{kwarg:
                True})
    ...
    def test_stopall_lifo(self):
        ...
        def get_patch(attribute):
            ...
            return mypatch(lambda : thing, attribute, None, None, False,
                None, None, None, {})
