...
try:
    ...
    _have_ssl = True
__all__ = ['Request', 'OpenerDirector', 'BaseHandler',
    ...
    'ProxyDigestAuthHandler', 'HTTPHandler', 'FileHandler', 'FTPHandler',
    'CacheFTPHandler', 'DataHandler', 'UnknownHandler',
...
def urlopen(url, data=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT, *,
    ...
    * getcode() - return the HTTP status code of the response.  Raises URLError
    ...
    if cafile or capath or cadefault:
        import warnings
        warnings.warn(
            'cafile, cpath and cadefault are deprecated, use a custom context instead.'
...
def urlretrieve(url, filename=None, reporthook=None, data=None):
    ...
    if size >= 0 and read < size:
        raise ContentTooShortError(
            'retrieval incomplete: got only %i out of %i bytes' % (read,
            size), result)
    return result
...
class Request:
    ...
    def has_header(self, header_name):
        return (header_name in self.headers or header_name in self.
            unredirected_hdrs)

    def get_header(self, header_name, default=None):
        return self.headers.get(header_name, self.unredirected_hdrs.get(
            header_name, default))
...
class OpenerDirector:
    ...
    def add_handler(self, handler):
        if not hasattr(handler, 'add_parent'):
            raise TypeError('expected BaseHandler instance, got %r' % type(
                handler))
        added = False
    ...
    def _open(self, req, data=None):
        ...
        if result:
            return result
        return self._call_chain(self.handle_open, 'unknown', 'unknown_open',
            req)
...
def build_opener(*handlers):
    ...
    default_classes = [ProxyHandler, UnknownHandler, HTTPHandler,
        ...
        FileHandler, HTTPErrorProcessor, DataHandler]
...
class HTTPErrorProcessor(BaseHandler):
    ...
    def http_response(self, request, response):
        ...
        if not 200 <= code < 300:
            response = self.parent.error('http', request, response, code,
                msg, hdrs)
        return response
...
class HTTPRedirectHandler(BaseHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        """
        m = req.get_method()
        if not (code in (301, 302, 303, 307) and m in ('GET', 'HEAD') or 
            code in (301, 302, 303) and m == 'POST'):
            raise HTTPError(req.full_url, code, msg, headers, fp)
        newurl = newurl.replace(' ', '%20')
        CONTENT_HEADERS = 'content-length', 'content-type'
        newheaders = dict((k, v) for k, v in req.headers.items() if k.lower
            () not in CONTENT_HEADERS)
        return Request(newurl, headers=newheaders, origin_req_host=req.
            origin_req_host, unverifiable=True)
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        ...
        if urlparts.scheme not in ('http', 'https', 'ftp', ''):
            raise HTTPError(newurl, code, 
                "%s - Redirection to url '%s' is not allowed" % (msg,
                newurl), headers, fp)
        ...
        if hasattr(req, 'redirect_dict'):
            ...
            if visited.get(newurl, 0) >= self.max_repeats or len(visited
                ) >= self.max_redirections:
                raise HTTPError(req.full_url, code, self.inf_msg + msg,
                    headers, fp)
        else:
            visited = new.redirect_dict = req.redirect_dict = {}
...
class ProxyHandler(BaseHandler):
    ...
    def __init__(self, proxies=None):
        ...
        for type, url in proxies.items():
            setattr(self, '%s_open' % type, lambda r, proxy=url, type=type,
...
class AbstractBasicAuthHandler:
    ...
    def http_error_auth_reqed(self, authreq, host, req, headers):
        ...
        if authreq:
            ...
            if scheme.lower() != 'basic':
                raise ValueError(
                    "AbstractBasicAuthHandler does not support the following scheme: '%s'"
                     % scheme)
            else:
    ...
    def http_request(self, req):
        if not hasattr(self.passwd, 'is_authenticated'
        ...
        if not req.has_header('Authorization'):
            ...
            req.add_unredirected_header('Authorization', 'Basic {}'.format(
                auth_str.strip()))
        return req
...
class AbstractDigestAuthHandler:
    ...
    def get_authorization(self, req, chal):
        ...
        base = (
            ...
            (user, realm, nonce, req.selector, respdig))
        if opaque:
...
class HTTPDigestAuthHandler(BaseHandler, AbstractDigestAuthHandler):
    ...
    def http_error_401(self, req, fp, code, msg, headers):
        ...
        retry = self.http_error_auth_reqed('www-authenticate', host, req,
            headers)
...
class ProxyDigestAuthHandler(BaseHandler, AbstractDigestAuthHandler):
    ...
    def http_error_407(self, req, fp, code, msg, headers):
        host = req.host
        retry = self.http_error_auth_reqed('proxy-authenticate', host, req,
            headers)
        self.reset_retry_count()
        return retry
...
class AbstractHTTPHandler(BaseHandler):
    ...
    def _get_content_length(self, request):
        return http.client.HTTPConnection._get_content_length(request.data,
            request.get_method())
        ...
        if request.data is not None:
            ...
            if not request.has_header('Content-length'
                ...
                if content_length is not None:
                    request.add_unredirected_header('Content-length', str(
                        content_length))
                else:
                    request.add_unredirected_header('Transfer-encoding',
                        'chunked')
        sel_host = host
    ...
    def do_open(self, http_class, req, **http_conn_args):
        ...
        headers.update(dict((k, v) for k, v in req.headers.items() if k not in
            headers))
...
if hasattr(http.client, 'HTTPSConnection'):
    ...
    class HTTPSHandler(AbstractHTTPHandler):
        ...
        def https_open(self, req):
            return self.do_open(http.client.HTTPSConnection, req, context=
...
class FileHandler(BaseHandler):
    ...
    def file_open(self, req):
        url = req.selector
        if url[:2] == '//' and url[2:3] != '/' and (req.host and req.host !=
            'localhost'):
    ...
    def get_names(self):
        if FileHandler.names is None:
            try:
                FileHandler.names = tuple(socket.gethostbyname_ex(
                    'localhost')[2] + socket.gethostbyname_ex(socket.
                    gethostname())[2])
...
""" %
            ...
            if not host or not port and _safe_gethostbyname(host
                ) in self.get_names():
                if host:
...
class URLopener:
    ...
    def _open_generic_http(self, connection_factory, url, data):
        ...
        if 200 <= response.status < 300:
            ...
            return self.http_error(url, response.fp, response.status,
                response.reason, response.msg, data)
    ...
    def open_file(self, url):
        ...
        if url[:2] == '//' and url[2:3] != '/' and url[2:12].lower(
            ) != 'localhost/':
...
""" %
    ...
    def open_ftp(self, url):
        ...
        try:
            ...
            raise URLError('ftp error %r' % exp).with_traceback(sys.
                exc_info()[2])
...
class FancyURLopener(URLopener):
    ...
    def redirect_internal(self, url, fp, errcode, errmsg, headers, data):
        ...
        urlparts = urlparse(newurl)
        if urlparts.scheme not in ('http', 'https', 'ftp', ''):
            raise HTTPError(newurl, errcode, errmsg + 
    ...
    def retry_proxy_http_basic_auth(self, url, realm, data=None):
        ...
        if not (user or passwd):
            return None
        proxyhost = '%s:%s@%s' % (quote(user, safe=''), quote(passwd, safe=
            ''), proxyhost)
    ...
    def retrfile(self, file, type):
        ...
        if file and not isdir:
            try:
                ...
                if str(reason)[:3] != '550':
                    raise URLError('ftp error: %r' % reason).with_traceback(sys
                        .exc_info()[2])
        if not conn:
...
def proxy_bypass_environment(host, proxies=None):
    ...
    for name in no_proxy_list:
        if name:
            ...
            name = re.escape(name)
            pattern = '(.+\\.)?%s$' % name
            if re.match(pattern, hostonly, re.I) or re.match(pattern, host,
                re.I):
                return 1
    return 0
