...
__all__ = ['Comment', 'dump', 'Element', 'ElementTree', 'fromstring',
    'fromstringlist', 'iselement', 'iterparse', 'parse', 'ParseError', 'PI',
    'ProcessingInstruction', 'QName', 'SubElement', 'tostring',
    'tostringlist', 'TreeBuilder', 'VERSION', 'XML', 'XMLID', 'XMLParser',
    'XMLPullParser', 'register_namespace']
VERSION = '1.3.0'
import sys
import re
import warnings
    ...
    *tag* is the element name.  *attrib* is an optional dictionary containing
    ...
    def __init__(self, tag, attrib={}, **extra):
        if not isinstance(attrib, dict):
            raise TypeError('attrib must be dict, not %s' % (attrib.
                __class__.__name__,))
        attrib = attrib.copy()
        attrib.update(extra)
        self.tag = tag
...
class ElementTree:
    ...
    *file* is an optional file handle or file name of an XML file whose
        ...
        if path[:1] == '/':
            ...
            warnings.warn(
                'This search is broken in 1.3 and earlier, and will be fixed in a future version.  If you rely on the current behaviour, change it to %r'
                 % path, FutureWarning, stacklevel=2)
    ...
    def write(self, file_or_filename, encoding=None, xml_declaration=None,
        default_namespace=None, method=None, *, short_empty_elements=True):
        ...
        with _get_writer(file_or_filename, enc_lower) as write:
            if method == 'xml' and (xml_declaration or xml_declaration is
                None and enc_lower not in ('utf-8', 'us-ascii', 'unicode')):
...
def _get_writer(file_or_filename, encoding):
    try:
        ...
        if encoding == 'unicode':
            ...
            file = open(file_or_filename, 'w', encoding=encoding, errors=
                'xmlcharrefreplace')
        with file:
            yield file.write
    else:
            ...
            with contextlib.ExitStack() as stack:
                if isinstance(file_or_filename, io.BufferedIOBase):
                    ...
                    try:
                        ...
                        pass
                file = io.TextIOWrapper(file, encoding=encoding, errors=
                    'xmlcharrefreplace', newline='\n')
                stack.callback(file.detach)
                yield file.write
...
HTML_EMPTY = ('area', 'base', 'basefont', 'br', 'col', 'frame', 'hr', 'img',
...
_serialize = {'xml': _serialize_xml, 'html': _serialize_html, 'text':
    _serialize_text}
...
def _raise_serialization_error(text):
    raise TypeError('cannot serialize %r (type %s)' % (text, type(text).
        __name__))
        ...
        if prefix == '&':
            ...
            try:
                ...
                err = expat.error('undefined entity %s: line %d, column %d' %
                    (text, self.parser.ErrorLineNumber, self.parser.
                    ErrorColumnNumber))
                err.code = 11
