...
def list_public_methods(obj):
    ...
    return [member for member in dir(obj) if not member.startswith('_') and
...
class SimpleXMLRPCDispatcher:
    ...
    def _marshaled_dispatch(self, data, dispatch_method=None, path=None):
        ...
        """
        try:
            params, method = loads(data, use_builtin_types=self.
                use_builtin_types)
            ...
            response = response,
            response = dumps(response, methodresponse=1, allow_none=self.
                allow_none, encoding=self.encoding)
        except Fault as fault:
            response = dumps(fault, allow_none=self.allow_none, encoding=
                self.encoding)
        except:
    ...
    def system_methodHelp(self, method_name):
        ...
        if method_name in self.funcs:
            ...
            if hasattr(self.instance, '_methodHelp'):
                ...
                try:
                    method = resolve_dotted_attribute(self.instance,
    ...
    def _dispatch(self, method, params):
        ...
        if self.instance is not None:
            ...
            try:
                func = resolve_dotted_attribute(self.instance, method, self
                    .allow_dotted_names)
            except AttributeError:
                pass
            else:
...
class SimpleXMLRPCRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if data is None:
                return
            response = self.server._marshaled_dispatch(data, getattr(self,
                '_dispatch', None), self.path)
...
class MultiPathXMLRPCServer(SimpleXMLRPCServer):
    ...
    def __init__(self, addr, requestHandler=SimpleXMLRPCRequestHandler,
        ...
        SimpleXMLRPCServer.__init__(self, addr, requestHandler, logRequests,
            allow_none, encoding, bind_and_activate, use_builtin_types)
        self.dispatchers = {}
        self.allow_none = allow_none
        self.encoding = encoding or 'utf-8'
    ...
    def _marshaled_dispatch(self, data, dispatch_method=None, path=None):
        try:
            response = self.dispatchers[path]._marshaled_dispatch(data,
                dispatch_method, path)
        except:
...
class CGIXMLRPCRequestHandler(SimpleXMLRPCDispatcher):
    ...
    def handle_request(self, request_text=None):
        ...
        headers.
        """
        if request_text is None and os.environ.get('REQUEST_METHOD', None
            ) == 'GET':
            self.handle_get()
        else:
            try:
...
class ServerHTMLDoc(pydoc.HTMLDoc):
    ...
    def docroutine(self, object, name, mod=None, funcs={}, classes={},
        methods={}, cl=None):
        """Produce HTML documentation for a function or method object."""
        anchor = (cl and cl.__name__ or '') + '-' + name
        note = ''
        title = '<a name="%s"><strong>%s</strong></a>' % (self.escape(
            anchor), self.escape(name))
        if inspect.ismethod(object):
            args = inspect.getfullargspec(object)
            argspec = inspect.formatargspec(args.args[1:], args.varargs,
                args.varkw, args.defaults, annotations=args.annotations,
                formatvalue=self.formatvalue)
        elif inspect.isfunction(object):
            args = inspect.getfullargspec(object)
            argspec = inspect.formatargspec(args.args, args.varargs, args.
                varkw, args.defaults, annotations=args.annotations,
                formatvalue=self.formatvalue)
        else:
            argspec = '(...)'
    ...
    def docserver(self, server_name, package_documentation, methods):
        ...
        result = result + self.bigsection('Methods', '#ffffff', '#eeaa77',
            ''.join(contents))
        return result
...
class XMLRPCDocGenerator:
    ...
    def generate_html_documentation(self):
        ...
        documentation = documenter.docserver(self.server_name, self.
            server_documentation, methods)
